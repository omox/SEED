...
public class NumberingUtility {
	...
	public static JSONObject calcCheckdigitSRCCD(User userInfo, String srccd, String sourcekbn) {
		...
		if(ArrayUtils.contains(eankbn, sourcekbn)){
			...
			if(ArrayUtils.contains(new String[]{"45", "49"}, StringUtils.left(srccd, 2))
				||NumberUtils.toInt(StringUtils.left(srccd, 2))<10
				||NumberUtils.toInt(StringUtils.left(srccd, 2))>97){
		...
		if(ArrayUtils.contains(upcakbn, sourcekbn)){
			...
			if(srccd.length()==10 &&
				(StringUtils.equals(StringUtils.left(srccd, 6), "000000") && NumberUtils.toInt(StringUtils.right(srccd, 4))>9999)){
	...
	private static int calcShncdCheckdigit(String cd) {
		...
		int res2 = NumberUtils.toInt(arr[0])*8 + NumberUtils.toInt(arr[1])*7 + NumberUtils.toInt(arr[2])*6 +NumberUtils.toInt(arr[3])*5 + NumberUtils.toInt(arr[4])*4 + NumberUtils.toInt(arr[5])*3 + NumberUtils.toInt(arr[6])*2;
	...
	private static int calcItfcdCheckdigit(String cd) {
		...
		int res2 = NumberUtils.toInt(arr[0]) + NumberUtils.toInt(arr[2]) + NumberUtils.toInt(arr[4]) +NumberUtils.toInt(arr[6]) + NumberUtils.toInt(arr[8]) + NumberUtils.toInt(arr[10]) + NumberUtils.toInt(arr[12]);
