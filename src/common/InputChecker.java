...
public class InputChecker {
   ...
   * @return チェック結果 (true=超えていない false=超えている)
   */
  public static boolean isNumericLessThanMaxLength(String buffer, String dataLen) {
   ...
   * @return チェック結果 (true=超えていない false=超えている)
   */
  public static boolean isNumericLessThanMaxLength(String buffer, String dataLen, String separator) {
    ...
    int decimalMax = 0;
    if (separatorCount > 0) {
      integerMax = Integer.parseInt(splitLen[0]) - Integer.parseInt(splitLen[1]);
    ...
    if (decimalLength > decimalMax && !(decimalMax == 0 && StringUtils.equals(decumalBuf, "0"))) {
      return false;
    }
   ...
   *
   * @param buffer チェックする文字列
   * @param integerDigits 整数部桁数（1以上）
   * @param fractionDigits 小数部桁数（0以上）
   * @return boolean チェック結果 (true=指定桁数以内 false=指定桁数オーバー)
   */
  public static boolean isNumericLessThanDigits(String buffer, int integerDigits, int fractionDigits) {
    ...
    return Pattern.matches("(-)?\\d{1," + integerDigits + "}(\\.\\d{1," + fractionDigits + "})?", buffer);
  }
   ...
   *
   * @param from 開始日として入力された文字列
   * @param to 終了日として入力された文字列
