...
public class GetSqlCommandChange {
	...
	public static String getSqlcommand(HttpServletRequest request, HashMap<String,String> maps, String datatype, String jndiName) throws Exception {
		...
		if (outobj.equals(DefineReport.Select.MOYSCD.getObj())) {
			if("-1".equals(value)){
				...
				if(moyocd.length >= 3){
					...
					if(StringUtils.isNotEmpty(moysKbn)
							&& StringUtils.isNotEmpty(moysStdt)
							&& StringUtils.isNotEmpty(moysRban)){
		...
		if (outobj.equals(DefineReport.InpText.TENCD.getObj())
			|| outobj.equals(DefineReport.InpText.TENCD.getObj()+"leader")
			|| outobj.equals(DefineReport.InpText.CENTERCD.getObj())
		...
		if (outobj.equals(DefineReport.InpText.SSIRCD.getObj())
			||outobj.equals(DefineReport.InpText.SIRCD.getObj())
			||outobj.equals(DefineReport.InpText.DSIRCD.getObj())
			||outobj.equals(DefineReport.InpText.RSIRCD.getObj())
			||outobj.equals(DefineReport.InpText.DF_RSIRCD.getObj())) {
			if(NumberUtils.isNumber(value)){
				paramData.add(value);
		...
		if (outobj.equals(DefineReport.InpText.BMNCD.getObj())
			||outobj.equals(DefineReport.InpText.YOT_BMNCD.getObj())
			||outobj.equals(DefineReport.InpText.URI_BMNCD.getObj())) {
			if(NumberUtils.isNumber(value)){
				paramData.add(value);
		...
		if (outobj.equals(DefineReport.InpText.DAICD.getObj())
			||outobj.equals(DefineReport.InpText.YOT_DAICD.getObj())
			||outobj.equals(DefineReport.InpText.URI_DAICD.getObj())) {
			String bmncd = obj.optString("bmncd");
		...
		if (outobj.equals(DefineReport.InpText.CHUCD.getObj())
			||outobj.equals(DefineReport.InpText.YOT_CHUCD.getObj())
			||outobj.equals(DefineReport.InpText.URI_CHUCD.getObj())) {
			String bmncd = obj.optString("bmncd");
		...
		if (outobj.equals(DefineReport.InpText.SHOCD.getObj())
			||outobj.equals(DefineReport.InpText.YOT_SHOCD.getObj())
			||outobj.equals(DefineReport.InpText.URI_SHOCD.getObj())) {
			String bmncd = obj.optString("bmncd");
			String daicd = obj.optString("daicd");
		...
		if (StringUtils.startsWith(outobj, DefineReport.InpText.SHNCD.getObj())&&!obj.containsKey("KEY")) {
			if(StringUtils.isNotEmpty(value)){
				...
				} else if(DefineReport.ID_PAGE_JU032.equals(outpage) || DefineReport.ID_PAGE_JU033.equals(outpage) ||
						DefineReport.ID_PAGE_JU012.equals(outpage) || DefineReport.ID_PAGE_JU013.equals(outpage)){
				...
				} else if(DefineReport.ID_PAGE_X213.equals(outpage) || DefineReport.ID_PAGE_X214.equals(outpage) ||
						DefineReport.ID_PAGE_X217.equals(outpage) || DefineReport.ID_PAGE_X218.equals(outpage)) {
					paramData.add(value);
...
//		}
		...
		if((outobj.equals(DefineReport.Text.RG_SOUBAIKA.getObj())&&obj.containsKey("KEY"))
		 ||(outobj.startsWith(DefineReport.Text.BG_SOUBAIKA.getObj())&&obj.containsKey("KEY"))){
			String bmncd = obj.optString("BMNCD");
		...
		if (!StringUtils.isEmpty(obj.optString("ID")) && (
				obj.optString("ID").equals(DefineReport.InpText.TAISYOTEN.getObj() + "_arr") ||
				obj.optString("ID").equals(DefineReport.InpText.JYOGAITEN.getObj() + "_arr") ||
				obj.optString("ID").equals(DefineReport.InpText.BMNCD.getObj() + "_arr"))
		) {
			// 入力値取得
			String bmnCd		= obj.optString("BMNCD");
