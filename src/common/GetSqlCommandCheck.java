	...
	public static String getSqlcommand(HttpServletRequest request, HashMap<String,String> maps, String datatype, String jndiName) throws Exception {
		...
		if(StringUtils.equals(key, "MST_CNT")){
			...
			if(outobj.equals(DefineReport.InpText.TENCD.getObj())) {
				tbl="INAMS.MSTTEN";
				col="TENCD";
				if (DefineReport.ID_PAGE_RP007.equals(outpage)
						|| DefineReport.ID_PAGE_SK003.equals(outpage)
						|| DefineReport.ID_PAGE_SK006.equals(outpage)
						|| "Out_ReportwinST008".equals(outpage)
		...
		if (outobj.equals(DefineReport.InpText.SHNCD.getObj()) && StringUtils.equals(key, "SEL") &&
				...
				DefineReport.ID_PAGE_JU032.equals(outpage) || DefineReport.ID_PAGE_JU033.equals(outpage))) {
		...
		if (StringUtils.equals(key, "PROHIBITED") && !StringUtils.isEmpty(value)) {
			...
			for (int i = 0; i < len; i++) {
				sqlcommand = "select * from (values('"+value.substring(i,(i+1))+"')) as T1("+DefineReport.VAL+")";
				try {
					new EasyToJSONServlet();
			...
			sqlcommand = "select * from (values('"+newValue+"')) as T1("+DefineReport.VAL+")";
			paramData.clear();
		}
