...
public class DefineReport {
  ...
  public final static String ID_SQL_SYSLOGIN =
      "select T1.SHAINCD as VALUE, RIGHT('00'||ROW_NUMBER() over(order by T1.SIMEIKN),2)||T1.SIMEIKN as TEXT, T1.SIMEIKN as TEXT2, RIGHT('00'||ROW_NUMBER() over(order by T1.SIMEIKN),2) as TEXT3 from (SELECT USER_ID AS SHAINCD, CASE WHEN YOBI_3 IS NULL OR YOBI_3='' THEN 0 ELSE CAST(YOBI_3 AS SIGNED) END AS SZKCD, NM_FAMILY || NM_NAME AS SIMEIKN FROM KEYSYS.SYS_USERS) T1";
