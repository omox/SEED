...
public class DefineReport {
  ...
  public final static String ID_SQL_GRD_CMN = "with T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) AS DUMMY union all select IDX + 1 from T1 where IDX < @M) ";
  public final static String ID_SQL_GRD_CMN0 = "with T1(IDX) as (select 0 from (SELECT 1 AS DUMMY) AS DUMMY union all select IDX + 1 from T1 where IDX < @M) ";
  ...
  public final static String ID_SQL_TENPO_STGT =
      "with T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) AS DUMMY union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.TENCD, T2.TENKN from T1"
  ...
  public final static String ID_SQL_EHSPTN_LIST =
      "with T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) DUMMY ) select T1.IDX, T2.HSPTN, T2.HSPTNKN, right ('000' || T2.CENTERCD, 3) as CENTERCD, right ('000' || TRIM(T2.YCENTERCD), 3) as YCENTERCD, right ('0000' || T2.TENGPCD, 4) as TENGPCD, T2.TENGPKN, T2.HSPTN, T2.TENDENFLG || '-' || RTRIM(M1.NMKN) as TEMBETSUDENPYOFLG, T2.VANKBN || '-' || RTRIM(M2.NMKN) as KEISANCENTER, T2.UNYOKBN || '-' || RTRIM(M3.NMKN) as UNYOKBN, T2.DENPKBN || '-' || RTRIM(M4.NMKN) as DENPYOKBN, T2.SHUHKBN || '-' || RTRIM(M5.NMKN) as SHUKEIHYO1, T2.PICKDKBN || '-' || RTRIM(M6.NMKN) as PICKINGDATA, T2.PICKLKBN || '-' || RTRIM(M7.NMKN) as PICKINGLIST, T2.WAPNKBN || '-' || RTRIM(M8.NMKN) as WAPPEN, T2.IDENPKBN || '-' || RTRIM(M9.NMKN) as IKKATSUDENPYO, T2.KAKOSJKBN || '-' || RTRIM(M10.NMKN) as KAKOSHIJI, T2.RYUTSUKBN || '-' || RTRIM(M11.NMKN) as RYUTSUKBN"
  ...
  public final static String ID_SQL_EHSPTN_HP012_INS =
      "with T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) AS DUMMY union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.TENGPCD, T2.TENGPKN, null as CENTERCD, null as YCENTERCD, null as TENKN_CENTER_G, null as TENKN_YCENTER_G, T2.SELCHECK as SEL from T1 left join (select ROW_NUMBER() over (order by TENGPCD) as IDX, * from (select HSTENGP.TENGPCD, HSTENGP.TENGPKN, 0 as SELCHECK from INAMS.MSTHSGP HSGP"
  ...
  public final static String ID_SQL_EHSPTN_HP012_UPD =
      "with T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) AS DUMMY union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.TENGPCD, T2.TENGPKN, case when int (T2.CENTERCD) = 0 then null else TRIM(T2.CENTERCD) end as CENTERCD, case when replace (T2.YCENTERCD, '0', '') = '' then null else TRIM(T2.YCENTERCD) end as YCENTERCD, case when int (T2.CENTERCD) = 0 then null else TEN1.TENKN end as TENKN_CENTER_G, case when replace (T2.YCENTERCD, '0', '') = '' then null else TEN2.TENKN end as TENKN_YCENTER_G, T2.SELCHECK as SEL from T1 left join (select ROW_NUMBER() over (order by TENGPCD) as IDX, * from (select HSTENGP.TENGPCD, HSTENGP.TENGPKN, AHS.CENTERCD as CENTERCD, case when TRIM(COALESCE(AHS.YCENTERCD, '')) = '' then null else TRIM(AHS.YCENTERCD) end as YCENTERCD, case when AHS.TENGPCD is not null then 1 else 0 end as SELCHECK from INAMS.MSTHSGP HSGP"
  ...
  public final static String ID_SQL_SETNO = ID_SQL_GRD_CMN
      ...
      + DefineReport.ValUpdkbn.NML.getVal() + " ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,STNO)) AS T2 ON T1.IDX=T2.IDX ORDER BY T1.IDX";
  // SQL :セット販売商品一覧
  ...
  public final static String ID_SQL_HATSTRTEN = "WITH SHORI as(" + ID_SQLSHORIDT
      ...
      + VAL + ",'YYYY/MM/DD') AND TN.TENCD=? AND COALESCE(TN.UPDKBN, 0) <> 1 AND COALESCE(MS.UPDKBN, 0) <> 1 ORDER BY TN.SHNCD";
  public final static String ID_SQL_HATJTRTEN = "WITH SHORI as(" + ID_SQLSHORIDT
      ...
      + VAL + ",'YYYY/MM/DD') AND TN.TENCD=? AND TN.SHUNO=? AND COALESCE(MS.UPDKBN, 0) <> 1 ORDER BY TN.SHNCD";
  ...
  public final static String ID_SQL_SYSLOGIN2 =
      "select T1.SHAINCD as VALUE, T1.SHAINCD||T1.SIMEIKN as TEXT, T1.SIMEIKN as TEXT2, T1.SHAINCD as TEXT3 from (SELECT USER_ID AS SHAINCD, CASE WHEN YOBI_3 IS NULL OR YOBI_3='' THEN 0 ELSE CAST(YOBI_3 AS SMALLINT) END AS SZKCD, NM_FAMILY || NM_NAME AS SIMEIKN FROM KEYSYS.SYS_USERS) T1";
