...
public class DefineReport {
	...
	public enum DataType {
		...
		public boolean isText() {
			return TEXT.equals(getObj()) || ZEN.equals(getObj()) || KANA.equals(getObj()) || ALPHA.equals(getObj())
					|| SUUJI.equals(getObj()) || LPADZERO.equals(getObj()) || TEL.equals(getObj())
					|| FAX.equals(getObj());
		}
	...
	public enum Download {
		/** CSV出力 */
		PAGE_X001("商品一覧画面出力CSV"),
		PAGE_X250("仕掛商品検索画面出力CSV");
	...
	public final static String ID_SQL_BLANK2 = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', ' ')) as X(value, TEXT) union all ";
	public final static String ID_SQL_BLANK3 = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', ' ')) as X(value, TEXT) union all ";
	public final static String ID_SQL_BLANK4 = "select VALUE, TEXT, TEXT as TEXT2, TEXT as TEXT3 from (values ('"
			+ Values.NONE.getVal() + "', ' ')) as X(value, TEXT) union all ";
	// SQL：すべて
	public final static String ID_SQL_ALL = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', 'すべて')) as X(value, TEXT)";
	public final static String ID_SQL_ALL2 = "select VALUE, TEXT, SEQ from (values ('" + Values.NONE.getVal()
			+ "', 'すべて', -1)) as X(value, TEXT, SEQ)";
	...
	public final static String ID_SQL_CMN_WHERE = " where nvl(UPDKBN,0) <> 1 ";
	public final static String ID_SQL_CMN_WHERE2 = " and nvl(UPDKBN,0) <> 1 ";
	...
	public final static String ID_SQL_ARR_CMN = ",ARRWK(IDX, RNK, S, ARR, LEN) as ("
			+ " select 1, SUBSTR(ARR, 1 , LEN), 1 + LEN, ARR, LEN from wk " + " union all"
			+ " select IDX+1, SUBSTR(ARR, S , LEN), S + LEN, ARR, LEN from ARRWK where S <= length(ARR)" + ") ";

	// SQL：カレンダーマスター
	public final static String ID_SQL_CAL = "select min(COMTOB) as " + MINDT + ", max(COMTOB) as " + MAXDT
			+ " from INAYS.MCALTT";
	...
	public final static String ID_SQL_MEISYO_HEAD = "select " + VAL + " as " + KBN + "," + VAL + "," + TXT + " as "
			+ KTXT + "," + TXT + "," + TXT + " as TEXT2," + TXT + " as " + STXT + "," + TXT + " as " + KANA
			+ " from (values ('" + Values.NONE.getVal() + "','　')) as X(" + VAL + "," + TXT + ") union all ";
	public final static String ID_SQL_MEISYO_HEAD2 = "select " + VAL + " as " + KBN + "," + VAL + "," + TXT + " as "
			+ KTXT + "," + TXT + "," + TXT + " as TEXT2," + TXT + " as " + STXT + "," + TXT + " as " + KANA
			+ " from (values ('" + Values.ALLKAN.getVal() + "','" + Values.ALLKAN.getTxt() + "')) as X(" + VAL + ","
			+ TXT + ") union all ";
	public final static String ID_SQL_MEISYO_HEAD3 = "select " + VAL + " as " + KBN + "," + VAL + "," + TXT + " as "
			+ KTXT + "," + TXT + "," + TXT + " as TEXT2," + TXT + " as " + STXT + "," + TXT + " as " + KANA
			+ " from (values ('" + Values.ALL.getVal() + "','" + Values.ALL.getTxt() + "')) as X(" + VAL + "," + TXT
			+ ") union all ";
	public final static String ID_SQL_MEISYO = "select MEISHOKBN as " + KBN + ", MEISHOCD as " + VAL
			+ ", MEISHOKBNKN as " + KTXT + ", MEISHOCD||'" + SEPARATOR + "'||rtrim(NMKN) as " + TXT
			+ ", rtrim(NMKN) as TEXT2, TNMKN as " + STXT + ", NMAN as " + KANA
			+ " from INAMS.MSTMEISHO where MEISHOKBN = ? ";
	public final static String ID_SQL_MEISYO2 = "select MEISHOKBN as " + KBN + ", MEISHOCD as " + VAL
			+ ", MEISHOKBNKN as " + KTXT + ", rtrim(NMKN) as " + TXT + ", rtrim(NMKN) as TEXT2, TNMKN as " + STXT
			+ ", NMAN as " + KANA + " from INAMS.MSTMEISHO where MEISHOKBN = ? ";
	...
	public final static String ID_SQL_MEISYO_PROHIBITED = "select * from (values ('" + DataType.KANA.getObj()
			+ "', 'ｧｨｩｪｫｯｬｭｮ', 'ｱｲｳｴｵﾂﾔﾕﾖ'),('" + DataType.ZEN.getObj()
			+ "', '〜', 'ー'),('ng', ',', null)) as MSTMEISHO(MOJI, " + TXT + ", " + TXT + "2)";
	...
	public final static String ID_SQLSHORIDT = "select SHORIDT as " + VAL
			+ " from INAAD.SYSSHORIDT where NVL(UPDKBN, 0) <> 1 order by ID desc fetch first 1 rows only";
	public final static String ID_SQLSHORIDT2 = "select SUBSTR(SHORIDT,3) as " + VAL
			+ " from INAAD.SYSSHORIDT where NVL(UPDKBN, 0) <> 1 order by ID desc fetch first 1 rows only";
	public final static String ID_SQLSHORIDT3 = "select SHORIDT as " + VAL
			+ "2,TO_CHAR(TO_DATE(SHORIDT,'yyyymmdd') +1 year,'yyyymmdd') as " + VAL
			+ "3 from INAAD.SYSSHORIDT where NVL(UPDKBN, 0) <> 1 order by ID desc fetch first 1 rows only";

	// SQL：処理日付曜日
	public final static String ID_SQLSHORIDTWEEK = "select DAYOFWEEK(TO_DATE(SHORIDT,'YYYYMMDD')) as " + VAL
			+ " from INAAD.SYSSHORIDT where NVL(UPDKBN, 0) <> 1 order by ID desc fetch first 1 rows only";

	// SQL：週№期間を取得
	public final static String ID_SQLSHUNOPERIOD = DefineReport.ID_SQL_CMN_WEEK
			+ ",SHORI as(SELECT CASE WHEN DAYOFWEEK(TO_DATE(" + VAL + ",'YYYYMMDD')) = 2 OR DAYOFWEEK(TO_DATE(" + VAL
			+ ",'YYYYMMDD')) = 3 THEN TO_CHAR(TO_DATE(" + VAL + ",'YYYYMMDD') + 7 DAY,'YYYYMMDD') ELSE TO_CHAR(TO_DATE("
			+ VAL + ",'YYYYMMDD') + 14 DAY,'yyyymmdd') END AS DT FROM(" + DefineReport.ID_SQLSHORIDT + ")) "
	...
	public final static String ID_SQL_MOYSCD_HEAD = " union all select VALUE, TEXT from (values ('-1', '　')) as X(value, TEXT)  ";
	public final static String ID_SQL_HEAD_MOYSCD = "select VALUE, TEXT, TEXT2, TEXT3, TEXT4 from (values ('-1', '　', 0, 0, 0)) as X(value, TEXT, TEXT2, TEXT3, TEXT4) union all select CONCAT(CONCAT(CONCAT(CONCAT(T1.MOYSKBN, '-'), T1.MOYSSTDT), '-'), T1.MOYSRBAN) as VALUE, CONCAT(CONCAT(CONCAT(CONCAT(T1.MOYSKBN, '-'), T1.MOYSSTDT), '-'), T1.MOYSRBAN) as TEXT, T1.MOYSKBN as TEXT2, T1.MOYSSTDT as TEXT3, T1.MOYSRBAN as TEXT4 from INATK.TOKMOYCD T1 where T1.MOYSKBN = 8 order by TEXT2, TEXT3, TEXT4;";
	...
	public final static String ID_SQL_TOKMOYCD = "SELECT COUNT(MOYSKBN) AS " + VAL
			+ " FROM INATK.TOKMOYCD WHERE MOYSKBN=? AND MOYSSTDT=? AND MOYSRBAN=? AND PLUSFLG='1'";

	// SQL PLU配信済のPLU配信日
	public final static String ID_SQL_PLUSDDT = "SELECT PLUSDDT AS " + VAL
			+ " FROM INATK.TOKMOYCD WHERE MOYSKBN=? AND MOYSSTDT=? AND MOYSRBAN=? AND PLUSFLG='1'";

	// SQL 分類割引 重複チェック
	public final static String ID_SQL_NEWDEPLICATECHEACK = "SELECT COUNT as " + VAL + " FROM INATK.TOKBT_KKK"
			+ " WHERE MOYSKBN =? AND MOYSSTDT =? AND BMNCD =? AND DAICD =?";

	// SQL 分類割引 重複チェック 中分類含む場合
	public final static String ID_SQL_NEWDEPLICATECHEACK_C = "SELECT COUNT as " + VAL + " from INATK.TOKBT_KKK"
			+ " WHERE MOYSKBN =? AND MOYSSTDT =? AND BMNCD =? AND DAICD =? AND CHUCD =?";
	...
	public final static String ID_SQL_SHUNO = " select RIGHT('0000'||T1.SHUNO,4) as " + VAL
			...
			+ "||'～'||TO_CHAR(TO_DATE(T1.ENDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.ENDDT, 'YYYYMMDD'))) as "
			+ TXT + "@C" + " from INAAD.SYSSHUNO T1" + " @T";
	public final static String ID_SQL_TOKSHUNO = " select RIGHT('0000'||T2.SHUNO,4) as " + VAL
			...
			+ "||'～'||TO_CHAR(TO_DATE(T1.ENDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.ENDDT, 'YYYYMMDD'))) END as "
			+ TXT + "@C"
			+ " from INATK.TOKMOYSYU T2 LEFT JOIN INAAD.SYSSHUNO T1 ON T1.SHUNO = T2.SHUNO where T2.UPDKBN = 0 "
			+ " @T";
	...
	public final static String ID_SQL_SHUNO2 = DefineReport.ID_SQL_CMN_WEEK
			...
			+ DefineReport.ID_SQLSHORIDT + ")) " + "SELECT SHUNO.SHUNO AS " + VAL + ",SUBSTR(SHUNO.SHUNO,1,2) || '"
			+ SEPARATOR
	...
	public final static String ID_SQL_SHUN_TR007 = DefineReport.ID_SQL_CMN_WEEK
			+ " select * from (select value, TEXT from (values ('-1', '　')) as X(value, TEXT)"
			+ " union all select right ('0000' || T1.SHUNO, 4) as value, right ('0000' || T1.SHUNO, 4) || '-' || TO_CHAR(TO_DATE(T1.STARTDT, 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.STARTDT, 'YYYYMMDD'))) || '～' || TO_CHAR(TO_DATE(T1.ENDDT, 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.ENDDT, 'YYYYMMDD'))) as TEXT from INAAD.SYSSHUNO T1 inner join (select distinct SHUNO from INATK.HATTR_CSV where NVL(UPDKBN, 0) <> 1) T2 on T1.SHUNO = T2.SHUNO"
			+ " ) T order by replace (value, '-1', '9999') desc";

	public final static String ID_SQL_SHUNO_HEAD = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_SHUNO_HEAD2 = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', '')) as X(value, TEXT) union all ";
	...
	/** 既存店条件 */
	public final static String ID_SQL_TEN_KZN = " and exists (select 'X' from INAMS.MTNPJIK M20 where M20.NENTUKI = ? and M20.MISECD = T1.MISECD and M20.KIZONTEN_KBN = '"
			+ DefineReport.ValKizonKbn.KIZON.getVal() + "') ";
	/** 新店条件 */
	public final static String ID_SQL_TEN_NEW = " and exists (select 'X' from INAMS.MTNPJIK M20 where M20.NENTUKI = ? and M20.MISECD = T1.MISECD and M20.KIZONTEN_KBN = '"
			+ DefineReport.ValKizonKbn.NEW.getVal() + "') ";
	...
	/** 共通（INAMS.MSTTENT） */
	public final static String ID_SQL_TENPO_MDM = "select right ('000' || TENCD, 3) as value, right ('000' || TENCD, 3) || '"
			+ SEPARATOR
			+ "' || NVL(RTRIM(RTRIM(TENKN), '　'), '') as TEXT, RTRIM(RTRIM(TENKN), '　') as TEXT2 from INAMS.MSTTEN order by VALUE";

	public final static String ID_SQL_TENPO_HEAD = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_TENPO_HEAD2 = "select VALUE, TEXT from (values " + " ("
			+ Values.TENPO_ALL.getVal() + ", '" + Values.TENPO_ALL.getTxt() + "'),(" + Values.TENPO_NEW.getVal() + ", '"
			+ Values.TENPO_NEW.getTxt() + "'),(" + Values.TENPO_EX.getVal() + ", '" + Values.TENPO_EX.getTxt() + "')"
			+ ") as X(value, TEXT) union all ";
	public final static String ID_SQL_TENPO_HEAD3 = "select VALUE, TEXT from (values ('', '')) as X(value, TEXT) union all ";
	public final static String ID_SQL_TENPO_MDM_HEAD = "select " + VAL + "," + TXT + "," + TXT
			+ " as TEXT2 from (values ('" + Values.NONE.getVal() + "','　')) as X(" + VAL + "," + TXT + ") union all ";

	// SQL：店舗 TODO:
	public final static String ID_SQL_TEN = "select TENCD as F1, TENAN as F2, CASE WHEN TENKN IS NULL OR TENKN = '' THEN '　' ELSE TENKN END as F3 from INAMS.MSTTEN where UPDKBN = "
	...
	public final static String ID_SQL_TENPO_LIST = ID_SQL_GRD_CMN + "select T1.IDX, T2.TENCD, T2.TENKN from T1"
			...
			+ DefineReport.ValUpdkbn.NML.getVal()
			+ " fetch first @M row only) T2 on T1.IDX = T2.IDX order by T1.IDX ,T2.TENCD";
	...
	public final static String ID_SQL_TENPO_HTS = "with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.F1 as TENCD, T2.F2 as TENKN, T2.F3 as HTSU, T2.F4 as HTSU_T, case when T2.F1 is not null then 0 else 1 end as SINKFLG from T1 left join (select ROW_NUMBER() over (order by HTEN.TENCD) as IDX, HTEN.TENCD as F1, MAX(TEN.TENKN) as F2, SUM(case when NVL(HTEN.INPUTDT, 0) < ? then HTEN.HTSU else 0 end) as F3, SUM(case when NVL(HTEN.INPUTDT, 0) = ? then HTEN.HTSU else 0 end) as F4"
	...
	public final static String ID_SQL_TENPO_SK = "with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.KANRINO, T2.SHNCD, T2.SHNKN, T2.SURYO from T1 left join (select ROW_NUMBER() over (order by HSKS.INPUTNO, HSKS.KANRINO) as IDX, HSKS.KANRINO, case when HSKS.SHNCD = 99999994 then '99999994' else SHN.SHNCD end as SHNCD, case when HSKS.SHNCD = 99999994 then '棚段変わります' else SHN.SHNKN end as SHNKN, HSKS.SURYO from INATK.HATSK_SHN HSKS left join INAMS.MSTSHN SHN on SHN.SHNCD = right ('00000000' || TRIM(HSKS.SHNCD), 8) where (NVL(SHN.UPDKBN, 0) <> 1 or right ('00000000' || TRIM(HSKS.SHNCD), 8) = '99999994') and HSKS.INPUTNO = ? order by HSKS.KANRINO) T2 on T1.IDX = T2.IDX @W order by T1.IDX";

	// SQL：店舗一覧(新店改装発注_構成別) TODO:
	public final static String ID_SQL_TENPO_SK_K = "with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.SHNCD, T2.SHNKN from T1 left join (select ROW_NUMBER() over (order by KSPG.LINENO) as IDX, case when KSPG.SHNCD = 99999994 then '99999994' else SHN.SHNCD end as SHNCD, case when KSPG.SHNCD = 99999994 then '棚段変わります' else SHN.SHNKN end as SHNKN, KSPG.LINENO from INAMS.MSTKSPAGE KSPG inner join INAMS.MSTTEN TEN on NVL(TEN.UPDKBN, 0) <> 1 and TEN.TENCD = KSPG.TENCD left join INAMS.MSTSHN SHN on SHN.SHNCD = right ('00000000' || TRIM(KSPG.SHNCD), 8) @W order by KSPG.LINENO) T2 on T1.IDX = T2.IDX order by T1.IDX, T2.LINENO";

	// SQL：店確認
	public final static String ID_SQL_ADTEN = "with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < @M) select T1.IDX, T2.TENCD as ADDTEN, T3.TENCD as DELTEN from T1 left join (select ROW_NUMBER() over (order by KKKNO, TENCD) as IDX, TENCD from INATK.TOKBT_ADTEN where ADDDELFLG = 1 and KKKNO = ? ) T2 on T1.IDX = T2.IDX left join (select ROW_NUMBER() over (order by KKKNO, TENCD) as IDX, TENCD from INATK.TOKBT_ADTEN where ADDDELFLG = 0 and KKKNO = ? ) T3 on T1.IDX = T3.IDX order by T1.IDX";
	...
	// SQL：部門
	public final static String ID_SQL_BUMON_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.ALL.getVal() + "','" + Values.ALL.getTxt() + "')) as X(" + VAL + "," + TXT + ") union all ";
	public final static String ID_SQL_BUMON_HEAD2 = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_BUMON_HEAD3 = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', '" + Values.BUMON_ALL_TOK.getTxt() + "')) as X(value, TEXT) union all ";
	public final static String ID_SQL_BUMON_CMN = "select right('0'||BMNCD,2) as VALUE, right('0'||BMNCD,2)||'"
			+ SEPARATOR + "'||nvl(rtrim(rtrim(max(BMNKN)), '　'),'') as TEXT, rtrim(rtrim(max(BMNKN)), '　') as TEXT2 ";
	public final static String ID_SQL_BUMON_CMN_C = "select right('0'||BMNCD,2) as VALUE, right('0'||BMNCD,2)||'"
			+ SEPARATOR + "'||nvl(rtrim(rtrim(BMNKN), '　'),'') as TEXT, rtrim(rtrim(BMNKN), '　') as TEXT2, * ";
	...
	public final static String ID_SQL_BUMON_UNSELECTED = " select value, TEXT, TEXT as TEXT2 from (values (- 1, '')) as X(value, TEXT) union all select BMNCD as value, right ('0' || BMNCD, 2) || '-' || RTRIM(RTRIM(MAX(BMNKN)), '　') as TEXT, RTRIM(RTRIM(MAX(BMNKN)), '　') as TEXT2 from INAMS.MSTBMN group by BMNCD order by value";
	public final static String ID_SQL_BUMON_ZIZEN = " select VALUE , TEXT , TEXT as TEXT2 from (values ('-1', '　')) as X(value, TEXT) union all select right ('0' || T1.BMNCD, 2) as VALUE , right ('0' || T1.BMNCD, 2) || '-' || rtrim(rtrim(max(T1.BMNKN)), '　') as TEXT , rtrim(rtrim(max(T1.BMNKN)), '　') as TEXT2  from INAMS.MSTBMN T1 left join INATK.TOKTJ_TEN T2 on T2.TENCD = ? and T2.BMNCD = T1.BMNCD where T2.LSTNO = ? and T2.TENCD = ? and T2.BMNCD = T1.BMNCD group by   T1.BMNCD order by VALUE";// TODO
																																																																																																																													// 定数を入れている為後で修正

	// SQL：大分類
	public final static String ID_SQL_DAI_BUN_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_DAI_BUN_HEAD2 = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_DAI_BUN_CMN = "select right('0'||BMNCD,2)||right('0'||DAICD,2) as VALUE, right('0'||BMNCD,2)||right('0'||DAICD,2)||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(DAIBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(DAIBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_DAI_BUN_SHC = "select right('0'||DAICD,2) as VALUE, right('0'||DAICD,2) ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(DAIBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(DAIBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_DAI_BUN_SHC2 = "select right('0'||BMNCD,2)||right('0'||DAICD,2) as VALUE, right('0'||DAICD,2) ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(DAIBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(DAIBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_DAI_BUN_SHC3 = "select right('0'||BMNCD,2)||right('0'||DAICD,2) as VALUE, DAICD ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(DAIBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(DAIBRUIKN)), '　') as TEXT2 ";
	...
	// SQL：中分類
	public final static String ID_SQL_CHU_BUN_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_CHU_BUN_HEAD2 = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_CHU_BUN_CMN = "select right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2) as VALUE, right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2)||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(CHUBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(CHUBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_CHU_BUN_SHC = "select right('0'||CHUCD,2) as VALUE, right('0'||CHUCD,2) ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(CHUBRUIKN)), '　'),'') as TEXT , rtrim(rtrim(max(CHUBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_CHU_BUN_SHC2 = "select right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2) as VALUE, right('0'||CHUCD,2) ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(CHUBRUIKN)), '　'),'') as TEXT , rtrim(rtrim(max(CHUBRUIKN)), '　') as TEXT2 ";
	public final static String ID_SQL_CHU_BUN_SHC3 = "select right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2) as VALUE, CHUCD ||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(CHUBRUIKN)), '　'),'') as TEXT , rtrim(rtrim(max(CHUBRUIKN)), '　') as TEXT2 ";
	...
	// SQL：小分類
	public final static String ID_SQL_SHO_BUN_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_SHO_BUN_CMN = "select right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2)||right('0'||SHOCD,2) as VALUE, right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2)||right('0'||SHOCD,2)||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(SHOBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(SHOBRUIKN)), '　') as TEXT2 ";
	...
	// SQL：小小分類
	public final static String ID_SQL_SSHO_BUN_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ('"
			+ Values.NONE.getVal() + "', 'すべて')) as X(value, TEXT) union all ";
	public final static String ID_SQL_SSHO_BUN = "select right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2)||right('0'||SHOCD,2)||SSHOCD as VALUE, right('0'||BMNCD,2)||right('0'||DAICD,2)||right('0'||CHUCD,2)||right('0'||SHOCD,2)||'"
			+ SEPARATOR
			+ "'||nvl(rtrim(rtrim(max(SHOBRUIKN)), '　'),'') as TEXT, rtrim(rtrim(max(SHOBRUIKN)), '　') as TEXT2 from INAMS.MSTSSHOBRUI";
	public final static String ID_SQL_SSHO_BUN_FOOTER = " group by BMNCD,DAICD,CHUCD,SHOCD order by VALUE";

	// SQL:部門マスタ存在チェック
	public final static String ID_SQL_BMN_CHK_HEAD = "with INP as (select BMNCD, DAICD, CHUCD, SHOCD, SSHOCD from (values (cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar))) as X(BMNCD, DAICD, CHUCD, SHOCD, SSHOCD))";
	public final static String ID_SQL_BMN_CHK = ID_SQL_BMN_CHK_HEAD
			+ "select NVL(M1.BMNCD || '', ''), NVL(M2.DAICD || '', ''), NVL(M3.CHUCD || '', ''), NVL(M4.SHOCD || '', ''), NVL(M5.SSHOCD || '', '') from INP T1 "
			+ " left outer join INAMS.MSTBMN M1 on T1.BMNCD = M1.BMNCD and nvl(M1.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTDAIBRUI@ M2  on T1.BMNCD = M2.BMNCD and T1.DAICD = M2.DAICD and nvl(M2.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTCHUBRUI@ M3  on T1.BMNCD = M3.BMNCD and T1.DAICD = M3.DAICD and T1.CHUCD = M3.CHUCD and nvl(M3.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTSHOBRUI@ M4  on T1.BMNCD = M4.BMNCD and T1.DAICD = M4.DAICD and T1.CHUCD = M4.CHUCD and T1.SHOCD = M4.SHOCD and nvl(M4.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTSSHOBRUI M5 on T1.BMNCD = M5.BMNCD and T1.DAICD = M5.DAICD and T1.CHUCD = M5.CHUCD and T1.SHOCD = M5.SHOCD and T1.SSHOCD = M5.SSHOCD and nvl(M5.UPDKBN,0) <> 1";
	// SQL:値付分類コードチェック
	public final static String ID_SQL_KRYO_CHK_HEAD = "with INP as (select BMNCD, DAICD, CHUCD, SHOCD from (values (cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar))) as X(BMNCD, DAICD, CHUCD, SHOCD))";
	public final static String ID_SQL_KRYO_CHK = ID_SQL_KRYO_CHK_HEAD
			+ "select NVL(M1.BMNCD || '', ''), NVL(M2.DAICD || '', ''), NVL(M3.CHUCD || '', ''), NVL(M4.SHOCD || '', '') from INP T1 "
			+ " left outer join INAMS.MSTBMN M1 on T1.BMNCD = M1.BMNCD and nvl(M1.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTDAIBRUI@ M2  on T1.BMNCD = M2.BMNCD and T1.DAICD = M2.DAICD and nvl(M2.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTCHUBRUI@ M3  on T1.BMNCD = M3.BMNCD and T1.DAICD = M3.DAICD and T1.CHUCD = M3.CHUCD and nvl(M3.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTSHOBRUI@ M4  on T1.BMNCD = M4.BMNCD and T1.DAICD = M4.DAICD and T1.CHUCD = M4.CHUCD and T1.SHOCD = M4.SHOCD and nvl(M4.UPDKBN,0) <> 1";
	...
	// SQL:商品マスタ
	/** 共通（INAMS.MSTSHN） */
	public final static String ID_SQL_SHN_CHK_UPDATECNT = "select CNT as VALUE from (select nvl(max(UPDATECNT), 0) as CNT from INAAD.SYSSHNCOUNT where UPDATEDT = ?) where CNT < (select nvl(MAXSU, 0) from INAAD.SYSSHNGENDOSU)";
	public final static String ID_SQL_SHN_CHK_UPDATECNT2 = "select (select nvl(MAXSU, 0) from INAAD.SYSSHNGENDOSU) - CNT as VALUE from (select nvl(max(UPDATECNT), 0) as CNT from INAAD.SYSSHNCOUNT where UPDATEDT = ?)";
	public final static String ID_SQL_SHN_SHNCD = "select SUBSTRING(right ('0000000' || RTRIM(CHAR (SHNCD)), 8), 1, 4) || '"
			+ SEPARATOR
	...
	// SQL：商品名（漢字） TODO:
	public final static String ID_SQL_SHNKN = "select case when SHNKN is null or SHNKN = '' then '　' else SHNKN end as F1 from INAMS.MSTSHN "
			+ DefineReport.ID_SQL_CMN_WHERE + " and SHNCD = ?";
	// SQL：商品一覧 TODO:
	public final static String ID_SQL_SHOHIN = DefineReport.ID_SQL_CMN_WEEK
			...
			+ " from (select SHN.UPDKBN, case when (select NNDT from INATK.HATYH_NNDT where KKKCD = SHN.KKKCD and SHNCD = SHN.SHNCD fetch first 1 rows only) = 0 then 0 else 1 end as NDTSET, SHN.CATALGNO, SHN.SHNCD, SHNM.SHNKN, SHN.HTDT, SHN.UKESTDT as UKESTDT_HIDE, SHN.UKEEDDT as UKEEDDT_HIDE, SHN.TENISTDT as TENISTDT_HIDE, SHN.TENIEDDT as TENIEDDT_HIDE, TO_CHAR(TO_DATE('20' || right ('0' || SHN.UKESTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKESTDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.UKESTDT, 6), 'YYYYMMDD')) as UKESTDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.UKEEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKEEDDT"
			+ ", DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.UKEEDDT, 6), 'YYYYMMDD')) as UKEEDDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.TENISTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENISTDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.TENISTDT, 6), 'YYYYMMDD')) as TENISTDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.TENIEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENIEDDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.TENIEDDT, 6), 'YYYYMMDD')) as TENIEDDT_WNUM, SHN.YOTEISU, SHN.GENDOSU"
	...
	public final static String ID_SQL_SHOHIN_HTS = DefineReport.ID_SQL_CMN_WEEK
			...
			+ ", T1.HTSU_T as F8, T1.YOTEISU as F9, T1.GENDOSU as F10, T1.UKESTDT as F11, T1.UKEEDDT as F12, T1.TENISTDT as F13, T1.TENIEDDT as F14, case when T1.NSTDT = T1.NEDDT then T1.NSTDT || W5.JWEEK else T1.NSTDT || W5.JWEEK || '～' || T1.NEDDT || W6.JWEEK end as F15 from (select SHN.NGFLG, SHN.SHNCD, SHNM.SHNKN, SHN.HTDT, TO_CHAR(TO_DATE('20' || right ('0' || SHN.UKESTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKESTDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.UKESTDT, 6), 'YYYYMMDD')) as UKESTDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.UKEEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKEEDDT"
			+ ", DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.UKEEDDT, 6), 'YYYYMMDD')) as UKEEDDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.TENISTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENISTDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.TENISTDT, 6), 'YYYYMMDD')) as TENISTDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || SHN.TENIEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENIEDDT, DAYOFWEEK(TO_DATE('20' || right ('0' || SHN.TENIEDDT, 6), 'YYYYMMDD')) as TENIEDDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || YHN.NSDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NSTDT"
			+ ", DAYOFWEEK(TO_DATE('20' || right ('0' || YHN.NSDT, 6), 'YYYYMMDD')) as NSTDT_WNUM, TO_CHAR(TO_DATE('20' || right ('0' || YHN.NEDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NEDDT, DAYOFWEEK(TO_DATE('20' || right ('0' || YHN.NEDT, 6), 'YYYYMMDD')) as NEDDT_WNUM, HTEN.HTSU_SUM, HTEN.HTSU_T, SHN.YOTEISU, SHN.GENDOSU, SHN.CATALGNO from INATK.HATYH_SHN SHN left join INAMS.MSTSHN SHNM on SHNM.SHNCD = SHN.SHNCD left join (select KKKCD, SHNCD, SUM(case when INPUTDT < ? then HTSU else 0 end) as HTSU_SUM, SUM(case when INPUTDT = ? then HTSU else 0 end) as HTSU_T"
	...
	public final static String ID_SQL_TOKRS = "select MTSH.SANCHIKN as F3,  MTSH.SHNKN as F4,  MTSH.KIKKN as F5, case when MTSH.HS_IRISU is NOT NULL then MTSH.HS_IRISU else MTSH.RG_IRISU END as F6 , "
			...
			+ " as F7, MTSH.RG_BAIKAAM as F8, MTSH.RG_GENKAAM as F9, MSBR.SHOBRUIKN as F13 from INAMS.MSTSHN MTSH left join INAMS.MSTMAKER MSMK on MTSH.MAKERCD = MSMK.MAKERCD left join INAMS.MSTSHOBRUI MSBR on MTSH.DAICD = MSBR.DAICD and MTSH.CHUCD = MSBR.CHUCD and MTSH.SHOCD = MSBR.SHOCD and MSBR.BMNCD = MTSH.BMNCD left outer join INAMS.MSTSHN M0 on M0.SHNCD = MTSH.SHNCD and NVL(M0.UPDKBN, 0) <> 1 left outer join INAMS.MSTBMN M1 on M1.BMNCD = MTSH.BMNCD and NVL(M1.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M2 on M2.ZEIRTKBN = M0.ZEIRTKBN and NVL(M2.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M3 on M3.ZEIRTKBN = M0.ZEIRTKBN_OLD and NVL(M3.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M4 on M4.ZEIRTKBN = M1.ZEIRTKBN and NVL(M4.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M5 on M5.ZEIRTKBN = M1.ZEIRTKBN_OLD and NVL(M5.UPDKBN, 0) <> 1 where MTSH.UPDKBN <> 1 and MTSH.SHNCD = ?";

	// SQL：冷凍食品（新売価） TODO:
	public final static String ID_SQL_BAIKAAM = "select "
			+ DefineReport.ID_SQL_TOKBAIKA_COL_SOU.replaceAll("@BAIKA", "?").replaceAll("@DT", "TRSH.HBSTDT")
			+ " as F11 from INATK.TOKRS_SHN TRSH left join INAMS.MSTSHN MTSH on TRSH.SHNCD = MTSH.SHNCD left join INAMS.MSTSHOBRUI MSBR on MTSH.DAICD = MSBR.DAICD and MTSH.CHUCD = MSBR.CHUCD and MTSH.SHOCD = MSBR.SHOCD and MSBR.BMNCD = MTSH.BMNCD left outer join INAMS.MSTSHN M0 on M0.SHNCD = TRSH.SHNCD and NVL(M0.UPDKBN, 0) <> 1 left outer join INAMS.MSTBMN M1 on M1.BMNCD = TRSH.BMNCD and NVL(M1.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M2 on M2.ZEIRTKBN = M0.ZEIRTKBN and NVL(M2.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M3 on M3.ZEIRTKBN = M0.ZEIRTKBN_OLD and NVL(M3.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M4 on M4.ZEIRTKBN = M1.ZEIRTKBN and NVL(M4.UPDKBN, 0) <> 1 left outer join INAMS.MSTZEIRT M5 on M5.ZEIRTKBN = M1.ZEIRTKBN_OLD and NVL(M5.UPDKBN, 0) <> 1 where TRSH.HBSTDT = ? and TRSH.BMNCD = ? and TRSH.WRITUKBN = ? and TRSH.SEICUTKBN = ? and TRSH.DUMMYCD = ? and TRSH.UPDKBN <> 1 and TRSH.SHNCD = ?";

	// SQL：税率区分
	public final static String ID_SQL_ZEIRT = "select value, TEXT from (values ('" + Values.NONE.getVal()
			+ "', '　')) as X(value, TEXT) union all select TO_CHAR(ZRT.ZEIRTKBN) as value, ZRT.ZEIRTKBN || '-' ||nvl(TRIM(ZRT.ZEIRTKN),'') as TEXT from INAMS.MSTZEIRT ZRT "
			+ DefineReport.ID_SQL_CMN_WHERE + " order by value";

	// SQL：分類区分
	public final static String ID_SQL_BUNRUI = "select value, TEXT from (values (1, '標準'), (2, '用途'), (3, '売場')) as X(value, TEXT) order by value";
	...
	// SQL：店舗名称(漢字)
	public final static String ID_SQL_TENKN = "select TENCD as VALUE, TENKN as TEXT from INAMS.MSTTEN  "
			+ DefineReport.ID_SQL_CMN_WHERE + DefineReport.ID_SQL_CMN_FOOTER;
	public final static String ID_SQL_TENKN2 = "select TENCD as VALUE, right ('00' || TENCD, 3) || '-' ||nvl(TENKN,'') as TEXT from INAMS.MSTTEN "
			+ DefineReport.ID_SQL_CMN_WHERE + " and TENCD = ? order by right ('00' || TENCD,3 )";
	public final static String ID_SQL_TENKN_HEAD = "select TENCD as VALUE, right ('00' || TENCD, 3) || '-' ||nvl(TENKN,'') as TEXT from INAMS.MSTTEN "
			+ DefineReport.ID_SQL_CMN_WHERE + "order by right ('00' || TENCD,3 )";
	public final static String ID_SQL_TENKN_HEAD2 = "select right ('00' || T1.TENCD, 3) as VALUE, right ('00' || T1.TENCD, 3) || '-' || T2.TENKN as TEXT from INATK.TOKTG_QATEN T1 left join INAMS.MSTTEN T2 on T2.TENCD = T1.TENCD where MOYSKBN = ? and T1.MOYSSTDT = ? and T1.MOYSRBAN = ? and T1.KYOSEIFLG = ? and T1.TENGPCD = ? and T2.TENCD = T1.TENCD order by VALUE";

	// SQL：センターコード
	public final static String ID_SQL_CENTER_HEAD = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_CENTER = "SELECT DISTINCT CENTERCD AS VALUE, CENTERCD TEXT FROM INAORR.ORRCOURSEMASTER WHERE TO_CHAR(EFFECTIVESTARTDATE,'YYYYMMDD') <= ? AND TO_CHAR(EFFECTIVEENDDATE,'YYYYMMDD') >= ? AND LOGICALDELFLG = "
			+ DefineReport.ValUpdkbn.NML.getVal() + " ORDER BY VALUE";

	// SQL：便コード
	public final static String ID_SQL_SUPPLYNO_HEAD = "select VALUE, TEXT from (values ('" + Values.NONE.getVal()
			+ "', '　')) as X(value, TEXT) union all ";
	...
	// SQL：商品区分
	public final static String ID_SQL_SHNKBN = "select MEISHOKBN as KBN, MEISHOCD as value, MEISHOKBNKN as KTEXT, MEISHOCD || '-' ||nvl(RTRIM(NMKN),'') as TEXT, RTRIM(NMKN) as TEXT2, TNMKN as STEXT, NMAN as KANA from INAMS.MSTMEISHO where MEISHOKBN = '10502' and MEISHOCD = 0 order by KBN, value, TEXT";
	...
	public final static String ID_SQL_SIR_ = "select * from INAMS.MSTSIR " + DefineReport.ID_SQL_CMN_WHERE
			+ " and SIRCD = ?";
	...
	public final static String ID_SQL_ZITSIR = ID_SQL_GRD_CMN
			+ " select T1.IDX, T2.RSIRCD, T3.SIRKN as SIRKN_R from T1 left join (select ROW_NUMBER() over (order by SIRCD) as IDX, * from INAMS.MSTFUKUSUSIR_R where SIRCD = ? and UPDKBN = "
			+ DefineReport.ValUpdkbn.NML.getVal() + " order by SIRCD fetch first @M rows only) T2 on T1.IDX = T2.SEQNO"
			+ " left join INAMS.MSTSIR T3 on T3.SIRCD = T2.RSIRCD order by T1.IDX";
	...
	// SQL：エリア別配送パターン TODO:
	/** 共通（INAMS.MSTHSPTN） */
	public final static String ID_SQL_EHSPTN = "select right('000' || EHPTN.HSPTN, 3) as F1, MAX(HPTN.HSPTNKN) as F2, case when EHPTN.AREAKBN = 1 then right ('0000' || EHPTN.TENGPCD, 4) when EHPTN.AREAKBN = 0 then right ('00' || EHPTN.TENGPCD, 2) end as F3, MAX(HTG.TENGPKN) as F4, right ('000' || MAX(EHPTN.CENTERCD), 3) as F5, right ('000' || MAX(trim(EHPTN.YCENTERCD)), 3) as F6"
	...
	// SQL：リードタイムパターン TODO:
	/** 共通（INAMS.MSTREADTM） */
	public final static String ID_SQL_READTMPTN_HEAD = "select VALUE, TEXT, TEXT as TEXT2 from (values ("
			+ Values.NONE.getVal() + ", '　')) as X(value, TEXT) union all ";
	public final static String ID_SQL_READTMPTN = "select READTMPTN as VALUE, right('000'||READTMPTN,3)||'" + SEPARATOR
			+ "'||nvl(rtrim(rtrim(READTMPTNKN), '　'),'') as TEXT, rtrim(rtrim(READTMPTNKN), '　') as TEXT2 from INAMS.MSTREADTM"
			+ ID_SQL_CMN_WHERE;
	...
	public final static String ID_SQL_SRCCD_JAN = " select " + ID_SQL_SRCCD_COL + " ,T.JAN1NO" + " from ("
			+ "  select SHNCD,SRCCD,YOYAKUDT,SEQNO,SOURCEKBN,YUKO_STDT,YUKO_EDDT"
			+ "  ,row_number() over(partition by SHNCD,YOYAKUDT order by nvl(SEQNO, 9),case when STNO <= 0 then abs(STNO) end,case when STNO > 0 then STNO end,SRCCD) as JAN1NO"
			+ "  from (select " + ID_SQL_SRCCD_COL
			+ ",days(to_date(T.YUKO_STDT,'YYYYMMDD'))-days(current date) as STNO,days(to_date(T.YUKO_EDDT,'YYYYMMDD'))-days(current date) as EDNO from @T ) WK"
			+ " ) T";
	public final static String ID_SQL_SRCCD_SEL = " select trim(T.SRCCD) as SRCCD, T.SOURCEKBN||'" + SEPARATOR
			+ "'||RTRIM(T2.NMKN) as SOURCEKBN, T.SOURCEKBN as SOURCEKBN2, nvl(right(T.YUKO_STDT, 6),'') as YUKO_STDT, nvl(right(T.YUKO_EDDT,6),'') as YUKO_EDDT, nvl(to_char(SEQNO), '') as SEQNO";
	public final static String ID_SQL_SRCCD_JOIN = " left outer join INAMS.MSTMEISHO T2 on T2.MEISHOKBN = "
			+ MeisyoSelect.KBN136.getCd() + " and TO_CHAR(T.SOURCEKBN) = T2.MEISHOCD";
	public final static String ID_SQL_SRCCD1 = ID_SQL_SRCCD_SEL + " from (select " + ID_SQL_SRCCD_COL + " from @T) T"
			+ ID_SQL_SRCCD_JOIN + " order by T.SEQNO";
	public final static String ID_SQL_SRCCD2 = ID_SQL_SRCCD_SEL + " from (" + ID_SQL_SRCCD_JAN + ") T"
			+ ID_SQL_SRCCD_JOIN + " order by T.JAN1NO";

	// SQL：配送グループ
	public final static String ID_SQL_HSGP = "select right('0000'||HSGPCD, 4) as F1 ,HSGPKN as F2 from INAMS.MSTHSGP "
			+ ID_SQL_CMN_WHERE + " and AREAKBN = ?";
	...
	public static final String ID_SQL_MSTGRP_Y = ID_SQL_GRD_CMN + "select T2.GRPID, T3.GRPKN from T1"
			...
			+ " left outer join INAMS.MSTGROUP T3 on T2.GRPID = T3.GRPID";
	public static final String ID_SQL_MSTGROUP = "select GRPID as " + VAL + ", GRPKN as " + TXT
			+ " from INAMS.MSTGROUP " + ID_SQL_CMN_WHERE + " and GRPKN like ? order by GRPKN fetch first @M rows only ";
	public static final String ID_SQL_MSTGROUP2 = "with INP as (select IDX, GRPKN from (values @V) as X(IDX, GRPKN))"
			+ "select T2.GRPID, T1.GRPKN from INP T1 left outer join INAMS.MSTGROUP T2 on T1.GRPKN = T2.GRPKN and nvl(T2.UPDKBN,0) <> 1 order by T1.IDX, T2.GRPID";

	// SQL：商品店グループ TODO:
	/** 共通（INAMS.MSTSHNTENGP） */
	public final static String ID_SQL_TENGP = "select T1.TENGPCD as VALUE, T1.TENGPCD||'" + SEPARATOR
			+ "'||rtrim(rtrim(T1.TENGPKN), '　') as TEXT, T1.TENGPKN as TEXT2, T1.AREAKBN from INAMS.MSTSHNTENGP T1 "
			+ ID_SQL_CMN_WHERE
	...
	public final static String ID_SQL_TENGP_CHK_TEN_CNT = "select count(TENCD) as CNT from INAMS.MSTSHNTENGP T1 inner join INAMS.MSTSHNTENGPTEN T2"
			+ " on T1.GPKBN = T2.GPKBN and T1.BMNCD = T2.BMNCD and T1.TENGPCD = T2.TENGPCD and T1.GPKBN = ? and T1.BMNCD = ? and T1.AREAKBN = ? and T1.TENGPCD in (@) and nvl(T1.UPDKBN,0) <> 1 group by TENCD having count(TENCD) > 1";

	/** 仕入グループ（INAMS.MSTSIRGPSHN） */
	public final static String ID_SQL_TENGP_SIR = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.SIRCD, T2.SIRCD as SSIRCD, T4.SIRKN, T2.HSPTN, T5.HSPTNKN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.SIR.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 "
			+ " left outer join INAMS.MSTSIR T4 on T2.SIRCD = T4.SIRCD and nvl(T4.UPDKBN,0) <> 1 "
			+ " left outer join INAMS.MSTHSPTN T5 on T2.HSPTN = T5.HSPTN and nvl(T5.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_SIR_Y = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.SIRCD, T2.SIRCD as SSIRCD, T4.SIRKN, T2.HSPTN, T5.HSPTNKN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSIR T4 on T2.SIRCD = T4.SIRCD and nvl(T4.UPDKBN,0) <> 1 "
			+ " left outer join INAMS.MSTHSPTN T5 on T2.HSPTN = T5.HSPTN and nvl(T5.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_SIR_C = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.SIRCD, T2.SIRCD as SSIRCD, T4.SIRKN, T2.HSPTN, T5.HSPTNKN, T2.AREAKBN from T1"
	...
	public final static String ID_SQL_TENGP_BAIKA = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.GENKAAM, T2.BAIKAAM, '' as BG_SOUBAIKA, '' as BG_NEIRE, T2.IRISU, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ORDER BY T2.TENGPCD ";
	public final static String ID_SQL_TENGP_BAIKA_Y = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.GENKAAM, T2.BAIKAAM, '' as BG_SOUBAIKA, '' as BG_NEIRE, T2.IRISU, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ORDER BY T2.TENGPCD ";
	public final static String ID_SQL_TENGP_BAIKA_C = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.GENKAAM, T2.BAIKAAM, '' as BG_SOUBAIKA, '' as BG_NEIRE, T2.IRISU, T2.AREAKBN from T1"
	...
	public final static String ID_SQL_TENGP_SHINA = ID_SQL_GRD_CMN + "select T2.TENGPCD, T3.TENGPKN, T2.ATSUKKBN||'"
			+ SEPARATOR + "'||T4.NMKN as ATSUKKBN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.SHINA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 "
			+ " left outer join INAMS.MSTMEISHO T4 on T4.MEISHOKBN = " + MeisyoSelect.KBN139.getCd()
			+ " and TO_CHAR(T2.ATSUKKBN) = T4.MEISHOCD";
	public final static String ID_SQL_TENGP_SHINA_Y = ID_SQL_GRD_CMN + "select T2.TENGPCD, T3.TENGPKN, T2.ATSUKKBN||'"
			+ SEPARATOR + "'||T4.NMKN as ATSUKKBN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTMEISHO T4 on T4.MEISHOKBN = " + MeisyoSelect.KBN139.getCd()
			+ " and TO_CHAR(T2.ATSUKKBN) = T4.MEISHOCD";
	public final static String ID_SQL_TENGP_SHINA_C = ID_SQL_GRD_CMN + "select T2.TENGPCD, T3.TENGPKN, T2.ATSUKKBN||'"
			+ SEPARATOR + "'||T4.NMKN as ATSUKKBN, T2.AREAKBN from T1"
	...
	public final static String ID_SQL_TENGP_TBMN = ID_SQL_GRD_CMN
			+ "select T2.TENSHNCD,T2.TENGPCD, T3.TENGPKN, T2.AREAKBN, trim(T2.SRCCD) as SRCCD from T1"
			+ " left outer join (select ROW_NUMBER() over (order by TENGPCD) as IDX, * from INAMS.MSTSHNTENBMN where SHNCD like ? and AREAKBN = ? order by TENGPCD fetch first @M rows only) T2 on T1.IDX = T2.IDX"
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_TBMN_Y = ID_SQL_GRD_CMN
			+ "select T2.TENSHNCD,T2.TENGPCD, T3.TENGPKN, T2.AREAKBN, trim(T2.SRCCD) as SRCCD from T1"
			+ " left outer join (select ROW_NUMBER() over (order by TENGPCD) as IDX, * from INAMS.MSTSHNTENBMN_Y where SHNCD like ?  and YOYAKUDT = ? and AREAKBN = ? order by TENGPCD fetch first @M rows only) T2 on T1.IDX = T2.IDX"
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_TBMN_C = ID_SQL_GRD_CMN
			+ "select T2.TENSHNCD,T2.TENGPCD, T3.TENGPKN, T2.AREAKBN, trim(T2.SRCCD) as SRCCD from T1"
			+ " left outer join (select ROW_NUMBER() over (order by TENGPCD) as IDX, * from INAMS.CSVMSTSHNTENBMN where SEQ = ? and INPUTNO = ? order by TENGPCD fetch first @M rows only) T2 on T1.IDX = T2.IDX"
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ";

	// SQL：添加物 TODO:
	/** 共通（INAMS.MSTTENKABUTSU） */
	public final static String ID_SQL_ALLERGY2 = "select T1.VALUE, T1.TEXT, case when T2.TENKABCD is null then '' else '1' end as SEL from ("
			+ DefineReport.ID_SQL_MEISYO + ") T1"
			+ " left outer join @T T2 on T2.TENKABCD = T1.VALUE @W order by VALUE";
	public final static String ID_SQL_TENKABUTSU2 = ID_SQL_GRD_CMN + "select T2.TENKABCD2||'" + SEPARATOR
			+ "'||RTRIM(T3.NMKN) as TENKABCD, T2.TENKABCD2 as TENKABCD2 from T1"
			...
			+ " left outer join INAMS.MSTMEISHO T3 on T3.MEISHOKBN = " + MeisyoSelect.KBN138.getCd()
			+ " and T2.TENKABCD2= T3.MEISHOCD ";

	// SQL：自動発注
	/** 共通（INAMS.MSTAHS） */
	public final static String ID_SQL_AHS = "select M1.TENCD, M1.TENKN, nvl(T1.AHSKB, '" + Values.OFF.getVal()
			+ "') as AHSKB, M2.TENITEMSU, M1.URIAERACD from INAMS.MSTTEN M1"
			+ " left outer join INAMS.MSTTENJSKI M2 on M1.TENCD = M2.TENCD "
			+ " left outer join @T T1  on M1.TENCD = T1.TENCD @W" + " where M1.UPDKBN = 0 order by TENCD";

	// SQL：催し_デフォルト設定
	/** 共通（INATK.TOKMOYDEF） */
	public static final String ID_SQL_TOKMOYDEF = "select BMNCD as " + VAL
			+ " from INATK.TOKMOYDEF where nvl(UPDKBN, 0) = 0";
	public static final String ID_SQL_TOKMOYDEF2 = "select BMNCD, SZKCD, HBSLIDEKBN, NNSLIDEKBN, KSGRPKBN, DSUEXKBN, DRTEXKBN, DZNENDSKBN, DDNENDSKBN, DCUTEXKBN, DCHIRASKBN, DBMNATRKBN from INATK.TOKMOYDEF where nvl(UPDKBN, 0) = 0";

	// SQL：週間発注計画_計画計表示
	/** 共通（INAMS.MSTAHS） */
	public final static String ID_SQL_KKH_TJ009 = "with WK_YSN as(select LSTNO, TENCD, BMNCD, SUM(NVL(BMNYSANAM, 0)) as BMNYSANAM_KEI from INATK.TOKTJ_BMNYSAN group by LSTNO, TENCD, BMNCD) select T3.LSTNO, T3.BMNCD, T3.TENCD, W1.BMNYSANAM_KEI, T3.HYOSEQNO, T3.IRISU_TB, T3.BAIKAAM_TB, T3.GENKAAM_MAE, T3.SHNKBN_01, T3.SHNKBN_02, T3.SHNKBN_03, T3.SHNKBN_04, T3.SHNKBN_05, T3.SHNKBN_06, T3.SHNKBN_07, T3.SHNKBN_08, T3.SHNKBN_09, T3.SHNKBN_10, T3.HTSU_01, T3.HTSU_02, T3.HTSU_03, T3.HTSU_04, T3.HTSU_05, T3.HTSU_06, T3.HTSU_07, T3.HTSU_08, T3.HTSU_09, T3.HTSU_10 from (select LSTNO from INATK.TOKTJ group by LSTNO) T1 inner join INATK.TOKTJ_SHN T2 on T1.LSTNO = T2.LSTNO inner join INATK.TOKTJ_TEN T3 on T2.LSTNO = T3.LSTNO and T2.BMNCD = T3.BMNCD and T2.HYOSEQNO = T3.HYOSEQNO left join WK_YSN W1 on W1.LSTNO = T3.LSTNO and W1.TENCD = T3.TENCD and W1.BMNCD = T3.BMNCD where T3.LSTNO = ? and T3.BMNCD = ? and T3.TENCD = ? ";

	// SQL：B/M番号一覧
	public final static String ID_SQL_BMNNO = "SELECT ROW_NUMBER() OVER (ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO) AS IDX,BMNNO,CASE WHEN (HBSTDTD IS NULL OR HBSTDTD='') AND (HBEDDTD IS NULL OR HBEDDTD='') THEN '' WHEN HBSTDTD = HBEDDTD THEN HBSTDTD || HBSTDTW ELSE HBSTDTD || HBSTDTW || '～' || HBEDDTD || HBEDDTW END AS HBPERIOD,BMNMAN,MOYSKBN,MOYSSTDT,MOYSRBAN FROM (SELECT MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,TO_CHAR(TO_DATE(HBSTDT,'YYYYMMDD'),'YY/MM/DD') AS HBSTDTD,TO_CHAR(TO_DATE(HBEDDT,'YYYYMMDD'),'YY/MM/DD') AS HBEDDTD,BMNMAN,CASE WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='1' THEN '(日)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='2' THEN '(月)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='3' THEN '(火)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='4' THEN '(水)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='5' THEN '(木)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='6' THEN '(金)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='7' THEN '(土)' END AS HBSTDTW,CASE WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='1' THEN '(日)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='2' THEN '(月)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='3' THEN '(火)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='4' THEN '(水)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='5' THEN '(木)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='6' THEN '(金)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='7' THEN '(土)' END AS HBEDDTW FROM INATK.TOKBM WHERE MOYSKBN=? AND MOYSSTDT = ? AND MOYSRBAN=? AND UPDKBN="
			+ DefineReport.ValUpdkbn.NML.getVal() + " ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO)";
	// SQL：B/M商品一覧
	public final static String ID_SQL_BMSHN = ID_SQL_GRD_CMN
			+ "SELECT T1.IDX,'0' AS UPDKBN, trim(T2.SHNCD) AS SHNCD,T2.SHNKN,T2.GENKAAM,T2.MOYSKBN,T2.MOYSSTDT,T2.MOYSRBAN,T2.BMNNO,T2.KANRINO FROM T1 LEFT JOIN (SELECT ROW_NUMBER() OVER (ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,KANRINO,SHNCD) AS IDX,MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,KANRINO,SHNCD,SHNKN,GENKAAM FROM (SELECT MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,KANRINO,MS.SHNCD,MS.SHNKN,GENKAAM FROM (SELECT MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,KANRINO,SHNCD,GENKAAM FROM INATK.TOKBM_SHN WHERE MOYSKBN=? AND MOYSSTDT=? AND MOYSRBAN=? AND BMNNO=? ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,BMNNO,KANRINO,SHNCD) BM LEFT JOIN INAMS.MSTSHN MS ON BM.SHNCD=MS.SHNCD)) AS T2 ON T1.IDX=T2.IDX ORDER BY T1.IDX";
	// SQL：B/M番号一覧
	public final static String ID_SQL_TOKBM_D1 = "SELECT DISTINCT 'D1' D1_1,right('0'||NVL(TO_CHAR(BM.MOYSKBN),''),1) D1_2,right('000000'||NVL(TO_CHAR(BM.MOYSSTDT),''),6) D1_3,right('000'||NVL(TO_CHAR(BM.MOYSRBAN),''),3) D1_4,left(NVL(MY.MOYKN,'')||'　　　　　　　　　　　　　　　　　　　　',40) D1_5,right('00'||SUBSTR(NVL(TO_CHAR(MY.SHUNO),'0000'),1,2),2) D1_6,right('00'||SUBSTR(NVL(TO_CHAR(MY.SHUNO),'0000'),3,2),2) D1_7,right('00000000'||NVL(TO_CHAR(MY.HBSTDT),''),8) D1_8,right('00000000'||NVL(TO_CHAR(MY.HBEDDT),''),8) D1_9,right('00000000'||NVL(TO_CHAR(MY.NNSTDT),''),8) D1_10,right('00000000'||NVL(TO_CHAR(MY.NNEDDT),''),8) D1_11 FROM INATK.TOKBM BM LEFT JOIN INATK.TOKMOYCD MY ON BM.MOYSKBN=MY.MOYSKBN AND BM.MOYSSTDT=MY.MOYSSTDT AND BM.MOYSRBAN=MY.MOYSRBAN WHERE BM.MOYSKBN=?AND BM.MOYSSTDT=?AND BM.MOYSRBAN=?AND BM.UPDKBN="
			+ DefineReport.ValUpdkbn.NML.getVal();
	// SQL：B/M番号一覧
	public final static String ID_SQL_TOKBM_D2 = "SELECT 'D2' D2_1,right('0'||NVL(BM.MOYSKBN,''), 1) D2_2,right('000000'||NVL(BM.MOYSSTDT,''), 6) D2_3,right('000'||NVL(TO_CHAR(BM.MOYSRBAN),''), 3) D2_4,right('0000'||NVL(TO_CHAR(BM.BMNNO),''), 4) D2_5,left(NVL(BM.BMNMAN,'')||'                    ', 20) D2_6,left(NVL(BM.BMNMKN,'')||'　　　　　　　　　　　　　　　　　　　　', 40) D2_7,right('000000'||NVL(TO_CHAR(BM.BAIKAAM),''), 6) D2_8,right('000'||NVL(TO_CHAR(BM.BD_KOSU1),''), 3) D2_9,right('000000'||NVL(TO_CHAR(BM.BD_BAIKAAN1),''), 6) D2_10,right('000'||NVL(TO_CHAR(BM.BD_KOSU2),''), 3) D2_11,right('000000'||NVL(TO_CHAR(BM.BD_BAIKAAN2),''), 6) D2_12,right('00000000'||NVL(TO_CHAR(BM.HBSTDT),''), 8) D2_13,right('00000000'||NVL(TO_CHAR(BM.HBEDDT),''), 8) D2_14,right('000'||NVL(TO_CHAR(BM.BMNCD_RANK),''), 3) D2_15,right('000'||NVL(TO_CHAR(BM.RANKNO_ADD),''), 3) D2_16,right('000'||NVL(TO_CHAR(BM.RANKNO_DEL),''), 3) D2_17,right('000'||NVL(TO_CHAR(TJ.TENCD),''), 3) D2_18_1,NVL(TO_CHAR(TJ.TJFLG),'') D2_18_2 FROM INATK.TOKBM BM LEFT JOIN INATK.TOKBM_TJTEN TJ ON BM.MOYSKBN=TJ.MOYSKBN AND BM.MOYSSTDT=TJ.MOYSSTDT AND BM.MOYSRBAN=TJ.MOYSRBAN AND BM.BMNNO=TJ.BMNNO WHERE BM.MOYSKBN=?AND BM.MOYSSTDT=?AND BM.MOYSRBAN=?AND BM.UPDKBN="
			+ DefineReport.ValUpdkbn.NML.getVal()
			+ " ORDER BY BM.MOYSKBN,BM.MOYSSTDT,BM.MOYSRBAN,BM.BMNNO,BM.BMNMAN,TJ.TJFLG DESC,TJ.TENCD";
	// SQL：B/M番号一覧
	public final static String ID_SQL_TOKBM_D3 = "SELECT 'D3' D3_1,right('0'||NVL(TO_CHAR(BM.MOYSKBN),''), 1) D3_2,right('000000'||NVL(TO_CHAR(BM.MOYSSTDT),''), 6) D3_3,right('000'||NVL(TO_CHAR(BM.MOYSRBAN),''), 3) D3_4,right('0000'||NVL(TO_CHAR(BM.BMNNO),''), 4) D3_5,left(NVL(BM.SHNCD,'')||'              ', 14) D3_6,right('00000000'||NVL(TO_CHAR(BM.GENKAAM),''), 8) D3_7,left(NVL(MS.POPKN,'')||'　　　　　　　　　　　　　　　　　　　　', 40) D3_8,left(NVL(MM.MAKERKN,'')||'　　　　　　　　　　　　　　', 28) D3_9,left(NVL(MS.KIKKN,'')||'　　　　　　　　　　　　　　　　　　　　　　　', 46) D3_10 FROM INATK.TOKBM_SHN BM LEFT JOIN INAMS.MSTSHN MS ON BM.SHNCD = MS.SHNCD LEFT JOIN INAMS.MSTMAKER MM ON MS.MAKERCD=MM.MAKERCD WHERE BM.MOYSKBN=?AND BM.MOYSSTDT=?AND BM.MOYSRBAN=? ORDER BY BM.MOYSKBN,BM.MOYSSTDT,BM.MOYSRBAN,BM.BMNNO,BM.SHNCD,MM.MAKERCD";

	// SQL：セット番号一覧
	public final static String ID_SQL_SETNO = ID_SQL_GRD_CMN
			+ "SELECT T1.IDX,right('0000'||T2.STNO, 4) as STNO ,T2.HBPERIOD,T2.STNM,T2.MOYSKBN,T2.MOYSSTDT,T2.MOYSRBAN FROM T1 LEFT JOIN (SELECT ROW_NUMBER() OVER (ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,STNO) AS IDX,STNO,CASE WHEN (HBSTDTD IS NULL OR HBSTDTD='') AND (HBEDDTD IS NULL OR HBEDDTD='') THEN '' WHEN HBSTDTD = HBEDDTD THEN HBSTDTD || HBSTDTW ELSE HBSTDTD || HBSTDTW || '～' || HBEDDTD || HBEDDTW END AS HBPERIOD,STNM,MOYSKBN,MOYSSTDT,MOYSRBAN FROM (SELECT MOYSKBN,MOYSSTDT,MOYSRBAN,STNO,TO_CHAR(TO_DATE(HBSTDT,'YYYYMMDD'),'YY/MM/DD') AS HBSTDTD,TO_CHAR(TO_DATE(HBEDDT,'YYYYMMDD'),'YY/MM/DD') AS HBEDDTD,STNM,CASE WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='1' THEN '(日)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='2' THEN '(月)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='3' THEN '(火)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='4' THEN '(水)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='5' THEN '(木)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='6' THEN '(金)' WHEN DAYOFWEEK(TO_DATE(HBSTDT,'YYYYMMDD'))='7' THEN '(土)' END AS HBSTDTW,CASE WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='1' THEN '(日)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='2' THEN '(月)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='3' THEN '(火)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='4' THEN '(水)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='5' THEN '(木)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='6' THEN '(金)' WHEN DAYOFWEEK(TO_DATE(HBEDDT,'YYYYMMDD'))='7' THEN '(土)' END AS HBEDDTW FROM INATK.TOKMM_KKK WHERE MOYSKBN=? AND MOYSSTDT = ? AND MOYSRBAN=? AND UPDKBN="
			+ DefineReport.ValUpdkbn.NML.getVal()
			+ " ORDER BY MOYSKBN,MOYSSTDT,MOYSRBAN,STNO)) AS T2 ON T1.IDX=T2.IDX ORDER BY T1.IDX";
	// SQL :セット販売商品一覧
	...
	// SQL：正規定量商品店一覧
	public final static String ID_SQL_HATSTRSHNTEN = "SELECT * FROM(SELECT 1 AS NUM,'月' AS WEEK,MEISHOCD||'" + SEPARATOR
			+ "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 2 AS NUM,'火' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 3 AS NUM,'水' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 4 AS NUM,'木' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 5 AS NUM,'金' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 6 AS NUM,'土' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0' UNION ALL SELECT 7 AS NUM,'日' AS WEEK,MEISHOCD||'"
			+ SEPARATOR + "'||RTRIM(NMKN) AS TEISEIKBN4,0 AS SURYO FROM INAMS.MSTMEISHO WHERE MEISHOKBN = '"
			+ MeisyoSelect.KBN105014.getCd() + "' AND MEISHOCD='0') ORDER BY NUM";
	// SQL：正規定量商品店一覧
	public final static String ID_SQL_HATSTRSHNTEN2 = "SELECT TEN.TENCD,MST.TENKN,TEN.SURYO_MON,TEN.SURYO_TUE,TEN.SURYO_WED,TEN.SURYO_THU,TEN.SURYO_FRI,TEN.SURYO_SAT,TEN.SURYO_SUN FROM INATK.HATSTR_TEN TEN LEFT JOIN INAMS.MSTTEN MST ON TEN.TENCD=MST.TENCD WHERE TEN.SHNCD=?AND TEN.BINKBN=?AND TEN.UPDKBN="
			+ DefineReport.ValUpdkbn.NML.getVal() + " ORDER BY TEN.TENCD";
	// SQL：入力済商品一覧
	public final static String ID_SQL_HATSTRTEN = "WITH SHORI as(" + ID_SQLSHORIDT
			...
			+ VAL
			+ ",'YYYY/MM/DD') AND TN.TENCD=? AND NVL(TN.UPDKBN, 0) <> 1 AND NVL(MS.UPDKBN, 0) <> 1 ORDER BY TN.SHNCD";
	public final static String ID_SQL_HATJTRTEN = "WITH SHORI as(" + ID_SQLSHORIDT
			+ ") SELECT TN.SHNCD,MS.SHNKN,TN.SURYO_MON,TN.SURYO_TUE,TN.SURYO_WED,TN.SURYO_THU,TN.SURYO_FRI,TN.SURYO_SAT,TN.SURYO_SUN,TO_CHAR(TN.ADDDT,'YY/MM/DD') AS ADDDT,TO_CHAR(TN.UPDDT,'YY/MM/DD') AS UPDDT,TN.OPERATOR FROM INATK.HATJTR_TEN TN,INAMS.MSTSHN MS,SHORI WHERE TN.SHNCD=MS.SHNCD AND TO_DATE(TO_CHAR(TN.UPDDT,'YYYYMMDD'),'YYYY/MM/DD')=TO_DATE(SHORI."
			+ VAL + ",'YYYY/MM/DD') AND TN.TENCD=? AND TN.SHUNO=? AND NVL(MS.UPDKBN, 0) <> 1 ORDER BY TN.SHNCD";

	// SQL：風袋マスタリスト TODO:
	/** 共通（INAMS.MSTFTAI） */
	public final static String ID_SQL_FUTAI = ID_SQL_GRD_CMN
			+ "select  MSFT.FTAISHUKBN, MSFT.FTAIECD,CONCAT(CONCAT(CONCAT(MSFT.FTAISHUKBN, MSFT.FTAIECD),'-' ),MSFT.FTAIKN ) as FUTAI  from INAMS.MSTFTAI MSFT where MSFT.FTAISHUKBN = ";
	...
	public final static String ID_SQL_TENHTSU_ARR = "select T1.TENCD,case when SUBSTRING(T2.TENHTSU_ARR, 1 + (T1.TENCD - 1) * 5, 5) = '' then null else T1.TENKN END as TENKN,case when SUBSTRING(T2.TENHTSU_ARR, 1 + (T1.TENCD - 1) * 5, 5) = '' then null else cast(SUBSTRING(T2.TENHTSU_ARR, 1 + (T1.TENCD - 1) * 5, 5) as INTEGER) END as SURYO from INAMS.MSTTEN as T1 left join  ? T2 on T2.MOYSKBN = ? and T2.MOYSSTDT = ? and T2.MOYSRBAN = ? and T2.KANRINO = ? where T1.TENCD <= ? and T1.MISEUNYOKBN <> 9 and T1.UPDKBN = 0 order by right('00'||T1.TENCD,3)";
	// 店発注数配列
	public final static String ID_SQL_SURYO2 = "select T1.SRYPTNNO as F1,max(T1.SRYPTNKN) as F2,sum(T2.SURYO) as F3 @C, T1.SRYPTNNO as IDX"
			+ " from (select BMNCD,SRYPTNNO,SRYPTNKN from INATK.TOKSRPTN where BMNCD = ? and UPDKBN = 0) T1"
			+ " left outer join INATK.TOKSRYRANK T2 on T1.BMNCD = T2.BMNCD and T1.SRYPTNNO = T2.SRYPTNNO"
	...
	public final static String ID_SQL_RANK = "select right('000'||RANKNO,3) as F1, RANKKN as F2,length(REGEXP_REPLACE(TENRANK_ARR, '[^A-Z]', '')) as F3, RANKNO as F4 from INATK.TOKRANK where nvl(UPDKBN, 0) <> 1 and BMNCD = ? order by RANKNO";
	...
	public final static String ID_SQL_RANKEX = "select right('000'||RANKNO,3) as F1, RANKKN as F2,length(REGEXP_REPLACE(TENRANK_ARR, '[^A-Z]', '')) as F3, RANKNO as F4, MOYSKBN as F5, MOYSSTDT as F6, MOYSRBAN as F7 "
			+ " from INATK.TOKRANKEX where nvl(UPDKBN, 0) <> 1 and BMNCD = ? and MOYSKBN = ? and MOYSSTDT = ? and MOYSRBAN = ? order by RANKNO";
	/** ランク・臨時マスタ共通 */
	...
	// TENCD, BMNCD, AREACD from INAMS.MSTTENBMN where BMNCD=? and nvl(UPDKBN,
	...
	// where BMNCD=? and nvl(UPDKBN, 0) <> 1) T2 on T1.TENCD=T2.TENCD where
	// T1.TENCD <=400";
	...
	// NVL(TEN.UPDKBN, 0) = 0 order by TEN.TENCD";
	// // SQL：通常率パターン店別分配率
	...
	// 400 and NVL(TEN.UPDKBN, 0) = 0 order by TEN.TENCD";
	...
	// TEN.TENCD <= 400 and NVL(TEN.UPDKBN, 0) = 0 order by TEN.TENCD";
	...
	public final static String ID_SQL_RANKSURYO_CHANGE_EX = "select SRYRANKEX.TENRANK as RANK, SRYRANKEX.SURYO from INATK.TOKSRYRANKEX SRYRANKEX where SRYRANKEX.BMNCD = ? and SRYRANKEX.SRYPTNNO = ? and SRYRANKEX.MOYSKBN = ? and SRYRANKEX.MOYSSTDT = ? and SRYRANKEX.MOYSRBAN = ? order by SRYRANKEX.TENRANK";

	// SQL：社員情報
	/** ログイン管理テーブル（INAAD.SYSLOGIN T1） */
	public final static String ID_SQL_SYSLOGIN = "select T1.SHAINCD as VALUE, RIGHT('00'||ROW_NUMBER() over(order by T1.SIMEIKN),2)||T1.SIMEIKN as TEXT, T1.SIMEIKN as TEXT2, RIGHT('00'||ROW_NUMBER() over(order by T1.SIMEIKN),2) as TEXT3 from (SELECT USER_ID AS SHAINCD, CASE WHEN YOBI_3 IS NULL OR YOBI_3='' THEN 0 ELSE CAST(YOBI_3 AS SMALLINT) END AS SZKCD, NM_FAMILY || NM_NAME AS SIMEIKN FROM KEYSYS.SYS_USERS) T1";
	public final static String ID_SQL_SYSLOGIN2 = "select T1.SHAINCD as VALUE, T1.SHAINCD||T1.SIMEIKN as TEXT, T1.SIMEIKN as TEXT2, T1.SHAINCD as TEXT3 from (SELECT USER_ID AS SHAINCD, CASE WHEN YOBI_3 IS NULL OR YOBI_3='' THEN 0 ELSE CAST(YOBI_3 AS SMALLINT) END AS SZKCD, NM_FAMILY || NM_NAME AS SIMEIKN FROM KEYSYS.SYS_USERS) T1";
	...
	public final static String ID_SQL_MD03100902 = "with FU as (select STARTNO, ENDNO, SUMINO from INAAD.SYSURICD_FU fetch first 1 rows only)"
			+ ", AKI as (select MIN(T1.URICD) as MIN_AKI_URICD from INAAD.SYSURICD_AKI T1 inner join FU T2 on T1.URICD between T2.STARTNO and T2.ENDNO and NVL(T1.USEFLG, 0) <> 1)"
	...
	public final static String ID_SQL_MD03112501 = "with INP as (select CD, KBN, BMNCD from (values (cast(? as varchar (14)), cast(? as varchar (2)), cast(? as varchar (2)))) as X(CD, KBN, BMNCD))"
			+ "select case when CD='' then right('0'||BMNCD,2)||'00001'"
			+ " when KBN='1' and left(CD,2)='45' and SUBSTR(CD,3,1)<= 5 then left(CD,7)"
			+ " when KBN='1' and left(CD,2)='45' and SUBSTR(CD,3,1) > 5 then left(CD,9)"
			+ " when KBN='1' and left(CD,2)='49' then left(CD,7)" + " when KBN='2' then left(CD,6)"
			+ " else right('0'||BMNCD,2)||'00002' end as value" + " from INP";

	/** SQL:添付資料（MD03111301）の総売価の取得 */
	public final static String ID_SQL_MD03111301_IN = "(values (cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar), cast(? as varchar))) as X(BMNCD, ZEIKBN, ZEIRTKBN, ZEIRTKBN_OLD, ZEIRTHENKODT, TENBAIKADT, BAIKAAM))";
	public final static String ID_SQL_MD03111301_IN_RG = "with INP as (select BMNCD, ZEIKBN, ZEIRTKBN, ZEIRTKBN_OLD, ZEIRTHENKODT, TENBAIKADT, BAIKAAM as RG_BAIKAAM from "
			+ ID_SQL_MD03111301_IN;
	public final static String ID_SQL_MD03111301_IN_HS = "with INP as (select BMNCD, ZEIKBN, ZEIRTKBN, ZEIRTKBN_OLD, ZEIRTHENKODT, TENBAIKADT, BAIKAAM as HS_BAIKAAM from "
			+ ID_SQL_MD03111301_IN;
	public final static String ID_SQL_MD03111301_COL_RG = " case when T1.ZEIKBN = 0 and nvl(T1.ZEIRTHENKODT,0) <= nvl(T4.SHORIDT, 0) then nvl(truncate(T1.RG_BAIKAAM,0),0) + nvl(truncate(decimal(T1.RG_BAIKAAM*M2.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 0 and nvl(T1.ZEIRTHENKODT,0) >  nvl(T4.SHORIDT, 0) then nvl(truncate(T1.RG_BAIKAAM,0),0) + nvl(truncate(decimal(T1.RG_BAIKAAM*M3.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) <= nvl(T4.SHORIDT, 0) then nvl(truncate(T1.RG_BAIKAAM,0),0) + nvl(truncate(decimal(T1.RG_BAIKAAM*M4.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) >  nvl(T4.SHORIDT, 0) then nvl(truncate(T1.RG_BAIKAAM,0),0) + nvl(truncate(decimal(T1.RG_BAIKAAM*M5.ZEIRT)/100, 0), 0)"
			+ " else T1.RG_BAIKAAM end";
	public final static String ID_SQL_MD03111301_COL_HS = " case when T1.ZEIKBN = 0 and nvl(T1.ZEIRTHENKODT,0) <= nvl(T4.SHORIDT, 0) then nvl(truncate(T1.HS_BAIKAAM,0),0) + nvl(truncate(decimal(T1.HS_BAIKAAM*M2.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 0 and nvl(T1.ZEIRTHENKODT,0) >  nvl(T4.SHORIDT, 0) then nvl(truncate(T1.HS_BAIKAAM,0),0) + nvl(truncate(decimal(T1.HS_BAIKAAM*M3.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) <= nvl(T4.SHORIDT, 0) then nvl(truncate(T1.HS_BAIKAAM,0),0) + nvl(truncate(decimal(T1.HS_BAIKAAM*M4.ZEIRT)/100, 0), 0)"
			+ " when T1.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) >  nvl(T4.SHORIDT, 0) then nvl(truncate(T1.HS_BAIKAAM,0),0) + nvl(truncate(decimal(T1.HS_BAIKAAM*M5.ZEIRT)/100, 0), 0)"
			+ " else T1.HS_BAIKAAM end";
	public final static String ID_SQL_MD03111301_COL_RG_Y = StringUtils.replace(ID_SQL_MD03111301_COL_RG, "T4.SHORIDT",
			"T1.TENBAIKADT");
	public final static String ID_SQL_MD03111301_COL_HS_Y = StringUtils.replace(ID_SQL_MD03111301_COL_HS, "T4.SHORIDT",
			"T1.TENBAIKADT");
	public final static String ID_SQL_MD03111301_JOIN_Y = " left outer join INAMS.MSTBMN M1 on T1.BMNCD = M1.BMNCD and nvl(M1.UPDKBN, 0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M2 on M2.ZEIRTKBN = T1.ZEIRTKBN and nvl(M2.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M3 on M3.ZEIRTKBN = T1.ZEIRTKBN_OLD and nvl(M3.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M4 on M4.ZEIRTKBN = M1.ZEIRTKBN and nvl(M4.UPDKBN,0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M5 on M5.ZEIRTKBN = M1.ZEIRTKBN_OLD and nvl(M5.UPDKBN,0) <> 1";
	public final static String ID_SQL_MD03111301_JOIN = " left outer join (select ID, SHORIDT as SHORIDT from INAAD.SYSSHORIDT where nvl(UPDKBN, 0) <> 1 fetch first 1 rows only) T4 on 1=1"
			+ ID_SQL_MD03111301_JOIN_Y;
	public final static String ID_SQL_MD03111301_RG = ID_SQL_MD03111301_IN_RG + " select " + ID_SQL_MD03111301_COL_RG
			+ " as VALUE from INP as T1" + ID_SQL_MD03111301_JOIN;
	public final static String ID_SQL_MD03111301_HS = ID_SQL_MD03111301_IN_HS + " select " + ID_SQL_MD03111301_COL_HS
			+ " as VALUE from INP as T1" + ID_SQL_MD03111301_JOIN;
	public final static String ID_SQL_MD03111301_RG_Y = ID_SQL_MD03111301_IN_RG + " select "
			+ ID_SQL_MD03111301_COL_RG_Y + " as VALUE from INP as T1" + ID_SQL_MD03111301_JOIN_Y;
	public final static String ID_SQL_MD03111301_HS_Y = ID_SQL_MD03111301_IN_HS + " select "
			+ ID_SQL_MD03111301_COL_HS_Y + " as VALUE from INP as T1" + ID_SQL_MD03111301_JOIN_Y;

	/** SQL:添付資料（特売基本情報）の本体売価/総売価の取得 */
	public final static String ID_SQL_TOKBAIKA_IN = "with INP as (select SHNCD, BAIKA, DT from (values (cast(? as varchar), cast(? as varchar), cast(? as varchar))) as X(SHNCD, BAIKA, DT))";
	public final static String ID_SQL_TOKBAIKA_COL_SOU = " case when M0.ZEIKBN = 0 and nvl(M0.ZEIRTHENKODT,0) <= nvl(@DT, 0) then nvl(truncate(@BAIKA,0),0) + nvl(truncate(decimal(@BAIKA*M2.ZEIRT)/100, 0), 0)"
			+ " when M0.ZEIKBN = 0 and nvl(M0.ZEIRTHENKODT,0) >  nvl(@DT, 0) then nvl(truncate(@BAIKA,0),0) + nvl(truncate(decimal(@BAIKA*M3.ZEIRT)/100, 0), 0)"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) <= nvl(@DT, 0) then nvl(truncate(@BAIKA,0),0) + nvl(truncate(decimal(@BAIKA*M4.ZEIRT)/100, 0), 0)"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) >  nvl(@DT, 0) then nvl(truncate(@BAIKA,0),0) + nvl(truncate(decimal(@BAIKA*M5.ZEIRT)/100, 0), 0)"
			+ " else @BAIKA end";
	/*
	 * public final static String ID_SQL_TOKBAIKA_COL_HON =
	 * " case when M0.ZEIKBN = 0 and nvl(M0.ZEIRTHENKODT,0) <= nvl(@DT, 0) then nvl(ceil(decimal(@BAIKA)/decimal(1+decimal(M2.ZEIRT)/100)), 0)"
	 * +" when M0.ZEIKBN = 0 and nvl(M0.ZEIRTHENKODT,0) >  nvl(@DT, 0) then nvl(ceil(decimal(@BAIKA)/decimal(1+decimal(M3.ZEIRT)/100)), 0)"
	 * +" when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) <= nvl(@DT, 0) then nvl(ceil(decimal(@BAIKA)/decimal(1+decimal(M4.ZEIRT)/100)), 0)"
	 * +" when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and nvl(M1.ZEIRTHENKODT,0) >  nvl(@DT, 0) then nvl(ceil(decimal(@BAIKA)/decimal(1+decimal(M5.ZEIRT)/100)), 0)"
	 * +" else @BAIKA end";
	 */

	/*
	 * public final static String ID_SQL_TOKBAIKA_COL_HON =
	 * " case when M0.ZEIKBN = 0 and NVL(M0.ZEIRTHENKODT, 0) <= NVL(@DT, 0) then ROUND(double (@BAIKA) / NULLIF(1 + decimal (M2.ZEIRT) / 100, 0), 0)"
	 * +" when M0.ZEIKBN = 0 and NVL(M0.ZEIRTHENKODT, 0) >  NVL(@DT, 0) then ROUND(double (@BAIKA) / NULLIF(1 + decimal (M3.ZEIRT) / 100, 0), 0)"
	 * +" when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(M1.ZEIRTHENKODT, 0) <= NVL(@DT, 0) then ROUND(double (@BAIKA) / NULLIF(1 + decimal (M4.ZEIRT) / 100, 0), 0)"
	 * +" when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(M1.ZEIRTHENKODT, 0) >  NVL(@DT, 0) then ROUND(double (@BAIKA) / NULLIF(1 + decimal (M5.ZEIRT) / 100, 0), 0)"
	 * +" else @BAIKA end";
	 */

	// 割引本体売価 小数点切上げ
	public final static String ID_SQL_TOKBAIKA_COL_HON = " case when M0.ZEIKBN = 0 and NVL(M0.ZEIRTHENKODT, 0) <= NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M2.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 0 and NVL(M0.ZEIRTHENKODT, 0) >  NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M3.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(M1.ZEIRTHENKODT, 0) <= NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M4.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(M1.ZEIRTHENKODT, 0) >  NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M5.ZEIRT) / 100, 0))"
			+ " else @BAIKA end";

	public final static String ID_SQL_TOKBAIKA_COL_HON2 = " case when M0.ZEIKBN = 0 and NVL(SUBSTR(M0.ZEIRTHENKODT, 3, 6), 0) <= NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M2.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 0 and NVL(SUBSTR(M0.ZEIRTHENKODT, 3, 6), 0) >  NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M3.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(SUBSTR(M1.ZEIRTHENKODT, 3, 6), 0) <= NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M4.ZEIRT) / 100, 0))"
			+ " when M0.ZEIKBN = 3 and M1.ZEIKBN = 0 and NVL(SUBSTR(M1.ZEIRTHENKODT, 3, 6), 0) >  NVL(@DT, 0) then CEILING(double (@BAIKA) / NULLIF(1 + decimal (M5.ZEIRT) / 100, 0))"
			+ " else @BAIKA end";

	public final static String ID_SQL_TOKBAIKA_JOIN = " left outer join INAMS.MSTSHN M0 on M0.SHNCD = @SHNCD   and nvl(M0.UPDKBN, 0) <> 1 "
			+ " left outer join INAMS.MSTBMN M1 on M1.BMNCD = M0.BMNCD and nvl(M1.UPDKBN, 0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M2 on M2.ZEIRTKBN = M0.ZEIRTKBN and nvl(M2.UPDKBN, 0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M3 on M3.ZEIRTKBN = M0.ZEIRTKBN_OLD and nvl(M3.UPDKBN, 0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M4 on M4.ZEIRTKBN = M1.ZEIRTKBN and nvl(M4.UPDKBN, 0) <> 1"
			+ " left outer join INAMS.MSTZEIRT M5 on M5.ZEIRTKBN = M1.ZEIRTKBN_OLD and nvl(M5.UPDKBN, 0) <> 1";
	public final static String ID_SQL_TOKBAIKA_SOU = ID_SQL_TOKBAIKA_IN + " select " + ID_SQL_TOKBAIKA_COL_SOU
			+ " as VALUE from INP as T1" + ID_SQL_TOKBAIKA_JOIN;
	public final static String ID_SQL_TOKBAIKA_HON = ID_SQL_TOKBAIKA_IN + " select " + ID_SQL_TOKBAIKA_COL_HON
			+ " as VALUE from INP as T1" + ID_SQL_TOKBAIKA_JOIN;
	public final static String ID_SQL_TOKBAIKA_HON2 = "SELECT T2.ZEIRT AS F1 FROM(SELECT CASE WHEN T1.ZEIKBN <> '3' THEN CASE WHEN T1.ZEIKBN <> '0' THEN NULL WHEN T1.ZEIKBN = '0' AND SUBSTR(T1.ZEIRTHENKODT, 3, 6) <= ? THEN T1.ZEIRTKBN WHEN T1.ZEIKBN = '0' AND SUBSTR(T1.ZEIRTHENKODT, 3, 6) > ? THEN T1.ZEIRTKBN_OLD END ELSE CASE WHEN T2.ZEIKBN <> '0' THEN NULL WHEN T2.ZEIKBN='0' AND SUBSTR(T2.ZEIRTHENKODT, 3, 6) <= ? THEN T2.ZEIRTKBN WHEN T2.ZEIKBN = '0' AND SUBSTR(T2.ZEIRTHENKODT, 3, 6) > ? THEN T2.ZEIRTKBN_OLD END END ZEIRTKBN FROM INAMS.MSTSHN T1 LEFT JOIN INAMS.MSTBMN T2 ON SUBSTR(T1.SHNCD, 1, 2) = T2.BMNCD WHERE T1.SHNCD=?) T1 LEFT JOIN INAMS.MSTZEIRT T2 ON T1.ZEIRTKBN=T2.ZEIRTKBN";

	// 特売共通処理：全品割引商品登録時のチェック用SQL
	public final static String ID_SQL_TOKRS_KKK_CNT = "with INP as ("
			+ "  select HBSTDT, int (left (LPAD(SHNCD, 8, '0'), 2)) as BMNCD, SHNCD, WRITUKBN"
			+ "  from (values (cast(? as varchar), cast(? as varchar), cast(? as varchar))) as X(HBSTDT, SHNCD, WRITUKBN)"
			+ " )" + " select count(T.HBSTDT) as VALUE from INATK.TOKRS_KKK T" + " inner join ("
			+ "  select MAX(T.HBSTDT) as HBSTDT, T.BMNCD, X.WRITUKBN, X.SHNCD" + "  from INATK.TOKRS_KKK T "
			+ "  inner join INP X on T.HBSTDT <= X.HBSTDT and T.BMNCD = X.BMNCD and T.DUMMYCD = SHNCD and T.SEICUTKBN = 0"
			+ "  group by T.BMNCD, X.WRITUKBN, X.SHNCD"
	 ...
	 */
	/** 商品条件（T_CTG_HEAD）検索 */
	public static final String ID_SQL_KEY_HEAD = "select VALUE, TEXT from (values ('', '')) as X(value, TEXT) union all "
	...
	/** 商品条件（T_CTG_HEAD）登録 */
	public static final String ID_SQL_KEY_SET_HEAD = "merge into KEYSYS.T_CTG_HEAD T1 using (select cast (? as integer) as CD_USER, cast (? as varchar (100)) as NM_CTG from SYSIBM.SYSDUMMY1) T2 on (T1.CD_USER = T2.CD_USER and T1.NM_CTG = T2.NM_CTG) when matched then update set T1.DT_UPDATE = current_timestamp when not matched then insert (T1.CD_USER, T1.NM_CTG, T1.DT_UPDATE) values (T2.CD_USER, T2.NM_CTG, current_timestamp) else ignore";
	...
	/** 商品条件（T_CTG_ITEM）登録 */
	public static final String ID_SQL_KEY_SET_ITEM = "merge into KEYSYS.T_CTG_ITEM T1 using (select CD_CTG, cast (? as varchar (20)) as CD_ITEM, cast (? as integer) as NO_LINE from KEYSYS.T_CTG_HEAD where CD_USER = ? and NM_CTG = ? ) T2 on (T1.CD_CTG = T2.CD_CTG and T1.CD_ITEM = T2.CD_ITEM and T1.NO_LINE = T2.NO_LINE) when not matched then insert (T1.CD_CTG, T1.CD_ITEM, T1.NO_LINE) values (T2.CD_CTG, T2.CD_ITEM, T2.NO_LINE) else ignore";
	...
	public static final String ID_SQL_KEY_HEAD_TG = "select VALUE, TEXT from (values ('', '')) as X(value, TEXT) union all "
	...
	public static final String ID_SQL_KEY_SET_HEAD_TG = "merge into KEYSYS.T_TNP_HEAD T1 using (select cast (? as integer) as CD_USER, cast (? as varchar (100)) as NM_CTG from SYSIBM.SYSDUMMY1) T2 on (T1.CD_USER = T2.CD_USER and T1.NM_CTG = T2.NM_CTG) when matched then update set T1.DT_UPDATE = current_timestamp when not matched then insert (T1.CD_USER, T1.NM_CTG, T1.DT_UPDATE) values (T2.CD_USER, T2.NM_CTG, current_timestamp) else ignore";
	...
	public static final String ID_SQL_KEY_SET_ITEM_TG = "merge into KEYSYS.T_TNP_ITEM T1 using (select CD_CTG, cast (? as varchar (20)) as CD_ITEM, cast (? as integer) as NO_LINE from KEYSYS.T_TNP_HEAD where CD_USER = ? and NM_CTG = ? ) T2 on (T1.CD_CTG = T2.CD_CTG and T1.CD_ITEM = T2.CD_ITEM and T1.NO_LINE = T2.NO_LINE) when not matched then insert (T1.CD_CTG, T1.CD_ITEM, T1.NO_LINE) values (T2.CD_CTG, T2.CD_ITEM, T2.NO_LINE) else ignore";
	 ...
	 * SQL：取引先
	 */
	public final static String ID_SQL_TORIHIKI_HEAD = "select VALUE, TEXT from (values ('"
					+ Values.NONE.getVal() + "',' ')) as X(" + VAL + "," + TXT + ") union all ";
	...
	public final static String ID_SQL_TORIHIKI_NO_YOBI_x245 =  ID_SQL_TORIHIKI_HEAD + "select SSM.SIRCD as VALUE, SSM.SIRCD || ' ' || SSM.SIRKN as TEXT from INAMS.MSTSIR SSM inner join (select MAX(STARTDT) as STARTDT, SIRCD from INAMS.MSTSIR where STARTDT <= ?  group by SIRCD) TMP on TMP.STARTDT = SSM.STARTDT and TMP.SIRCD = SSM.SIRCD order by VALUE";
	...
	/**
	 * Web商談
	 * 仕入グループ（INAWS.PIMTISIRGPSHN）, （INAWS.PIMSISIRGPSHN）
	 * */
	public final static String ID_SQL_TENGP_SIR_TEIAN = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.SIRCD, T2.SIRCD as SSIRCD, T4.SIRKN, T2.HSPTN, T5.HSPTNKN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSIR T4 on T2.SIRCD = T4.SIRCD and nvl(T4.UPDKBN,0) <> 1 "
			+ " left outer join INAMS.MSTHSPTN T5 on T2.HSPTN = T5.HSPTN and nvl(T5.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_SIR_SHIKAKARI = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.SIRCD, T2.SIRCD as SSIRCD, T4.SIRKN, T2.HSPTN, T5.HSPTNKN, T2.AREAKBN from T1"
	...
	/**
	 * Web商談
	 * 売価グループ（INAWS.PIMTIBAIKACTL）, （INAWS.PIMSIBAIKACTL）
	 * */
	public final static String ID_SQL_TENGP_BAIKA_TEIAN = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.GENKAAM, T2.BAIKAAM, '' as BG_SOUBAIKA, '' as BG_NEIRE, T2.IRISU, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTSHNTENGP T3 on T2.TENGPCD = T3.TENGPCD and T2.AREAKBN = T3.AREAKBN and T3.GPKBN = "
			+ ValGpkbn.BAIKA.getVal() + " and T3.BMNCD = ? and nvl(T3.UPDKBN,0) <> 1 ";
	public final static String ID_SQL_TENGP_BAIKA_SHIKAKARI = ID_SQL_GRD_CMN
			+ "select T2.TENGPCD, T3.TENGPKN, T2.GENKAAM, T2.BAIKAAM, '' as BG_SOUBAIKA, '' as BG_NEIRE, T2.IRISU, T2.AREAKBN from T1"
	...
	/**
	 * Web商談
	 * 品揃えグループ（INAWS.PIMTISHINAGP）, （INAWS.PIMSISHINAGP）
	 * */
	public final static String ID_SQL_TENGP_SHINA_TEIAN = ID_SQL_GRD_CMN + "select T2.TENGPCD, T3.TENGPKN, T2.ATSUKKBN||'"
			+ SEPARATOR + "'||T4.NMKN as ATSUKKBN, T2.AREAKBN from T1"
			...
			+ " left outer join INAMS.MSTMEISHO T4 on T4.MEISHOKBN = " + MeisyoSelect.KBN139.getCd()
			+ " and TO_CHAR(T2.ATSUKKBN) = T4.MEISHOCD";
	public final static String ID_SQL_TENGP_SHINA_SHIKAKARI = ID_SQL_GRD_CMN + "select T2.TENGPCD, T3.TENGPKN, T2.ATSUKKBN||'"
			+ SEPARATOR + "'||T4.NMKN as ATSUKKBN, T2.AREAKBN from T1"
	...
	/**
	 * Web商談
	 * メーカー（INAWS.PIMTIMAKER）, （INAWS.PIMSIMAKER）
	 * */
