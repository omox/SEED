...
public class GetSqlCommandInit {
	...
	public static String getSqlcommand(HttpServletRequest request, HashMap<String,String> maps, String datatype, String jndiName) throws Exception {
		...
		if (outobj.equals(DefineReport.Select.MSG_LIST.getObj())) {
			...
			} /*else if (DefineReport.ID_PAGE_X251.equals(outpage)) {
				paramData.add("E%");
				paramData.add("I%");
				paramData.add("W%");
				sqlcommand += DefineReport.ID_SQL_MSG_WHERE2;
		...
		if(outobj.equals(DefineReport.Select.MOYSCD.getObj())){
			...
			if(!StringUtils.isEmpty(moyoosi)){
				...
				sqlcommand = "(" + DefineReport.ID_SQL_MOYSCD;
				sqlcommand +=" where T1.MOYSKBN = 8 AND T1.UPDKBN=" + DefineReport.ValUpdkbn.NML.getVal() + " and NOT EXISTS(SELECT T2.MOYSKBN FROM INATK.TOKQJU_MOY T2 WHERE T1.MOYSKBN=T2.MOYSKBN AND T1.MOYSSTDT=T2.MOYSSTDT AND T1.MOYSRBAN=T2.MOYSRBAN AND T2.UPDKBN=" + DefineReport.ValUpdkbn.NML.getVal() + ") order by T1.MOYSKBN , T1.MOYSSTDT , T1.MOYSSTDT )";
		...
		if (StringUtils.startsWith(outobj, DefineReport.Select.BUMON.getObj())) {
			...
			} else if (DefineReport.ID_PAGE_X001.equals(outpage)||DefineReport.ID_PAGE_X231.equals(outpage)||DefineReport.ID_PAGE_X244.equals(outpage)||
					DefineReport.ID_PAGE_X246.equals(outpage)||DefineReport.ID_PAGE_X249.equals(outpage)||DefineReport.ID_PAGE_X250.equals(outpage)||
		...
		if (StringUtils.startsWith(outobj,DefineReport.Select.DAI_BUN.getObj())) {
			...
			if (array1 != null){
				...
				if (!DefineReport.ID_PAGE_X001.equals(outpage)&&!DefineReport.ID_PAGE_HT002.equals(outpage)&&!DefineReport.ID_PAGE_HT004.equals(outpage)
						&&!DefineReport.ID_PAGE_HT007.equals(outpage)&&!DefineReport.ID_PAGE_HT009.equals(outpage) && !DefineReport.ID_PAGE_X231.equals(outpage)
		...
		if(outobj.equals(DefineReport.Select.BYCD.getObj())){
			...
			|| DefineReport.ID_PAGE_ST016.equals(outpage)){
				String bumon = obj.optString("BUMON");
		...
		if (ArrayUtils.contains(DefineReport.getItemObjs(MeisyoSelect.values()), outobj)
			...
			if(!ArrayUtils.contains(DefineReport.getItemObjs(MeisyoSelect.values()), id)){ id = outobj; }
			...
			if((DefineReport.ID_PAGE_X001.equals(outpage)||DefineReport.ID_PAGE_X280.equals(outpage)) && DefineReport.MeisyoSelect.KBN142.getObj().equals(id)){
				sqlcommand = "select KBN,VALUE,'' as "+DefineReport.KTXT+",VALUE||'"+DefineReport.SEPARATOR+"'||TEXT as TEXT,TEXT as TEXT2,'' as "+DefineReport.STXT+",'' as "+DefineReport.KANA
						+" from (values ('"+DefineReport.MeisyoSelect.KBN142.getCd()+"','2','両方')) as X(KBN,VALUE,TEXT) union all " + sqlcommand;
			}
		}
		...
		// Web商談 取引先
		if (StringUtils.startsWith(outobj, DefineReport.Select.TORIHIKI.getObj()) ||
				(StringUtils.startsWith(outobj, DefineReport.InpText.SSIRCD.getObj()) && DefineReport.ID_PAGE_X280.equals(outpage))) {
			User userInfo = (User)session.getAttribute(Consts.STR_SES_LOGINUSER);
		...
		// 店舗一覧
			...
			if(DefineReport.ID_PAGE_TG003.equals(outpage)){
				...
				if(!StringUtils.isEmpty(txt_tengpcd)
						&& !StringUtils.isEmpty(gpkbn)
						&& !StringUtils.isEmpty(SelBumon)
						&& !StringUtils.equals(DefineReport.Values.NONE.getVal(), gpkbn)
		...
		// 店舗一覧(新店改装発注)
			...
			if(!StringUtils.isEmpty(txt_tencd)
				&& !StringUtils.isEmpty(txt_kspage)){
				...
				String sqlWhere = "";
				if(StringUtils.equals("TypeF", txt_dispModo)
						|| StringUtils.equals("TypeD", txt_dispModo)){
		...
		// 納入日一覧(予約発注)
			...
			if(DefineReport.ValTablekbn.CSV.getVal().equals( obj.optString("TABLEKBN"))){
				...
				if(obj.containsKey("SHNCD") && StringUtils.isNotEmpty(sel_shocd)){
					...
					/*
					if (DefineReport.ID_PAGE_X251.equals(outpage) || StringUtils.equals(outobj, DefineReport.Grid.SRCCD.getObj()+"_winIT031")) {
						sqlcommand = "SELECT * FROM(" + sqlcommand + ") order by case when SEQNO = '1' then '1' when SEQNO = '2' then '2' else '9' end, case when to_char(sysdate,'yymmdd') <= YUKO_STDT then 1 else 9 end, YUKO_STDT, SRCCD";
					}
					*/
				}
		...
		// B/M番号一覧
			...
			if(!StringUtils.isEmpty(txt_moyskbn) && !StringUtils.isEmpty(txt_moyskbn) &&
				!StringUtils.isEmpty(txt_moysstdt) && !StringUtils.isEmpty(txt_moysstdt) &&
				!StringUtils.isEmpty(txt_moysrban) && !StringUtils.isEmpty(txt_moysrban) &&
				!StringUtils.isEmpty(txt_bmnno) && !StringUtils.isEmpty(txt_bmnno)){
			paramData.add(txt_moyskbn);
			paramData.add(txt_moysstdt.substring(2));
			paramData.add(txt_moysrban);
		...
		if(StringUtils.startsWith(outobj, DefineReport.Grid.SUB.getObj())){
			...
			if(StringUtils.startsWith(outobj, DefineReport.Grid.SUB.getObj()+"_winTG018")){
				...
				&& StringUtils.isNotEmpty(txt_bmncd) && StringUtils.isNotEmpty(txt_kanrino) && StringUtils.isNotEmpty(txt_kanrieno)
		...
		/* 販促 */
		if(StringUtils.startsWith(outobj, DefineReport.Grid.MOYCD_R.getObj())
		|| StringUtils.startsWith(outobj, DefineReport.Grid.MOYCD_S.getObj())
