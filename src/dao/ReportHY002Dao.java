...
public class ReportHY002Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHS.UKESTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKESTDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHS.UKESTDT, 6), 'YYYYMMDD')) as UKESTDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHS.UKEEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKEEDDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHS.UKEEDDT, 6), 'YYYYMMDD')) as UKEEDDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHS.TENISTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENISTDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHS.TENISTDT, 6), 'YYYYMMDD')) as TENISTDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHS.TENIEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as TENIEDDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHS.TENIEDDT, 6), 'YYYYMMDD')) as TENIEDDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHN.MIN_NNDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as MIN_NNDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHN.MIN_NNDT, 6), 'YYYYMMDD')) as MIN_NNDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHN.MAX_NNDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as MAX_NNDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHN.MAX_NNDT, 6), 'YYYYMMDD')) as MAX_NNDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHS.HTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as HTDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHS.HTDT, 6), 'YYYYMMDD')) as HTDT_WNUM");
		sbSQL.append(", YHS.YOTEISU, YHT.HTSU from INATK.HATYH_KKK YHK left join INATK.HATYH_SHN YHS on YHS.KKKCD = YHK.KKKCD");
		sbSQL.append(" left join (select KKKCD, SHNCD, SUM(HTSU) as HTSU, SUM(case when TENCD = ? then HTSU else 0 end) as HTSU_T from INATK.HATYH_TEN group by KKKCD, SHNCD) YHT on YHT.KKKCD = YHS.KKKCD and YHT.SHNCD = YHS.SHNCD");
		paramData.add(tencd);
