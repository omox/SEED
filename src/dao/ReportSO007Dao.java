...
public class ReportSO007Dao extends ItemDao {
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		sbSQL.append(", TO_CHAR(MAX(CHAD.INPUT_DATE), 'YY/MM/DD')  || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(MAX(CHAD.INPUT_DATE))) as F2");
		sbSQL.append(", TO_CHAR(MAX(CHAD.INPUT_DATE), 'HH24:MI') as F3");
		...
		sbSQL.append(", TO_CHAR(TO_DATE(MAX(MYCD.HBSTDT), 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(MAX(MYCD.HBSTDT), 'YYYYMMDD')))");
		sbSQL.append(" || '～' || TO_CHAR(TO_DATE(MAX(MYCD.HBEDDT), 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(MAX(MYCD.HBEDDT), 'YYYYMMDD'))) as F7");
