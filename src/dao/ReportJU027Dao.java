...
public class ReportJU027Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    sbSQL.append(
        ",DATE_FORMAT(DATE_FORMAT(T1.NNSTDT, '%Y%m%d'), '%y/%m/%d')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(DATE_FORMAT(T1.NNSTDT, '%Y%m%d')))    ||'～'||    DATE_FORMAT(DATE_FORMAT(T1.NNEDDT, '%Y%m%d'), '%y/%m/%d')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(DATE_FORMAT(T1.NNEDDT, '%Y%m%d'))) as 納入期間");// F3納入期間
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T2.QASMDT, '%Y%m%d'), '%y/%m/%d')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(DATE_FORMAT(T2.QASMDT, '%Y%m%d'))) as 店舗締切日"); // F4店舗締切日
