 ...
 *
 */
public class ReportBM003Dao extends ItemDao {
   ...
   * @param source
   */
  public ReportBM003Dao(String JNDIname) {
    super(JNDIname);
  }
  ...
  private String createCommand() {
    ...
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T2.HBSTDT,'%Y%m%d'),'%y/%m/%d') AS HBSTDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T2.HBEDDT,'%Y%m%d'),'%y/%m/%d') AS HBEDDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T2.NNSTDT,'%Y%m%d'),'%y/%m/%d') AS NNSTDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T2.NNEDDT,'%Y%m%d'),'%y/%m/%d') AS NNEDDTD ");
    sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(DATE_FORMAT(T2.HBSTDT,'%Y%m%d'))) HBSTDTW");
    sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(DATE_FORMAT(T2.HBEDDT,'%Y%m%d'))) HBEDDTW");
    sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(DATE_FORMAT(T2.NNSTDT,'%Y%m%d'))) NNSTDTW");
    sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(DATE_FORMAT(T2.NNEDDT,'%Y%m%d'))) NNEDDTW ");
    sbSQL.append("FROM ");
    sbSQL.append("INATK.TOKBM T1 LEFT JOIN INATK.TOKMOYCD T2 ON T1.MOYSKBN=T2.MOYSKBN AND T1.MOYSSTDT=T2.MOYSSTDT AND T1.MOYSRBAN=T2.MOYSRBAN ");
    sbSQL.append("WHERE " + sqlWhere + " AND T1.UPDKBN=" + DefineReport.ValUpdkbn.NML.getVal());
    sbSQL.append(" ORDER BY ");
    sbSQL.append("T1.MOYSKBN ");
    sbSQL.append(",T1.MOYSSTDT ");
    sbSQL.append(",T1.MOYSRBAN)T3");
