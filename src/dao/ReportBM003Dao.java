...
public class ReportBM003Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(",TO_CHAR(TO_DATE(T2.HBSTDT,'YYYYMMDD'),'YY/MM/DD') AS HBSTDTD ");
		sbSQL.append(",TO_CHAR(TO_DATE(T2.HBEDDT,'YYYYMMDD'),'YY/MM/DD') AS HBEDDTD ");
		sbSQL.append(",TO_CHAR(TO_DATE(T2.NNSTDT,'YYYYMMDD'),'YY/MM/DD') AS NNSTDTD ");
		sbSQL.append(",TO_CHAR(TO_DATE(T2.NNEDDT,'YYYYMMDD'),'YY/MM/DD') AS NNEDDTD ");
		sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(TO_DATE(T2.HBSTDT,'YYYYMMDD'))) HBSTDTW");
		sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(TO_DATE(T2.HBEDDT,'YYYYMMDD'))) HBEDDTW");
		sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(TO_DATE(T2.NNSTDT,'YYYYMMDD'))) NNSTDTW");
		sbSQL.append(",(select JWEEK from WEEK where CWEEK=DAYOFWEEK(TO_DATE(T2.NNEDDT,'YYYYMMDD'))) NNEDDTW ");
		sbSQL.append("FROM ");
