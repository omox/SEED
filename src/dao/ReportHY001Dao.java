...
public class ReportHY001Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHK.NNSTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NNSTDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHK.NNSTDT, 6), 'YYYYMMDD')) as NNSTDT_WNUM");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || YHK.NNEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NNEDDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || YHK.NNEDDT, 6), 'YYYYMMDD')) as NNEDDT_WNUM");
		...
		/*sbSQL.append("with WEEK as (select");
		sbSQL.append(" CWEEK");
		sbSQL.append(", JWEEK");
		sbSQL.append(" from (values (0, '(日'), (1, '(月)'), (2, '(火)'), (3, '(水)'), (4, '(木)'), (5, '(金)'), (6, '(土)')) as TMP(CWEEK, JWEEK))");
		sbSQL.append(" select");
		...
		sbSQL.append(", SUM(SHN.COUNT) as COUNT");
		...
		sbSQL.append(" left join (select MOYSKBN, MOYSSTDT, MOYSRBAN, BMNCD, COUNT(SHNCD) as COUNT from INATK.TOKSO_SHN group by MOYSKBN, MOYSSTDT, MOYSRBAN, BMNCD) SHN on SHN.MOYSKBN = MYCD.MOYSKBN and SHN.MOYSSTDT = MYCD.MOYSSTDT and SHN.MOYSRBAN = MYCD.MOYSRBAN and SHN.BMNCD = BMN.BMNCD");
