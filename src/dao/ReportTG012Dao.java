...
public class ReportTG012Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" WHERE ");
		sbSQL.append(" T1.ITMANSFLG = 1 ");
		sbSQL.append(" AND T1.SCHANSFLG = 1 ");
		...
		sbSQL.append(" , TO_CHAR(TO_DATE(TMYD.HBSTDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(TMYD.HBSTDT, 'YYYYMMDD'))) ");
		sbSQL.append(" ||'～'|| ");
		sbSQL.append(" TO_CHAR(TO_DATE(TMYD.HBEDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(TMYD.HBEDDT, 'YYYYMMDD'))) as KIKAN ");
		sbSQL.append(" , CASE ");
...
//		}
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		sbSQL.append("MERGE INTO INATK.TOKTG_KHN AS T USING (SELECT ");
		...
		sbSQL.append("MOYSKBN ");													// F1	: 配送グループ
		sbSQL.append(",MOYSSTDT ");													// F2	: 配送グループ名称
		sbSQL.append(",MOYSRBAN ");													// F3	: 配送グループ名称（カナ）
		sbSQL.append(",SIMEFLG1_LD ");													// F7	: エリア区分
		sbSQL.append(",SIMEFLG2_LD ");													// F7	: エリア区分
		sbSQL.append(",SIMEFLG_MB ");													// F7	: エリア区分
		...
		sbSQL.append(",SIMEFLG1_LD=RE.SIMEFLG1_LD");
		sbSQL.append(",SIMEFLG2_LD=RE.SIMEFLG2_LD");
		sbSQL.append(",SIMEFLG_MB=RE.SIMEFLG_MB");
