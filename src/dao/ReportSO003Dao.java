...
import dao.ReportTG016Dao.SYSMOYBMNLayout;
...
public class ReportSO003Dao extends ItemDao {
	...
	// 名称検索
	public final static String ID_SQL_SHNKN_SO003 = ""
			+ "select"
			+ " case when SHN.POPKN is null or SHN.POPKN = '' then '　' else SHN.POPKN end as F1"
			+ ", SHN.KIKKN as F2"
			+ ", SHN.RG_IRISU as F3"
			//+ ", case when SHN.HS_SPOTMINSU is not null then SHN.HS_SPOTMINSU else 1 end as F4"
			+ ", case when NVL(SHN.HS_SPOTMINSU, 0) = 0 then 1 else SHN.HS_SPOTMINSU end as F4"
			+ ", SHN.RG_GENKAAM as F5"
			+ ", SHN.SANCHIKN as F6"
			//+ ", SHN.HS_BAIKAAM as F7"
			+ ", case"
			+ "  when NVL(SHN.HS_GENKAAM, 0) <> 0 and NVL(SHN.HS_BAIKAAM, 0) <> 0 and NVL(SHN.HS_IRISU, 0) <> 0 then SHN.HS_BAIKAAM"
			+ "  else SHN.RG_BAIKAAM end as F7"
			+ ", SHN.RG_IRISU as F8"
			+ ", case"
			+ "  when NVL(SHN.HS_GENKAAM, 0) <> 0 and NVL(SHN.HS_BAIKAAM, 0) <> 0 and NVL(SHN.HS_IRISU, 0) <> 0 then SHN.HS_GENKAAM"
			+ "  else SHN.RG_GENKAAM end as F9"
			+ ", '' as F10"
			+ ", '' as F11"
			+ ", '' as F12"
			+ ", '' as F13"
			...
			+ ", '' as F16"
			+ ", '' as F17"
			+ ", SHN.RG_BAIKAAM as F18"		// エラーチェック用：レギュラー標準売価
			+ " from INAMS.MSTSHN SHN"
			+ " left join INAMS.MSTMAKER MAK on MAK.MAKERCD = SHN.MAKERCD"
			+ " where NVL(SHN.UPDKBN, 0) <> 1"
			+ " and SHN.SHNCD = ?";
	...
	private String createCommand() {
		...
		if(StringUtils.equals(DefineReport.Button.ERR_CHANGE.getObj(), sendBtnid)){
			...
			sbSQL.append(", trunc(double(T2.A_HONBAIK - T2.GENKAAM) * 100 / T2.A_HONBAIK, 2)");			// F13:	A値入率
			sbSQL.append(", T2.B_BAIKAAM");																// F14:	B総売価
			sbSQL.append(", T2.B_HONBAIK");																// F15:	B本売価
			sbSQL.append(", T2.B_RANKNO");																// F16:	Bランク
			sbSQL.append(", case when T2.B_HONBAIK is null then null else trunc(double(T2.B_HONBAIK - T2.GENKAAM) * 100 / T2.B_HONBAIK, 2) end ");			// F17:	B値入率
			sbSQL.append(", T2.C_BAIKAAM");																// F18:	C総売価
			sbSQL.append(", T2.C_HONBAIK");																// F19:	C本売価
			sbSQL.append(", T2.C_RANKNO");																// F20:	Cランク
			sbSQL.append(", case when T2.C_HONBAIK is null then null else trunc(double(T2.C_HONBAIK - T2.GENKAAM) * 100 / T2.C_HONBAIK, 2) end ");			// F21:	C値入率
			...
			sbSQL.append(",case when CSO.B_BAIKAAM is null then null else "+DefineReport.ID_SQL_TOKBAIKA_COL_HON.replaceAll("@BAIKA", "CSO.B_BAIKAAM").replaceAll("@DT", "MYCD.MOYSSTDT")+ " end as B_HONBAIK");
			sbSQL.append(", CSO.B_RANKNO");
			sbSQL.append(", null");
			sbSQL.append(", CSO.C_BAIKAAM");
			sbSQL.append(",case when CSO.C_BAIKAAM is null then null else "+DefineReport.ID_SQL_TOKBAIKA_COL_HON.replaceAll("@BAIKA", "CSO.C_BAIKAAM").replaceAll("@DT", "MYCD.MOYSSTDT")+ " end as C_HONBAIK");
			...
			sbSQL.append(", NVL(CSO.UPDKBN, 0) as UPDKBN");
			sbSQL.append(", TO_CHAR(CSO.ADDDT, 'yy/mm/dd') as ADDDT");
			sbSQL.append(", TO_CHAR(CSO.UPDDT, 'yy/mm/dd') as UPDDT");
			...
			sbSQL.append(" left outer join INAMS.MSTSHN M0 on M0.SHNCD = CSO.SHNCD and NVL(M0.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTBMN M1 on M1.BMNCD = CSO.BMNCD and NVL(M1.UPDKBN, 0) <> 1");
			...
			sbSQL.append(" and NVL(CSO.ERRCD, 0) <> 4");
			sbSQL.append(" and NVL(CSO.UPDKBN , 0) <> 1");
			sbSQL.append(" ) T2");
			...
			sbSQL.append("with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < ("+countRows+" + 1))");
			sbSQL.append("select");
			sbSQL.append(" trim(T2.SHNCD)");															// F1:	商品コード
			sbSQL.append(", T2.MAKERKN");																// F2:	メーカー名
			sbSQL.append(", T2.SHNKN");																	// F3:	商品名称
			...
			sbSQL.append(", T2.POPSZ");																	// F23:	POPサイズ
			sbSQL.append(", T2.POPSU");																	// F24:	枚数
			sbSQL.append(", TO_CHAR(T2.ADDDT, 'yy/mm/dd')");											// F25:	登録
			sbSQL.append(", TO_CHAR(T2.UPDDT, 'yy/mm/dd')");											// F26:	更新
			sbSQL.append(", T2.OPERATOR");																// F27:	オペレーター
			sbSQL.append(", T2.KANRINO");																// F28:	管理番号
			sbSQL.append(", T2.PLUSFLG");																// F29:	PLG配信済フラグ
			sbSQL.append(", T2.UPDKBN");																// F30:	更新区分
			...
			sbSQL.append(", MYCD.PLUSFLG");
			sbSQL.append(", NVL(SHN.UPDKBN, 0) as UPDKBN");
			sbSQL.append(" from INATK.TOKMOYCD MYCD");
			...
			sbSQL.append(" left outer join INAMS.MSTSHN M0 on M0.SHNCD = SHN.SHNCD and NVL(M0.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTBMN M1 on M1.BMNCD = SHN.BMNCD and NVL(M1.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTZEIRT M2 on M2.ZEIRTKBN = M0.ZEIRTKBN and NVL(M2.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTZEIRT M3 on M3.ZEIRTKBN = M0.ZEIRTKBN_OLD and NVL(M3.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTZEIRT M4 on M4.ZEIRTKBN = M1.ZEIRTKBN and NVL(M4.UPDKBN, 0) <> 1");
			sbSQL.append(" left outer join INAMS.MSTZEIRT M5 on M5.ZEIRTKBN = M1.ZEIRTKBN_OLD and NVL(M5.UPDKBN, 0) <> 1");
			sbSQL.append(" where NVL(SHN.UPDKBN, 0) <> 1 and MYCD.MOYSKBN = "+szMoyskbn+" and MYCD.MOYSSTDT = "+szMoysstdt+" and MYCD.MOYSRBAN = "+szMoysrban+" and SHN.BMNCD = "+szBumon);
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		if(StringUtils.equals(DefineReport.Button.ERR_CHANGE.getObj(), sendBtnid)){
			...
			sbSQL.append(", TO_CHAR(TO_DATE(T1.HBSTDT, 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HBSTDT, 'YYYYMMDD'))) || '～' || TO_CHAR(TO_DATE(T1.HBEDDT, 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HBEDDT, 'YYYYMMDD'))) as F3");
			...
			sbSQL.append(" where NVL(UPDKBN, 0) <> 1 and SEQ = "+szSeq);
			...
			sbSQL.append(", TO_CHAR(T2.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as F6");
			sbSQL.append(", T1.PLUSFLG as F7");
			sbSQL.append(" from INATK.TOKMOYCD T1");
			sbSQL.append(" left join INATK.TOKSO_BMN T2 on NVL(T2.UPDKBN, 0) <> 1 and T2.MOYSKBN = T1.MOYSKBN and T2.MOYSSTDT = T1.MOYSSTDT and T2.MOYSRBAN = T1.MOYSRBAN and T2.BMNCD = "+szBumon);
			sbSQL.append(" left join INAMS.MSTBMN BMN on NVL(BMN.UPDKBN, 0) <> 1 and BMN.BMNCD = T2.BMNCD");
	...
	public String createSqlTOKSO(JSONObject data, HashMap<String, String> map, User userInfo){
		...
		for (int i = 0; i < len; i++) {
				...
				for (int k = 1; k <= maxField; k++) {
					...
					if(! ArrayUtils.contains(new String[]{""}, key)){
						...
						if(StringUtils.equals("F5", key)){
							...
							val = this.getTenCdAddABC(dataT.optString("F4"), dataT.optString("F1"), dataT.optString("F2"), dataT.optString("F3"), dataT.optString("F16"), dataT.optString("F17"), dataT.optString("F18"));
						}
			...
			if(valueData.length >= 100 || (i+1 == len && valueData.length > 0)){
				...
				// 生活応援_商品の登録・更新
				sbSQL = new StringBuffer();
				sbSQL.append(" merge into INATK.TOKSO_SHN as T using (select");
				sbSQL.append(" MOYSKBN");													// 催し区分
				sbSQL.append(", MOYSSTDT");													// 催し開始日
				sbSQL.append(", MOYSRBAN");													// 催し連番
				sbSQL.append(", BMNCD");													// 部門
				...
				sbSQL.append(" ))as RE on (T.MOYSKBN = RE.MOYSKBN and T.MOYSSTDT = RE.MOYSSTDT and T.MOYSRBAN = RE.MOYSRBAN and T.BMNCD = RE.BMNCD and T.KANRINO = RE.KANRINO)");
				sbSQL.append(" when matched then update set");
				sbSQL.append(" MOYSKBN=RE.MOYSKBN");
				sbSQL.append(", MOYSSTDT=RE.MOYSSTDT");
				sbSQL.append(", MOYSRBAN=RE.MOYSRBAN");
				...
				sbSQL.append(", MINSU=RE.MINSU");
				sbSQL.append(", GENKAAM=RE.GENKAAM");
				sbSQL.append(", A_BAIKAAM=RE.A_BAIKAAM");
				sbSQL.append(", B_BAIKAAM=RE.B_BAIKAAM");
				sbSQL.append(", C_BAIKAAM=RE.C_BAIKAAM");
				sbSQL.append(", A_RANKNO=RE.A_RANKNO");
				sbSQL.append(", B_RANKNO=RE.B_RANKNO");
				sbSQL.append(", C_RANKNO=RE.C_RANKNO");
				sbSQL.append(", POPCD=RE.POPCD");
				sbSQL.append(", POPSZ=RE.POPSZ");
				sbSQL.append(", POPSU=RE.POPSU");
				...
				sbSQL.append(", RE.IRISU");
				sbSQL.append(", RE.MINSU");
				sbSQL.append(", RE.GENKAAM");
				sbSQL.append(", RE.A_BAIKAAM");
				sbSQL.append(", RE.B_BAIKAAM");
				sbSQL.append(", RE.C_BAIKAAM");
				sbSQL.append(", RE.A_RANKNO");
				sbSQL.append(", RE.B_RANKNO");
				sbSQL.append(", RE.C_RANKNO");
				sbSQL.append(", RE.POPCD");
				sbSQL.append(", RE.POPSZ");
				sbSQL.append(", RE.POPSU");
	...
	// 対象店取得処理
		public String getTenCdAddABC(String bmnCd, String moysKbn, String moysStDt, String moysRban, String rankNo_A,String rankNo_B,String rankNo_C) {
			...
			ArrayList<String> rankNoList	= new ArrayList<String>();
			ArrayList<String> LankArray		= new ArrayList<String>();
			...
			// Aランクの値よりベースとなる配列を作成
			for (int i = 0; i < rankARR_A.length; i++) {
				if(rankARR_A.length > 0){
					if(StringUtils.isNotEmpty(rankARR_A[i].trim())){
						LankArray.add("A");
					}else{
						LankArray.add(" ");
					}
	...
	public String getKANRINO(String MOYSKBN, String MOYSSTDT, String MOYSRBAN, String BMNCD) {
		...
		/*sbSQL.append("select");
		sbSQL.append(" MAX(SOS.KANRINO) as KANRINO");
		sbSQL.append(" from INATK.TOKMOYCD MYCD");
	...
	public void setReturnData(String MOYSKBN, String MOYSSTDT, String MOYSRBAN, String BMNCD, JSONObject option) {
		...
		sbSQL = new StringBuffer();
		sbSQL.append("select");
		sbSQL.append(" TO_CHAR(SOB.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as UPDDT");
		...
		sbSQL.append(" where NVL(SOB.UPDKBN, 0) <> 1");
		...
		if(StringUtils.isNotEmpty(selectKanriNo)){
			...
			sbSQL = new StringBuffer();
			sbSQL.append(" select");
			sbSQL.append(" TO_CHAR(SOS.ADDDT, 'yy/mm/dd') as F25");			// 登録日
			sbSQL.append(", TO_CHAR(SOS.UPDDT, 'yy/mm/dd') as F26");		// 更新日
			...
			sbSQL.append(" where NVL(SOS.UPDKBN, 0) <> 1");
	...
	public List<JSONObject> checkData(HashMap<String, String> map, User userInfo, MessageUtility mu,
		...
		// テーブル名定義
		String tokso_bmn = "TOKSO_BMN";
		String tokso_shn = "TOKSO_SHN";
		String errCd = "";
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			/*if(StringUtils.isEmpty(data.optString("F1"))
					|| StringUtils.isEmpty(data.optString("F2"))
					|| StringUtils.isEmpty(data.optString("F3"))
					|| StringUtils.isEmpty(data.optString("F4"))
					){
				JSONObject o = MessageUtility.getDbMessageIdObj("E00001", new String[]{});
				// TODO
				//o = this.setCsvSoErrinfo(o, errCd, tokso_bmn, colinf.getCol(),String.valueOf(i+1), data);
				msg.add(o);
				return msg;
			}*/
		...
		for (int i = 0; i < dataArrayShn.size(); i++) {
			...
			targetCol = new TOKSO_SHNLayout[]{
					TOKSO_SHNLayout.MOYSKBN			// 催し区分
			...
			for(int j = 0; j < rankNo.size(); j++){
				...
				if(StringUtils.isNotEmpty(val)){
					...
					if(Integer.parseInt(val) > 900){
						emsgId = "E20466";
						paramData  = new ArrayList<String>();
						sqlcommand = "select COUNT(BMNCD) as value from INATK.TOKRANKEX where NVL(UPDKBN, 0) <> 1 and BMNCD = ? and MOYSKBN = ? and MOYSSTDT = ? and MOYSRBAN = ? and RANKNO = ? ";
						paramData.add(data.getString("F4"));
						paramData.add(data.getString("F1"));
						paramData.add(data.getString("F2"));
						paramData.add(data.getString("F3"));
						...
						emsgId = "E20057";
						paramData  = new ArrayList<String>();
						sqlcommand = "select COUNT(BMNCD) as value from INATK.TOKRANK where NVL(UPDKBN, 0) <> 1 and BMNCD = ? and RANKNO = ? ";
						paramData.add(data.getString("F4"));
						paramData.add(val);
					}
			...
			String rank_A	 = data.getString("F16");
	...
	public String createCommandFTP(String userId, JSONObject obj, String outpage) {
		...
		sbSQL.append(" || left (RPAD(NVL(T1.MOYKN, ''), 40, '　'), 40)");
		sbSQL.append(" || LPAD(NVL(TO_CHAR(T1.MOYSSTDT), ''), 8, '0')");
		sbSQL.append(" || LPAD(NVL(TO_CHAR(T1.HBEDDT), ''), 8, '0'), 241, ' ') as REC");
		sbSQL.append(" from INATK.TOKMOYCD T1");
		sbSQL.append(" inner join INATK.TOKSO_BMN T2 on NVL(T1.UPDKBN, 0) <> 1 and NVL(T2.UPDKBN, 0) <> 1 and T1.MOYSKBN = T2.MOYSKBN and T1.MOYSSTDT = T2.MOYSSTDT and T1.MOYSRBAN = T2.MOYSRBAN");
		...
		if (DefineReport.ID_PAGE_SO003.equals(outpage)) {
			...
			sbSQL.append(" || LPAD(T1.MOYSSTDT, 6, '0')");
			sbSQL.append(" || LPAD(T1.MOYSRBAN, 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T2.BMNCD), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.KANRINO), ''), 4, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.SHNCD), ''), 14, '0')");
			sbSQL.append(" || left (RPAD(NVL(T3.MAKERKN, ''), 14, '　'), 14)");
			sbSQL.append(" || left (RPAD(NVL(T3.SHNKN, ''), 40, '　'), 40)");
			sbSQL.append(" || left (RPAD(NVL(T3.KIKKN, ''), 46, '　'), 46)");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.DAICD), ''), 2, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.CHUCD), ''), 2, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.SHOCD), ''), 2, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.RG_IRISU), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.IRISU), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.MINSU), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.RG_GENKAAM), ''), 11, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.GENKAAM), ''), 11, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.RG_BAIKAAM), ''), 6, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.A_BAIKAAM), ''), 6, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.B_BAIKAAM), ''), 6, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.C_BAIKAAM), ''), 6, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.A_RANKNO), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.B_RANKNO), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.C_RANKNO), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.POPCD), ''), 10, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(T3.POPSU), ''), 2, '0')");
			sbSQL.append(" || left (RPAD(NVL(T3.POPSZ, ''), 3, '　'), 3)");
			sbSQL.append(" || left (RPAD(NVL(M1.ZEIKBN, ''), 1, '　'), 1)");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.ZEIRTKBN), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.ZEIRTKBN_OLD), ''), 3, '0')");
			sbSQL.append(" || LPAD(NVL(TO_CHAR(M1.ZEIRTHENKODT), ''), 8, '0'), 241, ' ') as REC");
			sbSQL.append(" from INATK.TOKMOYCD T1");
			sbSQL.append(" inner join INATK.TOKSO_BMN T2 on NVL(T1.UPDKBN, 0) <> 1 and NVL(T2.UPDKBN, 0) <> 1 and T1.MOYSKBN = T2.MOYSKBN and T1.MOYSSTDT = T2.MOYSSTDT and T1.MOYSRBAN = T2.MOYSRBAN");
			sbSQL.append(" inner join WK on T2.MOYSKBN = WK.MOYSKBN and T2.MOYSSTDT = WK.MOYSSTDT and T2.MOYSRBAN = WK.MOYSRBAN and T2.BMNCD = WK.BMNCD");
			...
			sbSQL.append(" inner join INAMS.MSTSHN M1 on M1.SHNCD = T3.SHNCD and NVL(M1.UPDKBN, 0) <> 1");
		}
