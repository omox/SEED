...
public class Reportx250Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    sbSQL.append(" ,substr(right('00000000' || rtrim(cast(T1.SHNCD as char)), 8), 1, 4) || '-' || substr(right('00000000' || rtrim(cast(t1.shncd as char)), 8), 5) as SHNCD"); // F3 ：商品コード
  ...
  private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
    ...
    try {
      ...
      String sqlUpdateCommand_1 = "update INAWS.PIMSIT set SITSTCD = 4, OPERATOR = ?, UPDDT = ? where SHNCD = ? and SITKNNO = ? and SITSTCD = 2";

      // 仕掛商品（INAWS.PIMSIT）の商品コードを更新
      String sqlUpdateCommand_2 = "update INAWS.PIMSIT set SHNCD = ?, URICD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ? and SITKNNO = ? and SITSTCD = 4";

      // 仕掛仕入グループ商品マスタ（INAWS.PIMSISIRGPSHN）の商品コードを更新
      String sqlUpdateCommand_2_2 =
          // "update INAWS.PIMSISIRGPSHN set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = (select SHNCD
          // from INAWS.PIMSIT where SITKNNO = ? and SHNCD = ? and SITSTCD = 4)";
          "update INAWS.PIMSISIRGPSHN set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ?";

      // 仕掛売価コントロールマスタ（INAWS.PIMSIBAIKACTL）の商品コードを更新
      String sqlUpdateCommand_2_3 = "update INAWS.PIMSIBAIKACTL set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ?";

      // 仕掛ソースコード管理マスタ（INAWS.PIMSISRCCD）の商品コードを更新
      String sqlUpdateCommand_2_4 = "update INAWS.PIMSISRCCD set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ?";

      // 仕掛添加物マスタ（INAWS.PIMSITENKABUTSU）の商品コードを更新
      String sqlUpdateCommand_2_5 = "update INAWS.PIMSITENKABUTSU set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ?";

      // 仕掛品揃グループマスタ（INAWS.PIMSISHINAGP）の商品コードを更新
      String sqlUpdateCommand_2_6 = "update INAWS.PIMSISHINAGP set SHNCD = ?, OPERATOR = ?, UPDDT = ? where SHNCD = ?";

      // 仕掛メーカーマスタは商品コードがないので、更新不要

      // 提案商品（INAWS.PIMTIT）の商品ステータス（SITSTCD）を完了（4）に更新
      String sqlUpdateCommand_3 =
          // "update INAWS.PIMTIT set TITSTCD = 4, OPERATOR = ?, UPDDT = ? where TITKNNO = (select SITKNNO
          // from INAWS.PIMSIT where SHNCD = ? and SITSTCD = 4) and SHNKN = ? and BMNCD = ? and DAICD = ? and
          // CHUCD = ? and TITSTCD = 3";
      ...
      for (int i = 0; i < dataArray.size(); i++) {
        ...
        // 販売コード付番管理テーブル更新
        prmData = new ArrayList<String>();
        String sqlUpdateCommand_uri = "merge into INAAD.SYSURICD_FU as T" + " using (" + "  select min(ID) as ID from INAAD.SYSURICD_FU" + " ) as RE on (T.ID = RE.ID) " + " when matched then "
            + " update set" + "  SUMINO = case when SUMINO+1 <=ENDNO then SUMINO+1 else SUMINO end" + // F4 : 付番済番号
        ...
        // 商品更新件数+1
        prmData = new ArrayList<String>();
        String sqlUpdateCommand_SYSSHN = "merge into INAAD.SYSSHNCOUNT as T" + " using (select" + "  cast(to_char(current date, 'YYYYMMDD') as integer) as UPDATEDT" + // F1 : 日付
            " ,cast(1 as SMALLINT) as UPDATECNT" + // F2 : 件数 insert用
            " ,current timestamp as UPDDT" + // F3 : 更新日
            " from SYSIBM.SYSDUMMY1" + " ) as RE on ( T.UPDATEDT = RE.UPDATEDT ) " + " when not matched then " + " insert" + " values(" + "  RE.UPDATEDT" + // F1 : 日付
            " ,RE.UPDATECNT" + // F2 : 件数
            " ,RE.UPDDT" + // F3 : 更新日
            " )";
   ...
   */
  @SuppressWarnings("static-access")
  private boolean updateDB(HashMap<String, String> map, User userInfo, String sysdate, Connection con, String code, String shohinSeq, String shohinCode, String userID, String strDateInfo,
      String sysDate, String sysTime, String kenno) throws Exception {
    // DB更新SQL
    StringBuffer sbSQL = new StringBuffer();
   ...
   */
  @SuppressWarnings("static-access")
  private boolean updateDBJNL(HashMap<String, String> map, User userInfo, String sysdate, Connection con, String code, String shohinSeq, String shohinCode, String userID, String strDateInfo,
      String sysDate, String sysTime, String kenno) throws Exception {
    // ジャーナル用変数取得
    /** ジャーナル更新のKEY保持用変数 */
    String jnlshn_seq = "";
  ...
  private JSONObject updateData2(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
    ...
    try {
      ...
      String sqlUpdateCommand_1 = "update INAWS.PIMSIT set SITSTCD = 9, OPERATOR = ?, UPDDT = ? where SHNCD = ? and SITKNNO = ? and SITSTCD = 2";
  ...
  private JSONObject updateData3(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
    ...
    try {
      ...
      String sqlUpdateCommand_1 = "update INAWS.PIMSIT set SITSTCD = 2, OPERATOR = ?, UPDDT = ? where SHNCD = ? and SITKNNO = ? and SITSTCD = 1";
