...
/*		// 必須チェック
		...
		sbSQL.append(") ");
		sbSQL.append(" select");
		sbSQL.append("    nvl(T3.CNT, 0) as F1");
		...
		sbSQL.append("    ||'～'||");
		sbSQL.append("    TO_CHAR(TO_DATE(T1.HBEDDT, 'YYYYMMDD'), 'YY/MM/DD') || (select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HBEDDT, 'YYYYMMDD'))) as F6");
		sbSQL.append("  , RIGHT(T2.GTSIMEDT, 6) as F7");
		sbSQL.append("  , (select JWEEK2 from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.GTSIMEDT, 'YYYYMMDD'))) as F8 ");
		sbSQL.append("  , T2.GTSIMEFLG as F9");
		sbSQL.append("  , RIGHT(T2.LSIMEDT, 6) as F10");
		sbSQL.append("  , (select JWEEK2 from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.LSIMEDT, 'YYYYMMDD'))) as F11");
		sbSQL.append("  , RIGHT(T2.QAYYYYMM, 4)  as F12");
		sbSQL.append("  , nvl(T2.QAENO, 0)  as F13");
		sbSQL.append("  , RIGHT(T2.QACREDT, 6)  as F14");
		sbSQL.append("  , (select JWEEK2 from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.QACREDT, 'YYYYMMDD'))) as F15");
		sbSQL.append("  , CASE WHEN T2.QARCREDT = '0' THEN null ELSE RIGHT(T2.QARCREDT, 6) END as F16");
		sbSQL.append("  , CASE WHEN T2.QARCREDT = '0' THEN null ELSE (select JWEEK2 from WEEK where T2.QARCREDT <> 0 and CWEEK = DAYOFWEEK(TO_DATE(T2.QARCREDT, 'YYYYMMDD'))) END as F17");
		sbSQL.append("  , RIGHT(T2.QADEVSTDT, 6)  as F18");
		sbSQL.append("  , (select JWEEK2 from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.QADEVSTDT, 'YYYYMMDD'))) as F19");
		...
		sbSQL.append("  , nvl(T5.CNT, 0) as F25");
		...
		sbSQL.append("  , TO_CHAR(T2.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as F31");
...
//			sbSQL.append(" inner join WK on T1.UPDKBN = 0 and T1.MOYSKBN= WK.MOYSKBN and T1.MOYSSTDT = WK.MOYSSTDT and T1.MOYSRBAN= WK.MOYSRBAN");
			...
			sbSQL.append(" ||lpad(nvl(to_char(T2.HBSTDT),''),8,'0')");				// 販売開始日 YYYYMMDDで設定 NULLの時ALL0/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T2.HBEDDT),''),8,'0')");				// 販売終了日 YYYYMMDDで設定 NULLの時ALL0/桁数に満たない時-
			sbSQL.append(" ||left(rpad(nvl(T2.MOYKN,''), 40, '　'), 40)");
			...
			sbSQL.append(" ||left(rpad(nvl(to_char(T1.HIGAWRFLG),''),1,' '),1)");	// 日替フラグ 0：通し　1：日替 NULLの時ALL半SP/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T1.HBSTDT),''),8,'0')");				// 販売開始日 YYYYMMDDで設定 NULLの時ALL0/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T1.HBEDDT),''),8,'0')");				// 販売終了日 YYYYMMDDで設定 NULLの時ALL0/桁数に満たない時-
			sbSQL.append(" ||left(rpad(nvl(T1.SHNCD,''),14,' '),14)");				// 商品コード  NULLの時ALL半SP/桁数に満たない時半SPACE
			sbSQL.append(" ||left(rpad(nvl(T1.SANCHIKN,''),40,'　'),40)");			// 産地  NULLの時ALL全SP/桁数に満たない時全SPACE
			sbSQL.append(" ||left(rpad(nvl(T1.MAKERKN,''),28,'　'),28)");			// メーカー名  NULLの時ALL全SP/桁数に満たない時全SPACE
			sbSQL.append(" ||left(rpad(nvl(T1.POPKN,''),40,'　'),40)");				// POP名称  NULLの時ALL全SP/桁数に満たない時全SPACE
			sbSQL.append(" ||left(rpad(nvl(T1.KIKKN,''),46,'　'),46)");				// 規格名称  NULLの時ALL全SP/桁数に満たない時全SPACE
			sbSQL.append(" ||left(rpad(nvl(to_char(T1.NAMANETUKBN),''),1,' '),1)");	// 生食加熱区分 1：生食　2：加熱 NULLの時ALL半SP/桁数に満たない時-
			sbSQL.append(" ||left(rpad(nvl(to_char(T1.KAITOFLG),''),1,' '),1)");	// 解凍フラグ 0：なし　1：解凍 NULLの時ALL半SP/桁数に満たない時-
			sbSQL.append(" ||left(rpad(nvl(to_char(T1.YOSHOKUFLG),''),1,' '),1)");	// 養殖フラグ 0：なし　1：養殖 NULLの時ALL半SP/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T1.GENKAAM_MAE),''),8,'0')");			// 事前原価 99999999で設定不定貫のときは1㎏原価 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.GENKAAM_ATO),''),8,'0')");			// 追加原価 99999999で設定不定貫のときは1㎏原価 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.IRISU),''),3,'0')");				// 入数  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.TKANPLUKBN),''),1,'0')");			// 定貫PLU・不定貫区分 1：定貫・PLU　2：不定貫　（鮮魚、精肉以外は0をセット） NULLの時ALL0/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T1.A_BAIKAAM),''),6,'0')");			// A総額売価 定貫：999999　不定貫：999999 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.B_BAIKAAM),''),6,'0')");			// B総額売価 定貫：999999　不定貫：999999 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.C_BAIKAAM),''),6,'0')");			// C総額売価 定貫：999999　不定貫：999999 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||left(rpad(nvl(to_char(T1.YORIFLG),''),1,' '),1)");		// よりどりフラグ 0：無　1：有 NULLの時ALL半SP/桁数に満たない時-
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD1_TENSU),''),3,'0')");			// バンドル1_点数  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD1_A_BAIKAAN),''),6,'0')");		// バンドル1_A総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD1_B_BAIKAAN),''),6,'0')");		// バンドル1_B総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD1_C_BAIKAAN),''),6,'0')");		// バンドル1_C総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD2_TENSU),''),3,'0')");			// バンドル2_点数  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD2_A_BAIKAAN),''),6,'0')");		// バンドル2_A総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD2_B_BAIKAAN),''),6,'0')");		// バンドル2_B総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.BD2_C_BAIKAAN),''),6,'0')");		// バンドル2_C総額売価  NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||lpad(nvl(to_char(T1.MEDAMAKBN),''),1,'0')");			// 目玉区分 1：大目玉　2：目玉　3：他 NULLの時ALL0/桁数に満たない時前0
			sbSQL.append(" ||left(rpad(nvl('先着'||lpad(to_char(T1.SEGN_NINZU),5,'　')||'名様'||'　', '')||nvl('お一人様'||lpad(to_char(T1.SEGN_GENTEI),3,'　')||'点限り', ''),58,'　'),58)");			// 制限 先着ＺＺＺＺ９名様　お一人様ＺＺ９点限り
			sbSQL.append(" ||left(rpad(nvl(T1.COMMENT_TB,''),60,'　'),60)");		// 特売コメント
			sbSQL.append(" ||left(rpad(nvl(T1.COMMENT_POP,''),100,'　'),100)");		// POPコメント
			sbSQL.append(" ||left(rpad(nvl(T1.COMMENT_HGW,''),100,'　'),100)");		// その他日替コメント
			sbSQL.append(" ||lpad(T2.DAICD,2,'0')");								// 大分類 前0埋め
			sbSQL.append(" ||lpad(T2.CHUCD,2,'0')");								// 中分類 前0埋め
			sbSQL.append(" ||lpad(T2.SHOCD,2,'0')");								// 小分類 前0埋め
			sbSQL.append(" ||left(rpad(nvl(to_char(T2.ZEIKBN),''),1,' '),1)");		// 税区分 0：外税　1：内税　2：非課税 3：部門に準拠
			sbSQL.append(" ||lpad(nvl(to_char(T2.ZEIRTKBN),''),3,'0')");			// 税率区分
			sbSQL.append(" ||lpad(nvl(to_char(T2.ZEIRTKBN_OLD),''),3,'0')");		// 旧税率区分
			sbSQL.append(" ||lpad(nvl(to_char(T2.ZEIRTHENKODT),''),8,'0')");		// 税率変更日 YYYYMMDDで設定
			sbSQL.append(" , "+cfi.getDataLen()+", ' ') as REC ");
			sbSQL.append(" from INATK.TOKTG_SHN T1");
			sbSQL.append(" inner join WK on T1.UPDKBN = 0 and T1.MOYSKBN = WK.MOYSKBN and T1.MOYSSTDT = WK.MOYSSTDT and T1.MOYSRBAN = WK.MOYSRBAN");
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		// --- 01-1.催し基本情報登録
		JSONObject result1 = this.createSqlTOKTG_KHN(userId, data);
		...
		if(newArray.size()==0 || !StringUtils.equals(data.optString(TOKTG_KHNLayout.QARCREDT.getId()), newArray.optJSONObject(0).optString(TOKTG_KHNLayout.QARCREDT.getCol()))){
			// --- 01-2.催し基本関連情報登録
			JSONObject result2 = this.createSqlTOKTG_KHN_RE(userId, data);
		}
	...
	public List<JSONObject> checkData(
			...
			HashMap<String, String> map, User userInfo, String sysdate1, MessageUtility mu, JSONArray dataArray			// 催し基本情報
		) {
	...
	/**
	 * 関連催し基本情報UPDATE SQL作成処理
	 * 再作成日の入力値は、同一アンケート月度、月度枝番のデータに対してUPDATEする。
	 * @param userId
	 * @param data
	 *
	 * @throws Exception
	...
	private JSONObject createSqlTOKTG_KHN_RE(String userId, JSONObject data) {
		...
		String[] target = new String[]{TOKTG_KHNLayout.MOYSKBN.getId(), TOKTG_KHNLayout.MOYSSTDT.getId(),TOKTG_KHNLayout.MOYSRBAN.getId(), TOKTG_KHNLayout.QAYYYYMM.getId(), TOKTG_KHNLayout.QAENO.getId(), TOKTG_KHNLayout.QARCREDT.getId()};
		...
		sbSQL.append(" using (select ");
		for(TOKTG_KHNLayout itm :TOKTG_KHNLayout.values()){
			if(!ArrayUtils.contains(target, itm.getId())){ continue;}	// 対象外
			if(itm.getNo() > 1){ sbSQL.append(","); }
			if(data.containsKey(itm.getId())){
				String value = StringUtils.strip(data.optString(itm.getId()));
				if(StringUtils.isNotEmpty(value)){
	...
	public static JSONArray getJSONArrayTOKTG_KHN(String sqlWhere, ArrayList<String> paramData) {
		...
		sbSQL.append(" where NVL(T1.UPDKBN, 0) <> 1" + sqlWhere);
