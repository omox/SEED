...
public class ReportTG003Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append("   ||'～'||");
		sbSQL.append("   TO_CHAR(TO_DATE(T1.HBEDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HBEDDT, 'YYYYMMDD'))) as DT");
		sbSQL.append("  ,nvl(TO_CHAR(TO_DATE(T2.QACREDT, 'YYYYMMDD'), 'YYYY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.QACREDT, 'YYYYMMDD'))), '____/__/__(_)') as QACREDT");
		sbSQL.append("  ,nvl(TO_CHAR(TO_DATE(T2.QARCREDT, 'YYYYMMDD'),'YYYY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.QARCREDT,'YYYYMMDD'))), '____/__/__(_)') as QARCREDT");
		...
		if(isNew){
			...
			sbSQL.append(" , max(nvl(TO_CHAR(T2.ADDDT, 'YY/MM/DD'),'__/__/__')) as ADDDT");
			sbSQL.append(" , max(nvl(TO_CHAR(T2.UPDDT, 'YY/MM/DD'),'__/__/__')) as UPDDT");
	...
	public String createSqlSelTOKTG_TEN(JSONObject data, boolean dispEmpty) {
		...
		sbSQL.append(" left join INAMS.MSTTEN M1 on T1.TENCD = M1.TENCD and nvl(M1.UPDKBN,0) <> 1");
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		JSONObject result1 = this.createSqlTOKTG_TENGP(userId, data, SqlType.MRG);

		// ************ 子テーブル処理 ***********
		JSONArray dataArrayDel = new JSONArray();
		// 子テーブルは、一度削除してから追加なので、キー項目に注意
		if(isChange){
			String[] keys = new String[]{TOKTG_TENLayout.MOYSKBN.getId(), TOKTG_TENLayout.MOYSSTDT.getId(), TOKTG_TENLayout.MOYSRBAN.getId(), TOKTG_TENLayout.TENGPCD.getId(), TOKTG_TENLayout.KYOSEIFLG.getId()};
		...
		if(dataArrayDel.size() > 0){
			JSONObject resultD2 = this.createSqlTOKTG_TEN(userId, dataArrayDel, SqlType.DEL);
		}
		if(dataArrayTENCD.size() > 0){
			JSONObject result2 = this.createSqlTOKTG_TEN(userId, dataArrayTENCD, SqlType.MRG);
		}
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		// --- 01.全店特売（アンケート有）_店グループ（論理削除）
		JSONObject result1 = this.createSqlTOKTG_TENGP(userId, data, SqlType.DEL);
...
//		if (outobj.equals(DefineReport.InpText.TENCD.getObj())||outobj.equals(DefineReport.InpText.TENCD.getObj()+"leader")) {
//			whr = " and nvl(UPDKBN, 0) =0 and MISEUNYOKBN = 9";
//			boolean isExist = super.getDbExist("INAMS.MSTTEN", "TENCD", val, whr, new ArrayList<>());
//			if(isExist){
//				// E20229	廃店は入力できません。	 	0	 	E
//				return MessageUtility.getDbMessageIdObj("E20229", new String[]{});
//			}
//		}
	...
	private JSONObject createSqlTOKTG_TENGP(String userId, JSONObject data, SqlType sql) {
		...
		for(TOKTG_TENGPLayout itm :TOKTG_TENGPLayout.values()){
			if(ArrayUtils.contains(notTarget, itm)){ continue;}	// 対象外
		...
		if(SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,TENGPCD=RE.TENGPCD");		// F4 : 店グループ
			sbSQL.append(" ,TENGPKN=RE.TENGPKN");		// F5 : 店グループ名称
			sbSQL.append(" ,KYOSEIFLG=RE.KYOSEIFLG");	// F6 : 強制グループフラグ
			sbSQL.append(" ,QASYUKBN=RE.QASYUKBN");		// F7 : アンケート種類
			sbSQL.append(" ,QACREDT_K=RE.QACREDT_K");	// F8 : アンケート作成日_強制
			sbSQL.append(" ,QARCREDT_K=RE.QARCREDT_K");	// F9 : アンケート再作成日_強制
	...
	private JSONObject createSqlTOKTG_TEN(String userId, JSONArray dataArray, SqlType sql) {
		...
		String sendflg = DefineReport.Values.SENDFLG_UN.getVal();
		if(SqlType.DEL.getVal() == sql.getVal()){
			szWhere += " and T.KYOSEIFLG = RE.KYOSEIFLG";
		}
	...
	public enum TOKTG_TENGPLayout implements MSTLayout{
		...
		/** 店グループ */
		TENGPCD(4,"TENGPCD","SMALLINT","店グループ"),
		/** 店グループ名称 */
		TENGPKN(5,"TENGPKN","VARCHAR(40)","店グループ名称"),
		/** 強制グループフラグ */
		KYOSEIFLG(6,"KYOSEIFLG","SMALLINT","強制グループフラグ"),
		/** アンケート種類 */
		QASYUKBN(7,"QASYUKBN","SMALLINT","アンケート種類"),
		/** アンケート作成日_強制 */
		QACREDT_K(8,"QACREDT_K","INTEGER","アンケート作成日_強制"),
		/** アンケート再作成日_強制 */
		QARCREDT_K(9,"QARCREDT_K","INTEGER","アンケート再作成日_強制"),
		/** 更新区分 */
		UPDKBN(10,"UPDKBN","SMALLINT","更新区分"),
		/** 送信フラグ */
		SENDFLG(11,"SENDFLG","SMALLINT","送信フラグ"),
		/** オペレータ */
		OPERATOR(12,"OPERATOR","VARCHAR(20)","オペレータ"),
		/** 登録日 */
	...
	public enum TOKTG_TENLayout implements MSTLayout{
		...
		MOYSRBAN(3,"MOYSRBAN","SMALLINT","催し連番"),
		/** 店コード */
		TENCD(4,"TENCD","SMALLINT","店コード"),
		/** 強制グループフラグ */
		KYOSEIFLG(5,"KYOSEIFLG","SMALLINT","強制グループフラグ"),
		/** 店グループ */
		TENGPCD(6,"TENGPCD","SMALLINT","店グループ"),
		/** リーダー店区分 */
		LDTENKBN(7,"LDTENKBN","SMALLINT","リーダー店区分"),
		/** 送信フラグ */
