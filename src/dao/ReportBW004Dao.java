...
public class ReportBW004Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" with T1(IDX) as ( select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < 500) ");
		sbSQL.append(" select");
		sbSQL.append("  T1.IDX");
		...
		sbSQL.append(" left outer join INAMS.MSTSHN M0 on M0.SHNCD = CTRH.SHNCD and NVL(M0.UPDKBN, 0) <> 1");
		sbSQL.append(" left outer join INAMS.MSTBMN M1 on M1.BMNCD = CTRH.BMNCD and NVL(M1.UPDKBN, 0) <> 1");
		...
		sbSQL.append(" order by CTRH.INPUTEDANO ) T2");
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		paramData.add(szSEQ);
		sqlcommand = "select COUNT(KKK.HBSTDT) as VALUE"
				+ " from INATK.TOKRS_KKK KKK"
				+ " inner join (select distinct HBSTDT, BMNCD, WRITUKBN, SEICUTKBN, DUMMYCD, MEISHOKN"
				+ " from INATK.CSVTOK_RSKKK where NVL(UPDKBN, 0) <> 1 and SEQ = ? ) CSV on CSV.HBSTDT = KKK.HBSTDT and CSV.BMNCD = KKK.BMNCD and CSV.WRITUKBN = KKK.WRITUKBN and CSV.SEICUTKBN = KKK.SEICUTKBN and CSV.DUMMYCD = KKK.DUMMYCD and CSV.MEISHOKN = KKK.MEISHOKN"
				+ " where NVL(KKK.UPDKBN, 0) <> 1";

		@SuppressWarnings("static-access")
		JSONArray result = iL.selectJSONArray(sqlcommand, paramData, Defines.STR_JNDI_DS);
		if(result.size() > 0 ){
		...
		paramData = new ArrayList<String>();
		sbSQL.append("  select");
		sbSQL.append(" RIGHT(TO_CHAR(CTRK.HBSTDT), 6) as F1");
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(array.size()==0){
			...
			sbSQL = new StringBuffer();
			sbSQL.append("MERGE INTO INATK.TOKRS_KKK AS T USING (SELECT ");
			...
			sbSQL.append(" T.HBSTDT = RE.HBSTDT and ");
			sbSQL.append(" T.BMNCD = RE.BMNCD and ");
			sbSQL.append(" T.WRITUKBN = RE.WRITUKBN and ");
			sbSQL.append(" T.SEICUTKBN = RE.SEICUTKBN and ");
			sbSQL.append(" T.DUMMYCD = RE.DUMMYCD ");
			...
			sbSQL.append(") values (");
			sbSQL.append(" RE.HBSTDT");			//販売開始日
			sbSQL.append(",RE.BMNCD");			//部門
			sbSQL.append(",RE.WRITUKBN");		//割引率区分
			sbSQL.append(",RE.SEICUTKBN");		//正規・カット
			sbSQL.append(",RE.DUMMYCD");		//ダミーコード
			sbSQL.append(",RE.MEISHOKN");		//管理番号
			sbSQL.append(",RE.UPDKBN");			//更新区分
			...
			if(StringUtils.isEmpty(getMessage())) {
				if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品_企画を " + MessageUtility.getMessage(Msg.S00003.getVal(),
			...
			values = StringUtils.removeStart(values, ",");
			sbSQL = new StringBuffer();
			sbSQL.append("MERGE INTO INATK.TOKRS_SHN AS T USING (SELECT ");
			sbSQL.append(" HBSTDT");			//販売開始日 F1
			sbSQL.append(",BMNCD");				//部門 F2
			sbSQL.append(",WRITUKBN");			//割引率区分 F3
			sbSQL.append(",SEICUTKBN");			//正規・カット F4
			...
			sbSQL.append(" T.WRITUKBN = RE.WRITUKBN and ");
			sbSQL.append(" T.SEICUTKBN = RE.SEICUTKBN and ");
			sbSQL.append(" T.DUMMYCD = RE.DUMMYCD and ");
			...
			sbSQL.append(",RE.SHNKN");			//商品名称
			sbSQL.append(",RE.KIKKN");			//規格名称
			sbSQL.append(",RE.IRISU");			//入数
			sbSQL.append(",RE.BAIKAAM");		//売価
			sbSQL.append(",RE.GENKAAM");		//原価
			sbSQL.append(",RE.MAKERKN");		//メーカー名
			sbSQL.append(",RE.UPDKBN");			//更新区分
			sbSQL.append(",RE.SENDFLG");		//送信フラグ
			sbSQL.append(",RE.OPERATOR");		//オペレーターコード
			sbSQL.append(",RE.ADDDT");			//登録日
			...
			if(StringUtils.isEmpty(getMessage())) {
				if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品_商品を " + MessageUtility.getMessage(Msg.S00003.getVal(),
						new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
				msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00001.getVal()));
			}else{
				msgObj.put(MsgKey.E.getKey(), getMessage());
			...
			values = StringUtils.removeStart(values, ",");
			sbSQL = new StringBuffer();
			sbSQL.append("MERGE INTO INATK.SYSRS AS T USING (SELECT ");
			sbSQL.append(" HBSTDT");			//販売開始日 F1
			sbSQL.append(",BMNCD");				//部門 F2
			sbSQL.append(",WRITUKBN");			//割引率区分 F3
			sbSQL.append(",SEICUTKBN");			//正規・カット F4
			...
			count = super.executeSQL(sbSQL.toString(), paramData);
			if(StringUtils.isEmpty(getMessage())) {
				if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品内部管理テーブルを " + MessageUtility.getMessage(Msg.S00003.getVal(),
						new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
				msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00001.getVal()));
			}else{
				msgObj.put(MsgKey.E.getKey(), getMessage());
			...
			count = super.executeSQL(sbSQL.toString(), paramData);
			if(StringUtils.isEmpty(getMessage())) {
				if (DefineReport.ID_DEBUG_MODE) System.out.println("CSV取込トラン_冷凍食品_企画を " + MessageUtility.getMessage(Msg.S00003.getVal(),
						new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
				msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00001.getVal()));
			}else{
				msgObj.put(MsgKey.E.getKey(), getMessage());
			...
			for(int i = 0; i < dataArray.size();i++){
				...
				//同一商品データが存在しない場合
					...
					iL = new ItemList();
					@SuppressWarnings("static-access")
					JSONArray array3 = iL.selectJSONArray(sbSQL.toString(), paramData, Defines.STR_JNDI_DS);
					int kanriNo ;
					boolean New;
					if(array3.size()==0){
						kanriNo = 1;
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo) throws Exception {
		...
		int count = super.executeSQL(sbSQL.toString(), paramData);
		if (StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("CSV取込トラン_冷凍食品_商品マスタを "
