...
public class ReportYH002Dao extends ItemDao {
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		if(DefineReport.Button.NEW.getObj().equals(sendBtnid)){
			...
			paramData.add(kkkcd);
			paramData.add(shncd);
			sqlcommand = "select COUNT(SHNCD) as value from INATK.HATYH_SHN where NVL(UPDKBN, 0) <> 1 and KKKCD = ? and SHNCD = ? ";
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(dataArray.size() > 0){
			...
			targetWhere = "NVL(UPDKBN, 0) <> "+DefineReport.ValUpdkbn.DEL.getVal()+" and KKKCD = ? and SHNCD = ? ";
	...
	public String createSqlSHN(JSONObject data, HashMap<String, String> map, User userInfo){
		...
		// 予約発注_商品の登録・更新
		sbSQL.append(" merge into INATK.HATYH_SHN as T using (select");
		...
		sbSQL.append(", CATALGNO=RE.CATALGNO");
		sbSQL.append(", HTDT=RE.HTDT");
		sbSQL.append(", UKESTDT=RE.UKESTDT");
		sbSQL.append(", UKEEDDT=RE.UKEEDDT");
		sbSQL.append(", TENISTDT=RE.TENISTDT");
		sbSQL.append(", TENIEDDT=RE.TENIEDDT");
		sbSQL.append(", YOTEISU=RE.YOTEISU");
		sbSQL.append(", GENDOSU=RE.GENDOSU");
		sbSQL.append(", NGFLG=RE.NGFLG");
		...
		sbSQL.append(", RE.CATALGNO");
		sbSQL.append(", RE.HTDT");
		sbSQL.append(", RE.UKESTDT");
		sbSQL.append(", RE.UKEEDDT");
		sbSQL.append(", RE.TENISTDT");
		sbSQL.append(", RE.TENIEDDT");
		sbSQL.append(", RE.YOTEISU");
		sbSQL.append(", RE.GENDOSU");
		sbSQL.append(", RE.NGFLG");
	...
	private String createCommand() {
		...
		if(DefineReport.Button.SEL_CHANGE.getObj().equals(sendBtnid)){
			...
			sbSQL.append(" select");
			sbSQL.append(" TO_CHAR(T1.KKKCD) || ' - ' || T1.KKKKM");
			...
			sbSQL.append(", TO_CHAR(SHN.UPDDT,'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT ");
			sbSQL.append(", SHN.OPERATOR");
			sbSQL.append(", TO_CHAR(SHN.ADDDT, 'yy/mm/dd') as ADDDT");
			sbSQL.append(", TO_CHAR(SHN.UPDDT, 'yy/mm/dd') as UPDDT");
			sbSQL.append(", SHN.NGFLG");
			sbSQL.append(" from (select");
			sbSQL.append(" T2.*, TO_CHAR(TO_DATE('20' || right ('0' || T2.NNSTDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NNSTDT_DSP");
			sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || T2.NNSTDT, 6), 'YYYYMMDD')) as STARTDTW");
			sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || T2.NNEDDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as NNEDDT_DSP");
			sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || T2.NNEDDT, 6), 'YYYYMMDD')) as ENDDTW");
