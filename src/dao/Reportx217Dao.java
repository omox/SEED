...
public class Reportx217Dao extends ItemDao {
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		ArrayList<JSONObject>	tenShncds	= new ArrayList<JSONObject>();
		HashSet<String>			tenShncds_	= new HashSet<String>();
		...
		for (int i=0; i<dataArrayG.size(); i++){
			...
			if(StringUtils.isNotEmpty(val)){
				tenShncds.add(dataArrayG.optJSONObject(i));
				tenShncds_.add(val);
			}
		}

		if(tenShncds.size() != tenShncds_.size()){
			msg.add(mu.getDbMessageObj("EX1024", new String[]{}));
			return msg;
		}
		*/
