 ...
 *
 */
public class Reportx217Dao extends ItemDao {
   ...
   * @param source
   */
  public Reportx217Dao(String JNDIname) {
    super(JNDIname);
  }
  ...
  public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
     ...
     * dataArrayG.optJSONObject(i).optString("F2") + dataArrayG.optJSONObject(i).optString("F4");
     * if(StringUtils.isNotEmpty(val)){ tenShncds.add(dataArrayG.optJSONObject(i)); tenShncds_.add(val);
     * } }
     *
     * if(tenShncds.size() != tenShncds_.size()){ msg.add(mu.getDbMessageObj("EX1024", new String[]{}));
     * return msg; }
     */
    ...
    for (int i = 0; i < dataArrayG.size(); i++) {
      ...
      if (StringUtils.isEmpty(data.optString("F2")) || StringUtils.isEmpty(data.optString("F4"))) {
        msg.add(mu.getDbMessageObj("E11049", new String[] {"店コードと商品コードを"}));
        return msg;
      }
  ...
  public String createSqlPc(JSONObject data, JSONArray dataArrayG, User userInfo) {
    ...
    if (StringUtils.isEmpty(data.optString("F4"))) {
      ...
      sbSQL.append("," + DefineReport.MaisuHohokbn.TEN.getVal());
