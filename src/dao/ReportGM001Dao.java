...
public class ReportGM001Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T1.HBSTDT,'%Y/%m/%d'),'%y/%m/%d') AS HBSTDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T1.HBEDDT,'%Y/%m/%d'),'%y/%m/%d') AS HBEDDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T1.NNSTDT,'%Y/%m/%d'),'%y/%m/%d') AS NNSTDTD ");
    sbSQL.append(",DATE_FORMAT(DATE_FORMAT(T1.NNEDDT,'%Y/%m/%d'),'%y/%m/%d') AS NNEDDTD ");
    ...
    sbSQL.append("FROM ");
    sbSQL.append(
        " INATK.TOKMOYCD T1 INNER JOIN INATK.TOKMM_KKK T2 ON T1.MOYSKBN=T2.MOYSKBN AND T1.MOYSSTDT=T2.MOYSSTDT AND T1.MOYSRBAN=T2.MOYSRBAN AND T2.UPDKBN=" + DefineReport.ValUpdkbn.NML.getVal());
    sbSQL.append(" WHERE " + sqlWhere + " AND T1.UPDKBN=" + DefineReport.ValUpdkbn.NML.getVal());
    sbSQL.append(" ORDER BY ");
    sbSQL.append(" 	T1.MOYSKBN ");
    sbSQL.append(",T1.MOYSSTDT ");
    sbSQL.append(",T1.MOYSRBAN)T2");
