...
public class ReportTG013Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append("  , MAX(case when TTQG.ITMANSFLG = 1 and TTQG.SCHANSFLG = 1 and TTQG.QASYUKBN = 1 then '〇' when (TTQG.ITMANSFLG = 0 or TTQG.SCHANSFLG = 0) and TTQG.QASYUKBN = 1 then '×' else '' END) as F11");
		sbSQL.append("  , MAX(case when TTQG.QASYUKBN = 2  then '1' else '0' END) as F12");
		sbSQL.append("  , MAX(case when TTQG.ITMANSFLG = 1 and TTQG.SCHANSFLG = 1 and TTQG.QASYUKBN = 2 then '〇' when (TTQG.ITMANSFLG = 0 or TTQG.SCHANSFLG = 0) and TTQG.QASYUKBN = 2  then '×' else '' END) as F13");
		sbSQL.append("  , MAX(case when TTQG.QASYUKBN = 3  then '1' else '0' END) as F14");
		sbSQL.append("  , MAX(case when TTQG.ITMANSFLG = 1 and TTQG.SCHANSFLG = 1 and TTQG.QASYUKBN = 3 then '〇' when (TTQG.ITMANSFLG = 0 or TTQG.SCHANSFLG = 0) and TTQG.QASYUKBN = 3  then '×' else '' END) as F15");
		sbSQL.append("  , MAX(case when TTQG.QASYUKBN = 4  then '1' else '0' END) as F16");
		sbSQL.append("  , MAX(case when TTQG.ITMANSFLG = 1 and TTQG.SCHANSFLG = 1 and TTQG.QASYUKBN = 4 then '〇' when (TTQG.ITMANSFLG = 0 or TTQG.SCHANSFLG = 0) and TTQG.QASYUKBN = 4  then '×' else '' END) as F17");
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		sbSQL.append("    ||'～'||");
		sbSQL.append("    TO_CHAR(TO_DATE(T1.HBEDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HBEDDT, 'YYYYMMDD'))) as F5");
