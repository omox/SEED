...
public class ReportRP006Dao extends ItemDao {
  ...
  public final static String ID_SQL_TENBETUBRT_RP006_UPD = "" + "WITH RECURSIVE T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) DUMMY union all select IDX + 1 from T1 where IDX < 400)"
      + ", T3 as (select MAX(TEN.TENCD) as MAXTEN ,TBN.BMNCD from INAMS.MSTTEN TEN inner join INAMS.MSTTENBMN TBN on TBN.TENCD = TEN.TENCD "
      + "and TBN.BMNCD = ? and COALESCE(TEN.UPDKBN, 0) <> 1 and COALESCE(TBN.UPDKBN, 0) <> 1)" + ", WK as (select TENRT_ARR as ARR, 5 as LEN, BMNCD" + " from INATK.TOKRTPTN" + " where BMNCD = ?"
      + " and RTPTNNO = ?" + " and UPDKBN = 0), ARRWK(IDX, BPRT, S, ARR, LEN, BMNCD) as (select 1, SUBSTR(ARR, 1, LEN), 1 + LEN, ARR, LEN ,BMNCD from WK"
      + " union all select IDX + 1, SUBSTR(ARR, S, LEN), S + LEN, ARR, LEN, BMNCD from ARRWK where S <= LENGTH(ARR))" + " select" + " right ('000' || T1.IDX, 3) as TENCD" // F1 : 店番
      + ", case when COALESCE(T2.MISEUNYOKBN, 9) = 9 then '' else T2.TENKN end as TENKN" // F2 : 店舗名
      + ", case when COALESCE(T2.MISEUNYOKBN, 9) = 9 then '' else case when TRIM(COALESCE(T4.BPRT, '')) <> '' then CAST(CAST(trim(T4.BPRT)AS CHAR)AS SIGNED) end end as BUNPAIRT" // F3 : 分配率(通常率)
      + ", case when COALESCE(T2.MISEUNYOKBN, 9) = 9 then 0 else 1 end as EDITFLG " // EDITFLG : 0:不可 1:可
      + " from T1 " + " left join ARRWK T4 on T4.IDX = T1.IDX INNER JOIN T3 ON T3.BMNCD=T4.BMNCD "
      + " left join (select TEN.* from INAMS.MSTTEN TEN inner join INAMS.MSTTENBMN TBN on TBN.TENCD = TEN.TENCD and TBN.BMNCD = ? and COALESCE(TEN.UPDKBN, 0) <> 1 and COALESCE(TBN.UPDKBN, 0) <> 1) T2 on T1.IDX = T2.TENCD and COALESCE(UPDKBN, 0) <> 1"
      + " where T1.IDX <= T3.MAXTEN" + " order by T1.IDX";

  public final static String ID_SQL_TENBETUBRT_RP006_NEW = "" + "WITH RECURSIVE T1(IDX) as (select 1 from (SELECT 1 AS DUMMY) DUMMY union all select IDX + 1 from T1 where IDX < 400)"
      + ", T3 as (select MAX(TEN.TENCD) as MAXTEN, TBN.BMNCD from INAMS.MSTTEN TEN inner join INAMS.MSTTENBMN TBN on TBN.TENCD = TEN.TENCD "
      + "and TBN.BMNCD = ? and COALESCE(TEN.UPDKBN, 0) <> 1 and COALESCE(TBN.UPDKBN, 0) <> 1)" + " select" + " right ('000' || T1.IDX, 3) as TENCD" // F1 : 店番
      + ", case when COALESCE(T2.MISEUNYOKBN, 9) = 9 then '' else T2.TENKN end as TENKN" // F2 : 店舗名
  ...
  public final static String ID_SQL_TENBETUBRT_RP010 = ""
      ...
      + " WITH RECURSIVE T1(IDX) as (select 1 from (SELECT 1 AS DUMMY)DUMMY union all select IDX + 1 from T1 where IDX < 400)"
      + ", T3 as (select MAX(TEN.TENCD) as MAXTEN ,TBN.BMNCD from INAMS.MSTTEN TEN inner join INAMS.MSTTENBMN TBN on TBN.TENCD = TEN.TENCD "
      + "and TBN.BMNCD = ? and COALESCE(TEN.UPDKBN, 0) <> 1 and COALESCE(TBN.UPDKBN, 0) <> 1)" + ", WK as (select" + " TENTEN_ARR as ARR_TEN" + ", TENURI_ARR as ARR_URI" + ", 9 as LEN, BMNCD"
      + " from INATK.TOKJRTPTN" + " where BMNCD = ?" + " and WWMMFLG = ?" + " and YYMM = ?" + " and DAICD = ?" + " and CHUCD = ?)"
      ...
      + ", ARRWK_URI(IDX, URI, S, ARR, LEN, BMNCD) as (select 1, SUBSTR(ARR_URI, 1, LEN), 1 + LEN, ARR_URI, LEN, BMNCD from WK union all select IDX + 1, SUBSTR(ARR, S, LEN), S + LEN, ARR, LEN, BMNCD from ARRWK_URI where S <= LENGTH(ARR))"
      + " select" + " right ('000' || T1.IDX, 3) as TENCD" // F1 : 店番
      + ", M1.TENKN" // F2 : 店舗名
      + ", case when TRIM(T4.TENSU) = '' then null else TRIM(T4.TENSU) end as TENSU" // F3 : 売上(実績率)
      + ", case when TRIM(T5.URI) = '' then null else TRIM(T5.URI) end as URIAGE" // F4 : 点数(実績率)
      + " from T1 left" + " join ARRWK_TENSU T4 on T4.IDX = T1.IDX" + " left join ARRWK_URI T5 on T5.IDX = T1.IDX"
      + " LEFT OUTER JOIN T3 ON T3.BMNCD=T5.BMNCD  left join INAMS.MSTTEN M1 on M1.TENCD = T1.IDX and COALESCE(M1.UPDKBN, 0) <> 1"
  ...
  private String createCommand() {
    ...
    if (DefineReport.Button.SEL_TENBETUBRT.getObj().equals(sendBtnid)) {
      ...
      sbSQL.append(", (select T2.BMNKN from INAMS.MSTBMN T2 where T2.BMNCD = " + szBmncd + ") AS BMNKN");
      sbSQL.append(", (select T3.DAIBRUIKN from INAMS.MSTDAIBRUI T3 where T3.BMNCD = " + szBmncd + " and T3.DAICD =" + szDaicd + ") AS DAIBRUIKN");
      sbSQL.append(", (select T4.CHUBRUIKN from INAMS.MSTCHUBRUI T4 where T4.BMNCD = " + szBmncd + " and T4.DAICD =" + szDaicd + " and T4.CHUCD =" + szChucd + ") AS CHUBRUIKN");
   ...
   *
   * sqlList.add(sbSQL.toString()); prmList.add(prmData); lblList.add("通常率パターン");
   *
  ...
  public String createSqlRtptn(JSONObject data, JSONArray dataArrayRtptn, User userInfo) {
    ...
    sbSQL.append(" INSERT INTO INATK.TOKRTPTN (");
    sbSQL.append("  BMNCD "); // F1 : 部門
    sbSQL.append(" ,RTPTNNO "); // F2 : 率パターン№
    ...
    sbSQL.append(",RTPTNNO=VALUES(RTPTNNO) ");
    sbSQL.append(",RTPTNKN=VALUES(RTPTNKN) ");
    sbSQL.append(",TENRT_ARR=VALUES(TENRT_ARR) ");
   ...
   * @throws Exception
   */
  public JSONArray checkData(MessageUtility mu, JSONArray dataArray, // キー項目
      JSONArray dataArrayRtptn, String sendBtnid) {
