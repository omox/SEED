...
public class ReportJU012Dao extends ItemDao {
   ...
   * @throws Exception
   */
  public JSONArray checkData(MessageUtility mu, JSONArray dataArray, // 事前打出し商品(アン付含む)
      String shoriDt, String report) {
    ...
    for (int i = 0; i < dataArray.size(); i++) {
      ...
      for (TOKJU_CKLayout colinf : TOKJU_CKLayout.values()) {
        ...
        if (StringUtils.isNotEmpty(val)) {
          ...
          if (colinf.getCol().equals(TOKJU_CKLayout.MOYSKBN.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.MOYSSTDT.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.MOYSRBAN.getCol())) {
            txt = "催しコードは";
          }
    ...
    targetCol = new TOKJU_CKLayout[] {TOKJU_CKLayout.MOYSKBN, TOKJU_CKLayout.MOYSSTDT, TOKJU_CKLayout.MOYSRBAN, TOKJU_CKLayout.SHNCD, TOKJU_CKLayout.SHNKBN, TOKJU_CKLayout.TSEIKBN,
        TOKJU_CKLayout.HTDT, TOKJU_CKLayout.NNDT, TOKJU_CKLayout.IRISU, TOKJU_CKLayout.GENKAAM, TOKJU_CKLayout.BAIKAAM, TOKJU_CKLayout.TENHTSU_ARR, TOKJU_CKLayout.SURYO};
    ...
    for (int i = 0; i < dataArray.size(); i++) {
      ...
      for (TOKJU_CKLayout colinf : targetCol) {
        ...
        if (StringUtils.isEmpty(val)) {
          ...
          if (colinf.getCol().equals(TOKJU_CKLayout.MOYSKBN.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.MOYSSTDT.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.MOYSRBAN.getCol())) {
            txt = "催しコード";
          } else if (shnkbn.equals("0")
              && (colinf.getCol().equals(TOKJU_CKLayout.IRISU.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.GENKAAM.getCol()) || colinf.getCol().equals(TOKJU_CKLayout.BAIKAAM.getCol()))) {
            continue;
          }
    ...
    for (int i = 0; i < dataArray.size(); i++) {
      ...
      // 発注日範囲チェック
      if (!StringUtils.isEmpty(data.optString("F11")) && !StringUtils.isEmpty(data.optString("F10")) && data.optInt("F11") <= data.optInt("F10")) {
        // 納入日 <= 発注日はエラー
      ...
      if (!moysCd.contains(key)) {
        // 納入日の範囲チェック
        if (dbDatas.getJSONObject(0).optInt("NNSTDT") > data.optInt("F11") || dbDatas.getJSONObject(0).optInt("NNEDDT") < data.optInt("F11")) {
          // 納入日 <= 発注日はエラー
  ...
  public JSONObject createSqlJuCsv(JSONArray dataArray, Set<Integer> addTen, HashMap<String, String> tenSuryoMap, String report, User userInfo) {
    ...
    for (int i = 0; i < dataArray.size(); i++) {
      ...
      sbSQL.append(", TSEIKBN = NEW.TSEIKBN "); 
      sbSQL.append(", JUKBN = NEW.JUKBN "); 
      sbSQL.append(", BDENKBN = NEW.BDENKBN "); 
      sbSQL.append(", WAPPNKBN = NEW.WAPPNKBN "); 
      ...
      sbSQL.append(", TENHTSU_ARR = NEW.TENHTSU_ARR "); 
      sbSQL.append(", KANRINO = NEW.KANRINO "); 
      sbSQL.append(", JUTENKAIKBN = NEW.JUTENKAIKBN "); 
  ...
  public enum TOKJULayout implements MSTLayout {
    ...
    /** 催し区分 */
    MOYSKBN(1, "MOYSKBN", "SIGNED"),
    /** 催し開始日 */
    MOYSSTDT(2, "MOYSSTDT", "SIGNED"),
    /** 催し連番 */
    MOYSRBAN(3, "MOYSRBAN", "SIGNED"),
    /** 商品コード */
    SHNCD(4, "SHNCD", "CHARACTER(14)"),
    /** 商品区分 */
    SHNKBN(5, "SHNKBN", "SIGNED"),
    /** 訂正区分 */
    TSEIKBN(6, "TSEIKBN", "SIGNED"),
    /** 事前区分 */
    JUKBN(7, "JUKBN", "SIGNED"),
    /** 別伝区分 */
    BDENKBN(8, "BDENKBN", "SIGNED"),
    /** ワッペン区分 */
    WAPPNKBN(9, "WAPPNKBN", "SIGNED"),
    /** 発注日 */
    HTDT(10, "HTDT", "SIGNED"),
    /** 納入日 */
    NNDT(11, "NNDT", "SIGNED"),
    /** 入数 */
    IRISU(12, "IRISU", "SIGNED"),
    /** 原価 */
    GENKAAM(13, "GENKAAM", "DECIMAL(8,2)"),
    /** 売価 */
    BAIKAAM(14, "BAIKAAM", "SIGNED"),
    /** 店発注数配列 */
    TENHTSU_ARR(15, "TENHTSU_ARR", "VARCHAR(2000)"),
    /** 行番号 */
    GYONO(16, "GYONO", "SIGNED"),
    /** 管理番号 */
    KANRINO(17, "KANRINO", "SIGNED");
