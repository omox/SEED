...
public class ReportSK003Dao extends ItemDao {
  ...
  public enum CSVSHNLayout implements MSTLayout {
    ...
    public String getId2() {
      return "F" + Integer.toString(no + HATSKLayout.values().length);
    }
  ...
  private String createCommand() {
    ...
    if (StringUtils.equals(DefineReport.Button.SEL_CHANGE.getObj(), sendBtnid) || StringUtils.equals(DefineReport.Button.SEL_REFER.getObj(), sendBtnid)) {
      ...
      sbSQL.append(", DATE_FORMAT(HSK.ADDDT, '%y/%m/%d')");
      sbSQL.append(", DATE_FORMAT(HSK.UPDDT, '%y/%m/%d')");
      sbSQL.append(", HSK.OPERATOR");
      sbSQL.append(", DATE_FORMAT(HSK.UPDDT, '%Y%m%d%H%i%s%f') as HDN_UPDDT");
      ...
      sbSQL.append(" and right ('00000' || HSK.INPUTNO, 5) = " + szInputNo);
    }
  ...
  public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
    ...
    // グリッドエラーチェック：店舗一覧
    /*
     * ArrayList<JSONObject> record = new ArrayList<JSONObject>(); HashSet<String> parameter_ = new
     * HashSet<String>(); for (int i = 0; i < dataArrayTEN.size(); i++) { data =
     * dataArray.getJSONObject(0);
     *
  ...
  public String createSqlSTKTHAT(JSONObject data, HashMap<String, String> map, User userInfo) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (valueData.length >= 100 || (i + 1 == len && valueData.length > 0)) {
        ...
        // 新店改装店発注_商品の登録・更新
        sbSQL = new StringBuffer();
        sbSQL.append(" REPLACE INTO INATK.HATSK_SHN (");
        sbSQL.append(" INPUTNO"); // 入力№
    ...
    /*
     * if(dataArrayTDel.size() > 0){ if(StringUtils.equals("1", allCheck)){ // 親テーブル削除
     * this.createSqlDelHATSK(userInfo, inputNo);
     *
     * }else{ ArrayList<String> paramData = new ArrayList<String>(); for (int j = 0; j <
     * dataArrayTDel.size(); j++) { data = dataArrayTDel.getJSONObject(j); if(data.isEmpty()){ continue;
     * }
     *
     * paramData = new ArrayList<String>(); paramData.add(data.getString("F1"));
     * paramData.add(data.getString("F2"));
     *
     * sbSQL = new StringBuffer();
     * sbSQL.append(" delete from INATK.HATSK_SHN where INPUTNO = ? and KANRINO = ? ");
     *
     * sqlList.add(sbSQL.toString()); prmList.add(paramData); lblList.add("新店改装店発注_商品"); } } }
     */
    return sbSQL.toString();
  }
  ...
  public JSONObject createSqlSYSSK(User userInfo, String Sqlprm, int count) {
    ...
    sbSQL.append(", UPDDT "); // 更新日：
    sbSQL.append(") VALUES ( ");
    sbSQL.append(Sqlprm);// 入力No.
    sbSQL.append("," + count);// 付番済番号
  ...
  public String getINPUTNO(String Sqlprm) {
    ...
    String sqlcommand = "select SUMI_KANRINO as F1 from INATK.SYSSK where right ('00000' || INPUTNO, 5) = ?";
    @SuppressWarnings("static-access")
    JSONArray array = iL.selectJSONArray(sqlcommand, paramData, Defines.STR_JNDI_DS);
    String value = "";
    if (array.size() > 0) {
