 ...
 *
 */
public class Reportx214Dao extends ItemDao {
	 ...
	 * @param source
	 */
	public Reportx214Dao(String JNDIname) {
		super(JNDIname);
	}
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		// コピー枚数にマイナス値の入力は
		if (dataArray.getJSONObject(0).optInt("F5") < 0) {
			msg.add(mu.getDbMessageObj("EX1047", new String[]{"コピー枚数 ≧ 0の値"}));
			return msg;
		}
	...
	public String createSqlPc(JSONObject data, JSONArray dataArrayG, User userInfo){
		...
		if(StringUtils.isEmpty(data.optString("F4"))){
			...
			sbSQL.append("," + DefineReport.MaisuHohokbn.DOMAISU.getVal());
	...
	private String createCommand() {
		...
		sbSQL.append(",COPYSU ");													// F5	: コピー枚数
		sbSQL.append(",OPERATOR ");													// F6	: オペレーター
		sbSQL.append(",TO_CHAR(ADDDT,'YY/MM/DD') AS ADDDT ");						// F7	: 登録日
		sbSQL.append(",TO_CHAR(UPDDT,'YY/MM/DD') AS UPDDT ");						// F8	: 更新日
		sbSQL.append(",TO_CHAR(UPDDT,'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT ");		// F9	: 更新日時
		sbSQL.append("FROM ");
		sbSQL.append("INAMS.TRNPCARD ");
		sbSQL.append("WHERE ");
		sbSQL.append(sqlWhere);
