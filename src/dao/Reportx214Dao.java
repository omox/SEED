...
public class Reportx214Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    sbSQL.append(",DATE_FORMAT(ADDDT,'%y/%m/%d') AS ADDDT "); // F7 : 登録日
    sbSQL.append(",DATE_FORMAT(UPDDT,'%y/%m/%d') AS UPDDT "); // F8 : 更新日
    sbSQL.append(",DATE_FORMAT(UPDDT,'%Y%m%d%H%i%s%f') as HDN_UPDDT "); // F9 : 更新日時
    sbSQL.append("FROM ");
    sbSQL.append("INAMS.TRNPCARD ");
    sbSQL.append("WHERE ");
    sbSQL.append(sqlWhere);
