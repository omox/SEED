...
public class Reportx131Dao extends ItemDao {
	...
	private String createCommand() {
		...
		String szTenkn	 = getMap().get("TENKN");				// 店舗名称（漢字）
		...
		sbSQL.append("with T1(IDX) as ( ");
		sbSQL.append("  select");
		sbSQL.append("    1");
		sbSQL.append("  from");
		sbSQL.append("    SYSIBM.SYSDUMMY1");
		...
		sbSQL.append("  , TO_CHAR(T1.ADDDT, 'yy/mm/dd')");
		sbSQL.append("  , TO_CHAR(T1.UPDDT, 'yy/mm/dd') ");
		sbSQL.append("  , 0 ");
		sbSQL.append(" , TO_CHAR(TYHB.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT");			// :更新日時
		sbSQL.append(" from");
		sbSQL.append("  INAMS.MSTTENYOBIHTBMN TYHB ");
		sbSQL.append("  left join (select TENCD, MIN(ADDDT) as ADDDT, MAX(UPDDT) as UPDDT from INAMS.MSTTENYOBIHTBMN where NVL(UPDKBN, 0) <> 1 group by TENCD) T1 on T1.TENCD = TYHB.TENCD");
		sbSQL.append("  left join (select distinct TENCD, OPERATOR from INAMS.MSTTENYOBIHTBMN where NVL(UPDKBN, 0) <> 1 and  TENCD = ? and UPDDT = (select MAX(UPDDT) from INAMS.MSTTENYOBIHTBMN where NVL(UPDKBN, 0) <> 1 and TENCD = ? group by TENCD)) T2 on T2.TENCD = TYHB.TENCD ");
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo) throws Exception {
		...
		if (StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("店舗曜日別発注部門マスタを "
	...
	public List<JSONObject> checkData(
		...
		JSONArray msg = new JSONArray();
		String ErrTblNm = "MSTTENYOBIHTBMN";
...
/*		if(this.checkMstExist(isNew,data.optString("F1"),data.optString("F2"))){
			JSONObject o;
		...
		if(!isChange&&this.checkMstExist(isNew,data.optString("F1"),data.optString("F3"))){
			JSONObject o;
			if(isNew){
				o = mu.getDbMessageObj("E00004", new String[]{});
			}else{
	...
	public String createSqlTENYOBIHTBMN(HashMap<String, String> map, User userInfo){
		...
		for (int i = 0; i < len; i++) {
			...
			if(valueData.length >= 100 || (i+1 == len && valueData.length > 0)){
				...
				sbSQL.append(" merge into INAMS.MSTTENYOBIHTBMN as T using (select");
				...
				sbSQL.append(" TENCD");
				sbSQL.append(", BMNCD");
				sbSQL.append(", HATFLG_MON");
				sbSQL.append(", HATFLG_TUE");
				sbSQL.append(", HATFLG_WED");
				sbSQL.append(", HATFLG_THU");
				sbSQL.append(", HATFLG_FRI");
				sbSQL.append(", HATFLG_SAT");
				sbSQL.append(", HATFLG_SUN");
				...
				sbSQL.append(", HATFLG_MON=RE.HATFLG_MON");
				sbSQL.append(", HATFLG_TUE=RE.HATFLG_TUE");
				sbSQL.append(", HATFLG_WED=RE.HATFLG_WED");
				sbSQL.append(", HATFLG_THU=RE.HATFLG_THU");
				sbSQL.append(", HATFLG_FRI=RE.HATFLG_FRI");
				sbSQL.append(", HATFLG_SAT=RE.HATFLG_SAT");
				sbSQL.append(", HATFLG_SUN=RE.HATFLG_SUN");
				...
				sbSQL.append(", RE.HATFLG_MON");
				sbSQL.append(", RE.HATFLG_TUE");
				sbSQL.append(", RE.HATFLG_WED");
				sbSQL.append(", RE.HATFLG_THU");
				sbSQL.append(", RE.HATFLG_FRI");
				sbSQL.append(", RE.HATFLG_SAT");
				sbSQL.append(", RE.HATFLG_SUN");
