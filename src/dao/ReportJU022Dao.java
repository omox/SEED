...
public class ReportJU022Dao extends ItemDao {
	...
	public String createSqlTokQju(JSONObject data, HashMap<String, String>map, User userInfo){
		...
		sbSQL.append(",QASMDT ");													// F4	: アンケート締め切り日
		...
		sbSQL.append(",RE.QASMDT");													// アンケート締め切り日：
	...
	private String createCommand() {
		...
		sbSQL.append(",TO_CHAR(TO_DATE(T2.QASMDT,'YYYYMMDD'),'YY/MM/DD') AS QASMDT ");							// F2	: アンケート締切日
		sbSQL.append(",T2.OPERATOR ");																			// F3	: オペレーター
		sbSQL.append(",TO_CHAR(T2.ADDDT,'YY/MM/DD') AS ADDDT ");												// F4	: 登録日
		sbSQL.append(",TO_CHAR(T2.UPDDT,'YY/MM/DD') AS UPDDT ");												// F5	: 更新日
		sbSQL.append(",TO_CHAR(T2.UPDDT,'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT ");								// F6	: 更新日時
		sbSQL.append("FROM ");
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		if (!isDel) {
			...
			sbSQL = new StringBuffer();
			sbSQL.append("SELECT ");
			sbSQL.append("TO_CHAR(TO_DATE(NNSTDT,'YYYYMMDD') - 2 DAYS,'YYYYMMDD') AS NNSTDT ");
