...
public class ReportTR016Dao extends ItemDao {
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		if (StringUtils.isEmpty(bmnCd)) {
			sqlWhere += "INTEGER(SUBSTR(SHNCD,1,2))=null AND ";
		} else {
			sqlWhere += "INTEGER(SUBSTR(SHNCD,1,2))=? AND ";
			paramData.add(bmnCd);
		}
	...
	public String createDelSqlHt(JSONObject data, User userInfo){
		...
		/*sbSQL.append("(SELECT SHN.SHNCD FROM INATK.HATSTR_SHN SHN ");
		sbSQL.append("WHERE ");
		sbSQL.append("EXISTS( ");
		sbSQL.append("SELECT ");
		sbSQL.append("1 ");
		...
		sbSQL.append("HT.SHNCD  ");
		sbSQL.append("FROM ");
		sbSQL.append("INATK.HATSTR_TEN HT ");
		sbSQL.append("WHERE ");

		// 部門コード
		if (StringUtils.isEmpty(bmnCd)) {
			sqlWhere = "SUBSTR(HT.SHNCD, 1, 2)=null AND ";
		}else{
			sqlWhere = "SUBSTR(HT.SHNCD, 1, 2)=? AND ";
			paramData.add(bmnCd);
		}
