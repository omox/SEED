...
public class ReportTG016Dao extends ItemDao {
  ...
  public String createSqlSelMSTSHN(String szMoysKbn, String szMoysstdt, String shncd) {
    ...
    // 【販売情報部分】
    sbSQL.append(" ,SUBSTRING(M0.SANCHIKN,1,14) as F104"); // F104：メーカー名
    ...
    if (isNew) {
      ...
      if (isTOKTG) {
        ...
        if (dataArrayNNDT.size() > 0) {
          ...
          this.deleteSqlTOKTG_NNDT(userId, dataArrayNNDT, data, SqlType.MRG);
        }
        ...
        if (dataArrayNNDT.size() > 0) {
          ...
          this.deleteSqlTOKSP_NNDT(userId, dataArrayNNDT, data, SqlType.MRG);
        }
        // 全店特売（アンケート無）_販売
  ...
  private JSONObject createSqlTOKTG_SHN(String userId, JSONObject data, SqlType sql) {
    ...
    String[] notTarget = new String[] {TOKTG_SHN_MySQL_Layout.UPDKBN.getId(), TOKTG_SHN_MySQL_Layout.SENDFLG.getId(), TOKTG_SHN_MySQL_Layout.OPERATOR.getId(), TOKTG_SHN_MySQL_Layout.ADDDT.getId(),
        TOKTG_SHN_MySQL_Layout.UPDDT.getId()};
  ...
  private JSONObject createSqlTOKSP_SHN(String userId, JSONObject data, SqlType sql) {
    ...
    String[] notTarget = new String[] {TOKSP_SHN_MySQL_Layout.UPDKBN.getId(), TOKSP_SHN_MySQL_Layout.SENDFLG.getId(), TOKSP_SHN_MySQL_Layout.OPERATOR.getId(), TOKSP_SHN_MySQL_Layout.ADDDT.getId(),
        TOKSP_SHN_MySQL_Layout.UPDDT.getId()};
    String[] keys = this.getIds(TOK_CMN_MySQL_Layout.values());
  ...
  protected JSONObject createSqlTOKTG_NNDT(String userId, JSONArray dataArray, JSONObject data, SqlType sql) {
    ...
    for (TOKTG_NNDT_MySQL_Layout itm : TOKTG_NNDT_MySQL_Layout.values()) {
      // 不要
      // パラメータ不要
      if (ArrayUtils.contains(notUse, itm.getId()) || ArrayUtils.contains(notTarget, itm.getId()) || ArrayUtils.contains(keys, itm.getId()) || itm.getId().equals(TOKTG_NNDT_MySQL_Layout.SENDFLG.getId())) {
        continue;
      } // 上記で実施
    ...
    sbSQL.append(" WHERE T1.SENDFLG = 1 ");
    sbSQL.append(") AS T1 ");
    sbSQL.append("ON DUPLICATE KEY UPDATE ");
  ...
  /**
   * 全店特売(アンケート有)_納入日_delete SQL作成処理
   *
   * @param userId
   * @param data
   *
   * @throws Exception
   */
  protected JSONObject deleteSqlTOKTG_NNDT(String userId, JSONArray dataArray, JSONObject data, SqlType sql) {
    JSONObject result = new JSONObject();
    ...
    for (TOKTG_NNDT_MySQL_Layout itm : TOKTG_NNDT_MySQL_Layout.values()) {
      // パラメータ不要
       // 上記で実施
      if (ArrayUtils.contains(notTarget, itm.getId()) || ArrayUtils.contains(keys, itm.getId()) || itm.getId().equals(TOKTG_NNDT_MySQL_Layout.SENDFLG.getId())) {
        continue;
      }
      sbSQL.append(",cast(T1." + itm.getCol() + " as " + itm.getTyp() + " ) as " + itm.getCol());
    }
    ...
    sbSQL.append("  FROM (values " + rows + ") as T1(" + names + ")");
    sbSQL.append(" WHERE T1.SENDFLG = 0 ");
    sbSQL.append(") AS T1 ");
    sbSQL.append(") ");
    sbSQL.append("DELETE FROM INATK.TOKTG_NNDT AS T2 ");
    sbSQL.append("WHERE T2.MOYSKBN IN (SELECT MOYSKBN FROM T1) ");
    sbSQL.append("AND T2.MOYSSTDT IN (SELECT MOYSSTDT FROM T1) ");
    sbSQL.append("AND T2.MOYSRBAN IN (SELECT MOYSRBAN FROM T1) ");
    sbSQL.append("AND T2.BMNCD IN (SELECT BMNCD FROM T1) ");
    sbSQL.append("AND T2.KANRINO IN (SELECT KANRINO FROM T1) ");
    sbSQL.append("AND T2.KANRIENO IN (SELECT KANRIENO FROM T1) ");
    sbSQL.append("AND T2.NNDT IN (SELECT NNDT FROM T1) ");
    ...
    sqlList.add(sbSQL.toString());
    prmList.add(prmData);
    lblList.add("全店特売(アンケート有)_納入日_削除");
    return result;
  }
  ...
  protected JSONObject createSqlTOKSP_NNDT(String userId, JSONArray dataArray, JSONObject data, SqlType sql) {
    ...
    for (TOKSP_NNDT_MySQL_Layout itm : TOKSP_NNDT_MySQL_Layout.values()) {
      // パラメータ不要
       // 上記で実施
      if (ArrayUtils.contains(notTarget, itm.getId()) || ArrayUtils.contains(keys, itm.getId()) || itm.getId().equals(TOKSP_NNDT_MySQL_Layout.SENDFLG.getId())) {
        continue;
      }
      sbSQL.append(",cast(T1." + itm.getCol() + " as " + itm.getTyp() + " ) as " + itm.getCol());
    }
  ...
  /**
   * 全店特売(アンケート無)_納入日_delete SQL作成処理
   *
   * @param userId
   * @param data
   *
   * @throws Exception
   */
  protected JSONObject deleteSqlTOKSP_NNDT(String userId, JSONArray dataArray, JSONObject data, SqlType sql) {
    JSONObject result = new JSONObject();
    ...
    sbSQL.append(") AS T1 ");
    sbSQL.append(") ");
    sbSQL.append("DELETE FROM INATK.TOKSP_NNDT AS T2 ");
    sbSQL.append("WHERE T2.MOYSKBN IN (SELECT MOYSKBN FROM T1) ");
    sbSQL.append("AND T2.MOYSSTDT IN (SELECT MOYSSTDT FROM T1) ");
    sbSQL.append("AND T2.MOYSRBAN IN (SELECT MOYSRBAN FROM T1) ");
    sbSQL.append("AND T2.BMNCD IN (SELECT BMNCD FROM T1) ");
    ...
    sqlList.add(sbSQL.toString());
    prmList.add(prmData);
    lblList.add("全店特売(アンケート無)_納入日_削除");
    return result;
  }
  ...
  public enum TOKTG_NNDT_MySQL_Layout implements MSTLayout {
    ...
    /** 店変更フラグ配列 */
    TENCHGFLG_ARR(9, "TENCHGFLG_ARR", "CHAR(400)", "店変更フラグ配列"),
    /** 発注総数 */
    HTASU(10, "HTASU", "SIGNED", "発注総数"),
    /** パターン№ */
    PTNNO(11, "PTNNO", "SIGNED", "パターン№"),
    /** 訂正区分 */
    TSEIKBN(12, "TSEIKBN", "SIGNED", "訂正区分"),
    /** 店舗数 */
    TPSU(13, "TPSU", "SIGNED", "店舗数"),
    /** 展開数 */
    TENKAISU(14, "TENKAISU", "SIGNED", "展開数"),
    /** 前年実績フラグ */
    ZJSKFLG(15, "ZJSKFLG", "SIGNED", "前年実績フラグ"),
    /** 週間発注処理日 */
    WEEKHTDT(16, "WEEKHTDT", "SIGNED", "週間発注処理日"),
    /** 送信フラグ */
    SENDFLG(17, "SENDFLG", "SIGNED", "送信フラグ"),
    /** オペレータ */
    OPERATOR(18, "OPERATOR", "CHAR(20)", "オペレータ"),
    /** 登録日 */
    ADDDT(19, "ADDDT", "TIMESTAMP", "登録日"),
    /** 更新日 */
    UPDDT(20, "UPDDT", "TIMESTAMP", "更新日")
  ...
  public enum TOKSP_HB_MySQL_Layout implements MSTLayout {
    ...
    KANRIENO(6, "KANRIENO", "SIGNED", "枝番"),
    /** 店扱いフラグ配列 */
    TENATSUK_ARR(7, "TENATSUK_ARR", "CHAR(400)", "店扱いフラグ配列"),
    /** 送信フラグ */
  ...
  public enum TOK_CMN_SHNHBDT_MySQL_Layout implements MSTLayout {
    ...
    /** 販売日 */
    HBDT(2, "HBDT", "SIGNED", "販売日"),
    /** 催しコード配列 */
    MOYCD_ARR(3, "MOYCD_ARR", "CHAR(4000)", "催しコード配列"),
    /** 管理番号配列 */
    KANRINO_ARR(4, "KANRINO_ARR", "CHAR(1600)", "管理番号配列"),
    /** オペレータ */
    OPERATOR(5, "OPERATOR", "CHAR(20)", "オペレータ"),
    /** 登録日 */
    ADDDT(6, "ADDDT", "TIMESTAMP", "登録日"),
    /** 更新日 */
    UPDDT(7, "UPDDT", "TIMESTAMP", "更新日");
  ...
  public enum TOK_CMN_SHNNNDT_MySQL_Layout implements MSTLayout {
    ...
    SHNCD(1, "SHNCD", "CHAR(14)", "商品コード"),
    /** 便区分 */
    BINKBN(2, "BINKBN", "SIGNED", "便区分"),
    /** 納入日 */
    NNDT(3, "NNDT", "SIGNED", "納入日"),
    /** 催しコード配列 */
    MOYCD_ARR(4, "MOYCD_ARR", "CHAR(4000)", "催しコード配列"),
    /** 管理番号配列 */
    KANRINO_ARR(5, "KANRINO_ARR", "CHAR(1600)", "管理番号配列"),
    /** オペレータ */
  ...
  public enum SYSMOYBMN_MySQL_Layout implements MSTLayout {
    ...
    BMNCD(4, "BMNCD", "SIGNED", "部門"),
    /** 付番済管理番号 */
    SUMI_KANRINO(5, "SUMI_KANRINO", "SIGNED", "付番済管理番号"),
    /** 付番済表示順番 */
    SUMI_HYOSEQNO(6, "SUMI_HYOSEQNO", "SIGNED", "付番済表示順番"),
    /** オペレータ */
    OPERATOR(7, "OPERATOR", "CHAR(20)", "オペレータ"),
    /** 登録日 */
    ADDDT(8, "ADDDT", "TIMESTAMP", "登録日"),
  ...
  public enum SYSMOYKANRIENO_MySQL_Layout implements MSTLayout {
    ...
    KANRINO(5, "KANRINO", "SIGNED", "管理番号"),
    /** 付番済枝番 */
    SUMI_KANRIENO(6, "SUMI_KANRIENO", "SIGNED", "付番済枝番"),
    /** オペレータ */
    OPERATOR(7, "OPERATOR", "CHAR(20)", "オペレータ"),
    /** 登録日 */
    ADDDT(8, "ADDDT", "TIMESTAMP", "登録日"),
