...
//			}
	...
	public String getUpdFlg(String szMoyskbn, String szMoysstdt, String szMoysrban) {
		...
		sbSQL.append(" SHUNO");
		sbSQL.append(" , INTEGER ( TO_CHAR( TO_DATE(STARTDT, 'YYYY/MM/DD') + 3 DAY, 'YYYYMMDD')) as STDT_T");
		...
		sbSQL.append(" INTEGER ( TO_CHAR( TO_DATE(T1.NNSTDT_TGF, 'YYYY/MM/DD') - 52 DAY, 'YYYYMMDD')) <= T3.SHORIDT THEN '1' ");
		...
		sbSQL.append(" WHERE");
		sbSQL.append(" INTEGER (TO_CHAR(TO_DATE(T1.NNSTDT_TGF, 'YYYYMMDD') - 14 DAY, 'YYYYMMDD')) >= T3.STARTDT ");
		sbSQL.append(" AND INTEGER (TO_CHAR(TO_DATE(T1.NNSTDT_TGF, 'YYYYMMDD') - 14 DAY, 'YYYYMMDD')) <= T3.ENDDT)");
	...
	private String createCommand() {
		...
		sbSQL.append(" ,CASE WHEN T3.BYCD='0' THEN right('00'||'"+szBmncd+"', 2) || '0000000' ELSE right('0000000'||T3.BYCD,7) END as BYCD");		// F9：BY
		...
		// 特売事前行
		sbSQL.append(" ,CASE WHEN INTEGER(T3.GENKAAM_MAE)=0 THEN null ELSE T3.GENKAAM_MAE END GENKAAM_MAE ");	// F63：原価
		sbSQL.append(" ,CASE WHEN NVL(T3.TKANPLUKBN,'1') <> '2' THEN T3.A_BAIKAAM ELSE NULL END A_BAIKAAM ");	// F64：A総売価
		sbSQL.append(" ,null");										// F65：本体売価 TODO※画面で実行
		sbSQL.append(" ,CASE WHEN NVL(T3.TKANPLUKBN,'1') <> '2' THEN T3.IRISU ELSE NULL END IRISU ");			// F66：入数
		...
		// B部分
		sbSQL.append(" ,CASE WHEN NVL(T3.TKANPLUKBN,'1') <> '2' THEN T3.B_BAIKAAM ELSE NULL END B_BAIKAAM ");	// F73：B総売価
		...
		// C部分
		sbSQL.append(" ,CASE WHEN NVL(T3.TKANPLUKBN,'1') <> '2' THEN T3.C_BAIKAAM ELSE NULL END C_BAIKAAM ");	// F77：C総売価
		...
		if(isCopy){
			...
			sbSQL.append(" ,nvl(TO_CHAR(T3.ADDDT, 'YY/MM/DD'),'__/__/__') as ADDDT");	// F151: 登録日
			sbSQL.append(" ,nvl(TO_CHAR(T3.UPDDT, 'YY/MM/DD'),'__/__/__') as UPDDT");	// F152: 更新日
			sbSQL.append(" ,TO_CHAR(T3.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT");	// F153: 更新日時
		}
		if(isNew||isCopyNotNN){
		...
		sbSQL.append("  and T1.MOYSRBAN = "+szMoysrban+"");
		sbSQL.append(" ) T1 ");
		sbSQL.append(" left join INATK.TOKTG_KHN T2 on T1.MOYSKBN = T2.MOYSKBN and T1.MOYSSTDT = T2.MOYSSTDT and T1.MOYSRBAN = T2.MOYSRBAN and nvl(T2.UPDKBN, 0) <> 1 ");
		sbSQL.append(" left join "+szTableSHN +" T3 on nvl(T3.UPDKBN, 0) <> 1 ");
		if(isCopy){
		...
		if(isCopy){
			sbSQL.append(" left join INATK.TOKMOYDEF DF on T3.BMNCD = DF.BMNCD and nvl(DF.UPDKBN, 0) <> 1");
		}else if(isNew){
			sbSQL.append(" left join INATK.TOKMOYDEF DF on DF.BMNCD = "+szBmncd+" and nvl(DF.UPDKBN, 0) <> 1");
		}
		sbSQL.append(" left join JTEN T4 on T3.MOYSKBN = T4.MOYSKBN and T3.MOYSSTDT = T4.MOYSSTDT and T3.MOYSRBAN = T4.MOYSRBAN and T3.BMNCD = T4.BMNCD and T3.KANRINO = T4.KANRINO and T3.KANRIENO = T4.KANRIENO and T4.TJFLG = 1");
		sbSQL.append(" left join JTEN T5 on T3.MOYSKBN = T5.MOYSKBN and T3.MOYSSTDT = T5.MOYSSTDT and T3.MOYSRBAN = T5.MOYSRBAN and T3.BMNCD = T5.BMNCD and T3.KANRINO = T5.KANRINO and T3.KANRIENO = T5.KANRIENO and T5.TJFLG = 2");
		sbSQL.append(DefineReport.ID_SQL_TOKBAIKA_JOIN.replace("@", "T3."));
		if(isToktg){
			sbSQL.append(" left join INAMS.MSTTEN MT1 on T3.RANKNO_ADD = MT1.TENCD and nvl(MT1.UPDKBN, 0) <> 1 ");
		}else{
			sbSQL.append(" left join INAMS.MSTTEN MT1 on T3.RANKNO_ADD_A = MT1.TENCD and nvl(MT1.UPDKBN, 0) <> 1 ");
			sbSQL.append(" left join INAMS.MSTTEN MT2 on T3.RANKNO_DEL = MT2.TENCD and nvl(MT1.UPDKBN, 0) <> 1 ");
		}
		...
		sbSQL.append(" left join INAMS.MSTMAKER MA on MA.MAKERCD = M0.MAKERCD and nvl(MA.UPDKBN,0) <> 1 ");
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		sbSQL.append(" (SELECT DBMNATRKBN FROM INATK.TOKMOYDEF WHERE BMNCD = "+szBmncd+" and nvl(UPDKBN, 0) <> 1) = '1' THEN T1.NNSTDT_TGF ");
		sbSQL.append(" ELSE T1.NNSTDT END AS CHK_NNSTDT "); // F21:チェック用納入開始日
		sbSQL.append(" ,CASE WHEN ");
		sbSQL.append(" (SELECT DBMNATRKBN FROM INATK.TOKMOYDEF WHERE BMNCD = "+szBmncd+" and nvl(UPDKBN, 0) <> 1) = '1' THEN T1.NNEDDT_TGF ");
	...
	public JSONArray getNNDTData(HashMap<String, String> map) {
		...
		sbSQL.append(",WK as (");
		sbSQL.append(" select * ");
		sbSQL.append(" ,case T1.JSKPTNSYUKBN "
					+ " when "+DefineReport.ValKbn10008.VAL1.getVal()+" then '売'"
					+ " when "+DefineReport.ValKbn10008.VAL2.getVal()+" then '点'"
					+ " else '' end as JSKPTNSYUKBN_NM");	// 実績率パタン数値txt
		sbSQL.append(" ,case T1.JSKPTNZNENMKBN "
					+ " when "+DefineReport.ValKbn10009.VAL1.getVal()+" then '"+DefineReport.ValKbn10009.VAL1.getTxt()+"'"
					+ " when "+DefineReport.ValKbn10009.VAL2.getVal()+" then '"+DefineReport.ValKbn10009.VAL2.getTxt()+"'"
					+ " when "+DefineReport.ValKbn10009.VAL3.getVal()+" then '"+DefineReport.ValKbn10009.VAL3.getTxt()+"'"
					+ " else '' end as JSKPTNZNENMKBN_NM");	// 実績率パタン前年同月txt
		sbSQL.append(" ,case T1.JSKPTNZNENWKBN "
					+ " when "+DefineReport.ValKbn10009.VAL1.getVal()+" then '"+DefineReport.ValKbn10009.VAL1.getTxt()+"'"
					+ " when "+DefineReport.ValKbn10009.VAL2.getVal()+" then '"+DefineReport.ValKbn10009.VAL2.getTxt()+"'"
					+ " when "+DefineReport.ValKbn10009.VAL3.getVal()+" then '"+DefineReport.ValKbn10009.VAL3.getTxt()+"'"
					+ " else '' end as JSKPTNZNENWKBN_NM");	// 実績率パタン前年同週txt
		sbSQL.append(" ,S1.SHUNO||'週' as ZNENW");			// 実績率パタン前年週	TODO
		sbSQL.append(" ,to_char(T1.ZNENDT, 'YYMM')||'月' as ZNENM");			// 実績率パタン前年月	TODO
		sbSQL.append(" from (");
		sbSQL.append("  select ");
		...
		if(isNew || (isCopy && !isCopyNN)){
			sbSQL.append("  ,T1.ADDDT -1 year as ZNENDT");			// 実績率パタン前年日
		}else{
			sbSQL.append("  ,T1.UPDDT -1 year as ZNENDT");			// 実績率パタン前年日
		}
		sbSQL.append("  from (");
		sbSQL.append("   select * from INATK.TOKMOYCD T1");
		sbSQL.append("   where nvl(T1.UPDKBN, 0) <> 1");
		...
		sbSQL.append("  left join INATK.TOKMOYDEF DF on DF.BMNCD = "+szBmncd+" and nvl(DF.UPDKBN, 0) <> 1");
		sbSQL.append(" ) T1");
		sbSQL.append(" left join INAAD.SYSSHUNO S1 on to_char(T1.ZNENDT, 'YYYYMMDD') between S1.STARTDT and S1.ENDDT");
		sbSQL.append(")");
		sbSQL.append(",CAL(IDX, DT) as (");
		sbSQL.append(" select 1 as IDX, NNSTDT as DT from WK");
		sbSQL.append(" union all ");
		sbSQL.append(" select IDX+1 as IDX, to_char(to_date(DT, 'YYYYMMDD') + 1 days, 'YYYYMMDD') as DT from CAL where IDX < 10");
		sbSQL.append(")");
		sbSQL.append(" select ");
		sbSQL.append("  nvl(TO_CHAR(TO_DATE(M1.DT, 'YYYYMMDD'), 'MM/DD'), '') as N1");	// F148：日付（1～10）
		...
		if(isNew || (isCopy && !isCopyNN)){
			...
			sbSQL.append(" ,max(case when int(M1.DT) between T1.HBSTDT and T1.HBEDDT then "+DefineReport.Values.ON.getVal()+" else "+DefineReport.Values.OFF.getVal()+" end) as N3");	// F168：販売日（1～10）
			sbSQL.append(" ,max(case when T6.NNDT IS NOT NULL AND T6.NNDT = int(M1.DT) then "+DefineReport.Values.ON.getVal()+" else "+DefineReport.Values.OFF.getVal()+" end) as N4");	// F178：納入日（1～10）
			sbSQL.append(" ,sum(case when T1.TENKAIKBN='2' then null else T6.HTASU end) as N5");				// F188：発注総数（1～10）+合計：
		}
		sbSQL.append(" ,max(case T1.TENKAIKBN "
						+ " when "+DefineReport.ValKbn10007.VAL1.getVal()+" then '通常率pt'"
						+ " when "+DefineReport.ValKbn10007.VAL2.getVal()+" then '通常数pt'"
						+ " when "+DefineReport.ValKbn10007.VAL3.getVal()+" then case T1.JSKPTNZNENMKBN when "+DefineReport.ValKbn10009.VAL0.getVal()
							+ " then ZNENW||'('||JSKPTNZNENWKBN_NM||JSKPTNSYUKBN_NM||')'"
							+ " else ZNENM||'('||JSKPTNZNENMKBN_NM||JSKPTNSYUKBN_NM||')' end"
						+ " else null end) as N6");	// F
		if(isNew || (isCopy && !isCopyNN)){
			...
			sbSQL.append(" ,null as N13");						// F252：本売価計(1～10)+合計：※画面にて実行
			sbSQL.append(" ,null as N14");						// F263：荒利額（1～10）+合計：※画面にて実行
			sbSQL.append(" ,max(to_char(T6.ADDDT, 'YYYYMMDDHH24MISSNNNNNN')) as N50");	// データ確認用
		}
		sbSQL.append(" ,DT as N90");							// 参照用
		sbSQL.append(" ,nvl(TO_CHAR(TO_DATE(M1.DT, 'YYYYMMDD'), 'YY/MM/DD')||max(JWEEK), '') as N91");	// 参照用
		...
		sbSQL.append(" inner join WEEK M2 on CWEEK = DAYOFWEEK(to_date(M1.DT, 'YYYYMMDD'))");
		sbSQL.append(" inner join WK T1 on 1=1");
		sbSQL.append(" left join "+szTableNNDT+" T6 on T1.MOYSKBN = T6.MOYSKBN and T1.MOYSSTDT = T6.MOYSSTDT and T1.MOYSRBAN = T6.MOYSRBAN and T1.BMNCD = T6.BMNCD and T1.KANRINO = T6.KANRINO and T1.KANRIENO = T6.KANRIENO and M1.DT = T6.NNDT");

		sbSQL.append(" group by grouping sets ((),(M1.DT))");
		sbSQL.append(" order by nvl(M1.DT, 99999999)");
	...
	public String createSqlSelMSTSHN(String szMoysKbn, String szMoysstdt,String shncd) {
		...
		sbSQL.append(" (SELECT"
				...
				+ " ,CHUCD "
				+ " ,SHOCD "
				+ " ,CASE WHEN TEIKANKBN='0' THEN '2' WHEN TEIKANKBN='1' THEN TEIKANKBN ELSE '1' END AS TEIKANKBN "
				+ " ,POPKN "
				+ " ,KIKKN "
				+ " ,PCKBN "
				+ " ,SHNKBN "
		...
		if (szMoysKbn.equals("0")) {
			sbSQL.append(" ,RG_GENKAAM GENKAAM "
					+ " ,RG_BAIKAAM BAIKAAM "
					+ " ,RG_IRISU IRISU ");
			...
			sbSQL.append(" ,CASE WHEN BMNCD IN ('2','4','5','6','9','15') THEN NULL ELSE "
					+ " CASE WHEN (HS_GENKAAM = 0 AND HS_BAIKAAM = 0 AND HS_IRISU = 0) OR "
					+ "  (HS_GENKAAM IS NULL AND HS_BAIKAAM IS NULL AND HS_IRISU IS NULL) THEN RG_GENKAAM ELSE HS_GENKAAM END END GENKAAM "
					+ " ,CASE WHEN BMNCD IN ('2','4','5','6','9','15') THEN NULL ELSE "
					+ " CASE WHEN (HS_GENKAAM = 0 AND HS_BAIKAAM = 0 AND HS_IRISU = 0) OR "
		...
		sbSQL.append(" left outer join INAMS.MSTBMN M1 on M1.BMNCD = M0.BMNCD and nvl(M1.UPDKBN, 0) <> 1"
		...
		sbSQL.append(" left join INAMS.MSTMEISHO MEI on MEI.MEISHOKBN = '"+DefineReport.MeisyoSelect.KBN102.getCd()+"' and MEI.MEISHOCD = M0.PCKBN ");
		sbSQL.append(" where M0.SHNCD = ? and nvl(M0.UPDKBN, 0) <> 1 ");
		return sbSQL.toString();
	}
...
//		全店特売(アンケート有/無)_納入日.前年実績フラグ
		...
		sbSQL.append("  ,T1.UPDDT -1 year as ZNENDT");			// 実績率パタン前年日
		sbSQL.append("  from INATK.TOKMOYCD T1 ");
		sbSQL.append("  inner join "+szTableSHN +" T3 on nvl(T1.UPDKBN, 0) <> 1 and nvl(T3.UPDKBN, 0) <> 1 ");
		sbSQL.append("    and T1.MOYSKBN  = T3.MOYSKBN and T1.MOYSSTDT = T3.MOYSSTDT and T1.MOYSRBAN = T3.MOYSRBAN");
...
//		sbSQL.append(" ,null as N13");						// F252：本売価計(1～10)+合計：※画面にて実行
//		sbSQL.append(" ,null as N14");						// F263：荒利額（1～10）+合計：※画面にて実行
//		sbSQL.append(" ,max(to_char(T6.UPDDT, 'YYYYMMDDHH24MISSNNNNNN')) as N50");	// データ確認用
		sbSQL.append(" ,DT as N90");							// 参照用
		sbSQL.append(" ,nvl(TO_CHAR(TO_DATE(M1.DT, 'YYYYMMDD'), 'YY/MM/DD')||max(JWEEK), '') as N91");	// 参照用
		sbSQL.append(" ,max(ZNENW) as N92,max(ZNENM) as N93");	// 参照用
		// 参照用基本情報
...
//		sbSQL.append(" ,max(T6.WEEKHTDT) as WEEKHTDT");			// 週間発注処理日
	...
	public JSONObject updateExec(String sendBtnid, String sendPageid, String szAddshukbn, String userId,
			JSONArray dataArray, JSONArray dataArrayTJTEN, JSONArray dataArrayNNDT, JSONArray dataArrayHB) {
		...
		boolean isNewKanrieno= false;	// (DefineReport.ID_PAGE_ST016.equals(sendPageid) && (StringUtils.startsWith(sendBtnid, DefineReport.Button.NEW.getObj())));
		...
		if(isNew){
			...
			if(isTOKTG){
				// 3.13.3.7.1．テーブル更新
				// 全店特売（アンケート有）_部門
				JSONObject result1 = this.createSqlTOK_CMN_BMN(userId, data, SqlType.MRG, isTOKTG);
				// 全店特売（アンケート有）_商品
				JSONObject result2 = this.createSqlTOKTG_SHN(userId, data, SqlType.MRG);
				// 全店特売（アンケート有）_対象除外店
				if(dataArrayTJTEN.size() > 0){
					JSONObject result3 = this.createSqlTOK_CMN_TJTEN(userId, dataArrayTJTEN, SqlType.MRG, isTOKTG);
				}
				// 全店特売（アンケート有）_納入日（店変更フラグ配列に400店分1:更新をセット）
				if(dataArrayNNDT.size() > 0){
					JSONObject result4 = this.createSqlTOKTG_NNDT(userId, dataArrayNNDT, data, SqlType.MRG);
				}
			...
			// 3.13.3.8．DB登録処理（アンケート無）：
				...
				JSONObject result1 = this.createSqlTOK_CMN_BMN(userId, data, SqlType.MRG, isTOKTG);
				// 全店特売（アンケート無）_商品
				JSONObject result2 = this.createSqlTOKSP_SHN(userId, data, SqlType.MRG);
				// 全店特売（アンケート無）_対象除外店
				if(dataArrayTJTEN.size() > 0){
					JSONObject result3 = this.createSqlTOK_CMN_TJTEN(userId, dataArrayTJTEN, SqlType.MRG, isTOKTG);
				}
				// 全店特売（アンケート無）_納入日
				if(dataArrayNNDT.size() > 0){
					JSONObject result4 = this.createSqlTOKSP_NNDT(userId, dataArrayNNDT, data, SqlType.MRG);
				}
				// 全店特売（アンケート無）_販売
				if( dataArrayHB.size() > 0){
					...
					HashMap<Integer,String> tenAtsuk = getTencds(
							szBmncd					// 部門コード
							,szMoyskbn				// 催し区分
							,szMoysstdt				// 催し開始日
							,szMoysrban				// 催し連番
					...
					JSONObject result5 = this.createSqlTOKSP_HB(userId, dataArrayHB, tenAtsuk ,SqlType.MRG);
				}
				...
				if (array.size() != 0) {
					...
					JSONObject result6 = this.createSqlTOKSP_SHNHBDT(userId, array, SqlType.MRG,table);
				}
				...
				if (array.size() != 0) {
					...
					JSONObject result7 = this.createSqlTOKSP_SHNNNDT(userId, array, SqlType.MRG,table);
				}
			...
			// 催し部門内部管理
			if(isNewKanrino){
				JSONObject result01 = this.createSqlSYSMOYBMN_SEL(data);
				JSONObject result8 = this.createSqlSYSMOYBMN(userId, data, SqlType.MRG);
			}
			// 管理番号内部管理
			if(isNewKanrieno){
				JSONObject result02 = this.createSqlSYSMOYKANRIENO_SEL(data);
				JSONObject result9 = this.createSqlSYSMOYKANRIENO(userId, data, SqlType.MRG);
			}
		// 3.14.3．更新時処理：
		}else{
			...
			if(isTOKTG){
				...
				// 全店特売（アンケート有）_対象除外店
				if(dataArrayTJTEN.size() > 0){
					// 全店特売（アンケート有）_対象除外店（複数枝番ある際は全て物理削除）
					JSONObject result3 = this.createSqlTOK_CMN_DELINS("INATK.TOKTG_TJTEN");
					result3 = this.createSqlTOK_CMN_TJTEN(userId, dataArrayTJTEN, SqlType.MRG, isTOKTG);
				}
				// 全店特売（アンケート有）_納入日（店変更フラグ配列に400店分1:更新をセット）
				if(dataArrayNNDT.size() > 0){
					JSONObject result4 = this.createSqlTOKTG_NNDT(userId, dataArrayNNDT, data, SqlType.MRG);
			...
			// 3.14.3.8．DB更新処理（全特アンケート無）
				...
				// 全店特売（アンケート無）_対象除外店
				if(dataArrayTJTEN.size() > 0){
					JSONObject result3 = this.createSqlTOK_CMN_DELINS("INATK.TOKSP_TJTEN");
					result3 = this.createSqlTOK_CMN_TJTEN(userId, dataArrayTJTEN, SqlType.MRG, isTOKTG);
				}
				// 全店特売（アンケート無）_納入日
				if(dataArrayNNDT.size() > 0){
				...
				/*
				// 3.15.2.2.2．管理テーブル更新
				if(!DefineReport.ValKbn10002.VAL3.getVal().equals(szMoyskbn)){
					// ①全特_商品販売日			催し区分 <> 3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
					JSONObject result6 = this.createSqlTOKSP_SHNHBDT(userId, dataArrayHB, SqlType.MRG);
					// ②全特（ア無）_商品納入日	催し区分 <> 3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
					JSONObject result7 = result7 = this.createSqlTOKSP_SHNNNDT(userId, dataArrayNNDT, SqlType.MRG);
				}else{
					// ③本部個特_商品販売日		催し区分=3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
					JSONObject result6 = this.createSqlTOK_CMN_SHNHBDT_DEL(userId, dataArrayHB, SqlType.MRG, "INATK.TOKHTK_SHNHBDT");
					// ④本部個特_商品納入日		催し区分=3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
					JSONObject result7 = this.createSqlTOK_CMN_SHNNNDT_DEL(userId, dataArrayNNDT, SqlType.MRG, "INATK.TOKHTK_SHNNNDT");
				}
				*/
			...
			try {
				...
				if(isError){
					...
					if(!StringUtils.isEmpty(targetValue)){
						sqlcommand = sqlcommand + " and TO_CHAR(UPDDT, 'YYYYMMDDHH24MISSNNNNNN') = ? ";
					}
					// 実行SQL設定
				...
				for (int index=0; index < commands.size(); index++) {
					...
					for (int i=0; i < paramData.size(); i++) {
						...
						String val = (String)paramData.get(i);
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		String szKanrino	= data.optString(TOK_CMNLayout.KANRINO.getId());	// 管理No.
		String szKanrieno	= data.optString(TOK_CMNLayout.KANRIENO.getId());	// 管理No.枝番	※月間チラシ・特売スポット遷移時キー
		...
		if(isTOKTG){
			// 3.15.2.1.1．テーブル更新
			// ① 関連テーブル：
			// 全店特売（アンケート有）_部門（論理削除）
			JSONObject result1 = this.createSqlTOK_CMN_BMN_DEL(userId, data, SqlType.DEL, isTOKTG);
			// 全店特売（アンケート有）_商品（複数枝番ある際は全て論理削除）
			JSONObject result2 = this.createSqlTOK_CMN_SHN_DEL(userId, data, SqlType.DEL, isTOKTG);
			// 全店特売（アンケート有）_対象除外店（複数枝番ある際は全て物理削除）
			JSONObject result3 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKTG_TJTEN");
			// 全店特売（アンケート有）_納入日（複数枝番ある際は全て物理削除）
			JSONObject result4 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKTG_NNDT");
			// 全店特売（アンケート有）_販売（複数枝番ある際は全て物理削除）
			JSONObject result5 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKTG_HB");
			// 管理番号内部管理
			JSONObject result9 = this.createSqlSYS_CMN_DEL(userId, data, SqlType.DEL, "INATK.SYSMOYKANRIENO", isTOKTG);

			// ② ST016の月締後今の内容を修正よりの場合：Gモード→廃止
		// 3.15.2.2．DB登録処理（全特アンケート無）：
		}else{
			// 3.15.2.2.1．テーブル更新
			// 全店特売（アンケート無）_部門（論理削除）
			JSONObject result1 = this.createSqlTOK_CMN_BMN_DEL(userId, data, SqlType.DEL, isTOKTG);
			// 全店特売（アンケート無）_商品（複数枝番ある際は全て論理削除）
			JSONObject result2 = this.createSqlTOK_CMN_SHN_DEL(userId, data, SqlType.DEL, isTOKTG);
			// 全店特売（アンケート無）_対象除外店（複数枝番ある際は全て物理削除）
			JSONObject result3 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKSP_TJTEN");
			// 全店特売（アンケート無）_納入日（複数枝番ある際は全て物理削除）
			JSONObject result4 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKSP_NNDT");
			// 全店特売（アンケート無）_販売（複数枝番ある際は全て物理削除）
			JSONObject result5 = this.createSqlTOK_CMN_DEL(userId, data, SqlType.DEL, "INATK.TOKSP_HB");
			...
			if (array.size() != 0) {
				...
				JSONObject result6 = this.createSqlTOK_CMN_SHNHBDT_DEL(userId, array, SqlType.MRG,table);
			}
			...
			if (array.size() != 0) {
				...
				JSONObject result7 = this.createSqlTOK_CMN_SHNNNDT_DEL(userId, array, SqlType.MRG,table);
			}
			...
			if(!DefineReport.ValKbn10002.VAL3.getVal().equals(szMoyskbn)){
				// ①全特_商品販売日			催し区分 <> 3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
				JSONObject result6 = this.createSqlTOK_CMN_SHNHBDT_DEL(userId, dataArrayHB, SqlType.MRG, "INATK.TOKSP_SHNHBDT");
				// ②全特（ア無）_商品納入日	催し区分 <> 3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
				JSONObject result7 = this.createSqlTOK_CMN_SHNNNDT_DEL(userId, dataArrayNNDT, SqlType.MRG, "INATK.TOKSP_SHNNNDT");
			}else{
				// ③本部個特_商品販売日		催し区分=3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
				JSONObject result6 = this.createSqlTOK_CMN_SHNHBDT_DEL(userId, dataArrayHB, SqlType.MRG, "INATK.TOKHTK_SHNHBDT");
				// ④本部個特_商品納入日		催し区分=3の場合、該当催しコード、管理番号をクリア（UPDATFE）する
			...
			*/
			...
			// ⑤部門内の全商品を論理削除した場合、以下のテーブルのレコードを物理削除する：
			// 催し部門内部管理
			JSONObject result8 = this.createSqlSYS_CMN_DEL(userId, data, SqlType.DEL, "INATK.SYSMOYBMN", isTOKTG);
			// 管理番号内部管理
			JSONObject result9 = this.createSqlSYS_CMN_DEL(userId, data, SqlType.DEL, "INATK.SYSMOYKANRIENO", isTOKTG);
		}
	...
	public JSONObject check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		JSONArray msgList = this.checkData(
				isNew, isChange, isCsverr, false,
				map, userInfo, mu, dataOther);
	...
	public JSONArray checkData(
			boolean isNew, boolean isChange, boolean isCsvErr, boolean isCsvUpload,
			HashMap<String, String> map, User userInfo, MessageUtility mu, JSONObject dataOther			// その他情報
		) {
	...
	public JSONArray getCheckResult(JSONArray dataArray, JSONArray dataArrayTJTEN, JSONArray dataArrayNNDT, JSONArray moycdData, String hobokure, MessageUtility mu) {
		...
		if(isToktg && !StringUtils.isEmpty(hbStDt) && !StringUtils.isEmpty(hbEdDt) && !StringUtils.isEmpty(pluSndFlg) && !pluSndFlg.equals("1")){
			...
			if (dbDatas.size() >= 1){
				...
				if (isNew) {
					...
					if (!szMoyskbn.equals(dbDatas.getJSONObject(0).optString("MOYSKBN")) ||
							!szMoysstdt.equals(dbDatas.getJSONObject(0).optString("MOYSSTDT")) ||
							!szMoysrban.equals(dbDatas.getJSONObject(0).optString("MOYSRBAN")) ||
							!bmncd.equals(dbDatas.getJSONObject(0).optString("BMNCD")) ||
							!kanrino.equals(dbDatas.getJSONObject(0).optString("KANRINO")) ||
							!kanrieno.equals(dbDatas.getJSONObject(0).optString("KANRIENO"))
					) {
						// 同一販売日の重複チェックエラー
						JSONObject o = mu.getDbMessageObj("E20449", new String[]{reqNo});
						msg.add(o);
		...
		/* TODO: jsからCOPYしてきた処理だがjava側に渡された時点で1,2の判断は不要の為、保留
		...
		*/
		...
		if (tkanPluKbn.equals("2")) {
			...
			if(!(StringUtils.isEmpty(aGenkaam1kg) && StringUtils.isEmpty(bGenkaam1kg) && StringUtils.isEmpty(cGenkaam1kg)) &&
					a_genkaam1kg == b_genkaam1kg && b_genkaam1kg == c_genkaam1kg){
		...
		if (tkanPluKbn.equals("2")) {
			...
			if(!(StringUtils.isEmpty(aBaikaamP) && StringUtils.isEmpty(bBaikaamP) && StringUtils.isEmpty(cBaikaamP))
					&& a_baikaamP == b_baikaamP && b_baikaamP == c_baikaamP){
		...
		*/
		...
		if (isFrm4 || isFrm5) {
			...
			if (!StringUtils.isEmpty(aWrituKbn) && !aWrituKbn.equals("-1") &&
					!StringUtils.isEmpty(bWrituKbn) && !bWrituKbn.equals("-1") &&
					!StringUtils.isEmpty(cWrituKbn) && !cWrituKbn.equals("-1")
			) {
		...
		if(!(StringUtils.isEmpty(aBaikaam100g) && StringUtils.isEmpty(bBaikaam100g) && StringUtils.isEmpty(cBaikaam100g)) &&
			a_baikaam100g == b_baikaam100g && b_baikaam100g == c_baikaam100g){
		...
		if(allcheck && id===$.id.chk_nndt){
			if((that.judgeRepType.frm1||that.judgeRepType.frm2||that.judgeRepType.frm3) && that.judgeRepType.st && (that.judgeRepType.toktg_h||that.judgeRepType.toksp)){
				...
				if($("[id^="+$.id.chk_nndt+"]:checked").length === 0){
					return "E20363";
				}
			}
		}
		*/
		...
		if (!StringUtils.isEmpty(tanknoAdd)) {
			if (StringUtils.isEmpty(saveTenrankArr)) {
				String checkTencd = dao.checkTenCdAdd(
						 szBmncd				// 部門コード
						,moyskbn				// 催し区分
						,moysstdt				// 催し開始日
						,moysrban				// 催し連番
					...
					if (dataN.optString(TOKSP_NNDTLayout.SENDFLG.getId()).equals("1")) {
						...
						for (HashMap.Entry<String, String> shnnn : map.entrySet()) {
							if (mapHtsu.containsKey(shnnn.getKey()) && Integer.valueOf(mapHtsu.get(shnnn.getKey())) >= 0) {
								...
								if ((isChange &&
										(!binKbn.equals(dataN.optString(TOKSP_NNDTLayout.OPERATOR.getId())) ||
												!shnnn.getValue().equals(moyscd) || (shnnn.getValue().equals(moyscd) && !mapKanri.get(shnnn.getKey()).equals(kanriNo)))) || !isChange) {
									if (tenSu == 0) {
	...
	public String checkNnDt(JSONObject data, JSONArray	nnDtArray) {
		...
		if ((StringUtils.isEmpty(jhtsuindt) || (!StringUtils.isEmpty(jhtsuindt) && jhtsuindt.equals("0"))) &&
				(StringUtils.isEmpty(weekhtdt) || (!StringUtils.isEmpty(weekhtdt) && weekhtdt.equals("0")))) {
			return "";
		} else {
	...
	public List<JSONObject> checkCsvNndt(MessageUtility mu,
			JSONArray	dataArray,		// メイン
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			for(TOKSP_NNDTLayout colinf: TOKSP_NNDTLayout.values()){
				...
				if(StringUtils.isNotEmpty(val)){
					...
					if (colinf.getCol().equals(TOKSP_NNDTLayout.MOYSKBN.getCol()) ||
							colinf.getCol().equals(TOKSP_NNDTLayout.MOYSSTDT.getCol()) ||
							colinf.getCol().equals(TOKSP_NNDTLayout.MOYSRBAN.getCol())) {
						txt = "催しコードは";
					}
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			for(HashMap.Entry<String, String> csv : arrMapCsv.entrySet()) {
				...
				sbSQL.append("nvl(UPDKBN, 0)=" + DefineReport.ValUpdkbn.NML.getVal());
	...
	public List<JSONObject> checkCsvNndt2(MessageUtility mu,
			JSONArray	dataArray,		// メイン
			JSONArray	dataArrayShn,	// 商品
			String reqNo
		) {
			...
			if (isToktg) {
				for(TOKTG_SHNLayout colinf: TOKTG_SHNLayout.values()){
					...
					if(StringUtils.isNotEmpty(val)){
						...
						String txt = colinf.getTxt() + "は";
						if (colinf.getCol().equals(TOKTG_SHNLayout.MOYSKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.MOYSSTDT.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.MOYSRBAN.getCol())) {
							txt = "催しコードは";
						}
				...
				for(TOKSP_SHNLayout colinf: TOKSP_SHNLayout.values()){
					...
					if(StringUtils.isNotEmpty(val)){
						...
						String txt = colinf.getTxt() + "は";
						if (colinf.getCol().equals(TOKSP_SHNLayout.MOYSKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.MOYSSTDT.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.MOYSRBAN.getCol())) {
							txt = "催しコードは";
						}
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			if (isToktg) {
				...
				binkbn = dataShn.optString(TOKTG_SHNLayout.BINKBN.getId());
			} else {
				shncd = dataShn.optString(TOKSP_SHNLayout.SHNCD.getId());
				binkbn = dataShn.optString(TOKSP_SHNLayout.BINKBN.getId());
			}
	...
	public List<JSONObject> checkCsvShn(MessageUtility mu,
			JSONArray	dataArray,		// メイン
				...
				for(TOKTG_SHNLayout colinf: TOKTG_SHNLayout.values()){
					...
					if(StringUtils.isNotEmpty(val)){
						...
						if (colinf.getCol().equals(TOKTG_SHNLayout.MAKERKN.getCol())) {
							...
							digit = new int[]{40, 0};
						} else if (colinf.getCol().equals(TOKTG_SHNLayout.NAMANETUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.KAITOFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.YOSHOKUFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.HIGAWRFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.CHIRASFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.HBSLIDEFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.NHSLIDEFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.HTGENBAIKAFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.TKANPLUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.PLUSNDFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.TENKAIKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.JSKPTNSYUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.JSKPTNZNENMKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.JSKPTNZNENWKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.YORIFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.MEDAMAKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.JUFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.CUTTENFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.SHUDENFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.A_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.B_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.C_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.KANRIENO.getCol())
						) {
							dtype = DefineReport.DataType.SUUJI;
							digit = new int[]{1, 0};
				...
				for(TOKSP_SHNLayout colinf: TOKSP_SHNLayout.values()){
					...
					if(StringUtils.isNotEmpty(val)){
						...
						if (colinf.getCol().equals(TOKSP_SHNLayout.MAKERKN.getCol())) {
							...
							dtype = DefineReport.DataType.ZEN;
							digit = new int[]{40, 0};
						} else if (colinf.getCol().equals(TOKSP_SHNLayout.NAMANETUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.KAITOFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.YOSHOKUFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.HIGAWRFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.CHIRASFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.HTGENBAIKAFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.TKANPLUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.PLUSNDFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.TENKAIKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.JSKPTNSYUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.JSKPTNZNENMKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.JSKPTNZNENWKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.YORIFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.MEDAMAKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.JUFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.CUTTENFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.SHUDENFLG.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.A_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.B_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.C_WRITUKBN.getCol()) ||
								colinf.getCol().equals(TOKSP_SHNLayout.KANRIENO.getCol())
						) {
							dtype = DefineReport.DataType.SUUJI;
							digit = new int[]{1, 0};
		...
		for (int i = 0; i < dataArrayTJTEN.size(); i++) {
			...
			for(TOK_CMN_TJTENLayout colinf: TOK_CMN_TJTENLayout.values()){
				...
				if(StringUtils.isNotEmpty(val)){
					...
					String txt = colinf.getTxt() + "は";
					if (colinf.getCol().equals(TOK_CMN_TJTENLayout.MOYSKBN.getCol()) ||
							colinf.getCol().equals(TOK_CMN_TJTENLayout.MOYSSTDT.getCol()) ||
			...
			if (isToktg) {
				for(TOKTG_NNDTLayout colinf: TOKTG_NNDTLayout.values()){
					...
					if(StringUtils.isNotEmpty(val)){
						...
						String txt = colinf.getTxt() + "は";
						if (colinf.getCol().equals(TOKTG_NNDTLayout.MOYSKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_NNDTLayout.MOYSSTDT.getCol()) ||
								colinf.getCol().equals(TOKTG_NNDTLayout.MOYSRBAN.getCol())) {
							txt = "催しコードは";
						}
		...
		if (isToktg) {
			...
			targetCol = new TOKTG_SHNLayout[]{TOKTG_SHNLayout.MOYSKBN,TOKTG_SHNLayout.MOYSSTDT,TOKTG_SHNLayout.MOYSRBAN,TOKTG_SHNLayout.SHNCD,
					TOKTG_SHNLayout.BYCD,TOKTG_SHNLayout.HBSTDT,TOKTG_SHNLayout.HBEDDT,TOKTG_SHNLayout.RANKNO_ADD,
					TOKTG_SHNLayout.BDENKBN,TOKTG_SHNLayout.WAPPNKBN,TOKTG_SHNLayout.SHUDENFLG,TOKTG_SHNLayout.POPKN};
			...
			for (int i = 0; i < dataArray.size(); i++) {
				...
				for(TOKTG_SHNLayout colinf: targetCol){
					...
					if ((colinf.getCol().equals(TOKTG_SHNLayout.HBSTDT.getCol()) ||
							colinf.getCol().equals(TOKTG_SHNLayout.HBEDDT.getCol())) && !isToktg) {
						conFlg = true;
					}
					...
					if (isFrm1) {
						...
						if (colinf.getCol().equals(TOKTG_SHNLayout.WAPPNKBN.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.SHUDENFLG.getCol()) ||
								colinf.getCol().equals(TOKTG_SHNLayout.BDENKBN.getCol())) {
							conFlg = true;
						}
					} else {
						if (colinf.getCol().equals(TOKTG_SHNLayout.RANKNO_ADD.getCol())) {
			...
			targetCol = new TOKSP_SHNLayout[]{TOKSP_SHNLayout.MOYSKBN,TOKSP_SHNLayout.MOYSSTDT,TOKSP_SHNLayout.MOYSRBAN,TOKSP_SHNLayout.SHNCD,
					TOKSP_SHNLayout.BYCD,TOKSP_SHNLayout.HBSTDT,TOKSP_SHNLayout.HBEDDT,
					TOKSP_SHNLayout.BDENKBN,TOKSP_SHNLayout.WAPPNKBN,TOKSP_SHNLayout.SHUDENFLG,TOKSP_SHNLayout.POPKN};
			...
			for (int i = 0; i < dataArray.size(); i++) {
				...
				for(TOKSP_SHNLayout colinf: targetCol){
					...
					if ((colinf.getCol().equals(TOKSP_SHNLayout.HBSTDT.getCol()) ||
							colinf.getCol().equals(TOKSP_SHNLayout.HBEDDT.getCol())) && !isToktg) {
						conFlg = true;
					}
	...
	public JSONObject createSqlShn(JSONArray dataArray, JSONArray dataArrayTJTEN, JSONArray dataArrayNNDT, JSONArray dataArrayHB, User userInfo , String updKbn) {
		...
		return updateExec(sendBtnid,DefineReport.ID_PAGE_ST016,dataArray.getJSONObject(0).optString(TOKTG_SHNLayout.ADDSHUKBN.getId()),userId,
				dataArray,dataArrayTJTEN,dataArrayNNDT,dataArrayHB);
	}
	...
	public JSONObject createSqlNnDtCsv(JSONArray dataArray, JSONArray dataArrayShn, boolean insTen, User userInfo){
		...
		ArrayList<String>	prmDataHb	= new ArrayList<String>();
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			htsuArr	= data.optString(TOKSP_NNDTLayout.TENHTSU_ARR.getId());
			...
			if(valueData.length >= 100 || (i+1 == dataArray.size() && valueData.length > 0)){
				sbSQL = new StringBuffer();
				if (isToktg) {
					sbSQL.append("MERGE INTO INATK.TOKTG_NNDT AS T USING (SELECT ");
				} else {
					sbSQL.append("MERGE INTO INATK.TOKSP_NNDT AS T USING (SELECT ");
				}
				...
				sbSQL.append("T.KANRINO=RE.KANRINO AND ");
				...
				sbSQL.append(",RE.KANRIENO");
				sbSQL.append(",RE.NNDT");
				sbSQL.append(",RE.TENKAISU");
				sbSQL.append(",RE.TENHTSU_ARR");
				if (isToktg) {
					sbSQL.append(",RE.TENCHGFLG_ARR");
				}
	...
	public void createSqlNnDtSub(JSONArray updKeys, String userId) {
		...
		sbSQL = new StringBuffer();
		if (isToktg) {
			sbSQL.append("MERGE INTO INATK.TOKTG_SHN AS T USING (SELECT ");
		} else {
			sbSQL.append("MERGE INTO INATK.TOKSP_SHN AS T USING (SELECT ");
		}
		sbSQL.append(" MOYSKBN");											// 催し区分
		sbSQL.append(",MOYSSTDT");											// 催し開始日
		sbSQL.append(",MOYSRBAN");											// 催し連番
	...
	public String getHtsuArr(JSONObject data, JSONObject dataShn,boolean insTen) {
		...
		if (isToktg) {
			...
			if (StringUtils.isEmpty(getArr) && dbDatas.size() == 0) {
				...
				dbDatas = new ReportBM015Dao(JNDIname).getST021UpdChk(
						moyskbn,moysstdt,moysrban,bmncd,kanrino,kanrieno,
						arr,
						tenkaikbn,
						syukbn,
						jskptnznenmkbn,
	...
	private JSONObject createSqlTOK_CMN_BMN(String userId, JSONObject data, SqlType sql, boolean isTOKTG) {
		...
		String[] notTarget = new String[]{TOK_CMN_BMNLayout.UPDKBN.getId(), TOK_CMN_BMNLayout.SENDFLG.getId(),TOK_CMN_BMNLayout.OPERATOR.getId(), TOK_CMN_BMNLayout.ADDDT.getId(), TOK_CMN_BMNLayout.UPDDT.getId()};
		...
		sbSQL = new StringBuffer();
		if(isTOKTG){
			sbSQL.append("merge into INATK.TOKTG_BMN as T");
		}else{
			sbSQL.append("merge into INATK.TOKSP_BMN as T");
		}
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append("  UPDKBN="+DefineReport.ValUpdkbn.NML.getVal());		// 更新区分
	...
	private JSONObject createSqlTOK_CMN_BMN_DEL(String userId, JSONObject data, SqlType sql, boolean isTOKTG) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into "+szTableBmn+" as T");
		sbSQL.append(" using (select ");
		sbSQL.append("   T1.MOYSKBN,T1.MOYSSTDT,T1.MOYSRBAN,T1.BMNCD,sum(case when T1.KANRINO = "+data.optString(TOK_CMNLayout.KANRINO.getId())+" then 0 else 1 end) as CNT");
		sbSQL.append("  from "+szTableShn+" T1 where ");
		sbSQL.append("   T1.MOYSKBN  = "+data.optString(TOK_CMNLayout.MOYSKBN.getId())+"");
		...
		sbSQL.append(" ) as RE on (");
		sbSQL.append(" T.MOYSKBN = RE.MOYSKBN and T.MOYSSTDT = RE.MOYSSTDT and T.MOYSRBAN = RE.MOYSRBAN and T.BMNCD = RE.BMNCD and CNT = 0");
		sbSQL.append(")");
		sbSQL.append(" when matched then ");
		sbSQL.append(" update set");
	...
	private JSONObject createSqlTOKTG_SHN(String userId, JSONObject data, SqlType sql) {
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched then ");
			sbSQL.append(" update set");
			sbSQL.append("  MOYSKBN=RE.MOYSKBN");	// F1 : 催し区分
			sbSQL.append(" ,MOYSSTDT=RE.MOYSSTDT");	// F2 : 催し開始日
			sbSQL.append(" ,MOYSRBAN=RE.MOYSRBAN");	// F3 : 催し連番
			sbSQL.append(" ,BMNCD=RE.BMNCD");		// F4 : 部門
			sbSQL.append(" ,KANRINO=RE.KANRINO");	// F5 : 管理番号
			sbSQL.append(" ,KANRIENO=RE.KANRIENO");	// F6 : 枝番
			sbSQL.append(" ,ADDSHUKBN=RE.ADDSHUKBN");	// F7 : 登録種別
			sbSQL.append(" ,HBSLIDEFLG=RE.HBSLIDEFLG");	// F8 : 1日遅スライド_販売
			sbSQL.append(" ,NHSLIDEFLG=RE.NHSLIDEFLG");	// F9 : 1日遅スライド_納品
			sbSQL.append(" ,BYCD=RE.BYCD");		// F10: BYコード
			sbSQL.append(" ,SHNCD=RE.SHNCD");		// F11: 商品コード
			sbSQL.append(" ,PARNO=RE.PARNO");		// F12: 親No
			sbSQL.append(" ,CHLDNO=RE.CHLDNO");	// F13: 子No
			sbSQL.append(" ,HIGAWRFLG=RE.HIGAWRFLG");	// F14: 日替フラグ
			sbSQL.append(" ,HBSTDT=RE.HBSTDT");	// F15: 販売期間_開始日
			sbSQL.append(" ,HBEDDT=RE.HBEDDT");	// F16: 販売期間_終了日
			sbSQL.append(" ,NNSTDT=RE.NNSTDT");	// F17: 納入期間_開始日
			sbSQL.append(" ,NNEDDT=RE.NNEDDT");	// F18: 納入期間_終了日
			sbSQL.append(" ,CHIRASFLG=RE.CHIRASFLG");	// F19: チラシ未掲載
			sbSQL.append(" ,RANKNO_ADD=RE.RANKNO_ADD");	// F20: 対象店ランク
			sbSQL.append(" ,HBYOTEISU=RE.HBYOTEISU");	// F21: 販売予定数
			sbSQL.append(" ,GENKAAM_MAE=RE.GENKAAM_MAE");	// F22: 原価_特売事前
			sbSQL.append(" ,GENKAAM_ATO=RE.GENKAAM_ATO");	// F23: 原価_特売追加
			sbSQL.append(" ,A_BAIKAAM=RE.A_BAIKAAM");	// F24: A売価（100ｇ）
			sbSQL.append(" ,B_BAIKAAM=RE.B_BAIKAAM");	// F25: B売価（100ｇ）
			sbSQL.append(" ,C_BAIKAAM=RE.C_BAIKAAM");	// F26: C売価（100ｇ）
			sbSQL.append(" ,IRISU=RE.IRISU");		// F27: 入数
			sbSQL.append(" ,HTGENBAIKAFLG=RE.HTGENBAIKAFLG");	// F28: 発注原売価適用フラグ
			sbSQL.append(" ,A_WRITUKBN=RE.A_WRITUKBN");	// F29: A売価_割引率区分
			sbSQL.append(" ,B_WRITUKBN=RE.B_WRITUKBN");	// F30: B売価_割引率区分
			sbSQL.append(" ,C_WRITUKBN=RE.C_WRITUKBN");	// F31: C売価_割引率区分
			sbSQL.append(" ,TKANPLUKBN=RE.TKANPLUKBN");	// F32: 定貫PLU・不定貫区分
			sbSQL.append(" ,A_GENKAAM_1KG=RE.A_GENKAAM_1KG");	// F33: A売価_1㎏
			sbSQL.append(" ,B_GENKAAM_1KG=RE.B_GENKAAM_1KG");	// F34: B売価_1㎏
			sbSQL.append(" ,C_GENKAAM_1KG=RE.C_GENKAAM_1KG");	// F35: C売価_1㎏
			sbSQL.append(" ,GENKAAM_PACK=RE.GENKAAM_PACK");	// F36: パック原価
			sbSQL.append(" ,A_BAIKAAM_PACK=RE.A_BAIKAAM_PACK");	// F37: A売価_パック
			sbSQL.append(" ,B_BAIKAAM_PACK=RE.B_BAIKAAM_PACK");	// F38: B売価_パック
			sbSQL.append(" ,C_BAIKAAM_PACK=RE.C_BAIKAAM_PACK");	// F39: C売価_パック
			sbSQL.append(" ,A_BAIKAAM_100G=RE.A_BAIKAAM_100G");	// F40: A売価_100ｇ相当
			sbSQL.append(" ,B_BAIKAAM_100G=RE.B_BAIKAAM_100G");	// F41: B売価_100ｇ相当
			sbSQL.append(" ,C_BAIKAAM_100G=RE.C_BAIKAAM_100G");	// F42: C売価_100ｇ相当
			sbSQL.append(" ,GENKAAM_1KG=RE.GENKAAM_1KG");	// F43: 原価_1㎏
			sbSQL.append(" ,PLUSNDFLG=RE.PLUSNDFLG");	// F44: PLU配信フラグ
			sbSQL.append(" ,TENKAIKBN=RE.TENKAIKBN");	// F45: 展開方法
			sbSQL.append(" ,JSKPTNSYUKBN=RE.JSKPTNSYUKBN");	// F46: 実績率パタン数値
			sbSQL.append(" ,JSKPTNZNENMKBN=RE.JSKPTNZNENMKBN");	// F47: 実績率パタン前年同月
			sbSQL.append(" ,JSKPTNZNENWKBN=RE.JSKPTNZNENWKBN");	// F48: 実績率パタン前年同週
			sbSQL.append(" ,DAICD=RE.DAICD");		// F49: 大分類
			sbSQL.append(" ,CHUCD=RE.CHUCD");		// F50: 中分類
			sbSQL.append(" ,SHOBUNCD=RE.SHOBUNCD");	// F50: 中分類
			sbSQL.append(" ,SANCHIKN=RE.SANCHIKN");	// F51: 産地
			sbSQL.append(" ,MAKERKN=RE.MAKERKN");	// F52: メーカー名
			sbSQL.append(" ,POPKN=RE.POPKN");		// F53: POP名称
			sbSQL.append(" ,KIKKN=RE.KIKKN");		// F54: 規格名称
			sbSQL.append(" ,SEGN_NINZU=RE.SEGN_NINZU");	// F55: 制限_先着人数
			sbSQL.append(" ,SEGN_GENTEI=RE.SEGN_GENTEI");	// F56: 制限_限定表現
			sbSQL.append(" ,SEGN_1KOSU=RE.SEGN_1KOSU");	// F57: 制限_一人当たり個数
			sbSQL.append(" ,SEGN_1KOSUTNI=RE.SEGN_1KOSUTNI");	// F58: 制限_一人当たり個数単位
			sbSQL.append(" ,YORIFLG=RE.YORIFLG");	// F59: よりどりフラグ
			sbSQL.append(" ,BD1_TENSU=RE.BD1_TENSU");	// F60: 点数_バンドル1
			sbSQL.append(" ,BD2_TENSU=RE.BD2_TENSU");	// F61: 点数_バンドル2
			sbSQL.append(" ,KO_A_BAIKAAN=RE.KO_A_BAIKAAN");	// F62: A売価_1個売り
			sbSQL.append(" ,BD1_A_BAIKAAN=RE.BD1_A_BAIKAAN");	// F63: A売価_バンドル1
			sbSQL.append(" ,BD2_A_BAIKAAN=RE.BD2_A_BAIKAAN");	// F64: A売価_バンドル2
			sbSQL.append(" ,KO_B_BAIKAAN=RE.KO_B_BAIKAAN");	// F65: B売価_1個売り
			sbSQL.append(" ,BD1_B_BAIKAAN=RE.BD1_B_BAIKAAN");	// F66: B売価_バンドル1
			sbSQL.append(" ,BD2_B_BAIKAAN=RE.BD2_B_BAIKAAN");	// F67: B売価_バンドル2
			sbSQL.append(" ,KO_C_BAIKAAN=RE.KO_C_BAIKAAN");	// F68: C売価_1個売り
			sbSQL.append(" ,BD1_C_BAIKAAN=RE.BD1_C_BAIKAAN");	// F69: C売価_バンドル1
			sbSQL.append(" ,BD2_C_BAIKAAN=RE.BD2_C_BAIKAAN");	// F70: C売価_バンドル2
			sbSQL.append(" ,MEDAMAKBN=RE.MEDAMAKBN");	// F71: 目玉区分
			sbSQL.append(" ,POPCD=RE.POPCD");		// F72: POPコード
			sbSQL.append(" ,POPSZ=RE.POPSZ");		// F73: POPサイズ
			sbSQL.append(" ,POPSU=RE.POPSU");		// F74: POP枚数
			sbSQL.append(" ,SHNSIZE=RE.SHNSIZE");	// F75: 商品サイズ
			sbSQL.append(" ,SHNCOLOR=RE.SHNCOLOR");	// F76: 商品色
			sbSQL.append(" ,COMMENT_HGW=RE.COMMENT_HGW");	// F77: その他日替わりコメント
			sbSQL.append(" ,COMMENT_POP=RE.COMMENT_POP");	// F78: POPコメント
			sbSQL.append(" ,NAMANETUKBN=RE.NAMANETUKBN");	// F79: 生食加熱区分
			sbSQL.append(" ,KAITOFLG=RE.KAITOFLG");	// F80: 解凍フラグ
			sbSQL.append(" ,YOSHOKUFLG=RE.YOSHOKUFLG");	// F81: 養殖フラグ
			sbSQL.append(" ,JUFLG=RE.JUFLG");		// F82: 事前打出フラグ
			sbSQL.append(" ,JUHTDT=RE.JUHTDT");	// F83: 事前打出日付
			sbSQL.append(" ,COMMENT_TB=RE.COMMENT_TB");	// F84: 特売コメント
			sbSQL.append(" ,CUTTENFLG=RE.CUTTENFLG");	// F85: カット店展開フラグ
			sbSQL.append(" ,BINKBN=RE.BINKBN");	// F86: 便区分
			sbSQL.append(" ,BDENKBN=RE.BDENKBN");	// F87: 別伝区分
			sbSQL.append(" ,WAPPNKBN=RE.WAPPNKBN");	// F88: ワッペン区分
			sbSQL.append(" ,SHUDENFLG=RE.SHUDENFLG");	// F89: 週次仕入先伝送フラグ
			sbSQL.append(" ,TENRANK_ARR=RE.TENRANK_ARR");	// F90: 店ランク配列
			sbSQL.append(" ,GTSIMECHGKBN=RE.GTSIMECHGKBN");	// F91: 月締変更理由
			sbSQL.append(" ,GTSIMEOKFLG=RE.GTSIMEOKFLG");	// F92: 月締変更許可フラグ
			sbSQL.append(" ,JLSTCREDT=RE.JLSTCREDT");	// F93: 事前発注リスト出力日
			sbSQL.append(" ,JHTSUINDT=RE.JHTSUINDT");	// F94: 事前発注数量取込日
			sbSQL.append(" ,WEEKHTDT=RE.WEEKHTDT");	// F95: 週間発注処理日
			sbSQL.append(" ,MYOSHBSTDT=RE.MYOSHBSTDT");	// F96: 催し販売開始日
			sbSQL.append(" ,MYOSHBEDDT=RE.MYOSHBEDDT");	// F97: 催し販売終了日
			sbSQL.append(" ,MYOSNNSTDT=RE.MYOSNNSTDT");	// F98: 催し納入開始日
			sbSQL.append(" ,MYOSNNEDDT=RE.MYOSNNEDDT");	// F99: 催し納入終了日
			sbSQL.append(" ,UPDKBN="+DefineReport.ValUpdkbn.NML.getVal());		// 更新区分
			sbSQL.append(" ,SENDFLG="+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
			sbSQL.append(" ,OPERATOR='"+userId+"'");		// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");				// 登録日
	...
	private JSONObject createSqlTOKSP_SHN(String userId, JSONObject data, SqlType sql) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.TOKSP_SHN as T");
		sbSQL.append(" using (select ");
		// キー情報はロックのため後で追加する
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
		...
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,"+DefineReport.ValUpdkbn.NML.getVal());		// F99: 更新区分
			sbSQL.append(" ,"+DefineReport.Values.SENDFLG_UN.getVal());	// F100: 送信フラグ
			sbSQL.append(" ,'"+userId+"'");				// F101: オペレータ
			sbSQL.append(" ,current timestamp");		// F102: 登録日
			sbSQL.append(" ,current timestamp");		// F103: 更新日
			sbSQL.append(")");
		}
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched then ");
			sbSQL.append(" update set");
			sbSQL.append("  ADDSHUKBN=RE.ADDSHUKBN");	// F7 : 登録種別
			sbSQL.append(" ,BYCD=RE.BYCD");		// F8 : BYコード
			sbSQL.append(" ,SHNCD=RE.SHNCD");		// F9 : 商品コード
			sbSQL.append(" ,PARNO=RE.PARNO");		// F10: 親No
			sbSQL.append(" ,CHLDNO=RE.CHLDNO");	// F11: 子No
			sbSQL.append(" ,HIGAWRFLG=RE.HIGAWRFLG");	// F12: 日替フラグ
			sbSQL.append(" ,HBSTDT=RE.HBSTDT");	// F13: 販売期間_開始日
			sbSQL.append(" ,HBEDDT=RE.HBEDDT");	// F14: 販売期間_終了日
			sbSQL.append(" ,NNSTDT=RE.NNSTDT");	// F15: 納入期間_開始日
			sbSQL.append(" ,NNEDDT=RE.NNEDDT");	// F16: 納入期間_終了日
			sbSQL.append(" ,CHIRASFLG=RE.CHIRASFLG");	// F17: チラシ未掲載
			sbSQL.append(" ,RANKNO_ADD_A=RE.RANKNO_ADD_A");	// F18: 対象店ランク_A売価
			sbSQL.append(" ,RANKNO_ADD_B=RE.RANKNO_ADD_B");	// F19: 対象店ランク_B売価
			sbSQL.append(" ,RANKNO_ADD_C=RE.RANKNO_ADD_C");	// F20: 対象店ランク_C売価
			sbSQL.append(" ,RANKNO_DEL=RE.RANKNO_DEL");	// F21: 除外店ランク
			sbSQL.append(" ,HBYOTEISU=RE.HBYOTEISU");	// F22: 販売予定数
			sbSQL.append(" ,GENKAAM_MAE=RE.GENKAAM_MAE");	// F23: 原価_特売事前
			sbSQL.append(" ,GENKAAM_ATO=RE.GENKAAM_ATO");	// F24: 原価_特売追加
			sbSQL.append(" ,A_BAIKAAM=RE.A_BAIKAAM");	// F25: A売価（100ｇ）
			sbSQL.append(" ,B_BAIKAAM=RE.B_BAIKAAM");	// F26: B売価（100ｇ）
			sbSQL.append(" ,C_BAIKAAM=RE.C_BAIKAAM");	// F27: C売価（100ｇ）
			sbSQL.append(" ,IRISU=RE.IRISU");		// F28: 入数
			sbSQL.append(" ,HTGENBAIKAFLG=RE.HTGENBAIKAFLG");	// F29: 発注原売価適用フラグ
			sbSQL.append(" ,A_WRITUKBN=RE.A_WRITUKBN");	// F30: A売価_割引率区分
			sbSQL.append(" ,B_WRITUKBN=RE.B_WRITUKBN");	// F31: B売価_割引率区分
			sbSQL.append(" ,C_WRITUKBN=RE.C_WRITUKBN");	// F32: C売価_割引率区分
			sbSQL.append(" ,TKANPLUKBN=RE.TKANPLUKBN");	// F33: 定貫PLU・不定貫区分
			sbSQL.append(" ,A_GENKAAM_1KG=RE.A_GENKAAM_1KG");	// F34: A売価_1㎏
			sbSQL.append(" ,B_GENKAAM_1KG=RE.B_GENKAAM_1KG");	// F35: B売価_1㎏
			sbSQL.append(" ,C_GENKAAM_1KG=RE.C_GENKAAM_1KG");	// F36: C売価_1㎏
			sbSQL.append(" ,GENKAAM_PACK=RE.GENKAAM_PACK");	// F37: パック原価
			sbSQL.append(" ,A_BAIKAAM_PACK=RE.A_BAIKAAM_PACK");	// F38: A売価_パック
			sbSQL.append(" ,B_BAIKAAM_PACK=RE.B_BAIKAAM_PACK");	// F39: B売価_パック
			sbSQL.append(" ,C_BAIKAAM_PACK=RE.C_BAIKAAM_PACK");	// F40: C売価_パック
			sbSQL.append(" ,A_BAIKAAM_100G=RE.A_BAIKAAM_100G");	// F41: A売価_100ｇ相当
			sbSQL.append(" ,B_BAIKAAM_100G=RE.B_BAIKAAM_100G");	// F42: B売価_100ｇ相当
			sbSQL.append(" ,C_BAIKAAM_100G=RE.C_BAIKAAM_100G");	// F43: C売価_100ｇ相当
			sbSQL.append(" ,GENKAAM_1KG=RE.GENKAAM_1KG");	// F44: 原価_1㎏
			sbSQL.append(" ,PLUSNDFLG=RE.PLUSNDFLG");	// F45: PLU配信フラグ
			sbSQL.append(" ,TENKAIKBN=RE.TENKAIKBN");	// F46: 展開方法
			sbSQL.append(" ,JSKPTNSYUKBN=RE.JSKPTNSYUKBN");	// F47: 実績率パタン数値
			sbSQL.append(" ,JSKPTNZNENMKBN=RE.JSKPTNZNENMKBN");	// F48: 実績率パタン前年同月
			sbSQL.append(" ,JSKPTNZNENWKBN=RE.JSKPTNZNENWKBN");	// F49: 実績率パタン前年同週
			sbSQL.append(" ,DAICD=RE.DAICD");		// F50: 大分類
			sbSQL.append(" ,CHUCD=RE.CHUCD");		// F51: 中分類
			sbSQL.append(" ,SANCHIKN=RE.SANCHIKN");	// F52: 産地
			sbSQL.append(" ,MAKERKN=RE.MAKERKN");	// F53: メーカー名
			sbSQL.append(" ,POPKN=RE.POPKN");		// F54: POP名称
			sbSQL.append(" ,KIKKN=RE.KIKKN");		// F55: 規格名称
			sbSQL.append(" ,SEGN_NINZU=RE.SEGN_NINZU");	// F56: 制限_先着人数
			sbSQL.append(" ,SEGN_GENTEI=RE.SEGN_GENTEI");	// F57: 制限_限定表現
			sbSQL.append(" ,SEGN_1KOSU=RE.SEGN_1KOSU");	// F58: 制限_一人当たり個数
			sbSQL.append(" ,SEGN_1KOSUTNI=RE.SEGN_1KOSUTNI");	// F59: 制限_一人当たり個数単位
			sbSQL.append(" ,YORIFLG=RE.YORIFLG");	// F60: よりどりフラグ
			sbSQL.append(" ,BD1_TENSU=RE.BD1_TENSU");	// F61: 点数_バンドル1
			sbSQL.append(" ,BD2_TENSU=RE.BD2_TENSU");	// F62: 点数_バンドル2
			sbSQL.append(" ,KO_A_BAIKAAN=RE.KO_A_BAIKAAN");	// F63: A売価_1個売り
			sbSQL.append(" ,BD1_A_BAIKAAN=RE.BD1_A_BAIKAAN");	// F64: A売価_バンドル1
			sbSQL.append(" ,BD2_A_BAIKAAN=RE.BD2_A_BAIKAAN");	// F65: A売価_バンドル2
			sbSQL.append(" ,KO_B_BAIKAAN=RE.KO_B_BAIKAAN");	// F66: B売価_1個売り
			sbSQL.append(" ,BD1_B_BAIKAAN=RE.BD1_B_BAIKAAN");	// F67: B売価_バンドル1
			sbSQL.append(" ,BD2_B_BAIKAAN=RE.BD2_B_BAIKAAN");	// F68: B売価_バンドル2
			sbSQL.append(" ,KO_C_BAIKAAN=RE.KO_C_BAIKAAN");	// F69: C売価_1個売り
			sbSQL.append(" ,BD1_C_BAIKAAN=RE.BD1_C_BAIKAAN");	// F70: C売価_バンドル1
			sbSQL.append(" ,BD2_C_BAIKAAN=RE.BD2_C_BAIKAAN");	// F71: C売価_バンドル2
			sbSQL.append(" ,MEDAMAKBN=RE.MEDAMAKBN");	// F72: 目玉区分
			sbSQL.append(" ,POPCD=RE.POPCD");		// F73: POPコード
			sbSQL.append(" ,POPSZ=RE.POPSZ");		// F74: POPサイズ
			sbSQL.append(" ,POPSU=RE.POPSU");		// F75: POP枚数
			sbSQL.append(" ,SHNSIZE=RE.SHNSIZE");	// F76: 商品サイズ
			sbSQL.append(" ,SHNCOLOR=RE.SHNCOLOR");	// F77: 商品色
			sbSQL.append(" ,COMMENT_HGW=RE.COMMENT_HGW");	// F78: その他日替わりコメント
			sbSQL.append(" ,COMMENT_POP=RE.COMMENT_POP");	// F79: POPコメント
			sbSQL.append(" ,NAMANETUKBN=RE.NAMANETUKBN");	// F80: 生食加熱区分
			sbSQL.append(" ,KAITOFLG=RE.KAITOFLG");	// F81: 解凍フラグ
			sbSQL.append(" ,YOSHOKUFLG=RE.YOSHOKUFLG");	// F82: 養殖フラグ
			sbSQL.append(" ,JUFLG=RE.JUFLG");		// F83: 事前打出フラグ
			sbSQL.append(" ,JUHTDT=RE.JUHTDT");	// F84: 事前打出日付
			sbSQL.append(" ,COMMENT_TB=RE.COMMENT_TB");	// F85: 特売コメント
			sbSQL.append(" ,CUTTENFLG=RE.CUTTENFLG");	// F86: カット店展開フラグ
			sbSQL.append(" ,BINKBN=RE.BINKBN");			// F87: 便区分
			sbSQL.append(" ,BDENKBN=RE.BDENKBN");		// F88: 別伝区分
			sbSQL.append(" ,WAPPNKBN=RE.WAPPNKBN");		// F89: ワッペン区分
			sbSQL.append(" ,SHUDENFLG=RE.SHUDENFLG");	// F90: 週次仕入先伝送フラグ
			sbSQL.append(" ,TENRANK_ARR=RE.TENRANK_ARR");	// F91: 店ランク配列
			sbSQL.append(" ,JLSTCREDT=RE.JLSTCREDT");	// F92: 事前発注リスト出力日
			sbSQL.append(" ,JHTSUINDT=RE.JHTSUINDT");	// F93: 事前発注数量取込日
			sbSQL.append(" ,WEEKHTDT=RE.WEEKHTDT");		// F94: 週間発注処理日
			sbSQL.append(" ,MYOSHBSTDT=RE.MYOSHBSTDT");	// F95: 催し販売開始日
			sbSQL.append(" ,MYOSHBEDDT=RE.MYOSHBEDDT");	// F96: 催し販売終了日
			sbSQL.append(" ,MYOSNNSTDT=RE.MYOSNNSTDT");	// F97: 催し納入開始日
			sbSQL.append(" ,MYOSNNEDDT=RE.MYOSNNEDDT");	// F98: 催し納入終了日
			sbSQL.append(" ,UPDKBN="+DefineReport.ValUpdkbn.NML.getVal());		// 更新区分
			sbSQL.append(" ,SENDFLG="+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
			sbSQL.append(" ,OPERATOR='"+userId+"'");		// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");				// 登録日
	...
	private JSONObject createSqlTOK_CMN_SHN_DEL(String userId, JSONObject data, SqlType sql, boolean isTOKTG) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into "+szTableShn+" as T");
		sbSQL.append(" using (select ");
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
	...
	protected JSONObject createSqlTOKTG_NNDT(String userId, JSONArray dataArray, JSONObject data, SqlType sql) {
		...
		for(int j=0; j < dataArray.size(); j++){
			...
			for (TOKTG_NNDTLayout itm :  TOKTG_NNDTLayout.values()) {
				if(ArrayUtils.contains(notUse, itm.getId())){ continue;}	// 不要
				if(ArrayUtils.contains(notTarget, itm.getId())){ continue;}	// パラメータ不要
				if(ArrayUtils.contains(keys, itm.getId())){ continue;}		// 共通で実施
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.TOKTG_NNDT as T");
		sbSQL.append(" using (select ");
		// キー情報はロックのため後で追加する
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
		...
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			for(TOKTG_NNDTLayout itm :TOKTG_NNDTLayout.values()){
				if(ArrayUtils.contains(notUse, itm.getId())){ continue;}	// 不要
				if(ArrayUtils.contains(notTarget, itm.getId())){ continue;}	// パラメータ不要
				if (itm.getId().equals(TOKTG_NNDTLayout.SENDFLG.getId())) { continue;}	// パラメータ不要
				if(itm.getNo() > 1){ sbSQL.append(","); }
				sbSQL.append("RE."+itm.getCol());
			}
			sbSQL.append(" ,"+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,TENCHGFLG_ARR=RE.TENCHGFLG_ARR");	// F9 : 店変更フラグ配列
			sbSQL.append(" ,HTASU=RE.HTASU");		// F10: 発注総数
			sbSQL.append(" ,PTNNO=RE.PTNNO");		// F11: パターン№
			sbSQL.append(" ,TSEIKBN=RE.TSEIKBN");	// F12: 訂正区分
			sbSQL.append(" ,TPSU=RE.TPSU");		// F13: 店舗数
			sbSQL.append(" ,TENKAISU=RE.TENKAISU");	// F14: 展開数
			sbSQL.append(" ,ZJSKFLG=RE.ZJSKFLG");	// F15: 前年実績フラグ
			sbSQL.append(" ,WEEKHTDT=RE.WEEKHTDT");	// F16: 週間発注処理日
			sbSQL.append(" ,SENDFLG="+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
			sbSQL.append(" ,OPERATOR='"+userId+"'");							// オペレータ
			sbSQL.append(" ,UPDDT=current timestamp");							// 更新日
	...
	protected JSONObject createSqlTOKSP_NNDT(String userId, JSONArray dataArray, JSONObject data,SqlType sql) {
		...
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			for(TOKSP_NNDTLayout itm :TOKSP_NNDTLayout.values()){
				if(ArrayUtils.contains(notTarget, itm.getId())){ continue;}	// パラメータ不要
				if (itm.getId().equals(TOKSP_NNDTLayout.SENDFLG.getId())) { continue;}	// パラメータ不要
				if(itm.getNo() > 1){ sbSQL.append(","); }
				sbSQL.append("RE."+itm.getCol());
			}
			sbSQL.append(" ,"+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
	...
	public HashMap<Integer,String> getTencds(String bmnCd, String moysKbn, String moysStDt, String moysRban, String rankNoAdd,
			String rankNoDel, String rankNoAddB, String rankNoAddC, JSONArray tenCdAdds, JSONArray tenCdDels, String tenAtsukArr, String tenRankArr) {
		...
		if (!StringUtils.isEmpty(tenAtsukArr)) {
			...
			if (!StringUtils.isEmpty(rankNoAddB)) {
				...
				bTencds = new ReportBM015Dao(JNDIname).getTenCdAdd(
						 bmnCd					// 部門コード
						,moysKbn				// 催し区分
						,moysStDt				// 催し開始日
						,moysRban				// 催し連番
			...
			if (!StringUtils.isEmpty(rankNoAddC)) {
				...
				cTencds =new ReportBM015Dao(JNDIname).getTenCdAdd(
						 bmnCd					// 部門コード
						,moysKbn				// 催し区分
						,moysStDt				// 催し開始日
						,moysRban				// 催し連番
	...
	private JSONObject createSqlTOKSP_HB(String userId, JSONArray dataArray, HashMap<Integer,String> tenAtsuk, SqlType sql) {
		...
		for(int j=0; j < dataArray.size(); j++){
			...
			for (TOKSP_HBLayout itm :  TOKSP_HBLayout.values()) {
				...
				String val = new ReportJU012Dao(JNDIname).spaceArr(arr,1);
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.TOKSP_HB as T");
		sbSQL.append(" using (select ");
		// キー情報はロックのため後で追加する
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched then");
			sbSQL.append(" update set");
			sbSQL.append("  TENATSUK_ARR=RE.TENATSUK_ARR");	// F7 : 店扱いフラグ配列
			sbSQL.append(" ,SENDFLG="+DefineReport.Values.SENDFLG_UN.getVal());	// 送信フラグ
			sbSQL.append(" ,OPERATOR='"+userId+"'");							// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");									// 登録日
			sbSQL.append(" ,UPDDT=current timestamp");							// 更新日
	...
	private JSONObject createSqlTOKSP_SHNHBDT(String userId, JSONArray dataArray, SqlType sql,String table) {
		...
		sbSQL.append("  from (values"+rows+") as T1("+names+")");
		sbSQL.append(" ) as RE on (");
		sbSQL.append(" T.SHNCD = RE.SHNCD and T.HBDT = RE.HBDT");
		sbSQL.append(" )");
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when not matched then ");
			sbSQL.append(" insert (");
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched AND RE.MOYCD_ARR IS NOT NULL then");
			sbSQL.append(" update set");
			sbSQL.append("  MOYCD_ARR=RE.MOYCD_ARR");		// F3 : 催しコード配列
			sbSQL.append(" ,KANRINO_ARR=RE.KANRINO_ARR");	// F4 : 管理番号配列
			sbSQL.append(" ,OPERATOR='"+userId+"'");							// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");									// 登録日
			sbSQL.append(" ,UPDDT=current timestamp");							// 更新日
			sbSQL.append(" when matched AND RE.MOYCD_ARR IS NULL then DELETE");
		}
		if (DefineReport.ID_DEBUG_MODE) System.out.println(this.getClass().getName()+ ":" + sbSQL.toString());
	...
	private JSONObject createSqlTOK_CMN_SHNHBDT_DEL(String userId, JSONArray dataArray, SqlType sql, String szTable) {
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched AND trim(RE.MOYCD_ARR) IS NOT NULL then");
			sbSQL.append(" update set");
			sbSQL.append("  MOYCD_ARR=RE.MOYCD_ARR");		// F3 : 催しコード配列
			sbSQL.append(" ,KANRINO_ARR=RE.KANRINO_ARR");	// F4 : 管理番号配列
			sbSQL.append(" ,OPERATOR='"+userId+"'");							// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");									// 登録日
			sbSQL.append(" ,UPDDT=current timestamp");							// 更新日
			sbSQL.append(" when matched AND trim(RE.MOYCD_ARR) IS NULL then DELETE");
		}
		if (DefineReport.ID_DEBUG_MODE) System.out.println(this.getClass().getName()+ ":" + sbSQL.toString());
	...
	public JSONObject createSqlSYSMOYBMN_SEL(JSONObject data) {
		...
		sbSQL.append("   and T1.MOYSRBAN = "+szMoysrban+"");
		sbSQL.append("   and T1.BMNCD    = "+szBmncd+"");
		sbSQL.append(" for update with rs");
	...
	private JSONObject createSqlSYSMOYBMN(String userId, JSONObject data, SqlType sql) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.SYSMOYBMN as T");
		sbSQL.append(" using (select ");
		// キー情報はロックのため後で追加する
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
		...
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,RE.BMNCD");				// F4 : 部門
			sbSQL.append(" ,RE.KANRINO");			// F5 : 付番済管理番号
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched then");
			sbSQL.append(" update set");
			sbSQL.append("  SUMI_KANRINO=RE.KANRINO");	// F5 : 付番済管理番号
			sbSQL.append(" ,SUMI_HYOSEQNO=null");		// F6 : 付番済表示順番
			sbSQL.append(" ,OPERATOR='"+userId+"'");	// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");			// 登録日
			sbSQL.append(" ,UPDDT=current timestamp");	// 更新日
		}
	...
	public JSONObject createSqlSYSMOYKANRIENO_SEL(JSONObject data) {
		...
		sbSQL.append(" ,nvl(T1.SUMI_KANRIENO+1, 0) as KANRIENO");	// F6 : 付番済枝番→枝番
	...
	private JSONObject createSqlSYSMOYKANRIENO(String userId, JSONObject data, SqlType sql) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.SYSMOYKANRIENO as T");
		sbSQL.append(" using (select ");
		// キー情報はロックのため後で追加する
		for(TOK_CMNLayout itm :TOK_CMNLayout.values()){
			if(itm.getNo() > 1){ sbSQL.append(","); }
		...
		if(SqlType.INS.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,RE.MOYSRBAN");			// F3 : 催し連番
			sbSQL.append(" ,RE.BMNCD");				// F4 : 部門
			sbSQL.append(" ,RE.KANRINO");			// F5 : 管理番号
			sbSQL.append(" ,RE.KANRIENO");			// F6 : 付番済枝番
			sbSQL.append(" ,'"+userId+"'");			// オペレータ
			sbSQL.append(" ,current timestamp");	// 登録日
			sbSQL.append(" ,current timestamp");	// 更新日
			sbSQL.append(")");
		...
		if(SqlType.UPD.getVal() == sql.getVal() || SqlType.MRG.getVal() == sql.getVal()){
			sbSQL.append(" when matched then");
			sbSQL.append(" update set");
			sbSQL.append("  SUMI_KANRIENO=RE.KANRIENO");	// F6 : 付番済枝番
			sbSQL.append(" ,OPERATOR='"+userId+"'");		// オペレータ
			//sbSQL.append(" ,ADDDT=RE.ADDDT");				// 登録日
			sbSQL.append(" ,UPDDT=current timestamp");		// 更新日
		}
	 ...
	 * @throws Exception
	 */
	private JSONObject createSqlTOKTG_CMN(int userId, JSONArray dataArray, SqlType sql, String table) {
		JSONObject result = new JSONObject();
	...
	public enum TOK_CMNLayout implements MSTLayout{
		...
		public DataType getDataType() {
			...
			if(typ.indexOf("DATE")!= -1||typ.indexOf("TIMSTAMP")!= -1){return DefineReport.DataType.DATE;}
