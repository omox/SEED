...
public class ItemDao implements ItemInterface {
	...
	public boolean selectBySQL(String command, boolean setDbTitle) {
		...
		try {
			...
			System.out.println(java8Disp+"\t[MDM]\t"+getMap().get("report")+"\t"+userInfo.getId()+"\t"+diff+"\t"+command+"\t"+(String)getParamData().toString());
	...
	public ArrayList<Integer> executeSQLs(ArrayList<String> commands, ArrayList<ArrayList<String>> paramDatas) throws Exception {
		...
		try {
			...
			for (int index=0; index < commands.size(); index++) {
				...
				// SQL実行
				if (DefineReport.ID_DEBUG_MODE) System.out.println("[sql]"+command+"[prm]"+(paramData == null?"":StringUtils.join(paramData.toArray(),","))+"[statement]");
				// SQL実行
	...
	public static int updateBySQL(String sqlCommand, ArrayList<String> paramData, Connection con) throws Exception {
		if(DefineReport.ID_DEBUG_MODE) System.out.println("[sql]"+sqlCommand+"[prm]"+(paramData == null?"":StringUtils.join(paramData.toArray(),",")));
