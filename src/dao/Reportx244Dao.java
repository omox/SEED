...
public class Reportx244Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" ,nvl(TO_CHAR(T1.INF_DATE, 'YYYY/MM/DD HH24:MI'),'__/__/__')");			// 更新情報_更新日時
	...
	private String getSqlWhere(ArrayList<String> paramData) {
		...
		if (!StringUtils.isEmpty(szShncd)){
			...
			if (bumonArray.optString(0).equals(DefineReport.Values.ALL.getVal())){
				szWhereCmd = " INTEGER(SUBSTR(T1.SHNCD,1,2)) IN ("+StringUtils.removeEnd(StringUtils.replace(StringUtils.replace(bumonAllArray.join(","),"\"0","\""),"\"",""),",")+")";
				szWhereCmd += " and T1.BMNCD IN ("+StringUtils.removeEnd(StringUtils.replace(StringUtils.replace(bumonAllArray.join(","),"\"0","\""),"\"",""),",")+")";
			}else{
				szWhereCmd = " INTEGER(SUBSTR(T1.SHNCD,1,2)) = "+StringUtils.removeEnd(StringUtils.replace(StringUtils.replace(bumonArray.join(","),"\"0","\""),"\"",""),",");
		...
		if ((!StringUtils.isEmpty(szFromDate)) && (StringUtils.isEmpty(szToDate))) {
			...
			szWhereCmd += " and to_char(T1.INF_DATE, 'YYYYMMDD') <= '" + convdt_to + "'";
		}
		if ((StringUtils.isEmpty(szFromDate)) && (!StringUtils.isEmpty(szToDate))) {
			...
			szWhereCmd += " and to_char(T1.INF_DATE, 'YYYYMMDD') >= '" + convdt_from + "'";
		}

		if (!StringUtils.isEmpty(szFromDate)){
			String convdt = CmnDate.getConvInpDate(szFromDate);
			szWhereCmd += " and to_char(T1.INF_DATE, 'YYYYMMDD') >= '" + convdt + "'";
		}
