...
public class ReportTG002Dao extends ItemDao {
  ...
  public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
    ...
    sbSQL.append("  , COALESCE(DATE_FORMAT(T1.ADDDT, '%y/%m/%d'),'__/__/__') as F5");
    sbSQL.append("  , COALESCE(DATE_FORMAT(T1.UPDDT, '%y/%m/%d'),'__/__/__') as F6");
    sbSQL.append("  , DATE_FORMAT(T1.UPDDT, '%Y%m%d%H%i%s%f') as F7");
