...
public class ReportTM002Dao extends ItemDao {
	...
	private String createCommand() {
		...
		if(isNew){
			...
			sbSQL.append(" left outer join INAAD.SYSSHUNO T2 on TO_CHAR(TO_DATE(T1.SHORIDT, 'YYYYMMDD') + 1 year, 'YYYYMMDD') between T2.STARTDT and T2.ENDDT");
		...
		// 変更/参照
			...
			sbSQL.append(" ,nvl(TO_CHAR(T1.ADDDT, 'YY/MM/DD'),'__/__/__') as ADDDT");	// F6 : 登録日
			sbSQL.append(" ,nvl(TO_CHAR(T1.UPDDT, 'YY/MM/DD'),'__/__/__') as UPDDT");	// F7 : 更新日
			sbSQL.append(" ,TO_CHAR(T1.UPDDT, 'YYYYMMDDHH24MISSNNNNNN') as HDN_UPDDT");	// F8:更新日時
			sbSQL.append(" ,CASE WHEN T1.TSHUFLG <> '1' AND T2.SHUNO IS NOT NULL THEN");
			sbSQL.append(" RIGHT('0000'||T2.SHUNO,4)||'-'||TO_CHAR(TO_DATE(T2.STARTDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.STARTDT, 'YYYYMMDD')))");
			sbSQL.append("||'～'||TO_CHAR(TO_DATE(T2.ENDDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T2.ENDDT, 'YYYYMMDD')))");
			...
			sbSQL.append(" where T1.SHUNO = " + szShuno + " and nvl(T1.UPDKBN, 0) <> 1 ");
		}
	...
	public JSONObject createSqlSelTOKMOYCD(HashMap<String, String> map) {
		...
		if(ValKbn10002.VAL0.getVal().equals(szMoyskbn)||ValKbn10002.VAL1.getVal().equals(szMoyskbn)||(!isOtherM && ValKbn10002.VAL2.getVal().equals(szMoyskbn))||ValKbn10002.VAL3.getVal().equals(szMoyskbn)){
			col_chk_use += "+nvl(T2.CNT_FLG, 0)+nvl(T4.CNT_FLG, 0)";
		...
		if(StringUtils.equals(key, "CNT")){
			...
			if(StringUtils.isNotEmpty(col_chk_use)){
				sbSQL.append(" ,case when "+col_chk_use+" > 0 then 1 else 0 end  as USE ");
			}else{
				sbSQL.append(" ,0 as USE ");
			}
			...
			if(ValKbn10002.VAL1.getVal().equals(szMoyskbn)){
				...
				sbSQL.append("    and nvl(T1.UPDKBN, 0) = 0");
				sbSQL.append("    and T1.MOYSKBN = "+szMoyskbn);
				if(!StringUtils.isEmpty(szSelShuno)){
					param.add(szSelShuno);
					sbSQL.append("    and T1.SHUNO = ?");
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(isNew){
			// --- 01.催し週
			JSONObject result1 = this.createSqlTOKMOYSYU(userId, data, SqlType.INS);

		// ②正 .変更
		}else if(isChange){
			// --- 02.催しコード削除
			if(dataArrayMOYCDD.size() > 0){
				JSONObject result2 = this.createSqlTOKMOYCD(userId, dataArrayMOYCDD, SqlType.DEL);
			}

			if(dataArrayTOKTGD.size() > 0){
				// --- 全店特売（アンケート有）系情報削除
				JSONObject resultD1 = this.createSqlTOKTG_CMN(userId, dataArrayTOKTGD, SqlType.DEL, "TOKTG_KHN");		// 全店特売（アンケート有）_基本
				JSONObject resultD2 = this.createSqlTOKTG_CMN(userId, dataArrayTOKTGD, SqlType.DEL, "TOKTG_TENGP");		// 全店特売（アンケート有）_店グループ

				// ---臨時系情報削除
				JSONObject resultD3 = this.createSqlTOKTG_CMN(userId, dataArrayTOKTGD, SqlType.DEL, "TOKRANKEX");		// 臨時ランクマスタ
				JSONObject resultD4 = this.createSqlTOKTG_CMN(userId, dataArrayTOKTGD, SqlType.DEL, "TOKSRPTNEX");		// 臨時数量パターンテーブル
			}

			if (delTokMoySyu.equals("1")) {
				// --- 01.催し週
				JSONObject result1 = this.createSqlTOKMOYSYU(userId, data, SqlType.DEL);
			}
		}
		...
		if(dataArrayMOYCDS.size() > 0){
			JSONObject result4 = this.createSqlTOKMOYCD(userId, dataArrayMOYCDS, SqlType.MRG);
		}
		// --- 05.催しコード登録-特売
		if(dataArrayMOYCDT.size() > 0){
			JSONObject result5 = this.createSqlTOKMOYCD(userId, dataArrayMOYCDT, SqlType.MRG);
		}

		// --- 06.ちらしのみ部門
		if(dataArrayTOKCHIRASBMN.size() > 0){
			JSONObject result6 = this.createSqlTOKCHIRASBMN(userId, dataArrayTOKCHIRASBMN, SqlType.MRG);
		}

		// --- 07.全店特売（アンケート有）_基本
		if(dataArrayTOKTG.size() > 0){
			JSONObject result7 = this.createSqlTOKTG_CMN(userId, dataArrayTOKTG, SqlType.MRG, "TOKTG_KHN");
		}
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		// チェック処理
		// 対象件数チェック
		if((dataArrayMOYCDR.size() == 0 || dataArrayMOYCDR.getJSONObject(0).isEmpty()) &&
				(dataArrayMOYCDS.size() == 0 || dataArrayMOYCDS.getJSONObject(0).isEmpty()) &&
				(dataArrayMOYCDT.size() == 0 || dataArrayMOYCDT.getJSONObject(0).isEmpty()) &&
				(dataArrayMOYCDD.size() == 0 || dataArrayMOYCDD.getJSONObject(0).isEmpty()) &&
				(dataArrayTOKCHIRASBMN.size() == 0 || dataArrayTOKCHIRASBMN.getJSONObject(0).isEmpty()) &&
				(dataArrayTOKTG.size() == 0 || dataArrayTOKTG.getJSONObject(0).isEmpty()) &&
		...
		List<JSONObject> msgList = this.checkData(
				...
				dataArray, dataArrayMOYCD, dataArrayMOYCDDel);
...
///*		if(isNew){
//			targetCol = new MSTSHNLayout[]{MSTSHNLayout.SHNAN,MSTSHNLayout.PCARD_SHUKBN,MSTSHNLayout.PCARD_IROKBN,MSTSHNLayout.BMNCD,MSTSHNLayout.DAICD,MSTSHNLayout.CHUCD,MSTSHNLayout.SHOCD
...
//		}else if(isChange){
//			targetCol = new MSTSHNLayout[]{MSTSHNLayout.SHUNO,MSTSHNLayout.SHNAN,MSTSHNLayout.PCARD_SHUKBN,MSTSHNLayout.PCARD_IROKBN,MSTSHNLayout.BMNCD
//					,MSTSHNLayout.TEIKEIKBN,MSTSHNLayout.TEISHIKBN,MSTSHNLayout.SHNKBN,MSTSHNLayout.PCKBN,MSTSHNLayout.TEIKANKBN,MSTSHNLayout.KAKOKBN,MSTSHNLayout.TANAOROKBN,MSTSHNLayout.ZEIKBN,MSTSHNLayout.SIMEKAISU,MSTSHNLayout.BINKBN,MSTSHNLayout.RG_IDENFLG};
//		}*/
...
//		}
	...
	private JSONObject createSqlTOKMOYSYU(String userId, JSONObject data, SqlType sql) {
		...
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.TOKMOYSYU as T");
		sbSQL.append(" using (select ");
		sbSQL.append("  cast(? as "+TOKMOYSYULayout.SHUNO.getTyp() + ") as " + TOKMOYSYULayout.SHUNO.getCol());
		sbSQL.append(" ,cast(? as "+TOKMOYSYULayout.TSHUFLG.getTyp() + ") as " + TOKMOYSYULayout.TSHUFLG.getCol());
		sbSQL.append(" from SYSIBM.SYSDUMMY1");
		sbSQL.append(" ) as RE on (T.SHUNO=RE.SHUNO) ");
		...
		sbSQL.append(")values(");
		sbSQL.append("  RE.SHUNO");									// F1 : 週№
		sbSQL.append(" ,RE.TSHUFLG");								// F2 : 特別週フラグ
		sbSQL.append(" ,"+DefineReport.ValUpdkbn.NML.getVal());		// F3 : 更新区分
		sbSQL.append(" ,0");										// F4 : 送信フラグ
	...
	private JSONObject createSqlTOKMOYCD(String userId, JSONArray dataArray, SqlType sql) {
		...
		if(SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,PLUSFLG= case when NVL(UPDKBN,0) = 1 then RE.PLUSFLG else PLUSFLG end");	// F15: PLU配信済フラグ
	...
	private JSONObject createSqlTOKCHIRASBMN(String userId, JSONArray dataArray, SqlType sql) {
		...
		String szWhere = "T.MOYSKBN = RE.MOYSKBN and T.MOYSSTDT= RE.MOYSSTDT and T.MOYSRBAN = RE.MOYSRBAN and T.BMNCD = RE.BMNCD";
		String sendflg = DefineReport.Values.SENDFLG_UN.getVal();

		// 基本Merge文
		StringBuffer sbSQL;
		sbSQL = new StringBuffer();
		sbSQL.append("merge into INATK.TOKCHIRASBMN as T");
		sbSQL.append(" using (select ");
		sbSQL.append("  cast(F1 as SMALLINT) as MOYSKBN");		// F1 : 催し区分
		sbSQL.append(" ,cast(F2 as INTEGER) as MOYSSTDT");		// F2 : 催し開始日
		sbSQL.append(" ,cast(F3 as SMALLINT) as MOYSRBAN");		// F3 : 催し連番
		sbSQL.append(" ,cast(F4 as SMALLINT) as BMNCD");		// F4 : 部門
		sbSQL.append(" from (values"+rows+") as T1("+names+")");
		sbSQL.append(" ) as RE on ("+szWhere+") ");
		...
		if(SqlType.MRG.getVal() == sql.getVal()){
			...
			sbSQL.append(" ,"+sendflg);				// F5 : 送信フラグ
			sbSQL.append(" ,'"+userId+"'");				// F6 : オペレータ
			sbSQL.append(" ,current timestamp");	// F7 : 登録日
			sbSQL.append(" ,current timestamp");	// F8 : 更新日
			sbSQL.append(" )");
			sbSQL.append(" when matched then ");
			sbSQL.append(" update set");
			sbSQL.append("  SENDFLG="+sendflg);			// F5 : 送信フラグ
			sbSQL.append(" ,OPERATOR='"+userId+"'");			// F6 : オペレータ
			sbSQL.append(" ,UPDDT=current timestamp");	// F8 : 更新日
