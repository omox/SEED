...
public class ReportYH203Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    if (!StringUtils.equals("合計", szNndt)) {
      ...
      sbSQL.append(" CAST(HKKK.KKKCD AS CHAR) || ' - ' || COALESCE(HKKK.KKKKM, '') as KKKKM");
      sbSQL.append(", left (TRIM(HSHN.SHNCD), 4) || '-' || right (TRIM(HSHN.SHNCD), 4) || ' ' || COALESCE(SHN.SHNKN, '') as SHNKN");
      sbSQL.append(", HKKK.KKKCD");
      sbSQL.append(", HSHN.SHNCD");
      sbSQL.append(", DATE_FORMAT(NNDT.UPDDT,'%Y%m%d%H%i%s%f') as HDN_UPDDT ");
      sbSQL.append(", DATE_FORMAT(DATE_FORMAT('20' || right ('0' || NNDT.NNDT, 6), '%Y%m%d'), '%y/%m/%d') as NNSTDT_DSP");
      sbSQL.append(", DAYOFWEEK(DATE_FORMAT('20' || right ('0' || NNDT.NNDT, 6), '%Y%m%d')) as STARTDTW, right (NNDT.NNDT, 6) as NNDT");
      sbSQL.append(", DATE_FORMAT(HKKK.ADDDT, '%y/%m/%d') as ADDDT");
      sbSQL.append(", DATE_FORMAT(HKKK.UPDDT, '%y/%m/%d') as UPDDT");
      ...
      sbSQL.append(", T1.HTDT");
      sbSQL.append(" from (select");
      sbSQL.append(" CAST(HKKK.KKKCD AS CHAR) || ' - ' || COALESCE(MAX(HKKK.KKKKM), '') as KKKKM");
      sbSQL.append(", left(TRIM(HSHN.SHNCD), 4) || '-' ||  right(TRIM(HSHN.SHNCD), 4) || ' ' || COALESCE(MAX(SHN.SHNKN), '') as SHNKN");
      sbSQL.append(", HKKK.KKKCD");
      sbSQL.append(", HSHN.SHNCD");
      sbSQL.append(", DATE_FORMAT(DATE_FORMAT(MIN(NNDT.NNDT), '%Y%m%d'), '%y/%m/%d') as NNSTDT_DSP");
      sbSQL.append(", DAYOFWEEK(DATE_FORMAT(MIN(NNDT.NNDT), '%Y%m%d')) as STARTDTW");
      sbSQL.append(", DATE_FORMAT(DATE_FORMAT(MAX(NNDT.NNDT), '%Y%m%d'), '%y/%m/%d') as NNEDDT_DSP");
      sbSQL.append(", DAYOFWEEK(DATE_FORMAT(MAX(NNDT.NNDT), '%Y%m%d')) as ENDDTW");
      sbSQL.append(", DATE_FORMAT(MAX(HKKK.ADDDT), '%y/%m/%d') as ADDDT");
      sbSQL.append(", DATE_FORMAT(MAX(HKKK.UPDDT), '%y/%m/%d') as UPDDT");
      sbSQL.append(", MAX(HKKK.OPERATOR) as OPERATOR");
      sbSQL.append(", DATE_FORMAT(MAX(NNDT.UPDDT),'%Y%m%d%H%i%s%f') as HDN_UPDDT");
  ...
  public String createSqlTEN(JSONObject data, HashMap<String, String> map, User userInfo) {
    ...
    sbSQL.append(", ADDDT "); // F5 : 登録日
    sbSQL.append(", UPDDT "); // F6 : 更新日
    sbSQL.append(") values (");
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (valueData.length >= 100 || (i + 1 == len && valueData.length > 0)) {
        ...
        sbSQL.append(",INPUTDT=VALUES(INPUTDT) ");
