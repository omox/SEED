...
public class Reportx004Dao extends ItemDao {
    ...
    try {
      ...
      if (errMsgList.size() == 0) {
        ...
        for (int i = idxHeader; i < dL.size(); i++) {
          ...
          JSONObject oshn = new JSONObject(), osrc = new JSONObject(), osir = new JSONObject(), obik = new JSONObject(), osin = new JSONObject(), oten = new JSONObject(), otbmn = new JSONObject(),
              ooth = new JSONObject();
          ...
          for (FileLayout itm : FileLayout.values()) {
            ...
            if (itm.getTbl() == RefTable.MSTSHN) {
              ...
              // 特殊データ加工
              if (StringUtils.equals(itm.getCol(), FileLayout.SRCCD1.getCol()) || StringUtils.equals(itm.getCol(), FileLayout.SRCCD2.getCol())) {
                // 先頭に'がある場合除去
        ...
        for (int i = 0; i < dataList.size(); i++) {
          ...
          // 基本情報チェック
          JSONArray msgListB = this.checkData(isSeiRep, dao, sendmap, userInfo, sysdate, mu, mstshnArray, // 対象情報（主要な更新情報）
              new JSONArray(), mstsrcArray, // ソースコード
              mstsinArray, // 品揃えグループ
          ...
          DefineReport.ValFileUpdkbn.YYK.getVal().equals(fUpdkbn);
          ...
          if (msgListB.size() == 0) {
            if (!isDel) {
              if (!isChangeY1Del) {
                msgList = dao.checkData(isNew, isChange, isYoyaku1, false, false, true, sendmap, userInfo, sysdate, mu, mstshnArray, // 対象情報（主要な更新情報）
                    new JSONArray(), mstsrcArray, // ソースコード
                    msttbmnArray, // 店別異部門
                    mstsinArray, // 品揃えグループ
                    mstbikArray, // 売価コントロール
              ...
              msgList_ = dao.checkDataDel(isNew, isChange, isYoyaku1, false, false, sendmap, userInfo, sysdate, mu, mstshnArray, // 対象情報（主要な更新情報）
                  mstsrcArray, // ソースコード
                  mstsinArray, // 品揃えグループ
                  mstbikArray, // 売価コントロール
                  mstsirArray, // 仕入グループ
          ...
          if (isError) {
            ...
            if (StringUtils.isNotEmpty(ooth.optString("SHNCD_RENEW"))) {
              NumberingUtility.execReleaseNewSHNCD(userInfo, ooth.optString("SHNCD_RENEW"));
            }
            // 採番実行時にエラーの場合、解除処理
            if (StringUtils.isNotEmpty(ooth.optString("URICD_NEW"))) {
              NumberingUtility.execReleaseNewURICD(userInfo, ooth.optString("URICD_NEW"));
            }

            // CVSトランSEQ情報取得
            if (StringUtils.equals(seq, "-1")) {
              seq = this.getCSVSHN_SEQ();
              this.createSqlCSVSHNHEAD(userId, sendBtnid, seq, szCommentkn);
            }
          ...
          this.createCommandUpdateData(
              // 更新処理コマンド作成
              dao, isNew, isSeiRep, isError, jnlshn_seq, sendmap, userInfo, sysdate, mstshnArray, // 対象情報（主要な更新情報）
              mstsrcArray, // ソースコード
              msttbmnArray, // 店別異部門
              mstsinArray, // 品揃えグループ
              mstbikArray, // 売価コントロール
   ...
   * @throws Exception
   */
  private JSONObject createCommandDeleteData(Reportx002Dao dao, boolean isNewtyp, boolean isSeiRep, boolean isErr, String jnlshn_seq, HashMap<String, String> map, User userInfo, String sysdate,
    ...
    DefineReport.Button.SEARCH.getObj().equals(sendBtnid);
    DefineReport.Button.SEI.getObj().equals(sendBtnid);
    ...
    dao.createSqlMSTSHN_DEL(userId, sendBtnid, data, TblType.YYK, SqlType.DEL);
    ...
    dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayDel, TblType.YYK, SqlType.DEL);

    dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayDel, TblType.YYK, SqlType.DEL);

    dao.createSqlMSTSRCCD(userId, sendBtnid, dataArrayDel, TblType.YYK, SqlType.DEL);

    dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayDelTENKABUTSU, TblType.YYK, SqlType.DEL);

    dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayDel, TblType.YYK, SqlType.DEL);


    dao.createSqlSYSSHNCOUNT(userId, sendBtnid, TblType.SEI, SqlType.MRG);
    ...
    dao.createSqlMSTSHN(userId, sendBtnid, data, TblType.JNL, SqlType.INS);

    // --- 16.ジャーナル_仕入グループ商品
    if (dataArrayTENGP1_Del.size() > 0) {
      dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayTENGP1_Del, TblType.JNL, SqlType.INS);
    }

    // --- 17.ジャーナル_売価コントロール
    if (dataArrayTENGP2_Del.size() > 0) {
      dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayTENGP2_Del, TblType.JNL, SqlType.INS);
    }

    // --- 18.ジャーナル_ソースコード管理
    if (dataArraySRCCD_Del.size() > 0) {
      dao.createSqlMSTSRCCD(userId, sendBtnid, dataArraySRCCD_Del, TblType.JNL, SqlType.INS);
    }

    // --- 19.ジャーナル_添加物
    if (dataArrayTENKABUTSU_Del.size() > 0) {
      dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayTENKABUTSU_Del, TblType.JNL, SqlType.INS);
    }

    // --- 20.ジャーナル_品揃グループ
    if (dataArrayTENGP3_Del.size() > 0) {
      dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayTENGP3_Del, TblType.JNL, SqlType.INS);
    }

    // --- 21.ジャーナル_グループ分類管理マスタ
    if (dataArrayGROUP_Del.size() > 0) {
      dao.createSqlMSTGRP(userId, sendBtnid, dataArrayGROUP, TblType.JNL, SqlType.INS);
    }

    // --- 22.ジャーナル_自動発注
    if (dataArrayAHS_Del.size() > 0) {
      dao.createSqlMSTAHS(userId, sendBtnid, dataArrayAHS, TblType.JNL, SqlType.INS);
    }

    // --- 23.ジャーナル_店別異部門
    if (dataArrayTENGP4_Del.size() > 0) {
      dao.createSqlMSTSHNTENBMN(userId, sendBtnid, dataArrayTENGP4_Del, TblType.JNL, SqlType.INS);
    }
    // }
  ...
  private JSONArray checkData(boolean isSeiRep, Reportx002Dao dao, HashMap<String, String> map, User userInfo, String sysdate1, MessageUtility mu, JSONArray dataArray, // 対象情報（主要な更新情報）
    ...
    // 予約画面で正更新
    if (!isSeiRep && (DefineReport.ValFileUpdkbn.NEW.getVal().equals(fUpdkbn) || DefineReport.ValFileUpdkbn.UPD.getVal().equals(fUpdkbn) || DefineReport.ValFileUpdkbn.DEL.getVal().equals(fUpdkbn))) {
      JSONObject o = mu.getDbMessageObj("E40009", new String[] {});
      this.setCsvshnErrinfo(o, errTbl, FileLayout.UPDKBN, fUpdkbn);
      msg.add(o);
      return msg;
   ...
   * @throws Exception
   */
  private JSONObject createCommandUpdateData(Reportx002Dao dao, boolean isNewtyp, boolean isSeiRep, boolean isErr, String jnlshn_seq, HashMap<String, String> map, User userInfo, String sysdate,
      JSONArray dataArray, // 対象情報（主要な更新情報）
      JSONArray dataArraySRCCD, // ソースコード
      JSONArray dataArrayTENGP4, // 店別異部門
      JSONArray dataArrayTENGP3, // 品揃えグループ
    ...
    if (isNew) {
      ...
      dao.createSqlMSTSHN(userId, sendBtnid, data, TblType.SEI, SqlType.MRG);

      // --- 08.商品コード空き番
      // JSONObject result8 = dao.createSqlSYSSHNCD_AKI(userId, sendBtnid, data, TblType.SEI,
      // SqlType.MRG);
      ...
      // --- 10.販売コード空き番
      // JSONObject result10 = dao.createSqlSYSURICD_AKI(userId, sendBtnid, data, TblType.SEI,
      // SqlType.UPD);
      ...
      dao.createSqlMSTSHN(userId, sendBtnid, data, TblType.SEI, SqlType.UPD);
      ...
      dao.createSqlMSTSHN(userId, sendBtnid, data, TblType.YYK, SqlType.MRG);
      ...
      dao.createSqlMSTSHN_DEL(userId, sendBtnid, data, TblType.SEI, SqlType.DEL);
      ...
      dao.createSqlMSTSHN(userId, sendBtnid, data, TblType.CSV, SqlType.INS);
    }
    ...
    if (isSeiRep && isErr && !isDel) {
      ...
      if (dataArrayTENGP1.size() > 0) {
        dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayTENGP1, baseTblType, SqlType.INS);
      }

      // --- 03.売価コントロール
      if (dataArrayTENGP2.size() > 0) {
        dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayTENGP2, baseTblType, SqlType.INS);
      }
      // --- 04.ソースコード管理
      if (dataArraySRCCD.size() > 0) {
        dao.createSqlMSTSRCCD(userId, sendBtnid, dataArraySRCCD, baseTblType, SqlType.INS);
      }

      // --- 05.添加物
      if (dataArrayTENKABUTSU.size() > 0) {
        dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayTENKABUTSU, baseTblType, SqlType.INS);
      }

      // --- 06.品揃グループ
      if (dataArrayTENGP3.size() > 0) {
        dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayTENGP3, baseTblType, SqlType.INS);
      }

      // --- 09.店別異部門
      if (dataArrayTENGP4.size() > 0) {
        dao.createSqlMSTSHNTENBMN(userId, sendBtnid, dataArrayTENGP4, baseTblType, SqlType.INS);
      }
      ...
      dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayDel, baseTblType, SqlType.DEL);
      if (dataArrayTENGP1.size() > 0) {
        dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayTENGP1, baseTblType, SqlType.INS);
      }

      dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayDel, baseTblType, SqlType.DEL);
      if (dataArrayTENGP2.size() > 0) {
        dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayTENGP2, baseTblType, SqlType.INS);
      }
      dao.createSqlMSTSRCCD(userId, sendBtnid, dataArrayDel, baseTblType, SqlType.DEL);
      if (dataArraySRCCD.size() > 0) {
        dao.createSqlMSTSRCCD(userId, sendBtnid, dataArraySRCCD, baseTblType, SqlType.INS);
      }

      dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayDelTENKABUTSU, baseTblType, SqlType.DEL);
      if (dataArrayTENKABUTSU.size() > 0) {
        dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayTENKABUTSU, baseTblType, SqlType.INS);
      }

      dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayDel, baseTblType, SqlType.DEL);
      if (dataArrayTENGP3.size() > 0) {
        dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayTENGP3, baseTblType, SqlType.INS);
      }

      dao.createSqlMSTSHNTENBMN(userId, sendBtnid, dataArrayDelMSTSHNTENBMN, baseTblType, SqlType.DEL);
      if (dataArrayTENGP4.size() > 0) {
        dao.createSqlMSTSHNTENBMN(userId, sendBtnid, dataArrayTENGP4, baseTblType, SqlType.INS);
      }
    }
    ...
    if (!isErr && !isChangeY1Del && !isDel) {
      ...
      if (StringUtils.isNotEmpty(makercd) && dataArraySRCCD.size() > 0) {
        ...
        if (!dao.checkMstExist(DefineReport.InpText.MAKERCD.getObj(), makercd)) {
          ...
          dao.createSqlMSTMAKER(userId, sendBtnid, dataMAKER, TblType.SEI, SqlType.INS);
        }
      }
    ...
    if (!isErr && !isChangeY1Del) {
      ...
      // --- 16.ジャーナル_仕入グループ商品
      if (dataArrayTENGP1.size() > 0) {
        dao.createSqlMSTSIRGPSHN(userId, sendBtnid, dataArrayTENGP1, TblType.JNL, SqlType.INS);
      }

      // --- 17.ジャーナル_売価コントロール
      if (dataArrayTENGP2.size() > 0) {
        dao.createSqlMSTBAIKACTL(userId, sendBtnid, dataArrayTENGP2, TblType.JNL, SqlType.INS);
      }

      // --- 18.ジャーナル_ソースコード管理
      if (dataArraySRCCD.size() > 0) {
        dao.createSqlMSTSRCCD(userId, sendBtnid, dataArraySRCCD, TblType.JNL, SqlType.INS);
      }

      // --- 19.ジャーナル_添加物
      if (dataArrayTENKABUTSU.size() > 0) {
        dao.createSqlMSTTENKABUTSU(userId, sendBtnid, dataArrayTENKABUTSU, TblType.JNL, SqlType.INS);
      }

      // --- 20.ジャーナル_品揃グループ
      if (dataArrayTENGP3.size() > 0) {
        dao.createSqlMSTSHINAGP(userId, sendBtnid, dataArrayTENGP3, TblType.JNL, SqlType.INS);
      }

      // --- 23.ジャーナル_店別異部門
      if (dataArrayTENGP4.size() > 0) {
        dao.createSqlMSTSHNTENBMN(userId, sendBtnid, dataArrayTENGP4, TblType.JNL, SqlType.INS);
      }
    }
  ...
  public JSONObject createSqlCSVSHNHEAD(String userId, String btnId, String seq, String commentkn) {
    ...
    values += ",current_timestamp"; // INPUT_DATE
    values += ",cast(? as char(" + MessageUtility.getDefByteLen(commentkn) + "))";
    ...
    for (MSTSHNLayout itm : MSTSHNLayout.values()) {
      ...
      if (data.containsKey(itm.getCol())) {
        ...
        if (StringUtils.isNotEmpty(value)) {
          prmData.add(value);
          sbSQLIn.append("cast(? as char(" + MessageUtility.getDefByteLen(value) + ")) as " + itm.getCol());
        } else {
    ...
    for (MSTSHNLayout itm : MSTSHNLayout.values()) {
      ...
      if (itm.isText()) {
        ...
        if (itm.getCol().equals(MSTSHNLayout.POPKN.getCol())) {
          sbSQL.append("CASE WHEN COALESCE(RE." + MSTSHNLayout.SHNKBN.getCol() + ", CAST(T." + MSTSHNLayout.SHNKBN.getCol() + " AS CHAR)) = '0' AND ");
          sbSQL.append("trim(COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR))) = '' THEN ");
          sbSQL.append("trim(COALESCE(RE." + MSTSHNLayout.SHNKN.getCol() + ", CAST(T." + MSTSHNLayout.SHNKN.getCol() + " AS CHAR))) ELSE ");
          sbSQL.append("trim(COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR))) END as " + itm.getId());
        } else {
          sbSQL.append("trim(COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR))) as " + itm.getId());
        }
      } else {
        sbSQL.append("COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR)) as " + itm.getId());
      }
    }
  ...
  public JSONArray selectCompData(JSONObject ShnData, JSONArray dataArray, String tbl, String keyCol, String groupCol, dao.Reportx002Dao.MSTLayout[] layouts) {
      ...
      for (dao.Reportx002Dao.MSTLayout itm : layouts) {
        ...
        if (dataArray.optJSONObject(j).containsKey(itm.getCol()) && !ArrayUtils.contains(nullSetCol, itm.getCol())) {
          if (StringUtils.isNotEmpty(val)) {
            ...
            if (itm.getTyp().equals("SIGNED") || itm.getTyp().equals("SIGNED")) {
              values += ",cast(cast(null as " + itm.getTyp() + ") as char (1))";
            } else if (itm.getTyp().equals("SMALLINT") || itm.getTyp().equals("INTEGER")) {
              values += ",cast(cast(null as SIGNED) as char (1))";
            } else {
              values += ",cast(null as " + itm.getTyp() + ")";
            }
          }
        ...
        if (!StringUtils.isEmpty(groupCol) && col.equals(groupCol)) {
          selNames += ", CAST(ROW_NUMBER() over (order by @O) AS CHAR) as " + col;
        } else {
          selNames += ", " + col;
          groupNames += ", " + col;
        }
    ...
    for (dao.Reportx002Dao.MSTLayout itm : layouts) {
      ...
      if (itm.isText()) {
        sbSQL.append("trim(COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR))) as " + itm.getCol());
      } else {
        sbSQL.append("COALESCE(RE." + itm.getCol() + ", CAST(T." + itm.getCol() + " AS CHAR)) as " + itm.getCol());
      }
    }
    for (CSVCMNLayout itm : CSVCMNLayout.values()) {
    ...
    sbSQL.append(" GROUP BY " + groupNames + ") RE left outer join @T T on @W) AS MT ");
  ...
  public enum MSTSHNLayout implements MSTLayout {
    ...
    /** レギュラー情報_一括伝票フラグ */
    RG_IDENFLG(37, "RG_IDENFLG", "CHAR(1)", "レギュラー情報_一括伝票フラグ"),
    /** レギュラー情報_ワッペン */
    RG_WAPNFLG(38, "RG_WAPNFLG", "CHAR(1)", "レギュラー情報_ワッペン"),
    /** 販促情報_取扱フラグ */
    ...
    /** 販促情報_ワッペン */
    HS_WAPNFLG(43, "HS_WAPNFLG", "CHAR(1)", "販促情報_ワッペン"),
    /** 販促情報_スポット最低発注数 */
    HS_SPOTMINSU(44, "HS_SPOTMINSU", "SIGNED", "販促情報_スポット最低発注数"),
    /** 販促情報_特売ワッペン */
    HP_SWAPNFLG(45, "HP_SWAPNFLG", "CHAR(1)", "販促情報_特売ワッペン"),
    /** 規格名称 */
    ...
    /** 期間 */
    KIKANKBN(85, "KIKANKBN", "CHAR(1)", "期間"),
    /** 酒級 */
    SHUKYUKBN(86, "SHUKYUKBN", "SIGNED", "酒級"),
    /** 度数 */
    DOSU(87, "DOSU", "SIGNED", "度数"),
    /** 陳列形式コード */
    CHINRETUCD(88, "CHINRETUCD", "CHAR(1)", "陳列形式コード"),
    /** 段積み形式コード */
    DANTUMICD(89, "DANTUMICD", "CHAR(2)", "段積み形式コード"),
    /** 重なりコード */
    KASANARICD(90, "KASANARICD", "CHAR(1)", "重なりコード"),
    /** 重なりサイズ */
    KASANARISZ(91, "KASANARISZ", "SIGNED", "重なりサイズ"),
    /** 圧縮率 */
    ASSHUKURT(92, "ASSHUKURT", "SIGNED", "圧縮率"),
    /** 種別コード */
    SHUBETUCD(93, "SHUBETUCD", "CHAR(2)", "種別コード"),
    /** 販売コード */
    ...
    /** 親商品コード */
    PARENTCD(98, "PARENTCD", "CHAR(14)", "親商品コード"),
    /** 便区分 */
    ...
    /** ITFコード */
    ITFCD(120, "ITFCD", "CHAR(14)", "ITFコード"),
    /** センター入数 */
