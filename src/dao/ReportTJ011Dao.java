 ...
 *
 */
public class ReportTJ011Dao extends ItemDao {
   ...
   * @param source
   */
  public ReportTJ011Dao(String JNDIname) {
    super(JNDIname);
  }
  ...
  private String createCommand() {
    ...
    JSONArray array2 = getTOKMOYCDData2(getMap());
    ...
    sbSQL.append("  when IDX = 3 then CAST(WK.F97 AS CHAR)"); // 3行目 F2 便
    ...
    sbSQL.append("  when IDX = 2 then CAST(WK.F17 AS CHAR)"); // 2行目 F3 入数
    sbSQL.append("  when IDX = 3 then CAST(WK.F104 AS CHAR)"); // 3行目 F3 ODS
    sbSQL.append("  when IDX = 4 then null"); // 4行目 F3
    sbSQL.append("  end as F3");
    sbSQL.append(", case");
    sbSQL.append("  when IDX = 1 then null"); // 1行目 F4
    sbSQL.append("  when IDX = 2 then CAST(WK.F18 AS CHAR)"); // 2行目 F4 特売原価
    sbSQL.append("  when IDX = 3 then CAST(WK.F52 AS CHAR)"); // 3行目 F4 追加原価
    sbSQL.append("  when IDX = 4 then CAST(WK.F101 AS CHAR)"); // 4行目 F4 標準原価
    sbSQL.append("  end as F4");
    sbSQL.append(", case");
    sbSQL.append("  when IDX = 1 then null"); // 1行目 F5
    sbSQL.append("  when IDX = 2 then CAST(WK.F19 AS CHAR)"); // 2行目 F5 特売総売
    sbSQL.append("  when IDX = 3 then CAST(WK.F53 AS CHAR)"); // 3行目 F5 特売本売
    sbSQL.append("  when IDX = 4 then CAST(WK.F102 AS CHAR)"); // 4行目 F5 標準売価
    sbSQL.append("  end as F5");
    // sbSQL.append(", case");
    // sbSQL.append(" when IDX = 1 then null"); // 1行目 F6 納品形態
    // sbSQL.append(" when IDX = 2 then null"); // 2行目 F6 特売総売
    // sbSQL.append(" when IDX = 3 then null"); // 3行目 F6 特売本売
    // sbSQL.append(" when IDX = 4 then null"); // 4行目 F6 パック売価
    ...
    sbSQL.append(" (select * from (values row(1),row(2),row(3),row(4)) as X(IDX) order by IDX) AS T1 ");
