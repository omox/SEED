...
public class ReportTJ011Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append("  when IDX = 3 then TO_CHAR(WK.F97)");	// 3行目 F2		便
		sbSQL.append("  when IDX = 4 then TO_CHAR(WK.F66)");	// 4行目 F2		コメント
		sbSQL.append("  end as F2");
		sbSQL.append(", case");
		sbSQL.append("  when IDX = 1 then null");				// 1行目 F3
		sbSQL.append("  when IDX = 2 then TO_CHAR(WK.F17)");	// 2行目 F3		入数
		sbSQL.append("  when IDX = 3 then TO_CHAR(WK.F104)");	// 3行目 F3		ODS
		sbSQL.append("  when IDX = 4 then null");				// 4行目 F3
		sbSQL.append("  end as F3");
		sbSQL.append(", case");
		sbSQL.append("  when IDX = 1 then null");				// 1行目 F4
		sbSQL.append("  when IDX = 2 then TO_CHAR(WK.F18)");	// 2行目 F4		特売原価
		sbSQL.append("  when IDX = 3 then TO_CHAR(WK.F52)");	// 3行目 F4		追加原価
		sbSQL.append("  when IDX = 4 then TO_CHAR(WK.F101)");	// 4行目 F4		標準原価
		sbSQL.append("  end as F4");
		sbSQL.append(", case");
		sbSQL.append("  when IDX = 1 then null");				// 1行目 F5
		sbSQL.append("  when IDX = 2 then TO_CHAR(WK.F19)");	// 2行目 F5		特売総売
		sbSQL.append("  when IDX = 3 then TO_CHAR(WK.F53)");	// 3行目 F5		特売本売
		sbSQL.append("  when IDX = 4 then TO_CHAR(WK.F102)");	// 4行目 F5		標準売価
