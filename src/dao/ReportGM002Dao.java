 ...
 *
 */
public class ReportGM002Dao extends ItemDao {
   ...
   * @param source
   */
  public ReportGM002Dao(String JNDIname) {
    super(JNDIname);
  }
  ...
  private String createCommand() {
    ...
    sbSQL.append(",DATE_FORMAT(MY.ADDDT,'%Y%m%d') AS ADDDT "); // F7 : 登録日
    sbSQL.append(",DATE_FORMAT(MY.UPDDT,'%Y%m%d') AS UPDDT "); // F8 : 更新日
    sbSQL.append(",DATE_FORMAT(MY.UPDDT,'%Y%m%d%H%i%s%f') as HDN_UPDDT "); // F9 : 更新日時
    sbSQL.append("FROM ");
    sbSQL.append("INATK.TOKMOYCD AS MY LEFT JOIN INATK.TOKMM_KKK AS BM ON ");
