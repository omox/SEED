...
public class ReportTJ003Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append("  select");
		sbSQL.append(" TO_CHAR(TO_DATE(T3.JTDT_01, 'YYYYMMDD'), 'MM/DD') as F1");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_02, 'YYYYMMDD'), 'MM/DD') as F2");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_03, 'YYYYMMDD'), 'MM/DD') as F3");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_04, 'YYYYMMDD'), 'MM/DD') as F4");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_05, 'YYYYMMDD'), 'MM/DD') as F5");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_06, 'YYYYMMDD'), 'MM/DD') as F6");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_07, 'YYYYMMDD'), 'MM/DD') as F7");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_08, 'YYYYMMDD'), 'MM/DD') as F8");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_09, 'YYYYMMDD'), 'MM/DD') as F9");
		sbSQL.append(" ,TO_CHAR(TO_DATE(T3.JTDT_10, 'YYYYMMDD'), 'MM/DD') as F10");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_01, 'YYYYMMDD'))) as F11");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_02, 'YYYYMMDD'))) as F12");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_03, 'YYYYMMDD'))) as F13");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_04, 'YYYYMMDD'))) as F14");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_05, 'YYYYMMDD'))) as F15");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_06, 'YYYYMMDD'))) as F16");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_07, 'YYYYMMDD'))) as F17");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_08, 'YYYYMMDD'))) as F18");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_09, 'YYYYMMDD'))) as F19");
		sbSQL.append(" ,(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_10, 'YYYYMMDD'))) as F20");
		sbSQL.append(" ,T3.JTDT_01 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_01, 'YYYYMMDD'))) as X1");
		sbSQL.append(" ,T3.JTDT_02 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_02, 'YYYYMMDD'))) as X2");
		sbSQL.append(" ,T3.JTDT_03 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_03, 'YYYYMMDD'))) as X3");
		sbSQL.append(" ,T3.JTDT_04 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_04, 'YYYYMMDD'))) as X4");
		sbSQL.append(" ,T3.JTDT_05 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_05, 'YYYYMMDD'))) as X5");
		sbSQL.append(" ,T3.JTDT_06 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_06, 'YYYYMMDD'))) as X6");
		sbSQL.append(" ,T3.JTDT_07 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_07, 'YYYYMMDD'))) as X7");
		sbSQL.append(" ,T3.JTDT_08 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_08, 'YYYYMMDD'))) as X8");
		sbSQL.append(" ,T3.JTDT_09 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_09, 'YYYYMMDD'))) as X9");
		sbSQL.append(" ,T3.JTDT_10 || '-' || (select CWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T3.JTDT_10, 'YYYYMMDD'))) as X10");
	...
	private String createCommand2() {
		...
		for (int i = 0; i < wkArray.size(); i++) {
			...
			HashMap<Integer,BigDecimal> kikanKei = new HashMap<Integer,BigDecimal>();
			// 10日分の発注売価、売上予算、荒利を kye:ソート番号-日付、value:金額 で保持
			HashMap<String,BigDecimal> bmnKei	= new HashMap<String,BigDecimal>();
	...
	public JSONArray getCMPRTData(HashMap<String, String> map) {
		...
		// 全ての情報を集約
		String values = "(values(null,null,null))";
		...
		sbSQL.append(" from");
		sbSQL.append("(select DAICD, TJDT, URICMPRT from " + values + " as ");
