 ...
 *
 */
public class Reportx215Dao extends ItemDao {
   ...
   * @param source
   */
  public Reportx215Dao(String JNDIname) {
    super(JNDIname);
  }
  ...
  public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
    ...
    /*
     * // 入力値の重複チェック for (int i=0; i<dataArrayG.size(); i++){ String val =
     * dataArrayG.optJSONObject(i).optString("F2"); if(StringUtils.isNotEmpty(val)){
     * tencds.add(dataArrayG.optJSONObject(i)); tencds_.add(val); } }
     *
     * if(tencds.size() != tencds_.size()){ msg.add(mu.getDbMessageObj("E11141", new String[]{}));
     * return msg; }
     */
    ...
    for (int i = 0; i < dataArrayG.size(); i++) {
      ...
      // 店コードに入力がある場合、構成ページ・枚数は必須
      if (StringUtils.isEmpty(data.optString("F2")) || StringUtils.isEmpty(data.optString("F4")) || StringUtils.isEmpty(data.optString("F5"))) {
        msg.add(mu.getDbMessageObj("E11049", new String[] {"店コードと構成ページと枚数を"}));
        return msg;
      }

      // 枚数・構成ページのみに入力が存在する場合
      if (StringUtils.isEmpty(data.optString("F2")) && (!StringUtils.isEmpty(data.optString("F4")) || !StringUtils.isEmpty(data.optString("F5")))) {
  ...
  public String createSqlPc(JSONObject data, JSONArray dataArrayG, User userInfo) {
    ...
    if (StringUtils.isEmpty(data.optString("F4"))) {
      ...
      sbSQL.append("," + DefineReport.MaisuHohokbn.TENMAISU.getVal());
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (valueData.length >= 100 || (i + 1 == len && valueData.length > 0)) {
        ...
        sbSQL.append(", KOSEPAGE = VALUES(KOSEPAGE) "); // 構成ページ：
