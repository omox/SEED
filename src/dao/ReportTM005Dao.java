...
public class ReportTM005Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" with T1(IDX) as (select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < "+idx+") ");
		sbSQL.append(" select");
		sbSQL.append("  T1.IDX");
	...
	private String createSqlTOKMOYDEF(int userId, JSONArray dataArray, String sysdate) {
		...
		sbSQL.append("merge into INATK.TOKMOYDEF as T using (select");
		...
		sbSQL.append(", current timestamp AS UPDDT");													// F17 : 更新日
		...
		sbSQL.append(", SZKCD = RE.SZKCD");											// F2 : 所属コード
		sbSQL.append(", HBSLIDEKBN = RE.HBSLIDEKBN");								// F3 : 1遅スライド_販売
		sbSQL.append(", NNSLIDEKBN = RE.NNSLIDEKBN");								// F4 : 1遅スライド_納品
		sbSQL.append(", KSGRPKBN = RE.KSGRPKBN");									// F5 : 検証の括り
		sbSQL.append(", DSUEXKBN = RE.DSUEXKBN");									// F6 : デフォルト_数展開
		sbSQL.append(", DRTEXKBN = RE.DRTEXKBN");									// F7 : デフォルト_実績率パタン数値
		sbSQL.append(", DZNENDSKBN = RE.DZNENDSKBN");								// F8 : デフォルト_前年同週
		sbSQL.append(", DDNENDSKBN = RE.DDNENDSKBN");								// F9 : デフォルト_同年同月
		sbSQL.append(", DCUTEXKBN = RE.DCUTEXKBN");									// F10 : デフォルト_カット店展開
		sbSQL.append(", DCHIRASKBN = RE.DCHIRASKBN");								// F11 : デフォルト_ちらしのみ
		sbSQL.append(", DBMNATRKBN = RE.DBMNATRKBN");								// F12 : デフォルト_部門属性
		sbSQL.append(", UPDKBN = RE.UPDKBN");										// F13 : 更新区分：
		sbSQL.append(", SENDFLG = RE.SENDFLG");										// F14 : 送信フラグ：
		sbSQL.append(", OPERATOR = RE.OPERATOR");									// F15 : オペレータ
		sbSQL.append(", ADDDT = case when NVL(UPDKBN, 0) = 1 then RE.ADDDT else ADDDT end"); // F16 : 登録日
		sbSQL.append(", UPDDT = RE.UPDDT");											// F17 : 更新日
		...
		sbSQL.append(", RE.SZKCD");													// F2 : 所属コード
		sbSQL.append(", RE.HBSLIDEKBN");											// F3 : 1遅スライド_販売
		sbSQL.append(", RE.NNSLIDEKBN");											// F4 : 1遅スライド_納品
		sbSQL.append(", RE.KSGRPKBN");												// F5 : 検証の括り
		sbSQL.append(", RE.DSUEXKBN");												// F6 : デフォルト_数展開
		sbSQL.append(", RE.DRTEXKBN");												// F7 : デフォルト_実績率パタン数値
		sbSQL.append(", RE.DZNENDSKBN");											// F8 : デフォルト_前年同週
		sbSQL.append(", RE.DDNENDSKBN");											// F9 : デフォルト_同年同月
		sbSQL.append(", RE.DCUTEXKBN");												// F10 : デフォルト_カット店展開
		sbSQL.append(", RE.DCHIRASKBN");											// F11 : デフォルト_ちらしのみ
		sbSQL.append(", RE.DBMNATRKBN");											// F12 : デフォルト_部門属性
		sbSQL.append(", RE.UPDKBN");												// F13 : 更新区分：
		sbSQL.append(", RE.SENDFLG");												// F14 : 送信フラグ：
		sbSQL.append(", RE.OPERATOR");												// F15 : オペレータ
		sbSQL.append(", RE.ADDDT");													// F16 : 登録日
		sbSQL.append(", RE.UPDDT");													// F17 : 更新日
		sbSQL.append(")");
