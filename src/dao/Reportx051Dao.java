...
public class Reportx051Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" from (select * from INAMS.MSTMAKER where NVL(UPDKBN, 0) = 0) MAK1");
		sbSQL.append(" left join (select * from INAMS.MSTMAKER where NVL(UPDKBN, 0) = 0) MAK2 on MAK2.DMAKERCD = MAK1.MAKERCD");
		sbSQL.append(" where NVL(MAK1.UPDKBN, 0) = 0");
		...
		for (int j = 0; j < dataArray.size(); j++) {
			...
			if(!isErr){
				if(StringUtils.equals("1", data.optString("TRUEUPDKBN"))){
					...
					sbSQL.append("select MAKERCD from INAMS.MSTMAKER where MAKERCD = ? and NVL(UPDKBN, 0) <> 1 fetch first 1 rows only");
					...
					prmData.add(data.optString("F1"));
					dbDatas = iL.selectJSONArray(sbSQL.toString(), prmData, Defines.STR_JNDI_DS);
					if (DefineReport.ID_DEBUG_MODE) System.out.println("既存データチェック:メーカーマスタ="+dbDatas.size());
					if(dbDatas.size() == 0 ){

						if (isduplication) {
							for (int j2 = 0; j2 < dataArrayAf.size(); j2++) {
								String check2 = dataArray.optJSONObject(j2).optString("F1");
					...
					sbSQL = new StringBuffer();
					prmData	 = new ArrayList<String>();
					sbSQL.append("select DMAKERCD from INAMS.MSTMAKER where DMAKERCD = ? and NVL(UPDKBN, 0) <> 1 fetch first 1 rows only");
					prmData.add(data.optString("F1"));
					dbDatas = iL.selectJSONArray(sbSQL.toString(), prmData, Defines.STR_JNDI_DS);
					if(dbDatas.size() > 0 ){
						isErr = true;
...
//				}
	...
	public enum FullFileLayout {
		/** 更新区分 */
		RECKBN(1,"レコード区分",RefTable.OTHER,"RECKBN"),
		DATAKIND(2,"データ種別",RefTable.OTHER,"DATAKIND"),
		UPDKBN(3,"更新区分",RefTable.MAKER,"UPDKBN"),
		HYOJUNTANSHUKUKBN(4,"標準/短縮識別区分",RefTable.OTHER,"HYOJUNTANSHUKUKBN"),
		MAKERCD(5,"標準メーカコード",RefTable.MAKER,"MAKERCD"),
		DMAKERCD(6,"代表メーカコード",RefTable.MAKER,"DMAKERCD"),
		MAKERKN(7,"会社名",RefTable.MAKER,"MAKERKN"),
		MAKERAN(8,"会社名カナ",RefTable.MAKER,"MAKERAN"),
		HOUJINKEITAIICHI(9,"法人形態（位置）",RefTable.MAKER,"HOUJINKEITAIICHI"),
		HOUJINKEITAIMOJI(10,"法人形態（文字）",RefTable.MAKER,"HOUJINKEITAIMOJI"),
		JUSYO1(11,"住所１",RefTable.MAKER,"JUSYO1"),
		ADDDT(12,"登録年月日",RefTable.OTHER,"ADDDT"),
		UPDDT(13,"内容変更年月日",RefTable.OTHER,"UPDDT"),
		DELDT(14,"廃止年月日",RefTable.MAKER,"DELDT");
	...
	public enum ShortFileLayout {
		...
		MAKERCD(5,"標準メーカコード",RefTable.MAKER,"MAKERCD"),
		DMAKERCD(6,"代表メーカコード",RefTable.MAKER,"DMAKERCD"),
		ADDDT(7,"登録年月日",RefTable.OTHER,"ADDDT"),
		UPDDT(8,"内容変更年月日",RefTable.OTHER,"UPDDT"),
		DELDT(9,"廃止年月日",RefTable.MAKER,"DELDT");
