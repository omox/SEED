...
public class Reportx003Dao extends ItemDao {
	...
	private String createCommand() {
		...
		String rows = "";
		String szCodes = "";
		String mark = "'"; //DefineReport.RadCode.ID3.getVal().equals(szRadCode) ? "":"'";
		for(int j=1; j <= 8; j++){
		...
		if(DefineReport.RadCode.ID1.getVal().equals(szRadCode)){
			...
			sbSQL.append(" where X.CD = T1.URICD and nvl(T1.UPDKBN, 0) <> 1" + szWhereUriCd);
		}
		...
		sbSQL.append("  ,case IDX when 1 then " + DefineReport.ID_SQL_MD03111301_COL_RG + " "
				+"when 2 then " + DefineReport.ID_SQL_MD03111301_COL_HS + " end");				// 総額売価
		...
		sbSQL.append("  ,case IDX when 1 then right('0'||nvl(T1.BMNCD,0),2)||right('0'||nvl(T1.DAICD,0),2)||right('0'||nvl(T1.CHUCD,0),2)||right('0'||nvl(T1.SHOCD,0),2) end");	// 分類コード
		sbSQL.append("  ,case IDX when 1 then nvl(TO_CHAR(T1.UPDDT, 'YY/MM/DD'),'__/__/__') end as UPDDT ");	// 更新日
		sbSQL.append(" from INAMS.MSTSHN T1");
		sbSQL.append(" inner join INP T0 on T1.SHNCD = T0.SHNCD and nvl(T1.UPDKBN, 0) <> 1");
		sbSQL.append(" inner join (values(1, 'レギュラー'),(2, '販促')) as T2(IDX, KTXT) on 1=1");
