...
public class ReportBW005Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(" select");
		sbSQL.append("  TO_CHAR(TO_DATE(TRKK.HBSTDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(TRKK.HBSTDT, 'YYYYMMDD')))");					// F1	：販売開始日
	...
	public String createCommandFTP(String userId, JSONObject obj, String outpage) {
		...
		sbSQL.append(" from INATK.TOKRS_KKK T1 ");
		sbSQL.append(" inner join INATK.TOKRS_SHN T2 on NVL(T1.UPDKBN, 0) <> 1 and NVL(T2.UPDKBN, 0) <> 1 and T1.HBSTDT = T2.HBSTDT and T1.BMNCD = T2.BMNCD and T1.WRITUKBN = T2.WRITUKBN and T1.SEICUTKBN = T2.SEICUTKBN and T1.DUMMYCD = T2.DUMMYCD");
