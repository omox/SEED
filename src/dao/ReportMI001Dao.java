...
public class ReportMI001Dao extends ItemDao {
	...
	private String createCommand() {
		...
		if (!StringUtils.isEmpty(szShncd)) {
			sqlWhere += ", (select SRCCD from (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SRCCD from INAMS.MSTSRCCD T2 where T2.SHNCD = ?) where ROWID=1) as SRCCD1";
			paramData.add(szShncd);
			sqlWhere += ", (select SRCCD from (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SRCCD from INAMS.MSTSRCCD T2 where T2.SHNCD = ?) where ROWID=2) as SRCCD2";
			paramData.add(szShncd);
			...
			sqlWhere += " where T2.SHNCD = (select SHNCD from (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SHNCD from INAMS.MSTSRCCD T2 where T2.SRCCD = ? ) where ROWID=1)";
			paramData.add(szSrccd);
			sqlWhere += " )where ROWID=1) as SRCCD1";
			sqlWhere += ",(select SRCCD from";
			sqlWhere += " (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SRCCD from INAMS.MSTSRCCD T2";
			sqlWhere += " where T2.SHNCD = (select SHNCD from (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SHNCD from INAMS.MSTSRCCD T2 where T2.SRCCD = ? ) where ROWID=1)";
			paramData.add(szSrccd);
			sqlWhere += " )where ROWID=2) as SRCCD2";
			sqlWhere2 += " where T1.SHNCD = (select SHNCD from (select ROW_NUMBER() OVER (ORDER BY T2.SEQNO) as ROWID, T2.SHNCD from INAMS.MSTSRCCD T2 where T2.SRCCD = ? ) where ROWID=1) and T1.UPDKBN = 0 ";
			paramData.add(szSrccd);
		}
		// タイトル情報(任意)設定
		...
		sbSQL.append(", (select T3.AVGPTANKAAM from INAMS.MSTAVGPTANKA T3 where T3.SHNCD=T1.SHNCD)");
		sbSQL.append(", T1.HS_GENKAAM");
		sbSQL.append(", T1.HS_BAIKAAM");
		sbSQL.append(", T1.HS_IRISU");
		sbSQL.append(", (select READTM_MON from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_TUE from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_WED from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_THU from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_FRI from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_SAT from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
		sbSQL.append(", (select READTM_SUN from INAMS.MSTREADTM T4 where T4.READTMPTN=T1.READTMPTN)");
