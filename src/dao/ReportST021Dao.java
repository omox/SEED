...
public class ReportST021Dao extends ItemDao {
	...
	public String createSqlSelTOKNNDTSplit(JSONObject obj) {
		...
		sbSQL.append(" select 0, 1 from SYSIBM.SYSDUMMY1");
		...
		sbSQL.append(" ,case when trim(T2.RNK) = '' then null else int(trim(T2.RNK)) end as RNK");
	...
	public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
		...
		String szBmncd		= dataArray.optJSONObject(0).optString(TOKTG_NNDTLayout.BMNCD.getId());		// 部門コード
		String szKanrino	= dataArray.optJSONObject(0).optString(TOKTG_NNDTLayout.KANRINO.getId());	// 管理No.
		String szKanrieno	= dataArray.optJSONObject(0).optString(TOKTG_NNDTLayout.KANRIENO.getId());	// 管理No.枝番
		...
		for (int i = 0; i < dataArray.size(); i++) {
			...
			for(dao.ReportTG016Dao.MSTLayout colinf: layouts){
				if(ArrayUtils.contains(notTarget, colinf.getId())){ continue;}	// チェック不要
				if(!ArrayUtils.contains(target, colinf.getId())){ continue;}	// チェック不要
			...
			if (!isTOKTG) {
				...
				if (dbDatas.size() >= 1){
					...
					if (!szMoyskbn.equals(dbDatas.getJSONObject(0).optString("MOYSKBN")) ||
							...
							!szKanrino.equals(dbDatas.getJSONObject(0).optString("KANRINO"))
					) {
						// 同一納入日の重複チェックエラー
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(isTOKTG){
			...
			if(dataArrayNNDT.size() > 0){
				JSONObject result4 = this.createSqlTOK_NNDT(userId, data, SqlType.UPD, isTOKTG, dao);
			}
		...
		// 3.14.3.8．DB更新処理（全特アンケート無）
			...
			if (array.size() != 0) {
				...
				JSONObject result7 = createSqlTOKSP_SHNNNDT2(userId, array, data, SqlType.MRG,table, dao);
			}
		}
		...
		if(StringUtils.isEmpty(getMessage())){
			...
			for (int i = 0; i < countList.size(); i++) {
				...
				if (DefineReport.ID_DEBUG_MODE) System.out.println(MessageUtility.getMessage(Msg.S00006.getVal(), new String[]{dao.lblList.get(i), Integer.toString(countList.get(i))}));
			}
			if(count==0){
	...
	protected JSONObject createSqlTOK_NNDT(String userId, JSONObject data, SqlType sql, boolean isTOKTG, ReportTG016Dao dao) {
		...
		for(dao.ReportTG016Dao.MSTLayout itm :layouts){
			if(!ArrayUtils.contains(target, itm.getId())){ continue;}	// パラメータ不要
