...
public class ReportBW002Dao extends ItemDao {
	...
	private String createCommand() {
		...
		String szWaribiki		= getMap().get("WARIBIKI");	// 店コード
		String szSeisi	= getMap().get("SEISI");	// ボタンID
		String szSendbtnid	= getMap().get("SENDBTNID");	// ボタンID
		...
		sbSQL.append(", T1(IDX) as ( select 1 from SYSIBM.SYSDUMMY1 union all select IDX + 1 from T1 where IDX < 500) ");
		sbSQL.append(" select");
		sbSQL.append("  T1.IDX");
		...
		sbSQL.append(", TO_CHAR(TRSH.ADDDT, 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(TO_CHAR(TRSH.ADDDT, 'YYYYMMDD'),'YYYYMMDD'))) as F13");			// F13	：登録日
		sbSQL.append(", TO_CHAR(TRSH.UPDDT, 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(TO_CHAR(TRSH.UPDDT, 'YYYYMMDD'),'YYYYMMDD'))) as F14");			// F14	：更新日
		sbSQL.append(", TRSH.OPERATOR  as F15");								// F15	：オペレーター
		sbSQL.append(", NVL(TRSH.UPDKBN,0) as F16");						// F16	：更新区分
		...
		sbSQL.append(" left outer join INAMS.MSTSHN M0 on M0.SHNCD = TRSH.SHNCD and NVL(M0.UPDKBN, 0) <> 1");
		sbSQL.append(" left outer join INAMS.MSTBMN M1 on M1.BMNCD = TRSH.BMNCD and NVL(M1.UPDKBN, 0) <> 1");
	...
	public String createSqlSelMSTSHN(JSONObject obj) {
		...
		StringBuffer sbSQL = new StringBuffer();
		sbSQL.append(" select ");
		sbSQL.append(DefineReport.ID_SQL_TOKBAIKA_COL_HON.replaceAll("@BAIKA", obj.optString("value")).replaceAll("@DT", "TRKK.HBSTDT")+" as F11 ");
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		sbSQL.append(DefineReport.ID_SQL_CMN_WEEK);
		sbSQL.append("  select");
		sbSQL.append("   TO_CHAR(TO_DATE(CTRK.HBSTDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(CTRK.HBSTDT, 'YYYYMMDD'))) as F1");
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(!shncd2.equals("")){
			...
			sbSQL.append(" and NVL(TRSH.UPDKBN, 0) <> 1)");
			...
			if(array.size()>0){
				...
				if(New){
					...
					sbSQL.append(" T.SEICUTKBN = RE.SEICUTKBN and ");
					sbSQL.append(" T.DUMMYCD = RE.DUMMYCD and ");
					sbSQL.append(" T.SUMI_KANRINO = RE.SUMI_KANRINO ");
					sbSQL.append(" ) WHEN NOT MATCHED THEN INSERT ( ");
					sbSQL.append(" HBSTDT");			//販売開始日
					sbSQL.append(",BMNCD");				//部門
					sbSQL.append(",WRITUKBN");			//割引率区分
				...
				if(StringUtils.isEmpty(getMessage())) {
					if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品_内部管理を " + MessageUtility.getMessage(Msg.S00003.getVal(),
							new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
					msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00001.getVal()));
				}else{
					msgObj.put(MsgKey.E.getKey(), getMessage());
		...
		if(StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品_企画を " + MessageUtility.getMessage(Msg.S00004.getVal(),
					new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo) throws Exception {
		...
		sbSQL.append(", ADDDT = current timestamp  ");								// 登録日：
		...
		int count = super.executeSQL(sbSQL.toString(), prmData);
		if (StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("冷凍食品企画マスタを "
