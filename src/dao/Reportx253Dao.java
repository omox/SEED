 ...
 *
 */
public class Reportx253Dao extends ItemDao {

	/**
	 * インスタンスを生成します。
	 * @param source
	 */
	public Reportx253Dao(String JNDIname) {
		super(JNDIname);
	}
...
//		sbSQL.append(" ,case when T6.SHNCD is not null and nvl(T7.CNT,0) = 1 then 1 else 0 end");	// 予約商品更新可
		...
		sbSQL.append(" ,T1.ERRFLD");
		sbSQL.append(" ,T1.ERRCD");
		sbSQL.append(" ,T1.ERRVL");
		sbSQL.append(" from INAWS.CSVSHNHEAD T0");
		sbSQL.append(" inner join INAWS.CSVSHN T1 on T1.SEQ = T0.SEQ and T0.SEQ = '" + szSeq + "' and nvl(T1.UPDKBN, 0) <> 1");
		sbSQL.append(" left outer join INAWS.CSVSRCCD T3 on T3.SEQ = T0.SEQ and T1.INPUTNO=T3.INPUTNO and T1.SHNCD = T3.SHNCD and T3.SEQNO = 1");
		sbSQL.append(" "+DefineReport.ID_SQL_MD03111301_JOIN+"");
		sbSQL.append(" left outer join INAWS.PIMTIT T5 on T1.SHNCD = T5.SHNCD and nvl(T5.UPDKBN, 0) <> 1");
...
//		sbSQL.append(" left outer join (select T7.SHNCD,count(T7.SHNCD) over(partition by T7.SHNCD) as CNT from INAMS.MSTSHN_Y T7 where NVL(T7.UPDKBN, 0) <> 1 group by T7.SHNCD) T7 on T1.SHNCD = T7.SHNCD");
	...
	public JSONArray getBaseData(HashMap<String, String> map) {
		...
		sbSQL.append(" , TO_CHAR(T1.INPUT_DATE, 'YYMMDD') as F3");
		sbSQL.append(" , TO_CHAR(T1.INPUT_DATE, 'HH24MISS') as F4");
		sbSQL.append(" , trim(COMMENTKN) as F5");
		sbSQL.append(" , TO_CHAR(T1.INPUT_DATE, 'YYYYMMDDHH24MISSNNNNNN') as F6");
		sbSQL.append(" from INAWS.CSVSHNHEAD T1");
