...
public class Reportx092Dao extends ItemDao {
	...
	/*private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		String values = "";
		int kryoColNum = 13;		// テーブル列数
		int futaiColNum = 7;		// テーブル列数
		// パラメータ確認
		...
		if (msg.size() > 0) {
			...
			if(StringUtils.isEmpty(getMessage())) {
				System.out.println("計量マスタを " + MessageUtility.getMessage(Msg.S00003.getVal(),
		...
		if(DefineReport.Button.SEL_CHANGE.getObj().equals(sendBtnid)){
			...
			ArrayList<String> prmDelData = new ArrayList<String>();
			delsql.append("delete from INAMS.MSTUGENRYO where BMNCD = ? and CALLCD = ?");
			prmDelData.add(bmoncd);
			prmDelData.add(callcd);
			int count = super.executeSQL(delsql.toString(), prmDelData);
			if(StringUtils.isEmpty(getMessage())){
				System.out.println("使用原料テーブルを "+MessageUtility.getMessage(Msg.S00004.getVal(), new String[]{Integer.toString(count)}));
			}else{
				msgObj.put(MsgKey.E.getKey(), getMessage());
			}
		}
		...
		if (dataArray2.size()>0) {
			for (int j = 0; j < dataArray2.size(); j++) {
				for (int i = 1; i <= futaiColNum; i++) {
					String col = "F" + i;
					String val = dataArray2.optJSONObject(j).optString(col);
			...
			sbSQL2.append("merge into INAMS.MSTUGENRYO as T using (select");
			sbSQL2.append("   SHNCD");// F1 : 商品コード
			sbSQL2.append(" , CALLCD");// F2 : 呼出コード
			sbSQL2.append(" , BMNCD");// F3 : 部門コード
			sbSQL2.append(" , NAIKN");// F4 : 内容量
			sbSQL2.append(" , GENKA");// F5 : 原料原価
			sbSQL2.append(" , BUDOMARI");// F6 : 歩留り
			sbSQL2.append(" , GENKAKEI");// F7 : 原価小計
			sbSQL2.append(" , 0 as SENDFLG");// F8:送信フラグ
			sbSQL2.append(" , 0 as UPDKBN");// F8:送信フラグ
			sbSQL2.append(" , '"+userId+"' AS OPERATOR ");// F9:オペレーター：
			sbSQL2.append(" , current timestamp AS ADDDT ");// F10:登録日：
			sbSQL2.append(" , current timestamp AS UPDDT ");// F11:更新日：
			sbSQL2.append(" from (values " + values + " ) as T1(");
			sbSQL2.append("   SHNCD");// F1 : 部門
			sbSQL2.append(" , CALLCD");// F2 : 呼出コード
			sbSQL2.append(" , BMNCD");// F3 : 風袋種別
			sbSQL2.append(" , NAIKN");// F3 : 風袋種別
			sbSQL2.append(" , GENKA");// F3 : 風袋種別
			sbSQL2.append(" , BUDOMARI");// F3 : 風袋種別
			sbSQL2.append(" , GENKAKEI)) as RE on (T.SHNCD = RE.SHNCD and T.CALLCD = RE.CALLCD and T.BMNCD = RE.BMNCD)");// F8:// 更新日
			sbSQL2.append(" when not matched then insert values (");
			sbSQL2.append("  RE.BMNCD");// F1 : 商品コード
			sbSQL2.append(" ,RE.CALLCD");// F2 : 呼出コード
			sbSQL2.append(" ,RE.SHNCD");// F3 : 部門コード
			sbSQL2.append(" ,RE.NAIKN");// F4 : 内容量
			sbSQL2.append(" ,RE.GENKA");// F5 : 原料原価
			sbSQL2.append(" ,RE.BUDOMARI");// F6 : 歩留り
			sbSQL2.append(" ,RE.GENKAKEI");// F7 : 原価小計
			sbSQL2.append(" ,RE.SENDFLG");// F8: 送信フラグ
			sbSQL2.append(" ,RE.UPDKBN");// F8: 送信フラグ
			sbSQL2.append(" ,RE.OPERATOR");// F9: オペレータ
			sbSQL2.append(" ,RE.ADDDT");// F10: 登録日
			sbSQL2.append(" ,RE.UPDDT )");// F11: 更新日
			...
			if (msg.size() > 0) {
				...
				// 更新処理実行
				System.out.println(sbSQL2);
				int count = super.executeSQL(sbSQL2.toString(), prmData);
				if (StringUtils.isEmpty(getMessage())) {
					System.out.println("使用原料マスタを " + MessageUtility.getMessage(Msg.S00003.getVal(),
							new String[] { Integer.toString(dataArray.size()), Integer.toString(count) }));
					msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00001.getVal()));
				} else {
					msgObj.put(MsgKey.E.getKey(), getMessage());
	...
	private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
		...
		if(dataArray.size() > 0){
			...
			targetWhere = "NVL(UPDKBN, 0) <> "+DefineReport.ValUpdkbn.DEL.getVal()+" and BMNCD = ? and CALLCD = ? and SHNCD = ?  ";
	...
	private JSONObject deleteData(HashMap<String, String> map, User userInfo) throws Exception {
		...
		int count = super.executeSQL(sbSQL.toString(), prmData);
		if (StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("値付器マスタを "
		...
		if (StringUtils.isEmpty(getMessage())) {
			if (DefineReport.ID_DEBUG_MODE) System.out.println("使用原料マスタを "
					+ MessageUtility.getMessage(Msg.S00004.getVal(), new String[] { Integer.toString(count) }));
			msgObj.put(MsgKey.S.getKey(), MessageUtility.getMessage(Msg.S00002.getVal()));
		} else {
			msgObj.put(MsgKey.E.getKey(), getMessage());
