...
public class ReportHT007Dao extends ItemDao {
  ...
  public String createSqlTenSu(HashMap<String, String> map, User userInfo) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (!dataT.isEmpty()) {
        for (int k = 1; k <= maxField; k++) {
          ...
          if (k == maxField) {
            ...
            values += "FROM INATK.HATJTR_TEN WHERE SHNCD = " + dataArrayT.optJSONObject(i).optString("F1") + " AND BINKBN = " + dataArrayT.optJSONObject(i).optString("F3") + " AND TENCD = " + ten
                + " AND SHUNO = " + shuno + ") T" + i + "    )";
      ...
      if (valueData.length >= 100 || (i + 1 == len && valueData.length > 0)) {
        sbSQL = new StringBuffer();
        sbSQL.append("REPLACE INTO INATK.HATJTR_TEN  ( ");
  ...
  private String createCommand() {
    ...
    if (!szIndex.equals("2")) {
      sbSQL.append(",ROW_NUMBER() OVER(ORDER BY TN.SHNCD, TN.BINKBN, DATE_FORMAT(TN.UPDDT, '%Y%m%d%H%i%s%f')) "); // F22 : 行番号
    }
    sbSQL.append(",DATE_FORMAT(SN.UPDDT,'%Y%m%d%H%i%s%f') as HDN_UPDDT2 "); // F23 : 更新日時
