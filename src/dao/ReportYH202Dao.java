...
public class ReportYH202Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(", SUM(NVL(TEN.HTSU_Z, 0)) as F2");
		sbSQL.append(", SUM(NVL(TEN.HTSU_T, 0)) as F3");
		sbSQL.append(", SUM(NNDT.YOTEISU) as F4");
		sbSQL.append(", SUM(NNDT.GENDOSU) as F5");
		sbSQL.append(", TO_CHAR(TO_DATE('20' || right ('0' || NNDT.NNDT, 6), 'YYYYMMDD'), 'YY/MM/DD') as UKESTDT");
		sbSQL.append(", DAYOFWEEK(TO_DATE('20' || right ('0' || NNDT.NNDT, 6), 'YYYYMMDD')) as UKESTDT_WNUM");
		sbSQL.append(" from INATK.HATYH_NNDT NNDT");
		sbSQL.append(" left join (select");
		sbSQL.append(" KKKCD");
		sbSQL.append(", SHNCD");
		sbSQL.append(", NNDT");
		sbSQL.append(", SUM(case when NVL(INPUTDT, 0) < "+szShoridt+" then NVL(HTSU, 0) else 0 end) as HTSU_Z");
		sbSQL.append(", SUM(case when NVL(INPUTDT, 0) = "+szShoridt+" then NVL(HTSU, 0) else 0 end) as HTSU_T");
		...
		sbSQL.append(" group by rollup (NNDT.KKKCD, NNDT.SHNCD, NNDT.NNDT) having NNDT.NNDT is not null or NNDT.KKKCD is null order by NNDT.KKKCD, NNDT.SHNCD, NNDT.NNDT) T1");
	...
	public JSONArray getTOKMOYCDData(HashMap<String, String> map) {
		...
		StringBuffer sbSQL = new StringBuffer();
		sbSQL.append(" select");
		sbSQL.append(" TO_CHAR(HKKK.KKKCD) || ' - ' || HKKK.KKKKM as F1");
