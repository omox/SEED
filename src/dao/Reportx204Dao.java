...
public class Reportx204Dao extends ItemDao {
  ...
  public JSONArray check(HashMap<String, String> map, User userInfo, String sysdate) {
    ...
    ArrayList<JSONObject> tencds = new ArrayList<>();
    HashSet<String> tencds_ = new HashSet<>();
  ...
  public String createSqlHs(JSONObject data, JSONArray dataArrayT, User userInfo) {
    ...
    sb.append("UPDATE INAMS.MSTHSTENGP ");
    sb.append("SET ");
    sb.append("OPERATOR='" + userId + "'");
    sb.append(",UPDDT=CURRENT_TIMESTAMP ");
    sb.append("WHERE ");
    sb.append(sqlWhere);
    ...
    // 配送店グループマスタの件数チェック
    sb = new StringBuffer();
    sb.append("DELETE FROM INAMS.MSTHSGPTEN ");
    sb.append("WHERE ");
    sb.append(sqlWhere);
    ...
    for (int i = 0; i < dataArrayT.size(); i++) {
      ...
      sb.append("INSERT INTO INAMS.MSTHSGPTEN( ");
      sb.append("HSGPCD ");
      sb.append(",TENGPCD ");
      sb.append(",TENCD ");
      sb.append(",UPDKBN ");
      ...
      sb.append("," + DefineReport.Values.SENDFLG_UN.getVal());
      sb.append(",'" + userId + "'");
  ...
  private String createCommand() {
    ...
    sbSQL.append(",DATE_FORMAT(HSG.ADDDT,'%y/%m/%d') AS ADDDT "); // F6 : 登録日
    sbSQL.append(",DATE_FORMAT(HSG.UPDDT,'%y/%m/%d') AS UPDDT "); // F7 : 更新日
    sbSQL.append(",DATE_FORMAT(HST.UPDDT,'%Y%m%d%H%i%s%f') as HDN_UPDDT "); // F8 : 更新日時（配送店グループ）
    sbSQL.append("FROM ");
