...
public class ReportJU031Dao extends ItemDao {
	...
	private String createCommand() {
		...
		sbSQL.append(",TO_CHAR(T1.GENKAAM) AS GENKAAM ");
		sbSQL.append(",CASE WHEN T2.ZEIRT IS NULL THEN T1.BAIKAAM  ");
		sbSQL.append("ELSE CEILING(DOUBLE(T1.BAIKAAM) / NULLIF(1 + DECIMAL (T2.ZEIRT) / 100, 0)) END HONBAIKA ");
		sbSQL.append(",T1.BAIKAAM ");
		sbSQL.append(",TO_CHAR(TO_DATE(T1.HTDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.HTDT, 'YYYYMMDD'))) ");
		sbSQL.append(",TO_CHAR(TO_DATE(T1.NNDT, 'YYYYMMDD'), 'YY/MM/DD')||(select JWEEK from WEEK where CWEEK = DAYOFWEEK(TO_DATE(T1.NNDT, 'YYYYMMDD'))) ");
