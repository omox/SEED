...
public class Reportx122Dao extends ItemDao {
  ...
  private String createCommand() {
    ...
    if (DefineReport.Button.SEARCH.getObj().equals(sendBtnid)) {
      ...
      sbSQL.append(", DATE_FORMAT(ADDDT, '%y/%m/%d')"); // F26 ：画面下部表示用_登録日
      sbSQL.append(", DATE_FORMAT(UPDDT, '%y/%m/%d')"); // F27 ：画面下部表示用_更新日
  ...
  private JSONObject updateData(HashMap<String, String> map, User userInfo, String sysdate) throws Exception {
    ...
    targetWhere = " COALESCE(UPDKBN, 0) <> " + DefineReport.ValUpdkbn.DEL.getVal() + " and TENCD = ? and BMNCD = ?";
    ...
    if (msg.size() > 0) {
      ...
      if (StringUtils.isEmpty(getMessage())) {
        if (DefineReport.ID_DEBUG_MODE)
          System.out.println("店舗部門を " + MessageUtility.getMessage(Msg.S00003.getVal(), new String[] {Integer.toString(dataArray.size()), Integer.toString(count)}));
  ...
  private JSONObject deleteData(HashMap<String, String> map, User userInfo) throws Exception {
    ...
    if (StringUtils.isEmpty(getMessage())) {
      if (DefineReport.ID_DEBUG_MODE)
        System.out.println("店舗部門マスタを " + MessageUtility.getMessage(Msg.S00004.getVal(), new String[] {Integer.toString(count)}));
