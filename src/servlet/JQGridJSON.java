...
public class JQGridJSON extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		List<String> actionList = Arrays.asList(DefineReport.ID_PARAM_ACTION_UPDATE,
				DefineReport.ID_PARAM_ACTION_DELETE);
		...
		if (StringUtils.isEmpty(sendBtnid) ||
				(!StringUtils.isEmpty(sendBtnid) && !(StringUtils.equals("workTableDel", sendBtnid) || StringUtils.equals("workTableInit", sendBtnid)))) {
		...
		if (execut && DefineReport.ID_PARAM_ACTION_UPDATE.equals(map.get(DefineReport.ID_PARAM_ACTION))) {
			...
			// 本部マスタ
			if ((!StringUtils.isEmpty(userInfo.getYobi6_()) && !userInfo.getYobi6_().equals("-1"))
					|| StringUtils.isEmpty(userInfo.getYobi6_())) {
				menuKbn = "4";

				// 本部特売
			} else if ((!StringUtils.isEmpty(userInfo.getYobi8_()) && !userInfo.getYobi8_().equals("-1"))
					|| StringUtils.isEmpty(userInfo.getYobi8_())) {
				menuKbn = "5";
			}
			...
			else if ((menuKbn.equals("4") && StringUtils.isEmpty(userInfo.getYobi6_()))
		...
		if (execut && DefineReport.ID_PARAM_ACTION_DELETE.equals(map.get(DefineReport.ID_PARAM_ACTION))) {
			...
			// 本部マスタ
			if ((!StringUtils.isEmpty(userInfo.getYobi7_()) && !userInfo.getYobi7_().equals("-1"))
					|| StringUtils.isEmpty(userInfo.getYobi7_())) {
				menuKbn = "4";
			}
	 ...
	 * @param con
	 */
	private void convertItem(ItemInterface shopItem, HttpServletRequest request, HashMap<String, String> map, int limit,
			HttpSession session, int start) {
	...
	private ArrayList<byte[]> excludeColumns(HttpSession session, JQEasyModel json, ArrayList<byte[]> al, int startIdx,
			int loopCnt, int rearCols) throws IOException {
	...
	@SuppressWarnings({ "unchecked", "unused" })
	private ArrayList<byte[]> excludeColumns(HttpSession session, JQEasyModel json, ArrayList<byte[]> al, int startIdx,
			int loopCnt, int rearCols, String[] notDeKeys) throws IOException {

		String[] titles = StringUtils
				.splitPreserveAllTokens(new String(Snappy.uncompress(al.get(0), 0, al.get(0).length), "UTF-8"), "\t");
		// 終了列インデックス
		...
		for (byte[] rows : al) {
			...
			String[] columnsList = StringUtils
					.splitPreserveAllTokens(new String(Snappy.uncompress(rows, 0, rows.length), "UTF-8"), "\t");
			for (int i = startIdx; i < endIdx; i++) {
				...
				try {
					...
					if (option != null && option.containsKey(DefineReport.ID_PARAM_OPT_TITLE)
							&& ArrayUtils.isNotEmpty(notDeKeys)) {
						JSONArray titArray = option.getJSONArray(DefineReport.ID_PARAM_OPT_TITLE);
		...
		while (itr.hasNext()) {
			...
			byte[] bytes = itr.next();
			String[] columnsList = StringUtils
					.splitPreserveAllTokens(new String(Snappy.uncompress(bytes, 0, bytes.length), "UTF-8"), "\t");
			index = -1;
			...
			newData.add(
					Snappy.compress(StringUtils.join(cols.toArray(new String[cols.size()]), "\t").getBytes("UTF-8")));
		}
	...
	@SuppressWarnings("unchecked")
	private void setErrData(HttpServletRequest request, HttpSession session, HashMap<String, String> map,
			String sesPrefix) {
