...
//		session.removeAttribute(Consts.STR_SES_LOGINDT);	// LoginDt
		...
		if(lusr != null){
			...
			request.setAttribute(Form.LOGIN_USER,User);
			request.setAttribute(Form.LOGIN_PASS,Pass);
		}
		...
		String tantoTenpo = "-1";
		if (request.getParameter(Form.PRM_TENPO) != null){
			tantoTenpo = request.getParameter(Form.PRM_TENPO);
		}
		...
		try {
			...
			// 入力された値の取得
			String strUserID = esc.htmlEscape(User.toString().trim());
			String strPass = esc.htmlEscape(Pass.toString().trim());
			...
			// パスワード有効期限確認
			if(!StringUtils.equals("6", strMenuKbn) && !vld.IsPasswordTrim(lUser)) {
				/* エラーメッセージの設定 */
			...
			if(StringUtils.isEmpty(tantoTen) || !StringUtils.isNumeric(tantoTen) ||
				(StringUtils.isNotEmpty(tantoTen) && StringUtils.isNumeric(tantoTen) && StringUtils.isNotEmpty(yobi9))
			){
				if(!StringUtils.equals("-1", tantoTenpo)){
					// URLに区分が含まれなかった場合に、区分を設定する
					loginusr.setYobi2_(tantoTenpo);


					/** 管理者以外の場合 */
					sql = SQL.USER_TENKN;
					Object[] param2 = {tantoTenpo};
					list = null;
					mu = null;
					...
					try {
						QueryRunner qr = new QueryRunner(DBConnection.getDataSource());
						ResultSetHandler rsh = new MapHandler();
						mu = (Map) qr.query(sql, param2, rsh);
					...
					if (mu != null) {
						if (StringUtils.isNotEmpty(yobi9)) {
							loginusr.setYobi2Nm_("【画面】" + mu.get("TENKN").toString());
						} else {
							loginusr.setYobi2Nm_(mu.get("TENKN").toString());
						}
					}
				}
			}
