...
public class LoginServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      // ユーザーのデータを保持
      User usr = new User(Integer.parseInt(mu.get("CD_USER").toString().trim()), mu.get("USER_ID").toString().trim(), mu.get("PASSWORDS").toString().trim(), mu.get("UNAME").toString().trim(), mu.get("CD_AUTH").toString().trim(), mu.get("DT_PW_TERM").toString().trim(),
          Integer.parseInt(mu.get("CD_GROUP").toString().trim()), Integer.parseInt(mu.get("CD_POSITION").toString().trim()), mu.get("CUSTOM_VALUE").toString().trim(), mu.get("LOGO").toString().trim(), mu.get("YOBI_1").toString().trim(), mu.get("YOBI_2").toString().trim(),
          mu.get("YOBI_3").toString().trim(), mu.get("YOBI_4").toString().trim(), mu.get("YOBI_5").toString().trim(), mu.get("YOBI_6").toString().trim(), mu.get("YOBI_7").toString().trim(), mu.get("YOBI_8").toString().trim(), mu.get("YOBI_9").toString().trim(), mu.get("YOBI_10").toString().trim());
      ...
      try {
        ...
        list = (List) qr.query(SQL.SIDE_SEL_001, new Object[] {new Long(strMenuKbn), new Long(loginusr.getCD_user()), new Long(loginusr.getCD_user())}, rsh);
      } catch (Exception en) {
      ...
      if (list != null) {
        ...
        for (int i = 0; i < list.size(); i++) {
          mu = (Map) list.get(i);
          menu.add(i, new Side(Integer.parseInt(mu.get("cd_report_side").toString().trim()), mu.get("NM_REPORT_SIDE").toString().trim(), mu.get("cd_disp_column").toString().trim()));
      ...
      if (menu != null) {
        for (int i = 0; i < menu.size(); i++) {
          ...
          try {
            QueryRunner qr = new QueryRunner(DBConnection.getDataSource());
            ResultSetHandler rsh = new MapListHandler();
            list = (List) qr.query(SQL.REPORT_SEL_001, new Object[] {new Long(loginusr.getCD_user()), new Long(loginusr.getCD_user()), new Long(m.getSide())}, rsh);
          ...
          if (list != null) {
            ...
            for (int j = 0; j < list.size(); j++) {
              new QueryRunner(DBConnection.getDataSource());
              new MapListHandler();
              mu = (Map) list.get(j);
              ...
              rep.add(j,
                  new Report(Integer.parseInt(mu.get("CD_REPORT_SIDE").toString().trim()), Integer.parseInt(mu.get("CD_REPORT_NO").toString().trim()), mu.get("NM_REPORT").toString().trim(), mu.get("NM_SHORT").toString().trim(), mu.get("NM_REPORT_JSP").toString().trim(),
                      mu.get("CD_DISP_NUMBER").toString().trim(), mu.get("ENABLE_MENU").toString().trim(), mu.get("custom_value").toString().trim().toString().trim(), users_custom, getParameter(list1), getParameter(list2), getParameter(list3), mu.get("YOBI_1").toString().trim(),
                      mu.get("YOBI_2").toString().trim(), mu.get("YOBI_3").toString().trim(), mu.get("YOBI_4").toString().trim(), mu.get("YOBI_5").toString().trim()));
      ...
      /**
       * 別ページ遷移設定 ①本部ユーザー マスタ/特売 ：Login2へ ②本部ユーザー 店舗特売 ：TenSelectへ ③店舗ユーザー ：Menuへ
       */
  ...
  protected User getTopLoad(String strUserID, String strPass) {
    ...
    try {
      ...
      if (mu != null) {
        // ユーザーのデータを保持
        usr = new User(Integer.parseInt(mu.get("CD_USER").toString().trim()), mu.get("USER_ID").toString().trim(), mu.get("PASSWORDS").toString().trim(), mu.get("uname").toString().trim(), mu.get("CD_AUTH").toString().trim(), mu.get("DT_PW_TERM").toString().trim(),
            Integer.parseInt(mu.get("CD_GROUP").toString().trim()), Integer.parseInt(mu.get("cd_position").toString().trim()), mu.get("custom_value").toString().trim(), mu.get("LOGO").toString().trim(), mu.get("YOBI_1").toString().trim(), mu.get("YOBI_2").toString().trim(),
            mu.get("YOBI_3").toString().trim(), mu.get("YOBI_4").toString().trim(), mu.get("YOBI_5").toString().trim());
      }
