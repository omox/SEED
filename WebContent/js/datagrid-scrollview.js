...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	scrolling: function(target){
		...
		// いなげや様特殊処理
		var editObj = dc.body2.children('table.datagrid-btable').find('.datagrid-row-editing');
		if (editObj.length !== 0) {
			var index = editObj.attr('datagrid-row-index');
			if (editObj.position().top < headerHeight || editObj.position().top > dc.footer1.prevObject.position().top) {
				$(target).datagrid('endEdit', index);
				this.editRow = index;
			}
		} else if (!$.isEmptyVal(this.editRow)) {
			editObj = dc.body2.children('table.datagrid-btable').find('[datagrid-row-index='+this.editRow+']');
			if (editObj.length !== 0 && !(editObj.position().top < headerHeight || editObj.position().top > dc.footer1.prevObject.position().top)) {
				$(target).datagrid('beginEdit', this.editRow);
				this.editRow = '';
			}
		}
