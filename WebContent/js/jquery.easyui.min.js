...
_6ea.each(function(){
	if (document.uniqueID && typeof window.matchMedia == "undefined"){
		var that=this;
		setTimeout(function(){
			$(that)._outerWidth(fit?$(that).parent().width():10);
		}, 1000);
	}else{
...
if(!_a54){
	...
	var gl=ss.length;
	if(gl<=1){
	}else if(gl===$(_a52).combobox("getData").length && (typeof opts.simpleView==="undefined"||opts.simpleView)){
		s='すべて';
	}else if(gl>1) {
		s='複数（'+gl+'）';
	}
$(_a52).combo("setText",s);//ss.join(opts.separator));
}
...
if(!_aea){
var s=ss.join(opts.separator);
if($(_ae8).combo("getText")!=s && (typeof opts.updateView==="undefined"||opts.updateView)){
	var gl=_af4.length;//
	if(gl<=1){
