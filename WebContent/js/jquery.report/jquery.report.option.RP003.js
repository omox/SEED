...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportRP003',			// （必須）レポートオプションの確認
...
//			}
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("RP003", "数量パターンマスタ　一覧");
		},
...
//			}
			...
			if (rt) {
				if (chk_rinji == "0" && txt_moyscd)  {
					$.showMessage('EX1027', undefined, function(){$.addErrState(that, $('#'+$.id_inp.txt_moyscd), true)});
					rt = false;
				}
			}
			// 入力エラーなしの場合に検索条件を格納
...
//			$($.id.gridholder).datagrid('loading');
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
					{field:'F1',	title:'数量パターンNo.',			width: 200,halign:'center',align:'left'},
					{field:'F2',	title:'数量パターン名称',			width: 500,halign:'center',align:'left'}
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_new:
				...
				// 転送先情報
				index = 13;		// RP004 ランク別数量(新規・変更・参照)
				childurl = href[index];
				if (!txt_bmncd) {
					$.showMessage('EX1025', undefined, function(){$.addErrState(that, $('#'+$.id_inp.txt_sousu), true)});
					return false;
				}
				if (chk_rinji == "1" && !txt_moyscd) {
					$.showMessage('EX1026', undefined, function(){$.addErrState(that, $('#'+$.id_inp.txt_sousu), true)});
					return false;
				}
				...
				if(msgid !==null){
					...
					msgid = that.checkInputboxFunc($.id_inp.txt_moyscd,txt_moyscd);
					if(msgid !==null){
			...
			case $.id.btn_sel_change:
				...
				$.setJSONObject(sendJSON, $.id_inp.txt_moyscd, txt_moyscd, txt_moyscd);		// 催しコード
				$.setJSONObject(sendJSON, $.id.chk_rinji, chk_rinji, chk_rinji);			// 臨時
				$.setJSONObject(sendJSON, $.id_inp.txt_sryptnno, row.F1, row.F1);			// 数量パターンNo.
				break;
