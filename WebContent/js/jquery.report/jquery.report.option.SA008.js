...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSA008',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			$.setCheckboxInit2(that.jsonHidden, $.id.urisel_flg+1, that);
			$.setCheckboxInit2(that.jsonHidden, $.id.urisel_flg+2, that);
			$.setCheckboxInit2(that.jsonHidden, $.id.urisel_flg+3, that);
			// Load処理回避
			//$.tryChangeURL(null);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("SA008", "特売アンケート状況　売価選択(商品別)", "一覧");
...
//							$.setButtonState('#'+$.id.btn_search, true, id);
		...
		extenxDatagridEditorIds:{
			  F8		: "urisel_flg1"		// チェックボックス（店不採用禁止)
			, F12		: "urisel_flg2"		// チェックボックス（店不採用禁止)
			, F16		: "urisel_flg3"		// チェックボックス（店不採用禁止)
		},
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$('#'+id).datagrid({
				...
				columns:[[
					...
					{field:'F8',	title:'',styler:cstyler, formatter:cformatter,editor:{type:'checkbox'},width: 40,halign:'center',align:'center'},
					...
					{field:'F12',	title:'',styler:cstyler, formatter:cformatter,editor:{type:'checkbox'},width: 40,halign:'center',align:'center'},
					{field:'F13',	title:'B総売価',						width: 60,	halign:'center',align:'right'},
					{field:'F14',	title:'B総売価<br>（バンドル１）',		width: 100,	halign:'center',align:'right'},
					{field:'F15',	title:'B総売価<br>（バンドル２）',		width: 100,	halign:'center',align:'right'},
					{field:'F16',	title:'',styler:cstyler, formatter:cformatter,editor:{type:'checkbox'},width: 40,halign:'center',align:'center'},
					{field:'F17',	title:'C総売価',		        		width: 60,	halign:'center',align:'right'},
					{field:'F18',	title:'C総売価<br>（バンドル１）',		width: 100,	halign:'center',align:'right'},
					{field:'F19',	title:'C総売価<br>（バンドル２）',		width: 100,	halign:'center',align:'right'},
				]],
				...
				onBeginEdit:function(index,row){
					...
					if(row.F23=="0" || row.F23==""){
						obj.find("[id^='"+$.id.urisel_flg+1+"_"+"']").each(function(){
							$.setInputBoxDisable($('#'+$.id.urisel_flg+1+"_"));
						});
					}
					if(row.F24=="0" || row.F24==""){
						obj.find("[id^='"+$.id.urisel_flg+2+"_"+"']").each(function(){
							$.setInputBoxDisable($('#'+$.id.urisel_flg+2+"_"));
						});
					}
					if(row.F25=="0" || row.F25==""){
						obj.find("[id^='"+$.id.urisel_flg+3+"_"+"']").each(function(){
							$.setInputBoxDisable($('#'+$.id.urisel_flg+3+"_"));
						});
					}
