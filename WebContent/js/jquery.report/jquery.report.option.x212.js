...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx212',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("PC008", "プライスカード一覧");
		},
...
//			this.success(this.name, false);
		...
		getGridColumns:function(that, id){
			...
			var fields = ["F1","F2","F3","F4","F5","F6","F7","F8","F9"];
			var titles = ["処理日","入力者","コメント","マスタ予約日付","作成売価区分","枚数指定方法","入力No","プライスカードサイズ","コピー枚数"];
			...
			columnBottom.push({field:fields[3],	title:titles[3],	width:100,halign:'center',align:'left', formatter:dformatter});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_sel_refer:
				...
				if (row.F5 === "1") {
					...
					$.setJSONObject(sendJSON, $.id.rad_pcardsz1, row.F8, row.F8);
				// 作成売価区分 2:店別売価
				} else if (row.F5 === "2") {
					...
					$.setJSONObject(sendJSON, $.id.rad_pcardsz2, row.F8, row.F8);
				}

				childurl = href[index];

				// オブジェクト作成
				var txt_mst_yoyakudt = row.F4

				$.setJSONObject(sendJSON, $.id_inp.txt_coman, row.F3, row.F3);
				$.setJSONObject(sendJSON, $.id_inp.txt_mst_yoyakudt, txt_mst_yoyakudt.substr(2), txt_mst_yoyakudt.substr(2));
				$.setJSONObject(sendJSON, $.id.txt_inputno, row.F7, row.F7);
				$.setJSONObject(sendJSON, $.id_inp.txt_copysu, row.F9, row.F9);
				break;
			case $.id.btn_back:
				// 転送先情報
