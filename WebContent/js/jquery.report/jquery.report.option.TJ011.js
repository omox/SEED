...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportTJ011',			// （必須）レポートオプションの確認
			...
			$.post(
				...
				function(json){
					...
					if(opts && opts.rows_){
						...
						that.setData3(opts.rows_2, opts);
		...
		updSuccess: function(id){	// validation OK時 の update処理
			...
			$.post(
				$.reg.jqgrid ,
				{
					...
					DATA_YSN:		JSON.stringify([]),						// 事前発注_部門予算
			...
			if(rows.length > 0){
				...
				for(var i = 0;i<10; i++){
					var j = i+1;
						...
						columns[0][i+6].title = rows[0]["V"+j];
						columns[2][i+6].title = rows[0]["W"+j];
		...
		setEditableGrid2: function (id, reportNumber){	// グリッドの構築
			...
			var formatterCasesu_F18 = function(value,row,index){
				if(row.F1 === '3'){
			...
			columnBottom.push({field:'F3',	title:'入り数',		width:  50,halign:'center',align:'right',styler:cstyler2});
			columnBottom.push({field:'F4',	title:'事前原価',	width:  60,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			columnBottom.push({field:'F5',	title:'特売総売',	width:  60,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			//columnBottom.push({field:'F6',	title:'F6',		width:  70,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			columnBottom.push({field:'F7',	title:'日付',		width:  70,halign:'center',align:'right',rowspan:2,styler:cstyler2});
			columnBottom.push({field:'F8',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F9',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F10',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F11',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F12',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F13',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F14',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F15',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F16',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F17',	title:'',			width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F18',	title:'単位：千円',	width:  80,halign:'center',align:'right',styler:cstyler1, formatter:formatterCasesu});
			...
			$('#'+id).datagrid({
				...
				onLoadSuccess:function(data){
					...
					if(that.thisPageNumber == that.maxPageNumber){
						// 最終ページの場合
						$('#'+$.id.btn_addline).attr('tabindex', 6).show();	// 行追加ボタンを再表示
					}else{
				...
				onSelect:function(index,row){
					...
					if(row && row.F1 == '2' ){
						that.extenxDatagridEditorIds.F2 = "txt_shncd"
					}else if(row && row.F1 == '3' ){
						that.extenxDatagridEditorIds.F2 = "txt_binkbn"
					}
				},
		...
		addLine: function(){		// 新規入力行を表示する
			...
			$($.id.gridholder).datagrid('appendRow',{ F1:"4",F7:"発注売価"	,F21:dispNo	,F22:'2', F39:maxPageSize});

			// 追加行が生じた為、入力可能なグリッドをセルの背景色を再設定する。
