...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx052',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			if($.getInputboxValue($('#'+$.id_inp.txt_makercd)).length > 0){
				// 入力制限：メーカーコード
				$('#'+$.id_inp.txt_makercd).attr('readonly', 'readonly')
				$('#'+$.id_inp.txt_makercd).attr('tabindex', -1).combobox('disable');

				// 入力制限：代表メーカーコード
				$('#'+$.id_inp.txt_makercd + "_dai").attr('readonly', 'readonly')
				$('#'+$.id_inp.txt_makercd + "_dai").attr('tabindex', -1).combobox('disable');

				// 入力制限：代表メーカー名
				$('#'+$.id_inp.txt_makerkn + "_dai").attr('readonly', 'readonly')
				$('#'+$.id_inp.txt_makerkn + "_dai").attr('tabindex', -1).combobox('disable');
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				$.initReportInfo("MR002", "メーカーマスタ　選択(参照)", "参照");
				...
				// 参照時入力不可
				$.setInputBoxDisable($("#"+$.id_inp.txt_makerkn));
				$.setInputBoxDisable($("#"+$.id_inp.txt_makeran));
				$.setInputBoxDisable($("#"+$.id_inp.txt_jancd));

			}else{
				$.initReportInfo("MR002", "メーカーマスタ　選択(変更)", "変更");
				// 各種遷移ボタン
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case "btn_return":
				...
				var toReportName = $.getJSONValue(this.jsonHidden, 'reportName')		// 遷移元の画面名
				if(toReportName === 'Out_Reportx051'){
					if(that.reportYobiInfo()==='1'){
						index = 2;
					}else{
						index = 1;
					}
				}else if(toReportName === 'Out_Reportx053'){
