...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportHT009',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			for ( var sel in meisyoSelect ) {
				if($('#'+$.id_mei[meisyoSelect[sel]]).length > 0){
					that.setMeisyoCombo(that, reportno, $.id_mei[meisyoSelect[sel]], isUpdateReport);
					count++;
				}
			}
...
//							$.setButtonState('#'+$.id.btn_search, true, id);
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			// 検索実行
			var sel_yobi	= $.getJSONObject(this.jsonString, $.id_mei.kbn315).value;		// 曜日
			var txt_shncd	= $.getJSONObject(this.jsonString, $.id_inp.txt_shncd).value;	// 商品コード
			var sel_bumon	= $.getJSONObject(this.jsonString, $.id.SelBumon).value;		// 部門
			...
			var sel_shuno	= $.getJSONObject(this.jsonString, $.id.sel_shunoperiod).value;	// 週№
			if (sel_shuno===$('#'+$.id.sel_shunoperiod).combobox('getData')[0].VALUE) {
				sel_shuno = '1'+sel_shuno;
			} else {
				sel_shuno = '2'+sel_shuno;
			}
			...
			$.post(
				...
				function(json){
					...
					if(titles != undefined && titles.length > 0){
						$($.id.gridholder).datagrid('options').columns[0][4].title = titles[0];
						// datagrid のタイトル再設定
		...
		updValidation: function (){	// （必須）批准
			...
			// 週№
			var sel_shuno = $.getJSONObject(this.jsonString, $.id.sel_shunoperiod).value;
			if (!$.isEmptyVal(sel_shuno) && sel_shuno===$('#'+$.id.sel_shunoperiod).combobox('getData')[0].VALUE) {
				$.showMessage('EX1037');
				return false;
			}
		...
		updSuccess: function(id){	// validation OK時 の update処理
			...
			var seltenpo	= parseInt($.getJSONObject(this.jsonInit, 'SelTenpo').value);						// ログイン店舗
			var selindex	= sel_shuno.slice(0,1);	// ログイン店舗
			var sel_yobi	= $.getJSONObject(this.jsonString, $.id_mei.kbn315).value;		// 曜日
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				id:		$.id_mei.kbn315,
				value:	$('#'+$.id_mei.kbn315).combobox('getValue'),
				text:	$('#'+$.id_mei.kbn315).combobox('getText')
			});
			// 商品コード
			this.jsonTemp.push({
				id:		$.id_inp.txt_shncd,
...
//				});
		...
		getGridColumns:function(that, id){
			var columns = [];
			var columnBottom=[];
			var fields = ["F1","F2","F3","F4","F6","F7","F5","F8","F9","F10"];
			var titles = ["商品コード","商品名","便","","登録日","更新日","入力者","排他用更新日時","訂正区分","No"];

			var iformatter = function(value,row,index){ return $.getFormat(value, '##,##0');};

			columnBottom.push({field:fields[9],		title:titles[9],	width:40,	halign:'center',align:'center',editor:{options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			...
			columnBottom.push({field:fields[4],	title:titles[4],	width:65,	halign:'center',align:'left',editor:{options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			columnBottom.push({field:fields[5],	title:titles[5],	width:65,	halign:'center',align:'left',editor:{options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			columnBottom.push({field:fields[6],	title:titles[6],	width:70,	halign:'center',align:'left',editor:{options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			columnBottom.push({field:fields[7],	title:titles[7],	hidden:true});
			columnBottom.push({field:fields[8],	title:titles[8],	hidden:true});
