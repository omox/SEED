...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportJU027',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("JU026", "店舗アンケート付き送付け 参照 催し一覧");
			}else{
				$($.id.buttons).show();
				// 各種ボタン
				...
				$.initReportInfo("JU027", "店舗アンケート付き送付け 新規・変更 催し一覧");
			}
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				columns:[[
					...
					{field:'F2',	title:'催し名称',		width:250,halign:'center',align:'left'},
					{field:'F3',	title:'納入期間',		width:200,halign:'center',align:'left'},
					{field:'F4',	title:'応募締切日',		width:100,halign:'center',align:'left'},
					{field:'F5',	title:'商品数',			width:80 ,halign:'center',align:'right'}
				]],
...
//				else if(that.pushBtnId===$.id.btn_csv){
//					if(data.total!==0){
//						setTimeout(function(){
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_sel_change:
				...
				sendMode = 1;
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id.sel_moyscd, row.F1, row.F1);
				break;
			case $.id.btn_sel_refer:
				...
				sendMode = 1;
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_moyscd, row.F6, row.F6);
				$.setJSONObject(sendJSON, $.id_inp.txt_moykn, row.F2, row.F2);
				$.setJSONObject(sendJSON, 'kikan_dummy', row.F3, row.F3);
				$.setJSONObject(sendJSON, $.id_inp.txt_qasmdt, row.F4, row.F4);
				break;
