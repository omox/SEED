...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportTJ001',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$($.id.buttons).show();
			// 各種ボタン
			$('#'+$.id.btn_saikeisan).on("click", function(){
				that.setKeisan2(false)
			});
			$('#'+$.id.btn_addline).bind('click', function(){
				that.addLine();
		    });
			$.initReportInfo("TJ001", "特売販売計画＆事前発注");
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			$.post(
				...
				function(json){
					...
					if(opts && opts.rows_){
						// 基本データ表示
						that.setData(opts.rows_, opts);			// ヘッダー部(リストNo、タイトル、期間、送信締切日、部門)
						that.setData2(opts.rows_2, opts);		// ヘッダー部(部門予算）
						that.setData3(opts.rows_3, opts);		// ヘッダー部(日付、曜日）
						that.grd_all_data = opts.allRows;
						that.setBMNYSNKEI()						// ヘッダー部(部門予算計)
		...
		setEditableGrid2: function (id, reportNumber){	// グリッドの構築
			...
			columnBottom.push({field:'F1',	title:'',			width:  80,halign:'center',align:'right',hidden:true});
			...
			columnBottom.push({field:'F18',	title:'',			width:  70,halign:'center',align:'right',styler:cstyler1, formatter:formatterCasesu_F18});
			...
			$('#'+id).datagrid({
				...
				onLoadSuccess:function(data){
					...
					// 部門予算のデータを保持
					var ysnData = that.getGridDataBMNYSN()
					that.setGridData(ysnData, grd_bumonyosan);
...
//				});
			...
			for (var i=0; i<allRows.length; i++){
				...
				if(update){
					if(row.F1 == '1'){
						...
						NHBIK_SUM_ = NHBIK_SUM	// 丸める前の値を保持
						NHBIK_SUM = Math.round(parseFloat(NHBIK_SUM) / 1000 );		// 小数点以下で四捨五入
						row_L4.F18 = NHBIK_SUM;

						// 計算：構成比
						if(!YSNKEI || YSNKEI == "" || YSNKEI== 0){
							// 部門予算計が空の場合計算をしない
							KOSEIHI = "";
						}else{
							KOSEIHI =  (parseFloat(NHBIK_SUM_) / (parseFloat(YSNKEI) * 1000) * 100)
							KOSEIHI = Math.round(KOSEIHI * 100) / 100				// 小数点2以下で四捨五入
						}
						row_L3.F18 = KOSEIHI;
						...
						$($.id.gridholder).datagrid('mergeCells',{index:FirstLineIdx + 3,field:'F2',colspan:3});
					}
				}
			}
		},
		addLine: function(){		// 新規入力行を表示する
			...
			$($.id.gridholder).datagrid('appendRow',{ F1:"4",F7:"納品売価"	,F21:dispNo	,F22:'2', F39:maxPageSize});
