...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx141',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("TP013", "店舗休日マスタ　参照");
			}else{
				$($.id.buttons).show();
				// 各種遷移ボタン
				...
				$.initReportInfo("TP013", "店舗休日マスタ　一覧");
			}
		},
...
//			this.success(this.name, false);
		...
		success: function(reportno, sortable, btnId){	// （必須）正処理
			...
			var that = this;
			// 検索実行
			var szTxtTencd	= $.getJSONObject(this.jsonString, $.id_inp.txt_tencd).value;			// ナンバーボックス：店コード
			var szTxtTenkyustdt	= $.getInputboxValue($('#'+$.id_inp.txt_tenkyustdt));	// ナンバーボックス：店休日
			var szTxtTenkyuendt	= $.getInputboxValue($('#'+$.id_inp.txt_tenkyuendt));	// ナンバーボックス：店休日
			...
			$.post(
				$.reg.jqgrid ,
				{
					...
					BTN:				btnId,
					TXT_TENCD:			szTxtTencd,	// 風袋枝番
					TXT_TENKYUSTDT:		szTxtTenkyustdt,		// 風袋種別
					TXT_TENKYUENDT:		szTxtTenkyuendt,		// 風袋種別
					t:					(new Date()).getTime(),
					sortable:			sortable,
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			// 店休日from
			this.jsonTemp.push({
				id:		$.id_inp.txt_tenkyustdt,
				value:	$('#'+$.id_inp.txt_tenkyustdt).textbox('getValue'),
				text:	$('#'+$.id_inp.txt_tenkyustdt).textbox('getText')
			});
			// 店休日to
			this.jsonTemp.push({
				id:		$.id_inp.txt_tenkyuendt,
				value:	$('#'+$.id_inp.txt_tenkyuendt).textbox('getValue'),
				text:	$('#'+$.id_inp.txt_tenkyuendt).textbox('getText')
			});
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
						{field:'F1',	title:'日付',					width: 80,halign:'center',align:'left'},
						{field:'F2',	title:'店コード',				width: 80,halign:'center',align:'left'},
						{field:'F3',	title:'店舗名',					width: 200,halign:'center',align:'left'},
						{field:'F4',	title:'店休フラグ',				width: 80,halign:'center',align:'left'}
				]],
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_new:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_tencd,'', '');
				$.setJSONObject(sendJSON, $.id_inp.txt_tenkyuendt,'', '');
				$.setJSONObject(sendJSON, $.id_mei.sel_tenkyuflg,'', '');
				break;
			case $.id.btn_copy:
			...
			case $.id.btn_sel_refer:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_tencd,row.F2, row.F2);
				$.setJSONObject(sendJSON, $.id_inp.txt_tenkyudt, row.F5, row.F5);
				$.setJSONObject(sendJSON, $.id_mei.kbn316, row.F6, row.F6);
				break;
