...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx218',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.sendBtnid===$.id.btn_sakubaikakb2){
				...
				$.initReportInfo("PC005", "プライスカード　（全店）　新規", "新規");
			} else if (that.sendBtnid===$.id.btn_sel_change) {
				$.initReportInfo("PC005", "プライスカード　（全店）　変更", "変更");
			} else {
				...
				$.initReportInfo("PC005", "プライスカード　（全店）　参照", "参照");
			}
		...
		setGrid: function(that, reportno, id, chk){		// データ表示
			...
			var funcBeforeLoad = function(param){
				...
				var json = [{"callpage":"Out_Reportx218","INPUTNO":txt_inputno}];
				// 情報設定
				param.page		=	reportno;
				param.obj		=	id;
