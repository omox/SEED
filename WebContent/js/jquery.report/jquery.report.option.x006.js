...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx006',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("IT026", "商品マスタ　CSV　エラー一覧", "一覧");
			}else{
				$($.id.buttons).show();
				// 各種ボタン
				...
				$.initReportInfo("IT026", "商品マスタ　CSV　エラー選択", "選択");
			}
...
*/
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
					{field:'F1',	title:'オペレーターコード',	width:120,halign:'center',align:'left'},
					{field:'F2',	title:'処理日',				width: 70,halign:'center',align:'center',
						formatter:function(value,row,index){
							return $.getFormatPrompt(value, '##/##/##');
						}
					},
					{field:'F3',	title:'時刻',				width: 50,halign:'center',align:'center',
						formatter:function(value,row,index){
							return $.getFormatPrompt(value, '##:##');
						}
					},
					{field:'F4',	title:'コメント',			width:500,halign:'center',align:'left'},
					{field:'F5',	title:'SEQ',				hidden:true}
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_err_change:
				...
				$.setJSONObject(sendJSON, $.id.txt_seq, row.F5, row.F5);		// SEQ
				$.setJSONObject(sendJSON, $.id.txt_operator, row.F1, row.F1);	// オペレータ
				$.setJSONObject(sendJSON, $.id.txt_upddt, row.F2, row.F2);		// 更新日
				$.setJSONObject(sendJSON, $.id.txt_updtm, row.F3, row.F3);		// 更新時刻
				$.setJSONObject(sendJSON, $.id.txt_commentkn, row.F4, row.F4);	// コメント
