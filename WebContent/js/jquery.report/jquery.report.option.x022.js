...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx022',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			// 部門コード
			$.setInputbox(that, reportno, $.id.txt_sel_bmncd, isUpdateReport);
			// 税率区分
			...
			// 新規以外は検索実行
			if($.getInputboxValue($('#'+$.id.txt_sel_bmncd)).length > 0){
				// 入力制限：部門コード
				$('#'+$.id.txt_sel_bmncd).attr('readonly', 'readonly')
				$('#'+$.id.txt_sel_bmncd).attr('tabindex', -1).combobox('disable');
				$.initReportInfo("IT001", "部門マスタ　変更" ,'変更');
				...
				$.initReportInfo("IT001", "部門マスタ　新規登録" ,'新規');
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			$.post(
				...
				function(json){
					...
					$('#'+$.id_mei.kbn501).combo("textbox").focus();
		...
		updValidation: function (){	// （必須）批准
			...
			// 全社部門
			var param = $.getInputboxValue($('#'+$.id_inp.txt_corpbmncd));
			if(param && param!==''){
				var msgid = that.checkInputboxFunc($.id_inp.txt_corpbmncd, param , '');

				if(msgid !==null){
					$.showMessage(msgid, undefined, function(){$.addErrState(that, $('#'+$.id_inp.txt_corpbmncd), true)});
					return false;
				}
			}

			// 売上計上部門
			var param = $.getInputboxValue($('#'+$.id_inp.txt_uribmncd));
			if(param && param!==''){
				var msgid = that.checkInputboxFunc($.id_inp.txt_uribmncd, param , '');
				if(msgid !==null){
					$.showMessage(msgid, undefined, function(){$.addErrState(that, $('#'+$.id_inp.txt_uribmncd), true)});
					return false;
				}
			}

			// 税区分
			var param = $.getInputboxValue($('#'+$.id_mei.kbn120));
			if(param && param!==''){
				var msgid = that.checkInputboxFunc($.id_mei.kbn120, param , '');
				if(msgid !==null){
					$.showMessage(msgid, undefined, function(){$.addErrState(that, $('#'+$.id_mei.kbn120), true)});
					return false;
				}
			}
...
//					DATA:			JSON.stringify(targetRows),		// 更新対象情報
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				id:		$.id.txt_sel_bmncd,
				value:	$.getJSONValue(this.jsonHidden, $.id.txt_sel_bmncd),
				text:	''
...
//				});
		...
		checkInputboxFunc:function(id, newValue, kbn, record, isNew){
			...
			// 全社部門
			if(id===$.id_inp.txt_corpbmncd){
				if(newValue !== '' && newValue){
					var param = $.getInputboxValue($('#'+$.id_mei.kbn501));			// 部門区分
			...
			if(id===$.id_mei.kbn120){
				if(newValue !== '' && newValue){
					var param = $.getInputboxValue($('#'+$.id.sel_zeirtkbn));				// 税率区分
