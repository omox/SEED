...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSA003',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("SA003", "特売アンケート状況　売価選択(一括)", "一覧");
			$("#searchCombo").hide();
			$("#"+$.id.btn_search).linkbutton('disable');
			$("#"+$.id.btn_search).attr('tabindex', -1);
...
//			var qasyukbn	= "4";			//アンケート種類
		...
		updSuccess: function(id){	// validation OK時 の update処理
			...
			var qasyukbn		= $.getJSONValue(that.jsonHidden, $.id_inp.sel_qasyukbn);			// アンケート種類
			var szAdopt		= $("input[name="+$.id.rad_adopt+"]:checked").val();					// 採用区分
			var szAuriaselkbn	= $("input[name="+$.id.rad_auriaselkbn+"]:checked").val();			// 売価区分
			// 変更行情報取得
			var targetDatas = [{}];
