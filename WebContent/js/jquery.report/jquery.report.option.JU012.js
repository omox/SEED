...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportJU012',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){		// （必須）初期化
			...
			$.winST008.init(that);	// 店ランク情報
			$.winST009.init(that);	// 店情報
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("JU012", "事前打出し　新規　商品情報", "新規");
		},
		...
		setEditableGrid: function(that, reportno, id){		// データ表示
			...
			$('#'+id).datagrid({
				...
				onBeforeLoad:function(param){
					...
					var json = [{"callpage":"Out_ReportJU012"}];
					// 情報設定
					param.page		=	reportno;
					param.obj		=	id;
