...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportJU001',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("JU006", "事前打出し　参照　催し一覧");
			}else{
				$($.id.buttons).show();
				// 各種ボタン
				$('#'+$.id.btn_sel_refer).on("click", $.pushChangeReport);
				$('#'+$.id.btn_csv_import).on("click", $.pushChangeReport);
				$.initReportInfo("JU001", "事前打出し　新規・更新　催し一覧");
			}
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				columns:[[
					...
					{field:'F2',	title:'催し名称',			width:250,halign:'center',align:'leftt'},
					{field:'F3',	title:'納入期間',			width:200,halign:'center',align:'left'},
					{field:'F4',	title:'商品数',				width:60,halign:'center',align:'right'},
				]],
				...
				onLoadSuccess:function(data){
					...
					if(getRowIndex !== ""){
						...
						// 一度適用したら要素を削除する。
						var targetName = "scrollToIndex_"+$.id.gridholder;
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_sel_refer:
				...
				sendMode = 1;
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_moyscd, row.F5, row.F5);
