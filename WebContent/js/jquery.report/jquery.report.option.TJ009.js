...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportTJ009',			// （必須）レポートオプションの確認
		...
		grd_kouseihi_data:[],					// グリッド情報:構成比情報
		...
		initialize: function (reportno){		// （必須）初期化
			...
			$.setInputbox(that, reportno, 'txt_kouseihi1', isUpdateReport);
			$.setInputbox(that, reportno, 'txt_kouseihi2', isUpdateReport);
			$.setInputbox(that, reportno, 'txt_kouseihi3', isUpdateReport);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("TJ009", "特売販売計画＆事前発注");
			...
			$.post(
				...
				function(json){
					...
					if(opts && opts.rows_){
						...
						that.setData4(opts.rows_4, opts);
		...
		updValidation: function (){	// （必須）批准
			...
			that.updWorkTable("gridholder", false);
			...
			if(that.updateData){
				for(var i = 0; i < that.updateData.length; i ++){
					if(that.updateData[i].type == 'addShnData'){
						...
						// 本画面のパターンでは便区分の入力欄が存在しない為チェックが行えない
						/*if(!binKbn || binKbn == ""){
							// 便区分コード未入力の場合
		...
		updSuccess: function(id){	// validation OK時 の update処理
			...
			//var targetDatasShn2 = that.getMergeGridDate3($.id.gridholder);
			var targetDatasShn2 = [];
			if(that.updateData){
				for(var i = 0; i < that.updateData.length; i ++){
					if(that.updateData[i].type == 'addShnData'){
			...
			//var targetDatasShn 		= that.getMergeGridDate($.id.gridholder);//取得
			//var targetDatasShn2 	= that.getMergeGridDate3($.id.gridholder);//取得
			var szLstno				= $.getJSONValue(this.jsonHidden, $.id_inp.txt_lstno);		// リスト№
			$('#'+that.focusRootId).find('[col^=F]').each(function(){
				var col = $(this).attr('col');
				targetDatas[0][col] = $.getInputboxValue($(this));
			...
			var targetDatasKouseihi = that.getMergeGridDate4();//取得
			// 処理時間計測用
			that.timeData = (new Date()).getTime();
			...
			$.post(
				$.reg.jqgrid ,
				{
					...
					DATA_KOUSEIHI:	JSON.stringify(targetDatasKouseihi),	// 事前発注_構成比
					LSTNO:			szLstno,
					BMNCD:			szBmncd.split("-")[0],
					rows:			60,										// 表示可能レコード数(時間外利用時に登録時、行情報を保持しなければページング処理に影響が出る為、行情報を送る必要がある)
					t:				(new Date()).getTime()
				},
				function(data){
					// 検索処理エラー判定
		...
		updWorkTable: function(id, init){	// validation OK時 の update処理
			...
			$.post(
				$.reg.jqgrid ,
				{
					...
					LSTNO:			szLstno,
					BMNCD:			szBmncd.split("-")[0],
					rows:			60,							// 表示可能レコード数(遷移毎にサーブレット経由で行情報が上書きされてしまう為、常に行情報を送る必要がある)
					t:				(new Date()).getTime()
				},
				function(data){
		...
		setData4: function(rows, opts){		// データ表示
			var that = this;
			if(rows.length > 0){

				var value1 = $.getFormat(rows[0]["F1"], '#,###.#0');
				var value2 = $.getFormat(rows[0]["F2"], '#,###.#0');
				var value3 = $.getFormat(rows[0]["F3"], '#,###.#0');

					$.setInputboxValue($('#txt_kouseihi1'), value1);
					$.setInputboxValue($('#txt_kouseihi2'), value2);
					$.setInputboxValue($('#txt_kouseihi3'), value3);
					...
					that.grd_kouseihi_data.push(rowDate);
			}
		},
		getMergeGridDate: function( target){
			...
			// scrollViewを使用している為、getRowsした際に全行取得できない。
		...
		getMergeGridDate3: function( target){
			...
			//var newrows = that.getGridData2(target)[target];			// 変更データ
			//var rows	 = $($.id.gridholder).datagrid('getRows');		// 商品一覧
		...
		getMergeGridDate4: function( ){
			// 保持したデータと入力データ比較を比較する。
			var that = this;
			var newrows = [];		// 変更データ

			var kouseihiDate = {
					F1	 : $.getInputboxValue($('#txt_kouseihi1')),
					F2	 : $.getInputboxValue($('#txt_kouseihi2')),
					F3	 : $.getInputboxValue($('#txt_kouseihi3')),
			}
			newrows.push(kouseihiDate);
			var oldrows = [];
			var oldrows2 = [];
			var targetRows= [];

			if (that.grd_kouseihi_data.length===0) {
				that.grd_kouseihi_data.push(kouseihiDate);
			}

			oldrows = that.grd_kouseihi_data;

			if((newrows[0]['F1'] ? newrows[0]['F1'] : "") != (oldrows[0]['F1'] ? oldrows[0]['F1'] : "")
			|| (newrows[0]['F2'] ? newrows[0]['F2'] : "") != (oldrows[0]['F2'] ? oldrows[0]['F2'] : "")
			|| (newrows[0]['F3'] ? newrows[0]['F3'] : "") != (oldrows[0]['F3'] ? oldrows[0]['F3'] : "")
			){
				var rowDate = {
						F1	 : newrows[0]["F1"],
						F2	 : newrows[0]["F2"],
		...
		setEditableGrid2: function (id, reportNumber){	// グリッドの構築
			...
			columnBottom.push({field:'F1',	title:'F1',			width:  80,halign:'center',align:'right',hidden:true});
			...
			columnBottom.push({field:'F3',	title:'',			width:  70,halign:'center',align:'right',styler:cstyler2});
			columnBottom.push({field:'F4',	title:'',			width:  70,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			columnBottom.push({field:'F5',	title:'',			width:  70,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			columnBottom.push({field:'F6',	title:'',			width:  60,halign:'center',align:'right',styler:cstyler2, formatter:iformatter});
			columnBottom.push({field:'F7',	title:'日付',		width:  60,halign:'center',align:'right',rowspan:2,styler:cstyler2});
			columnBottom.push({field:'F8',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F9',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F10',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F11',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F12',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F13',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F14',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F15',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F16',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F17',	title:'',			width:  52,halign:'center',align:'right',rowspan:2,styler:cstyler1, formatter:formatterCasesu,editor:{type:'numberbox'}});
			columnBottom.push({field:'F18',	title:'単位：千円',	width:  70,halign:'center',align:'right',styler:cstyler1, formatter:formatterCasesu});
			...
			$('#'+id).datagrid({
				...
				onLoadSuccess:function(data){
					...
					// ページ番号を保持
					that.thisPageNumber	 =  $('#'+id).datagrid('options').pageNumber															// 現在のページの番号
					that.maxPageNumber	 = Math.ceil(parseFloat($('#'+id).datagrid('getData').total / $('#'+id).datagrid('options').pageSize))	// 最終のページの番号
					...
					for (var i=0; i<data.length; i++){
						if(data[i]["F1"]=="1"){
							...
							$('#'+id).datagrid('mergeCells',{index:i+(offset),field:'F2',colspan:2});
						}
					}
		...
		// 部門予算の合計を設定する。
		setBMNYSNKEI: function(){
			var that = this;
			var ysnKei = 0;

			for(var i = 0;i<10; i++){
				var value = $.getInputboxValue($('#'+'txt_yosan'+(i+1)));
				if(value && value != ""){
					ysnKei += Number(value);
				}
			}
			...
			$.setInputboxValue($('#txt_yosan_sum'), $.getFormat(ysnKei, '#,##0'));
		},
		setKeisan2: function(){
			var that = this;
			...
			//var allRows = $($.id.gridholder).datagrid('getRows');
			...
			for (var i=0; i<allRows.length; i++){
				...
				if(update){
					if(row.F1 == '1'){
						...
						// 計算：実績過不足
						if(row_L3.F6 && row_L3.F6 != ''){
							// 平均実績=0の時
							ZITKFSK = Number(SURYO) - Number(row_L3.F6)
						}else{
							ZITKFSK = "";				// パック売価
						}
						row_L4.F18 = ZITKFSK;
						...
						// 行データのUPDATE
						$($.id.gridholder).datagrid('updateRow',{index: FirstLineIdx + 2,row: row_L3})
						$($.id.gridholder).datagrid('updateRow',{index: FirstLineIdx + 3,row: row_L4})
						...
						// セルのマージ
						$($.id.gridholder).datagrid('mergeCells',{index:FirstLineIdx	,field:'F2',colspan:3});
						$($.id.gridholder).datagrid('mergeCells',{index:FirstLineIdx + 3,field:'F2',colspan:2});
					}
				}
			}
		},
		addLine: function(){		// 新規入力行を表示する
			...
			var maxPageSize = Math.ceil((data.total + 4) / maxRowLength)

			$($.id.gridholder).datagrid('appendRow',{ F1:"1",F7:"販売日"	,F21:dispNo	,F22:'2', F39:maxPageSize});
			$($.id.gridholder).datagrid('appendRow',{ F1:"2",F7:"発注"		,F21:dispNo	,F22:'2', F39:maxPageSize});
			$($.id.gridholder).datagrid('appendRow',{ F1:"3",F7:"ケース数"	,F21:dispNo	,F22:'2', F39:maxPageSize});
			$($.id.gridholder).datagrid('appendRow',{ F1:"4",F7:""			,F21:dispNo	,F22:'2', F39:maxPageSize});
		...
		getPageNum:function(id, index){
			var that = this
			var maxRowLength = 15 * 4	// 1ページ内の最大表示行数(1ページ当たり15商品まで表示、1商品につき5行使用している)
			var thisPage = that.thisPageNumber	// 現在のページ番号

			if(index <= maxRowLength){
				return thisPage;
			}else{
				//var pageNum = Math.floor(parseFloat((index - maxRowLength)) / maxRowLength)
		...
		changeInputboxFunc:function(that, id, newValue, obj){
			...
			/* TODO: お客様検証完了後に要望があれば復活
			if (id.indexOf('txt_kouseihi')!==-1) {
				newValue = Math.round(newValue * 10);
				newValue = newValue / 10;
				$.setInputboxValue($('#'+id),newValue);
			}*/
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_upd:
				...
				// オブジェクト作成
				//$.setJSONObject(sendJSON, $.id_inp.txt_lstno,txt_lstno, txt_lstno);
