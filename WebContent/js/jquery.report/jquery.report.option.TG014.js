...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportTG014',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("TG014", "特売アンケート状況　グループ・店別", "一覧");
...
//			this.success(this.name, false);
		...
		success: function(reportno, sortable, btnId){	// （必須）正処理
			...
			var szTengpcd	= $.getJSONObject(this.jsonString, $.id_inp.txt_tengpcd).value;	// 催し連番
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
					{field:'F1',	title:'店コード',		width: 40,	halign:'center',align:'left'},
					{field:'F2',	title:'店舗名',			width:220,	halign:'center',align:'left'},
					{field:'F3',	title:'回答',		    width: 60,	halign:'center',align:'center'},
					{field:'F4',	title:'参加',			width: 60,	halign:'center',align:'center',	formatter:cformatter,	styler:cstyler},
					{field:'F6',	title:'不参加',		    width: 60,	halign:'center',align:'center',	formatter:cformatter,	styler:cstyler}
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			$.setJSONObject(sendJSON, 'returnPageInfo1', 'TG014', 'TG014');		// 参照情報保持
			...
			case $.id.btn_sel_change+3:
				...
				sendMode = 1;
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_tencd, row.F1, row.F1);	// 店グループ
				break;
			case $.id.btn_back:
			case $.id.btn_cancel:
				// 転送先情報
