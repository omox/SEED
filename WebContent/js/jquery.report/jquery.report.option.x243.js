...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx243',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			// 表示する・しない
			$.setMeisyoCombo(that, reportno, $.id.sel_display, isUpdateReport);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if (that.reportYobiInfo()==='1') {
				...
				$.initReportInfo("x243", "ユーザー権限変更履歴　参照", "");
			}else{
				$.initReportInfo("x243", "ユーザー権限変更履歴　参照", "");
			}
			...
			// TODO:表示する・しないは非表示
			$.setInputBoxDisableVariable($('#'+$.id.sel_display));
			$('#'+$.id.sel_display+'_td').hide();
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			var txt_fromdate	= $.getInputboxValue($('#txt_fromdate'));	// 開始日
			var txt_todate		= $.getInputboxValue($('#txt_todate'));		// 終了日
			var txt_name		= $.getInputboxValue($('#txt_name'));		// 姓名
			var sel_tenpo		= $.getInputboxValue($('#SelTenpo'));		// 店舗
			var sel_display		= $.getInputboxValue($('#'+$.id.sel_display));	// 表示する・しない
