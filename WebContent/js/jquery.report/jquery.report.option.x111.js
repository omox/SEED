...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx111',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("TP001", "店舗基本マスタ一覧");
		},
...
//			this.success(this.name, false);
		...
		validation: function (){	// （必須）批准
			...
			var txt_tenkn		= $('#'+$.id_inp.txt_tenkn).textbox('getValue'); 			// 店舗名

			if(!txt_tencd && !txt_tenkn){
				$.showMessage('EX1013');
				rt = false;
			}
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			var txt_tenkn		= $.getJSONObject(this.jsonString, $.id_inp.txt_tenkn).value;		// 店舗名
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
					{field:'F1',	title:'店コード',				width: 100,halign:'center',align:'left'},
					{field:'F2',	title:'店舗名(漢字)',			width: 300,halign:'center',align:'left'}
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_select:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_tencd, row.F1, row.F1);
				break;
			case $.id.btn_back:
				// 転送先情報
