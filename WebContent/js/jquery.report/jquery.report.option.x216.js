...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx216',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.sendBtnid===$.id.btn_sakubaikakb2){
				...
				$.initReportInfo("PC007", "プライスカード　（店、部門、枚数指定）　新規", "新規");
			} else if (that.sendBtnid===$.id.btn_sel_change) {
				$.initReportInfo("PC007", "プライスカード　（店、部門、枚数指定）　変更", "変更");
			} else {
				...
				$.initReportInfo("PC007", "プライスカード　（店、部門、枚数指定）　参照", "参照");
			}
		...
		setGrid: function(that, reportno, id, chk){		// データ表示
			...
			var funcBeforeLoad = function(param){
				...
				var json = [{"callpage":"Out_Reportx216","INPUTNO":txt_inputno}];
				// 情報設定
				param.page		=	reportno;
				param.obj		=	id;
...
//				});
		...
		getGridColumns:function(that, id){
			...
			var targetTenId = $.id_inp.txt_tencd;
			var checkTen = $('#'+targetTenId).attr("check") ? JSON.parse('{'+$('#'+targetTenId).attr("check")+'}'): JSON.parse('{}');	// コードのcheck要素を取得
			var formatterLPadTen = function(value){
				return $.getFormatLPad(value, checkTen.maxlen);
			};
			var targetBmnId = $.id_inp.txt_bmncd;
			var checkBmn = $('#'+targetBmnId).attr("check") ? JSON.parse('{'+$('#'+targetBmnId).attr("check")+'}'): JSON.parse('{}');	// コードのcheck要素を取得
			var formatterLPadBmn = function(value){
				return $.getFormatLPad(value, checkBmn.maxlen);
			};
			var fields = ["TENCD","TENKN","BMNCD","BMNKN","MAISU","SEQ"];
			var titles = ["店コード","店名","部門コード","部門名","枚数","SEQ"];
			...
			columnBottom.push({field:fields[0],	title:titles[0],	width:60,	halign:'center',align:'left',formatter:formatterLPadTen,editor:{type:'numberbox'}});
			...
			columnBottom.push({field:fields[2],	title:titles[2],	width:80,	halign:'center',align:'left',formatter:formatterLPadBmn,editor:{type:'numberbox'}});
			columnBottom.push({field:fields[3],	title:titles[3],	width:150,	halign:'center',align:'left',	editor:{type:'textbox',options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			columnBottom.push({field:fields[4],	title:titles[4],	width:60,	halign:'center',align:'right', formatter:iformatter, editor:{type:'numberbox'}});
		...
		changeInputboxFunc:function(that, id, newValue, obj){
			...
			// 商品コードに入力があった場合枚数に0を設定
			if (id===$.id_inp.txt_tenkn &&
				...
				if(row){
					...
					parentObj.find("[datagrid-row-index='"+idx+"']").find('[id='+$.id_inp.txt_maisu+'_]').each(function(){
			...
			if(target===undefined || target===$.id.grd_pcardsu+'_list'){
				...
				for (var i=0; i<rowsPcardsu.length; i++){
					if((rowsPcardsu[i]["TENCD"] == "" || rowsPcardsu[i]["TENCD"] == null) &&
							(rowsPcardsu[i]["BMNCD"] == "" || rowsPcardsu[i]["BMNCD"] == null) &&
							(rowsPcardsu[i]["MAISU"] == "" || rowsPcardsu[i]["MAISU"] == null)){
						...
						var rowDate = {
								...
								F4	 : rowsPcardsu[i]["BMNCD"],
								F5	 : rowsPcardsu[i]["BMNKN"],
								F6	 : rowsPcardsu[i]["MAISU"],
								F7	 : rowsPcardsu[i]["SEQ"],
							};
		...
		checkInputboxFunc: function(){
			...
			for (var i=0; i<targetRows.length; i++){
				...
				if (!$.isEmptyVal(bmncd)) {
					...
					if(chk_cnt==="" || chk_cnt==="0"){
						$.showMessage("E11044", undefined, function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_bmncd})});
						return false;
					}
					errFlg = false;
				}
				...
				if ((!$.isEmptyVal(tencd) || !$.isEmptyVal(bmncd) || !$.isEmptyVal(maisu))) {
					if (($.isEmptyVal(tencd) || $.isEmptyVal(bmncd) || $.isEmptyVal(maisu))) {
						$.showMessage("E11049", ["店コードと部門コードと枚数を"], function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_tencd})});
						return false;
					}
				}
				...
				// 入力値が空じゃないときのみチェック
				if ($.isEmptyVal(tencd) && $.isEmptyVal(bmncd) && !$.isEmptyVal(maisu)) {
					$.showMessage("EX1040", undefined, function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_maisu})});
					return false;
				}
				...
				if (!$.isEmptyVal(maisu)) {
					// 枚数にマイナス値の入力は
					if (maisu*1 < 0) {
						$.showMessage("EX1047", ["枚数 ≧ 0の値"], function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_maisu})});
						return false;
					}
				}
			}
