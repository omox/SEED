...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx101',			// （必須）レポートオプションの確認
...
//			this.success(this.name, false);
		...
		success: function(reportno, sortable, btnId){	// （必須）正処理
			...
			var that = this;
			// 検索実行
			var szTxt617	= $.getJSONObject(this.jsonString, $.id_mei.kbn617).value;			// ナンバーボックス：風袋種別
			var szTxtFutaiEdaban	= $.getJSONObject(this.jsonString, $.id_inp.txt_futaiedaban).value;			// ナンバーボックス：風袋枝番
			...
			$.post(
				$.reg.jqgrid ,
				{
					...
					BTN:				btnId,
					TXT_FUTAIEDABAN:	szTxtFutaiEdaban,	// 風袋枝番
					TXT_FUTAISHUKUBUN:	szTxt617,		// 風袋種別
					t:					(new Date()).getTime(),
					sortable:			sortable,
...
*/
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				id:		$.id_mei.kbn617,
				value:	$('#'+$.id_mei.kbn617).combobox('getValue'),
				text:	$('#'+$.id_mei.kbn617).combobox('getText')
			});
			// 風袋枝番
			this.jsonTemp.push({
				id:		$.id_inp.txt_futaiedaban,
				value:	$('#'+$.id_inp.txt_futaiedaban).numberbox('getValue'),
				text:	$('#'+$.id_inp.txt_futaiedaban).numberbox('getText')
			});
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
						{field:'F1',	title:'種別',					width: 80,halign:'center',align:'left'},
						{field:'F2',	title:'漢字名',					width: 200,halign:'center',align:'left'},
						{field:'F3',	title:'風袋コード',				width: 80,halign:'center',align:'left'},
						{field:'F4',	title:'漢字名',				width: 200,halign:'center',align:'left'},
						{field:'F5',	title:'カナ名',				width: 120,halign:'center',align:'left'},
						{field:'F6',	title:'重量',				width: 120,halign:'center',align:'left'}
				]],
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_new:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_mei.kbn617,'', '');
				$.setJSONObject(sendJSON, $.id_inp.txt_futaiedaban,'', '');
				break;
			case $.id.btn_copy:
			...
			case $.id.btn_sel_change:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_mei.kbn617,row.F1, row.F1);
				$.setJSONObject(sendJSON, $.id_inp.txt_futaiedaban, row.F3, row.F3);
				break;
			case $.id.btn_back:
				// 転送先情報
