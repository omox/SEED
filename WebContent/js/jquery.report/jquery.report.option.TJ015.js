...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportTJ015',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("TJ015", "週間発注計画　週一覧");
...
//			this.success(this.name, false);
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
				    {field:'F1',	title:'リストNo',					width: 70,halign:'center',align:'left',formatter:function(value, rowData, rowIndex) {return $.getFormatPrompt(value, '####-##');},styler:cstyler},
				    {field:'F2',	title:'タイトル',					width: 200,halign:'center',align:'left'},
					{field:'F3',	title:'期間',						width: 200,halign:'center',align:'left'},
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_new:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_lstno,row.F1, row.F1);
				break;
			...
			case $.id.btn_sel_change:
				...
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id_inp.txt_lstno,row.F1, row.F1);
				$.setJSONObject(sendJSON, $.id_inp.txt_title,row.F2, row.F2);
