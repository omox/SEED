...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSO004',			// （必須）レポートオプションの確認
			...
			$('#if').on('load', function (e) {
				try {
					...
					if(json.opts.E_MSG === undefined){
						...
						if(Number(json.opts[$.id.txt_err_number]) == 0){
							$.setInputBoxDisable($("#btn_err_list"));
							$.setInputBoxDisable($("#btn_err_change"));
						}
					}else{
						$('#'+$.id.txt_status).textbox('setValue', json.opts[$.id.txt_status]);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			// 初期化
			var status		 = $.getJSONValue(that.jsonHidden, $.id.txt_status);
			var upd_number	 = $.getJSONValue(that.jsonHidden, $.id.txt_upd_number);
			var err_number	 = $.getJSONValue(that.jsonHidden, $.id.txt_err_number);
			var file		 = $.getJSONValue(that.jsonHidden, $.id.txt_file);

			$('#'+$.id.txt_status).textbox('setValue',status);				// ステータス
			$('#'+$.id.txt_upd_number).textbox('setValue',upd_number);		// 取込件数
			$('#'+$.id.txt_err_number).textbox('setValue', err_number);		// エラー件数
			$("#"+$.id.txt_file+"_").textbox('setValue', file);				// ファイル名

			// シーケンスが引き継がれた場合、ボタンを有効化する。
			var seq = $.getJSONValue(that.jsonHidden, $.id.txt_seq);
			if(seq && seq != ""){
				$('#'+$.id.txt_seq).val(seq);
				that.setUploadObjectState();
			}
			...
			$.initReportInfo("SO004", "生活応援　CSV取込", "取込");
		...
		updConfirm: function(func){
			$.showMessage('W20012', undefined, func);
		},
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				id:		$.id.txt_status,
				value:	$('#'+$.id.txt_status).textbox('getValue'),
				text:	$('#'+$.id.txt_status).textbox('getText')
			});
			// 取込件数
			this.jsonTemp.push({
				id:		$.id.txt_upd_number,
				value:	$('#'+$.id.txt_upd_number).val(),
				text:	$('#'+$.id.txt_upd_number).val()
			});
			// エラー件数
			this.jsonTemp.push({
				id:		$.id.txt_err_number,
				value:	$('#'+$.id.txt_err_number).val(),
				text:	$('#'+$.id.txt_err_number).val()
			});
			// ファイル名
			this.jsonTemp.push({
				id:		$.id.txt_file,
				value:	$('#'+$.id.txt_file).val(),
				text:	$('#'+$.id.txt_file).val()
			});
		},
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_err_change:
				...
				// 仮数値
				//$.setJSONObject(sendJSON, $.id.txt_seq, '101', '101');

				// 転送先情報
				if(btnId === $.id.btn_err_list){
					index = 8;
