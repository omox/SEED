...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportST024',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			// CSV取込ボタン押下時:tokubai
			if(that.sendBtnid===$.id.btn_csv+"1"){
				$.initReportInfo("ST018", "特売・スポット計画　特売原稿　CSV取込", "取込");
			}else if(that.sendBtnid===$.id.btn_csv+"2"){
				$.initReportInfo("ST018", "特売・スポット計画　店別数量　CSV取込", "取込");
			}else if(that.sendBtnid===undefined){
				$.initReportInfo("ST024", "特売・スポット計画　店一括数量　CSV取込", "取込");
			}
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			// 戻る実行時用に現在の画面情報を保持する
			var isStartpage = that.sendBtnid===undefined;
			var states = $.getBackBaseJSON(that);
			var newrepinfos = $.getBackJSON(that, states, isStartpage);
			...
			case $.id.btn_back:
				// 転送先情報
				if(that.sendBtnid===$.id.btn_csv+"1"
				|| that.sendBtnid===$.id.btn_csv+"2"){
					index = that.repgrpInfo.ST022.idx;
					childurl = href[index];
					sendMode = 2;
				}else if(that.sendBtnid===undefined){
