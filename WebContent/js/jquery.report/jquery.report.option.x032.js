...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx032',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("BR012", "中分類マスタ　一覧", "新規・変更");
		...
		updSuccess: function(id){	// validation OK時 の update処理
			...
			for (var i = 0; i < that.jsonHidden.length; i++) {
				if (that.jsonHidden[i].id === $.id_inp.txt_chucd) {
		...
		setCheckInfOtherDatasChu: function(){
			...
			for (var i=0; i< targetRowsx032.length; i++){
				if(targetRowsx032[i]['F14'] == '1'){
					// 削除データ有の場合
					targetRowsx032_Del.push(targetRowsx032[i])
				}
			}
			...
			if(dataLength == targetRowsx032_Del.length){
				// 表示件数 = 削除件数の場合
				allDelete = true
			}
			...
			for (var j=0; j< targetRowsx031.length; j++){
				...
				countFlg = false;
				if(allDelete){
					var newLines = targetRowsx032_Del.filter(function(item, index){
			...
			for (var j=0; j< targetRowsx033.length; j++){
				...
				if(targetRowsx032_Del.length > 0){
					var newLines = targetRowsx032_Del.filter(function(item, index){
						if((item.F16).indexOf(bunrui) >= 0
								&&(item.F17).indexOf(bmncd) >= 0
								&&(item.F18).indexOf(daicd) >= 0){
		...
		setUpdateDataThis: function(targetdata){
			...
			for (var i=0; i< that.updateDatas[$.id_update.Reportx032].length; i++){
				thisdatasAf.push(that.updateDatas[$.id_update.Reportx032][i])
			}

			for (var i=0; i< targetdata.length; i++){
				thisdatasAf.push(targetdata[i]);
			}
			that.updateDatas[$.id_update.Reportx032] = thisdatasAf;
		},
		setUpdateDatasCridIn: function(that, id){		// 保持されて遷移されてきた更新データ一覧をグリッドに設定
			...
			if(rows.length > 0){
				for (var i=0; i<rows.length; i++){
					if(rows[i]['F16'] === szBunrui && rows[i]['F17'] === szBumon && rows[i]['F18'] === szDaicd){
						// データグリッドを更新
		...
		setEditableGrid: function(that, reportno, id){		// データ表示
			...
			$('#'+id).datagrid({
				...
				columns:[[
				          	...
				          	{field:'F1',		title:'中分類コード', 			width:  90,halign:'center',align:'left',formatter:formatterLPad,editor:{type:'numberbox'}},
				          	{field:'F3',		title:'中分類名（漢字）',		width: 130,halign:'center',align:'left',editor:{type:'textbox',options:{editable:true,disabled:false,readonly:false}}},
							{field:'F2',		title:'中分類名（カナ）',		width: 130,halign:'center',align:'left',editor:{type:'textbox',options:{editable:true,disabled:false,readonly:false}}},
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_select:
				...
				$.setJSONObject(sendJSON, $.id_inp.txt_chucd, row.F1, row.F1);								// 検索条件：中分類コード
				$.setJSONObject(sendJSON, "beforIndex", 2, 2);												// 遷移前のindex
			...
			if(btnId==$.id.btn_select || btnId==$.id.btn_cancel || btnId=='btn_back' ){
				...
				if(btnId==$.id.btn_cancel){
					that.cancelInptData(that.updateDatas[$.id_update.Reportx032]);
				}
		...
		cancelInptData: function(targetrow){
			...
			targetrow.some(function(v, i){
			    if (v.F16==szBunrui && v.F17==szBumon && v.F18==szDaicd){
			    	if(v.F13 === '1'){
			    		// 新規行だった場合。
			    		szChucd = v.F1
			    	}
			...
			// 新規行の子要素削除
			if(szChucd !== ''){
				var childrowBf = that.updateDatas[$.id_update.Reportx033]

				childrowBf.some(function(v, i){
				    if (v.F16==szBunrui && v.F17==szBumon && v.F18==szDaicd && v.F18==szDaicd && v.F19==szChucd){
				    	// 新規行だった場合。
				    }else{
				    	childrowsAf.push(childrowBf[i])
				    }
				});
				that.updateDatas[$.id_update.Reportx033] = childrowsAf
			}
			that.updateDatas[$.id_update.Reportx032] = arterrows
		},
