...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportJU033',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("JU033", "店舗アンケート付き送付け　参照　商品情報");
			}else{
				$($.id.buttons).show();
				// 各種ボタン
				$.initReportInfo("JU033", "店舗アンケート付き送付け　変更 商品情報");
			}
				...
				function(data){
					...
					var afterFunc = function(){
						...
						if (opts.shnCnt==1) {
							...
							that.changeReport(that.name, 'btn_return_del');
						}
					};
		...
		setEditableGrid: function(that, reportno, id){		// データ表示
			...
			$('#'+id).datagrid({
				...
				onBeforeLoad:function(param){
					...
					var json = [{"callpage":"Out_ReportJU033","MOYSCD":txt_moyscd,"KANRINO":txt_kanrino}];
					// 情報設定
					param.page		=	reportno;
					param.obj		=	id;
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case "btn_return":
				...
				for (var i = 0; i < newrepinfos.length; i++) {
					...
					var reportYobi1 = $.getJSONValue(that.jsonHidden, "reportYobi1");
					if(callpage==='Out_ReportMM001'&&i==0){
						if (reportYobi1 === '0') {
							index = 1;
							sendMode = 2;
						} else {
					...
					if(callpage==='Out_ReportMM002'&&i==1){
						index = 3;
						sendMode = 2;
					}
					if(callpage==='Out_ReportJU033'&&mmFlg==false){
						index = 4;
						sendMode = 2;
					}
				}
