...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx215',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.sendBtnid===$.id.btn_sakubaikakb2){
				...
				$.initReportInfo("PC006", "プライスカード　（店、構成ページ、枚数指定）　新規", "新規");
			} else if (that.sendBtnid===$.id.btn_sel_change) {
				$.initReportInfo("PC006", "プライスカード　（店、構成ページ、枚数指定）　変更", "変更");
			} else {
				...
				$.initReportInfo("PC006", "プライスカード　（店、構成ページ、枚数指定）　参照", "参照");
			}
		...
		setGrid: function(that, reportno, id, chk){		// データ表示
			...
			var funcBeforeLoad = function(param){
				...
				var json = [{"callpage":"Out_Reportx215","INPUTNO":txt_inputno}];
				// 情報設定
				param.page		=	reportno;
				param.obj		=	id;
...
//				});
		...
		getGridColumns:function(that, id){
			...
			var fields = ["TENCD","TENKN","KOSEPAGE","MAISU","SEQ"];
			var titles = ["店コード","店名","構成ページ","枚数","SEQ"];
			...
			columnBottom.push({field:fields[2],	title:titles[2],	width:80,	halign:'center',align:'left',	editor:{type:'numberbox'}});
			columnBottom.push({field:fields[3],	title:titles[3],	width:60,	halign:'center',align:'right', formatter:iformatter,editor:{type:'numberbox'}});
			...
			if(target===undefined || target===$.id.grd_pcardsu+'_list'){
				...
				for (var i=0; i<rowsPcardsu.length; i++){
					if((rowsPcardsu[i]["TENCD"] == "" || rowsPcardsu[i]["TENCD"] == null) &&
							(rowsPcardsu[i]["KOSEPAGE"] == "" || rowsPcardsu[i]["KOSEPAGE"] == null) &&
						...
						var rowDate = {
								...
								F4	 : rowsPcardsu[i]["KOSEPAGE"],
								F5	 : rowsPcardsu[i]["MAISU"],
		...
		checkInputboxFunc: function(){
			...
			for (var i=0; i<targetRows.length; i++){
				...
				kosepage = targetRows[i]["KOSEPAGE"];

				if (!$.isEmptyVal(tencd) && ($.isEmptyVal(maisu) || $.isEmptyVal(kosepage))) {
					$.showMessage("E11049", ["店コードと構成ページと枚数を"], function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_tencd})});
					return false;
				}
				...
				if (!$.isEmptyVal(kosepage)) {
					// 商品コードの入力がない場合エラー
					if ($.isEmptyVal(tencd)) {
						$.showMessage("EX1040", undefined, function(){$.addErrState(that, $('#'+$.id.grd_pcardsu+'_list'), true, {NO:i, ID:$.id_inp.txt_kosepage})});
						return false;
					}
				}
