...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx248',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			$('#if').on('load', function (e) {
				try {
					...
					that.queried = true;
					var msgid = "";
					if(json.opts === null || json.opts.E_MSG === undefined){
						// ログ情報の格納
						$.post(
							$.reg.easy ,
							{
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			// 引き継ぎ情報セット
			var sendBtnid = $.id.btn_csv_import;
			if(!sendBtnid){
			...
			$.initReportInfo("x248", "提案商品　CSVアップロード", "取込");
