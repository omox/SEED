...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportJU011',			// （必須）レポートオプションの確認
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_new:
				...
				that.getEasyUI();
				that.validation();
				var txt_moyscd	= $.getInputboxValue($('#'+$.id_inp.txt_moyscd));		// メーカーコード
				var txt_moykn		= $.getInputboxValue($('#'+$.id_inp.txt_moykn));		// メーカー名（漢字）
				var kikan_dummy		= $.getInputboxValue($('#'+'kikan_dummy'));		// メーカー名（漢字）
