    ...
    var defaults = {
      ...
      setRadioInit2: function (jsonHidden, id, that) {
        ...
        if (that) {
          ...
          if (isRefer) {
            $.setInputStateRefer(that, $('input[name="' + id + '"]'));
          }
      ...
      isReferUpdateInput: function (that, target, isUpdateReport) {
        ...
        if (isUpdateReport) {
          if ($("#reportYobi1").val() === "1") {
            // 予備情報1(reportYobi1)が1の場合
            isRefer = true;
          } else if (that.judgeRepType && that.judgeRepType.ref) {
            // 帳票タイプがあり、参照がtrueの場合
            isRefer = true;
          } else if (target && !target.is(":enabled")) {
            // 項目の設定がそもそも使用不可の場合
            isRefer = true;
          }
        }
            ...
            if (options.prompt) {
              ...
              if (isYY) {
                formatter = function (value) {
                  return $.getFormatDt(value, isYY, addWeek);
                };
              } else {
                validType = "yyyymmdd";
                formatter = function (value) {
                  return $.getFormatDt2(value, addWeek);
                };
              }
              parser = function (value) {
                return $.getParserDt(value, false);
              };
            }
          } else if (check.datatyp == "yymm") {
            validType = "ym";
            formatter = function (value) {
              return $.getFormatYm(value);
            };
            parser = function (value) {
              return $.getParserYm(value, false);
            };
          } else if (check.datatyp == "mmdd") {
            validType = "mmdd";
            formatter = function (value) {
              return $.getFormatMmdd(value);
            };
            parser = function (value) {
              return $.getParserMmdd(value);
            };
          } else if (check.datatyp == "hhmm") {
            validType = "hhmm";
            formatter = function (value) {
              return $.getFormatHhmm(value);
            };
            parser = function (value) {
              return $.getParserHhmm(value);
            };
          ...
          onChange: function (newValue, oldValue, obj) {
            ...
            if (!obj.hasClass("datagrid-editable-input")) {
              if (!$.setComboReload(obj, true) && !editableCheck) {
                $.showMessage("E11302", ["入力値"], function () {
                  $.addErrState(this, obj, false);
                });
                obj.combobox("reload");
      ...
      setTextBox: function (that, id, required, validType, isSearch) {
        ...
        if ($("#" + id).attr("maxlength") != undefined) {
          $("#" + id)
            .combo("textbox")
            .attr("maxlength", $("#" + id).attr("maxlength"));
        }
        if (that) {
        ...
        $.extend($.fn.datagrid.defaults.editors, {
          combobox: {
            init: function (container, options) {
              ...
              var gfrmid = $(container)
                .parents("form[id^=" + $.id.gridform.replace("#", "") + "]")
                .eq(0)
                .attr("id");
...
*/
            ...
            .on("keydown", function (e) {
              ...
              if (code === 13 || code === 9) {
                ...
                if (targetsAll.filter(criteria).length == 0) {
                  ...
                  nextTarget.focus();
                  if (nextTarget.is(":text") && !nextTarget.hasClass("validatebox-readonly")) {
                    nextTarget.select();
                  }
                }
                e.preventDefault();
        ...
        if (focusTarget !== null) {
          ...
          $(focusTarget).focus();
          if ($(focusTarget).is(":text") && !$(focusTarget).hasClass("validatebox-readonly")) {
            $(focusTarget).select();
          }
        }
      },
      ...
      addErrState: function (that, obj, setCss, opt) {
        ...
        if (target.is(":hidden")) {
          ...
          for (var i = tabpanels.length - 1; i >= 0; i--) {
            if (!tabpanels.eq(i).filter(":hidden")) {
              break;
            }
          ...
          setTimeout(function () {
            ...
            setTimeout(function () {
              ...
              for (var i = 0; i < jsonId.length; i++) {
                ...
                if (i === 0) {
                  target.focus();
                  if (target.is(":text") && !target.hasClass("validatebox-readonly")) {
                    target.select();
                  }
                }
              }
      ...
      addErrStateGrid: function (id, indexArray, colindexArray) {
        ...
        if (Array.isArray(indexArray) == false) {
          indexArray = [indexArray];
        }
        if (Array.isArray(colindexArray) == false) {
          colindexArray = [colindexArray];
        }
      ...
      initKeyEvent: function (that) {
        ...
        $("#" + that.focusRootId)
          .find("[tabindex]")
          .each(function () {
            $.setKeyEvent(that, $(this));
          }); // tabindexが設定された項目に対し、キーイベントの設定を行う
      ...
      setReadyKeyEvent: function (js) {
        ...
        $("#" + js.focusRootId)
          .find(".easyui-datagrid_")
          .removeClass("easyui-datagrid_")
          .addClass("easyui-datagrid");
        $("#" + js.focusRootId)
          .find(".easyui-datagrid")
          .each(function () {
            var that = this;
            // targetに設定をコピー
                ...
                if ($("#" + gridId).datagrid("getRows").length > 0) {
                  ...
                  if (doEnter) {
                    ...
                    if (type === "mousedown") {
                      ...
                      this.prevEvent = "";
                      var mouseOver = $("#" + gridId)
                        .parent()
                        .children(".datagrid-view2")
                        .find(".datagrid-row-over");
        ...
        $("#" + js.focusRootId)
          .find(".easyui-combobox_")
          .removeClass("easyui-combobox_")
          .addClass("easyui-combobox");
        $("#" + js.focusRootId)
          .find(".easyui-combobox")
          .each(function () {
            var that = this;
            // targetに設定をコピー
        ...
        $("#" + js.focusRootId)
          .find(".easyui-numberbox_")
          .removeClass("easyui-numberbox_")
          .addClass("easyui-numberbox");
        $("#" + js.focusRootId)
          .find(".easyui-numberbox")
          .each(function () {
            var that = this;
            // targetに設定をコピー
        ...
        obj.on("keydown", function (e) {
            ...
            if (($(this).is("a") && $(this).is("[id^='btn']")) || this.type === "button" || this.type === "submit") {
              ...
              if (isEditing === undefined) {
                ...
                if (
                  $("#" + that.focusGridId)
                    .datagrid("getPanel")
                    .find(".datagrid-header-check").length === 1
                ) {
          ...
          if (!isSearch && (code === 13 || code === 9)) {
            ...
            if (!isSet && this.type !== "submit" && ($(this).is(":input") || $(this).is("a") || $(this).is(".datagrid"))) {
              ...
              var targetsAll = $("#" + that.focusParentId)
                .find("[tabindex]")
                .filter("[tabindex!=-1]")
                .filter("[disabled!=disabled]")
                .sort(function (a, b) {
              ...
              if (targetsAll.length < 2) {
                ...
                if (targetsAll.length === targetsAll.filter(":visible").length) {
                  ...
                  // tabIndex有効項目と画面上の表示項目の数が一致しない場合、タブによる非表示項目ありと考え、タブ表示を考慮した遷移を行う
                  var index = targetsAll.index(this);
                  var criteria = e.shiftKey ? ":lt(" + index + "):last" : ":gt(" + index + "):first";
                  if (targetsAll.filter(criteria).length == 0) criteria = e.shiftKey ? ":last" : ":eq(0)";
                  ...
                  if (target.is(":hidden")) {
                    ...
                    if ($(this).parents(".easyui-tabs").filter(":first").attr("id") === tabid) {
                      ...
                      if (panelIndex < 0) {
                        panelIndex = tabs.length - 1;
                      }
                      if (panelIndex >= tabs.length) {
                        panelIndex = 0;
                      }
      ...
      changeReportByTabs: function (that) {
        if ($.reg.changeReportByTabs) {
          $("#tabs a", window.parent.document).click(function () {
            // タブ要素(a)取得
            var elems = $("#tabContent", window.parent.document)
              .map(function (i, e) {
                return e;
              })
              .get();
      ...
      confirmUnregist: function () {
        ...
        var obj = $($.id.hiddenChangedIdx);
        if (obj.length === 0) {
          obj = $(this).parent($.id.hiddenChangedIdx);
        }
        // 未登録の警告メッセージが必要かどうかチェック
        if ($.getConfirmUnregistFlg(obj)) {
      ...
      pushUpd: function (e) {
        ...
        // チェック・確認処理
        var rtn = true;
        if ($.isFunction(that.updValidation)) {
          rtn = that.updValidation(id);
        }
        // 変更情報チェック
      ...
      pushDel: function (e) {
        ...
        if ($.isFunction(that.delValidation)) {
          rtn = that.delValidation(id);
        }
        if (rtn) {
          var func_ok = function (r) {
            // セッションタイムアウト、利用時間外の確認
      ...
      ctrlFocusSubWin: function (that) {
        if (that.suffix && that.suffix != "") {
          ...
          $("#" + that.focusRootId)
            .find(".textbox-text.validatebox-text")
            .on("focus", function () {
      ...
      checkColumnSize: function (that, ids) {
        ...
        for (var i = 0; i < ids.length; ++i) {
          ...
          if (isFinite(ids[i])) {
            // 列数直接指定
            num = ids[i] * 1;
          } else if ($("#" + ids[i]).combogrid("options").multiple) {
            // 複数選択可
            num = $("#" + ids[i]).combogrid("getValues").length;
            // 最上段は総合計考慮
            if (i == 0) num = num + 1;
          } else if ($("#" + ids[i]).combogrid("getValue") == "") {
            // 複数選択不可で「全て」選択
            num = $("#" + ids[i])
              .combogrid("grid")
            ...
            // 複数選択不可で「全て」以外選択、かつ最上段の場合
            num = 2;
          }
          size = size * num;
        }
        // レポートごとの特殊計算
        if (that.name == "livins_rep09") {
          // 加盟店別商品別在庫表
      ...
      setDateBox: function (that, id, required, validType, isSearch, setHiddenValue) {
        ...
        if ($("#" + id).attr("tabindex")) {
          $("#" + id)
            .combo("textbox")
            .attr("tabindex", $("#" + id).attr("tabindex"));
        }
      },
      ...
      onInitAllSelect: function (prefix) {
        ...
        if ($($.id.hiddenSelectIdx + prefix).val() !== "") {
          ...
          var index = $($.id.hiddenSelectIdx + prefix)
            .val()
            .split(",");
      ...
      setChangeIdx_grid: function (rowIndex, prefix) {
        ...
        if (dg) {
          var opts = dg.datagrid("options");
          var totalIdx = $.getTotalIdx(opts.pageNumber, opts.pageSize, rowIndex);
          var index = $($.id.hiddenChangedIdx + prefix)
            .val()
            .split(",");
          if ($.inArray(totalIdx + "", index) === -1) {
            index.push(totalIdx);
