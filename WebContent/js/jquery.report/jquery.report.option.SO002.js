...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSO002',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				$.initReportInfo("SO002", "生活応援　部門別件数(参照)","一覧");
				...
				$.initReportInfo("SO002", "生活応援　部門別件数","一覧");
				// 各種遷移ボタン
				/*$('#'+$.id.btn_new).on("click", $.pushChangeReport);
...
//			this.success(this.name, false);
		...
		getGridColumns:function(that, id){
			...
			columnBottom.push({field:'F1',	title:'部門',		width:  50,halign:'center',align:'left'});
			columnBottom.push({field:'F2',	title:'部門名',		width: 300,halign:'center',align:'left'});
			columnBottom.push({field:'F3',	title:'登録件数',	width:  80,halign:'center',align:'right',formatter:iformatter});
