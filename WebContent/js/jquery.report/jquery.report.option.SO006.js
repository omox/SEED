...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSO006',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("SO006", "生活応援　CSV取込　エラー選択","選択");
...
//			this.success(this.name, false);
		...
		getGridColumns:function(that, id){
			...
			columnBottom.push({field:'F1',	title:'オペレーターコード',		width:  85,halign:'center',align:'left'});
			columnBottom.push({field:'F2',	title:'処理日',					width:  80,halign:'center',align:'left'});
			columnBottom.push({field:'F3',	title:'時刻',					width:  60,halign:'center',align:'left'});
			columnBottom.push({field:'F4',	title:'コメント',				width: 500,halign:'center',align:'left'});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_err_change:
				...
				if(btnId===$.id.btn_err_list){
					index = 8;
				}else if(btnId===$.id.btn_err_change){
					index = 4;
				}
