...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx213',			// （必須）レポートオプションの確認
		...
		initialize: function (reportno){	// （必須）初期化
			...
			// ラジオボタン系
			that.setRadio(that, $.id.rad_pcardsz1);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			// 当帳票を「新規」で開いた場合
			if(that.sendBtnid===$.id.btn_sakubaikakb1){
				$('#'+$.id.btn_del).linkbutton('disable').attr('disabled', 'disabled').hide();
				$.initReportInfo("PC002", "プライスカード　（枚数指定）　新規", "新規");
			} else if (that.sendBtnid===$.id.btn_sel_change) {
				$.initReportInfo("PC002", "プライスカード　（枚数指定）　変更", "変更");
			} else {
				...
				$.initReportInfo("PC002", "プライスカード　（枚数指定）　参照", "参照");
			}
			...
			$.setInputBoxDisable($("input[name="+$.id.rad_pcardsz1+"]"));
		},
		...
		setGrid: function(that, reportno, id, chk){		// データ表示
			...
			var funcBeforeLoad = function(param){
				...
				var json = [{"callpage":"Out_Reportx213","INPUTNO":txt_inputno}];
				// 情報設定
				param.page		=	reportno;
				param.obj		=	id;
...
//				});
		...
		getGridColumns:function(that, id){
			...
			var fields = ["SHNCD","SHNKN","MAISU","SEQ"];
			var titles = ["商品コード","商品名","枚数","SEQ"];
...
//			columnBottom.push({field:fields[2],	title:titles[2],	width:60,	halign:'center',align:'right', formatter:iformatter, editor:{type:'numberbox'}});
//			columnBottom.push({field:fields[3],	title:titles[3],	hidden:true});

			columnBottom.push({field:'SHNCD',	title:'商品コード',	width: 80	,halign:'center',align:'left', formatter:function(value,row,index){return $.getFormatPrompt(value, '####-####');},editor:{type:'numberbox'}});

			columnBottom.push({field:'SHNKN',	title:'商品名',		width: 300	,halign:'center',align:'left', editor:{type:'textbox',options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}});
			columnBottom.push({field:'MAISU',	title:'枚数',		width: 60	,halign:'center',align:'left', formatter:iformatter, editor:{type:'numberbox'}});
			columnBottom.push({field:'SEQ',	title:'SEQ',			hidden:true});
				...
				for (var i=0; i<rowsPcardsu.length; i++){
					if((rowsPcardsu[i]["SHNCD"] == "" || rowsPcardsu[i]["SHNCD"] == null) &&
						...
						var rowDate = {
								...
								F4	 : rowsPcardsu[i]["MAISU"],
								F5	 : rowsPcardsu[i]["SEQ"],
							};
