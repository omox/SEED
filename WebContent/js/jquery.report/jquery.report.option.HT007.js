...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportHT007',			// （必須）レポートオプションの確認
		...
		setEditableGrid: function(that, reportno, id){		// データ表示
			...
			$(id).datagrid({
				...
				onBeforeEdit:function(index,row){
					if(row.F15==='3'&&row.F16==='3'&&row.F17==='3'&&row.F18==='3'&&row.F19==='3'&&row.F20==='3'&&row.F21==='3'){
						// 次の行に移るか、次の項目に移るかする
...
//				});
		...
		getGridColumns:function(that, id){
			var columns = [];
			var columnBottom=[];
			var fields = ["F1","F2","F3","F4","F5","F6","F7","F8","F9","F10","F12","F13","F11","F14","F15","F22"];
