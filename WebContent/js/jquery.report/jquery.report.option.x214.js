...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx214',			// （必須）レポートオプションの確認
		...
		setGrid: function(that, reportno, id, chk){		// データ表示
			...
			var funcBeforeLoad = function(param){
				...
				var json = [{"callpage":"Out_Reportx214","INPUTNO":txt_inputno}];
				// 情報設定
				param.page		=	reportno;
				param.obj		=	id;
...
//				});
		...
		getGridColumns:function(that, id){
			...
			var fields = ["SHNCD","SHNKN","SEQ"];
			var titles = ["商品コード","商品名","SEQ"];
			...
			columnBottom.push({field:fields[2],	title:titles[2],	hidden:true});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			$.setJSONObject(sendJSON, 'txt_coman', $.getInputboxValue($('#'+$.id_inp.txt_coman)), $.getInputboxValue($('#'+$.id_inp.txt_coman)));	// 参照情報保持
		...
		checkInputboxFunc: function(){
			...
			// 枚数にマイナス値の入力は
			if ($.getInputboxValue($('#'+$.id_inp.txt_copysu))*1 < 0) {
				$.showMessage("EX1047", ["コピー枚数 ≧ 0の値"], function(){$.addErrState(that, $('#'+$.id_inp.txt_copysu),true)});
				return false;
			}
