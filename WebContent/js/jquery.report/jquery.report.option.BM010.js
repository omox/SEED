...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportBM010',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("BM012", "B/M別送信情報　B/M別　参照　B/M番号一覧", "参照");
			}else{
				$('#'+$.id.btn_sel_refer).linkbutton('disable').attr('disabled', 'disabled').hide();
				$.initReportInfo("BM010", "B/M別送信情報　B/M別　新規・変更　B/M番号一覧", "変更");
			}
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			$.post(
				...
				function(json){
					...
					// データグリッド初期化
					that.setGrid(that, reportno, $.id.grd_bmnno+'_list');
		...
		setGrid: function(that, reportno, id){		// データ表示
			...
			$('#'+id).datagrid({
				...
				frozenColumns:[[]],
				columns:[[
							{field:'BMNNO',		title:'B/M番号'			,width: 60  ,halign:'center',align:'left',formatter:formatterLPad},
							{field:'HBPERIOD',	title:'販売期間'		,width: 190 ,halign:'center',align:'left'},
							{field:'BMNMAN',	title:'B/M名称（ｶﾅ）'	,width: 200 ,halign:'center',align:'left'},
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			var row = $('#'+$.id.grd_bmnno+'_list').datagrid("getSelected");
			...
			case $.id.btn_sel_refer:
				...
				if (row.BMNNO!=="" && row.BMNNO!==null && row.BMNNO!==undefined) {
					$.setJSONObject(sendJSON, $.id_inp.txt_bmnno, row.BMNNO, row.BMNNO);
				}
		...
		getGridData: function (target){
			...
			// 配送店グループ
			if(target===undefined || target===$.id.grd_bmnno+'_list'){
				var rowsBmnNo= $('#'+$.id.grd_bmnno+'_list').datagrid('getRows');
				for (var i=0; i<rowsBmnNo.length; i++){
					if(rowsBmnNo[i]["BMNNO"] == "" || rowsBmnNo[i]["BMNNO"] == null ){

					}else{
						var rowDate = {
								F1	 : rowsBmnNo[i]["BMNNO"],
								F2	 : rowsBmnNo[i]["HBPERIOD"],
								F3	 : rowsBmnNo[i]["BMNMAN"],
								F4	 : rowsBmnNo[i]["MOYSKBN"],
								F5	 : rowsBmnNo[i]["MOYSSTDT"],
								F6	 : rowsBmnNo[i]["MOYSRBAN"],
							};
