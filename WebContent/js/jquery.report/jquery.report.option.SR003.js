...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportSR003',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("SR003", "商品コード履歴検索　催し一覧","一覧");
		},
...
//			this.success(this.name, false);
		...
		success: function(reportno, sortable){	// （必須）正処理
			...
			var that = this;
			// 検索実行
			var szShncd		= $.getJSONObject(this.jsonString, $.id_inp.txt_shncd).value;	// 催し連番
			// 仮数値
			var szShoridt	= "20170101";	// 催し連番
		...
		getGridColumns:function(that, id){
			...
			columnBottom.push({field:'F1',	title:'催しコード',		width: 100,halign:'center',align:'left'});
			columnBottom.push({field:'F2',	title:'販売期間',		width: 180,halign:'center',align:'left'});
			columnBottom.push({field:'F3',	title:'納入期間',		width: 180,halign:'center',align:'right'});
			columnBottom.push({field:'F4',	title:'山積入数',	width:  90,halign:'center',align:'right'});
			columnBottom.push({field:'F5',	title:'最低発注数',		width:  80,halign:'center',align:'right'});
			columnBottom.push({field:'F6',	title:'原価',			width:  90,halign:'center',align:'right', formatter:iformatter_s});
			columnBottom.push({field:'F7',	title:'総売価',			width:  80,halign:'center',align:'right', formatter:iformatter});
			columnBottom.push({field:'F8',	title:'B総売価',		width:  80,halign:'center',align:'right', formatter:iformatter});
			columnBottom.push({field:'F9',	title:'Bランク',		width:  80,halign:'center',align:'right'});
			columnBottom.push({field:'F10',	title:'C総売価',		width:  80,halign:'center',align:'right', formatter:iformatter});
			columnBottom.push({field:'F11',	title:'Cランク',		width:  80,halign:'center',align:'right'});
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_sel_refer:
				...
				$.setJSONObject(sendJSON, $.id_inp.txt_moyskbn, szMoyskbn, szMoyskbn);				// 催し区分
				$.setJSONObject(sendJSON, $.id_inp.txt_moysstdt, szMoysstdt, szMoysstdt);			// 催しコード（催し開始日）
				$.setJSONObject(sendJSON, $.id_inp.txt_moysrban, szMoysrban, szMoysrban);			// 催し連番
				$.setJSONObject(sendJSON, $.id_inp.txt_bmncd, row.F1, row.F1);						// 部門コード
				break;
			case $.id.btn_back:
			case $.id.btn_cancel:
				// 転送先情報
