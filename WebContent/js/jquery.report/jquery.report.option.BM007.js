...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_ReportBM007',			// （必須）レポートオプションの確認
			...
			$('#if').on('load', function (e) {
				try {
					...
					if(json.opts.E_MSG === undefined){
						...
						var seq = json.opts[$.id.txt_seq];

						if (seq==='-1') {
							$('#'+$.id.txt_status).textbox('setValue', json.message);
							$('#'+$.id.txt_upd_number).textbox('setValue','');
							$.showMessage("E00014");
						} else {
							$('#'+$.id.txt_seq).val(seq);
							if (json.message.length >= 2) {
								$('#'+$.id.txt_status).textbox('setValue', json.message);
								$('#'+$.id.txt_upd_number).textbox('setValue','0');
							} else {
								$('#'+$.id.txt_status).textbox('setValue', json.opts[$.id.txt_status]);
								$('#'+$.id.txt_upd_number).textbox('setValue', json.opts[$.id.txt_upd_number]);
							}

							$('#'+$.id.txt_err_number).val(json.opts[$.id.txt_err_number]);
							$.setInputboxValue($('#'+$.id.txt_err_number),json.opts[$.id.txt_err_number]);
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			// アップロード処理実行後に利用可能
			//$('#'+$.id.btn_err_list).linkbutton('enable').attr('tabindex', 5); // TODO:暫定対応
			$('#'+$.id.btn_err_list).on("click", $.pushChangeReport);

			$.initReportInfo("BM007", "B/M別送信情報　新規・変更　CSV取込", "取込");
		...
		setUploadObjectState: function(){	// 登録時の項目制御
			...
			//if(isAbleChange){
			//	$('#'+$.id.btn_err_list).linkbutton('enable').attr('tabindex', 5);
			//}

			var errCnt = $.getInputboxValue($('#'+$.id.txt_err_number));
			if (errCnt!=='0') {
				$('#'+$.id.btn_err_list).linkbutton('enable').attr('tabindex', 5);
			}
		},
		...
		getEasyUI: function(){	// （必須）情報の取得
			...
			this.jsonTemp.push({
				...
				value:	$('#'+$.id_inp.txt_commentkn).val(),
				text:	$('#'+$.id_inp.txt_commentkn).val()
			});
		},
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			var txt_seq = $.getInputboxValue($('#'+$.id.txt_seq));
			...
			case $.id.btn_err_list:
				// 選択行
				$.setJSONObject(sendJSON, $.id.txt_seq, txt_seq, txt_seq);		// SEQ
