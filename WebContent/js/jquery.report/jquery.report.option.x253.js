...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx253',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			if(that.reportYobiInfo()==='1'){
				...
				$.initReportInfo("x253", "提案商品　CSVエラー選択", "一覧");
			}else{
				$($.id.buttons).show();
				// 各種ボタン
				$('#'+$.id.btn_err_change).on("click", $.pushChangeReport);

				$.initReportInfo("x253", "提案商品　CSVエラー選択", "選択");
			}
...
//			this.success(this.name, false);
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				pageList:pageList,
				columns:[[
					{field:'F22',	title:'件名No',			width: 80,halign:'center',align:'left'},
					{field:'F23', title:'状態', width:60, align:'center',	halign:'center', resizable:false,
						formatter:function(value, rowData, rowIndex) {
							if(rowData['F23'] == 1){
								return '作成中';
							} else if (rowData['F23'] == 2){
								return '確定';
							} else if (rowData['F23'] == 3){
								return '仕掛';
							} else if (rowData['F23'] == 4){
								return '完了';
							} else if (rowData['F23'] == 9){
								return '却下';
							}
						}
					},
					{field:'F1',	title:'商品コード', hidden:true},
					...
					{field:'F24',	title:'エラー箇所',				width:200,halign:'center',align:'left'},
					{field:'F25',	title:'エラー理由',				width:100,halign:'center',align:'left'},
					{field:'F26',	title:'エラー値',				width:300,halign:'center',align:'left'},
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_err_change:
				...
				$.setJSONObject(sendJSON, $.id.hiddenNoTeian, row.F22, row.F22);	// 件名No
				$.setJSONObject(sendJSON, $.id.hiddenStcdTeian, row.F23, row.F23);	//  提案状態、デフォルトは作成中
				break;
			default:
				break;
			}
