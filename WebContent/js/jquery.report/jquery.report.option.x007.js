...
;(function($) {
	...
	$.extend({
		reportOption: {
		name:		'Out_Reportx007',			// （必須）レポートオプションの確認
		...
		setInitObjectState: function(){	// 画面初期化時の項目制御
			...
			$.initReportInfo("IT027", "商品マスタ　CSV取込　エラーリスト", "リスト");
		},
...
//			this.success(this.name, false);
		...
		setGrid: function (id, reportNumber){	// グリッドの構築
			...
			$(id).datagrid({
				...
				pageList:pageList,
				frozenColumns:[[
					{field:'F1',	title:'行番号',				width: 45,halign:'center',align:'right'},
					{field:'F2',	title:'商品コード',			width: 80,halign:'center',align:'left'},
				]],
				columns:[[
					{field:'F3',	title:'商品名',				width:200,halign:'center',align:'left'},
					{field:'F4',	title:'エラー箇所',			width:200,halign:'center',align:'left'},
					{field:'F5',	title:'エラー理由',			width:200,halign:'center',align:'left'},
					{field:'F6',	title:'エラー値',			width:200,halign:'center',align:'left'}
				]],
...
//				});
		...
		changeReport:function(reportno, btnId){				// 画面遷移
			...
			case $.id.btn_sel_change:
				...
				sendMode = 1;
				// オブジェクト作成
				$.setJSONObject(sendJSON, $.id.txt_sel_shncd, row.F1, row.F1);
				$.setJSONObject(sendJSON, $.id_inp.txt_shncd, row.F1, row.F1);
				break;
			default:
				break;
			}
