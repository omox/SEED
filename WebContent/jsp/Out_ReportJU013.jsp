...
<table class="t-layout1">
	...
	<tr>
		<td>
			<input class="easyui-numberbox" col="F4" tabindex="-1" id="<%=DefineReport.InpText.SHNCD.getObj()%>" check='<%=DefineReport.InpText.SHNCD.getMaxlenTag()%>' data-options="prompt:'____-____',label:'商品コード',labelWidth:80" style="width:200px;" disabled="disabled">
		</td>
		<td colspan=2>
			<input class="easyui-textbox" tabindex="-1" col="F5" id="<%=DefineReport.InpText.SHNKN.getObj()%>" check='<%=DefineReport.InpText.SHNKN.getMaxlenTag()%>' style="width:350px;" data-options="label:'商品名',labelWidth:80" disabled="disabled">
		</td>
	</tr>
	<tr>
		<td>
			<input class="easyui-combobox" tabindex="1" col="F6" id="<%=DefineReport.MeisyoSelect.KBN103051.getObj()%>" data-options="label:'商品区分',labelWidth:80,required:true" style="width:200px;">
		</td>
		<td>
			<input class="easyui-combobox" tabindex="2" col="F7" id="<%=DefineReport.MeisyoSelect.KBN103061.getObj()%>" data-options="label:'訂正区分',labelWidth:80,required:true" style="width:230px;">
		</td>
		<td>
			<input class="easyui-textbox" tabindex="-1" col="F8" id="<%=DefineReport.InpText.JUKBN.getObj()%>" value="" data-options="label:'　事前区分',labelWidth:80,required:true" disabled="disabled" style="width:140px;text-align: left;">
		</td>
		<td>
			<input class="easyui-numberbox" tabindex="3" col="F9" id="<%=DefineReport.InpText.BDENKBN.getObj()%>" value="" check='<%=DefineReport.InpText.BDENKBN.getMaxlenTag()%>' data-options="label:'　別伝区分',labelWidth:80,min:0,max:8" style="width:140px;text-align: left;">
		</td>
		<td>
			<input class="easyui-combobox" tabindex="4" col="F10" id="<%=DefineReport.MeisyoSelect.KBN103081.getObj()%>" for_inp="<%=DefineReport.InpText.SHNCD.getObj()%>_F2" data-options="label:'　ワッペン区分',labelWidth:100,required:true" style="width:220px;">
		</td>
	</tr>
	<tr>
		<td>
			<input class="easyui-numberbox" tabindex="5" col="F11" id="<%=DefineReport.InpText.IRISU.getObj()%>" check='<%=DefineReport.InpText.IRISU.getMaxlenTag()%>' for_inp="<%=DefineReport.InpText.SHNCD.getObj()%>_F3" data-options="label:'入数',labelWidth:80,required:true,min:0" style="width:200px;text-align: right;">
		</td>
		<td>
			<input class="easyui-numberbox" tabindex="6" col="F12" id="<%=DefineReport.InpText.GENKAAM.getObj()%>" check='<%=DefineReport.InpText.GENKAAM.getMaxlenTag()%>' for_inp="<%=DefineReport.InpText.SHNCD.getObj()%>_F4" data-options="label:'原価',labelWidth:80,required:true,min:0" style="width:200px;text-align: right;">
		</td>
		<td>
			<input class="easyui-numberbox" tabindex="7" col="F13" id="<%=DefineReport.InpText.RG_BAIKAAM.getObj()%>" check='<%=DefineReport.InpText.RG_BAIKAAM.getMaxlenTag()%>' for_inp="<%=DefineReport.InpText.SHNCD.getObj()%>_F5" data-options="label:'　総売価',labelWidth:80,required:true,min:0" style="width:140px;text-align: right;">
		</td>
		<td>
			<input class="easyui-numberbox" tabindex="-1"  col="F14" id="<%=DefineReport.InpText.BAIKAAM.getObj()%>"  for_inp="<%=DefineReport.InpText.SHNCD.getObj()%>_F6" data-options="label:'　本体売価',labelWidth:80,required:true" style="width:140px;text-align: right;" disabled="disabled">
		</td>
	</tr>
	<tr>
		<td>
			<input class="easyui-numberbox" tabindex="8" col="F15" id="<%=DefineReport.InpText.HTDT.getObj()%>" check='<%=DefineReport.InpText.HTDT.getMaxlenTag()%>' data-options="prompt:'<%=DefineReport.Label.PROMPT_DTW.getTxt()%>',label:'発注日',labelWidth:80,required:true" style="width:200px;">
		</td>
		<td>
			<input class="easyui-numberbox" tabindex="9" col="F16" id="<%=DefineReport.InpText.NNDT.getObj()%>" check='<%=DefineReport.InpText.NNDT.getMaxlenTag()%>' data-options="prompt:'<%=DefineReport.Label.PROMPT_DTW.getTxt()%>',label:'納入日',labelWidth:80,required:true" style="width:200px;">
			<!-- 非表示でデータを保持 -->
			<input type="hidden" name="<%=DefineReport.Text.SHORIDT.getObj()%>" id="<%=DefineReport.Text.SHORIDT.getObj()%>" />
			<input type="hidden" col="F21" name="txt_jutenkaikbn" id="txt_jutenkaikbn" />
			<input type="hidden" col="F22" name="txt_rankno_add" id="txt_rankno_add" />
			<input type="hidden" col="F23" name="txt_htsu" id="txt_htsu" />
			<input type="hidden" col="F24" name="txt_suryoptn" id="txt_suryoptn" />
		</td>
	</tr>
</table>
</div>
...
<div id="buttons" data-options="region:'south',border:false" style="display:none;height:30px;padding:0 20px;">
	...
	<div id="disp_record_info" style="float: right;">
		<span class="labelName" tabindex="-1" style="padding-top: 3px;">
			登録日 <span col="F18" id="<%=DefineReport.Text.ADDDT.getObj()%>"></span>　更新日 <span col="F19" id="<%=DefineReport.Text.UPDDT.getObj()%>"></span>　オペレータ <span col="F17" id="<%=DefineReport.Text.OPERATOR.getObj()%>"></span>
		</span>
		<input type="hidden" col="F20" name="<%=DefineReport.Hidden.UPDDT.getObj()%>" id="<%=DefineReport.Hidden.UPDDT.getObj()%>" />
	</div>
</div>
</div>
...
<script type="text/javascript" src="../js/jquery.report/jquery.report.option.JU013.js?v=<%=prm %>"></script>
