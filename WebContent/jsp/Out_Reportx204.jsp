 	...
 	<tr><th style="max-width:600px;"></th><th style="max-width:600px;"></th></tr>
		<tr>
			<td style="height:30px;">
				...
				<input class="easyui-textbox" tabindex="-1" col="F2" id="<%=DefineReport.InpText.HSGPKN.getObj()%>" check='<%=DefineReport.InpText.HSGPKN.getMaxlenTag()%>' style="width:350px;">
			</td>
		</tr>
		<tr>
			<td style="height:30px;">
				<input class="easyui-numberbox" tabindex="-1" col="F3" id="<%=DefineReport.InpText.HSTENGPCD.getObj()%>" check='<%=DefineReport.InpText.HSTENGPCD.getMaxlenTag()%>' data-options="label:'店グループ',labelWidth:90,required:true" style="width:140px;">
				<input class="easyui-textbox" tabindex="-1" col="F4" id="<%=DefineReport.InpText.TENGPKN.getObj()%>" check='<%=DefineReport.InpText.TENGPKN.getMaxlenTag()%>' style="width:350px;">
			</td>
		</tr>
</table>
</div>
<div data-options="region:'center',border:false" style="display:none;padding:2px 5px 0;">
<table id="<%=DefineReport.Grid.HSGPT.getObj()%>_list" class="easyui-datagrid" tabindex="1" data-options="singleSelect:true,rownumbers:true, fit:true">
<thead>
	<tr>
		<th data-options="field:'TENCD',width: 70,halign:'center',editor:'numberbox'">店番</th>
		<th data-options="field:'TENKN',width:279,halign:'center',editor:{type:'textbox',options:{cls:'labelInput',editable:false,disabled:true,readonly:true}},styler:function(value,row,index){return 'background-color:#f5f5f5;';}">店舗名称</th>
	</tr>
</thead>
</table>
...
<div id="buttons" data-options="region:'south',border:false" style="display:none;height:30px;padding:0 20px;">
	<div class="btn" style="float: left;">
		...
		<td></td>
		<td><a href="#" title="キャンセル" class="easyui-linkbutton" tabindex="3" id="<%=DefineReport.Button.CANCEL.getObj()%>" iconCls="icon-cancel" style="width:110px;"><%=DefineReport.Button.CANCEL.getTxt()%></a></td>
		<td><a href="#" title="登録" class="easyui-linkbutton" tabindex="4" id="<%=DefineReport.Button.UPD.getObj()%>" title="<%=DefineReport.Button.UPD.getTxt()%>" iconCls="icon-save" style="width:110px;"><span class="btnTxt"><%=DefineReport.Button.UPD.getTxt()%></span></a></td>
		<td><a href="#" title="削除" class="easyui-linkbutton" tabindex="5" id="<%=DefineReport.Button.DEL.getObj()%>" title="<%=DefineReport.Button.DEL.getTxt()%>" iconCls="icon-remove" style="width:110px;"><span class="btnTxt"><%=DefineReport.Button.DEL.getTxt()%></span></a></td>
		</tr>
		</table>
	</div>
...
<script type="text/javascript" src="../js/jquery.report/jquery.report.option.x204.js?v=<%=prm %>"></script>
